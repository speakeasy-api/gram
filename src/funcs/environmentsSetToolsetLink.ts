...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.SetToolsetEnvironmentLinkRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON(
    "body",
    payload.SetToolsetEnvironmentLinkRequestBody,
    { explode: true },
  );

  const path = pathToFunc("/rpc/environments.setToolsetLink")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ToolsetEnvironmentLink$inboundSchema),
    M.jsonErr(
