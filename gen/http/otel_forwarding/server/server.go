...
// Server lists the otelForwarding service endpoint HTTP handlers.
type Server struct {
	Mounts       []*MountPoint
...
// New instantiates HTTP handlers for all the otelForwarding service endpoints
...
func New(
	...
	return &Server{
		Mounts: []*MountPoint{
			{"GetConfig", "GET", "/rpc/otelForwarding.getConfig"},
			{"UpsertConfig", "POST", "/rpc/otelForwarding.upsertConfig"},
			{"DeleteConfig", "POST", "/rpc/otelForwarding.deleteConfig"},
		},
...
func (s *Server) Service() string { return "otelForwarding" }
...
func (s *Server) MethodNames() []string { return otelforwarding.MethodNames[:] }

// Mount configures the mux to serve the otelForwarding endpoints.
...
// MountGetConfigHandler configures the mux to serve the "otelForwarding"
...
func MountGetConfigHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("GET", "/rpc/otelForwarding.getConfig", f)
}

// NewGetConfigHandler creates a HTTP handler which loads the HTTP request and
// calls the "otelForwarding" service "getConfig" endpoint.
func NewGetConfigHandler(
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		ctx = context.WithValue(ctx, goa.ServiceKey, "otelForwarding")
...
// MountUpsertConfigHandler configures the mux to serve the "otelForwarding"
...
func MountUpsertConfigHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("POST", "/rpc/otelForwarding.upsertConfig", f)
}

// NewUpsertConfigHandler creates a HTTP handler which loads the HTTP request
// and calls the "otelForwarding" service "upsertConfig" endpoint.
...
// MountDeleteConfigHandler configures the mux to serve the "otelForwarding"
...
func MountDeleteConfigHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("POST", "/rpc/otelForwarding.deleteConfig", f)
}

// NewDeleteConfigHandler creates a HTTP handler which loads the HTTP request
// and calls the "otelForwarding" service "deleteConfig" endpoint.
