...
Notes for running Gram inside the Cursor Cloud VM. The startup update script only refreshes deps (`mise install`; `mise run install`). Tools (Go/Node/pnpm/uv/…), Docker, generated keys (`mise.local.toml`), TLS certs, and the migrated DB volumes are already present from the environment snapshot — you generally only need to (re)start Docker and the services.

- **Shell/mise**: `mise` is activated for login shells via `~/.bashrc`. In a fresh non-login shell, run `eval "$(mise activate bash)"` (or prefix commands with `mise exec --`) before using tools.
- **Docker is not a managed service here.** It must be started manually once per VM: `sudo dockerd &` (config already set to the `fuse-overlayfs` storage driver with the containerd snapshotter disabled — required for this nested VM; don't change it). Then `docker ps` should work without sudo.
- **Start infra**: `mise infra:start` (Postgres, Redis, Temporal, ClickHouse, Jaeger, Pub/Sub, etc.). A local, untracked `compose.override.yaml` clears ClickHouse's `mem_limit` because this VM's cgroup v2 root is "domain threaded" and cannot delegate the memory controller, so any container with a memory limit fails to start. Keep that file; without it ClickHouse won't boot.
- **Start app services**: `mise run start:headless` runs dev-idp, server, worker, and dashboard as background processes (logs in `/tmp/gram-dev/*.log`). `go run` compiles the server/worker on first start (can take a couple minutes). Dashboard: `https://localhost:5173` (self-signed mkcert cert — accept the browser warning). API: `https://localhost:8080` (`/healthz` = 200). dev-idp on `:35291` auto-approves login (mock-workos), so no real credentials are needed. `madprocs` (the default `mise run start`) is a TUI and is awkward headless; prefer `start:headless`.
- **Node version shadowing**: `/exec-daemon/node` (v22.x) can shadow mise's Node 24 inside `mise run` tasks, so `.mts` mise tasks fail with `ERR_UNKNOWN_FILE_EXTENSION`. Run those directly with `mise exec -- node <file.mts>`. The Vite dashboard runs fine regardless (it only prints an "Unsupported engine" warning).
- **Migrations / Atlas**: Atlas Pro login is unavailable in this environment. Postgres schema was applied directly from `server/database/schema.sql` via `psql` (the versioned `mise db:migrate` needs an Atlas login for a `txtar` pre-migration check). ClickHouse uses the `golang-migrate` engine (`CLICKHOUSE_MIGRATION_ENGINE=golang-migrate` in `mise.local.toml`); two migrations contain a `;` inside a `COMMENT` string that golang-migrate's naive splitter mis-parses — apply those with `clickhouse-client -mn < <file>.up.sql` and then `migrate ... force <version>`. DB state lives in Docker volumes and persists in the snapshot, so a full re-migration is usually unnecessary.
- **Enterprise gate**: new orgs are blocked behind a "book a demo" modal unless whitelisted. For local dev: `docker compose exec -T gram-db psql -U gram -d gram -c "UPDATE organization_metadata SET whitelisted = true;"` then log out/in (whitelisting is captured into the session at login).
- **Lint/test**: see the `mise tasks` list — e.g. `mise lint:server` / `mise lint:client`, `mise test:server ./path/...` (uses `testenv` against the running Postgres, so infra must be up), `pnpm -F dashboard type-check`.
