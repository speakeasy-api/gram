...
- **Docker daemon (required before setup).** Docker is installed but not auto-started (no systemd). Start it once per session before running setup or infra: `sudo service docker start`. It is configured for this VM with the `fuse-overlayfs` storage driver and `iptables-legacy`. Note: a daemon restart recreates `/var/run/docker.sock` as `root:docker 0660`; fresh login shells pick up the `docker` group automatically, but a long-lived shell started before group membership may need `sudo chmod 666 /var/run/docker.sock` (or `newgrp docker`) to reach the daemon.
- **One-shot setup: `./zero --agent`.** With the Docker daemon running (and the `mise.local.toml` Node-shim fix above in place), `./zero --agent` runs the full non-interactive setup: installs tools/deps, generates keys/TLS + the dev-idp RSA signing key, builds `elements`, starts infra, runs Postgres + ClickHouse migrations, and prints a service-status summary. It intentionally does NOT start the app services or `madprocs` (agent mode never execs the start task), so bring those up yourself afterwards. `zero` in agent mode auto-installs `mise` if missing and skips all interactive prompts (GitHub/OpenRouter/WorkOS/Fly/hooks).
- **Or bring up the stack manually (order matters):**
  1. `mise run infra:start` — Postgres, Redis, Temporal, ClickHouse, Jaeger, Prometheus, OTEL collector, Pub/Sub emulator, Presidio (via `compose.yml`). Waits for both Postgres and ClickHouse to accept queries before returning.
  2. `mise run db:migrate` then `mise run clickhouse:migrate` — ClickHouse uses the `atlas` engine locally (this VM is already logged into Atlas Cloud; `golang-migrate` is the fallback).
