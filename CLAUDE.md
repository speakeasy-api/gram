...
- **`.mts` tasks need Node 24.** Several `zero`/`seed` steps are TypeScript `.mts` scripts requiring Node ≥ 24. The VM's `/exec-daemon/node` (v22) can shadow mise's Node 24 in task subshells (`ERR_UNKNOWN_FILE_EXTENSION ".mts"`); this is pinned via `_.path = ["/home/ubuntu/.local/share/mise/shims"]` under `[env]` in the gitignored `mise.local.toml`. Re-add that line if the error reappears.
- **`mise` is the tool entrypoint** (`~/.local/bin/mise`, activated in `~/.bashrc`). In non-interactive shells invoke it explicitly or `export PATH="$HOME/.local/bin:$PATH"` first.
