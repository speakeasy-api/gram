...
Full environment setup is handled by `./zero --agent` (idempotent — re-run any time to reconcile): it installs tools/deps, generates keys/TLS + the dev-idp RSA key, starts the container infra (rootless Podman), and runs the Postgres + ClickHouse migrations and finally starts all local services. Run it per session. It is deliberately NOT the startup update script — that stays minimal (`mise install` / `mise run install`), because starting infra and running migrations are too heavy and failure-prone for pod boot. Non-obvious caveats:

- **No container daemon to start.** Local infra runs on rootless Podman (a static build installed by mise — no root needed to install or run). `./zero --agent` writes the Podman config files (`fuse-overlayfs` storage when `/dev/fuse` exists, `vfs` otherwise), then starts the Podman API socket explicitly with `podman system service --time=0` (daemonized, idempotent — no systemd socket activation). `DOCKER_HOST` is exported via mise env to that socket, so compose, testcontainers-go, and atlas `docker://` dev-urls all talk to Podman. In uid-0 sandboxes Podman runs rootful (socket at `/run/podman/podman.sock`) and no uidmap setup applies; on regular machines a missing `newuidmap`/`/etc/subuid` setup makes `./zero` print a ONE-TIME `sudo` instruction — it never runs sudo itself.
- **One compose entrypoint**: `local/lib/compose.sh` defines the `compose` function (docker-compose pointed at the Podman socket, with images pre-built via `podman build`). Scripts source it; never invoke `docker compose` directly. The `docker` CLI on PATH is a repo shim that execs `podman`.
