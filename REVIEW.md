...
- Verify the change assessed whether an existing Platform MCP tool must change or a new outcome-oriented tool should be added. A new endpoint does not automatically need a tool, but a useful repeatable administrator/member outcome must not be silently omitted.
