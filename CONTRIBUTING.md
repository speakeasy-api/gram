...
`wt remove` runs `mise run nuke --keep-shared --delete-namespace` inside the worktree first, so the right Compose stack and its Temporal namespace are torn down before the directory disappears (shared services stay up for other worktrees). Removing a worktree by hand (`git worktree remove`) skips that and leaves its containers, volumes, and namespace running.
