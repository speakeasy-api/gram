...
- **Server Function** (`/chat/server-fn`) — RPC-style `createServerFn` using `createChatSession` from `@gram-ai/elements/server/core`
...
| `src/session.functions.ts`       | Server function using `createChatSession` from `@gram-ai/elements/server/core`     |
