...
16. **Organization setup workstreams** — open `/acme-demo/setup`. Confirm all
    ...
    **Done**, overriding any local manual status.
    Check the seeded task API separately if verifying server assignments or
...
### Setup workstream merge regression

Open `/<ORG_SLUG>/setup` and verify all four workstreams. In **Observe agents**,
open **Set up Anthropic observability** and check logging, inference-hook
configuration, and inference traffic confirmation. In **Secure agent traffic**,
open **Set up Anthropic admin controls** and check logging, marketplace
publication, Claude Cowork, Claude Code, Cursor, and hook traffic confirmation.
