...
| Identities (roster + per-person pages) | PG memberships + `user_accounts`; CH telemetry keyed by email (agent-metrics view) and by `user.id` (raw logs) — one member-less address and one agent id so all three kinds render | `[~]`  |
