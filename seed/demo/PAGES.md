...
Organization onboarding renders these existing durable task rows as workstreams at `/acme-demo/setup`, without Kanban or audience-specific board defaults. Authorized admins can switch between Workstreams and the independent wizard at `/acme-demo/setup/wizard`, preserving task, section, and project context. Card clicks and task deep links open the linear wizard, not a popup. Assignment lives on workstream headers: MCP Gateway has one member owner (including its hidden optional task), Observe retains legacy per-task owners and explicitly shows Mixed owners or Partially assigned, with a concise owner breakdown in the header picker until an owner is explicitly selected for all tasks; Connect is unassigned and shows its suggested IT admin role. Suggested roles appear only in unassigned headers and disappear when a person or email is assigned. Workstream headers show completed/total required tasks and aggregate progress from the existing task statuses; hidden and optional tasks are excluded, and the Assigned to me filter does not change these totals. Customized-preset controls and seed data are inherited from the staff preset layer (#6328).

Workstream reassignment is one atomic server mutation, including hidden and optional tasks. External email assignment sends a task notification, not organization membership. Admins may optionally send a separate team invite with a selected role after assignment succeeds; failed invitations can be retried in the picker without reassigning tasks. Existing mixed-owner seed rows exercise these states; no schema or seed SQL changes are required.

Cards show compact status indicators. Admins can inspect blocked tasks from their menu; the normal card action opens prerequisite navigation. Completing a task entered from Workstreams returns to the board, while the standalone wizard advances. Platform admins can inspect explicitly selected hidden tasks without adding them to the normal wizard sequence. Reader access stays on the board with an explanation that setup execution requires an organization admin.

The ordered workstream catalog in `server/internal/organizations/setup_workstreams.go` is the source of titles, task membership, and ordering for the customer board, Admin onboarding selection, and whole-workstream assignment. Both onboarding read APIs return this catalog, including hidden task keys; visibility still comes from the organization's task rows. The existing seed rows cover this projection without a SQL or schema change.
