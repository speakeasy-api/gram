...
| Organization onboarding (wizard, settings, product-page guidance) | PG `organization_onboarding_answers` ×1 (observability, Jamf) + `organization_onboarding_providers` ×2 (Anthropic on Enterprise, Cursor on Teams) + `organization_onboarding_products` ×2 (Claude Code CLI, Cursor; resolved by slug from the server-synced catalog) + `organization_onboarding_steps` ×1 verified, so the next step reads "Turn on Anthropic inference hooks"                                                                                                                                                                                                                                                                                                                 | `[~]`  |
