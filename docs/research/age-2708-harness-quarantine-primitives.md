...
2. That write also populates a Redis circuit entry, e.g. `quarantine:session:{org}:{conversation_id}`, mirroring `spend_gate:actor:*` (TTL + evaluator/API refresh so a lift propagates within seconds).
...
4. Deny rendering reuses the block machinery (`recordToolCallBlockAsync`, block view links) so quarantine denies audit identically to policy blocks.

Fail-mode: follow the spend gate and fail open on cache infrastructure errors, but note the asymmetry: quarantine is a containment action, so an org-level fail-closed toggle (as AIS-402 introduced for scan deadlines) is worth carrying through the design rather than hard-coding fail-open.
...
1. AGE-2707's `quarantine` action writes session quarantine state and mirrors it into a Redis circuit keyed `(org, conversation_id)`, modeled on `spendrules.Gate` (evaluator/API refresh + TTL so lifts propagate without waiting for expiry).
2. Add `checkQuarantineGate` beside `checkSpendGate` in `evaluateCanonicalHook` and the three legacy provider paths, denying `prompt.submitted` and `tool.requested` (covers before_tool_use, before_mcp_execution, permission_request). Existing adapters render the deny natively for Claude (`permissionDecision: "deny"`), Codex (`decision: "deny"`), and Cursor (`permission: "deny"`); prompt denies freeze new turns, tool denies freeze the current one. Reuse block recording so quarantine denies audit like policy blocks.
...
- Cursor: hook denies across shell/MCP/read/preToolUse plus `beforeSubmitPrompt` `continue: false`. Set `failClosed: true` on the gated hooks so a Gram outage cannot be exploited to escape quarantine (unique among harnesses). Gap: cloud agents skip `beforeMCPExecution` and prompt hooks; the Cloud Agents cancel API is the fallback there if we ever manage those.
...
Fail-mode: default fail-open at the Gram layer (matching the spend gate) because the harness side already fails open on timeout everywhere except Cursor failClosed; a slow fail-closed check would punish every session to guard a rare outage. Carry an org-level fail-closed option through the design (as AIS-402 did for scan deadlines) for orgs that prefer containment over availability, and pair it with Cursor `failClosed` and deny-by-default permission postures where the org's managed config allows.
