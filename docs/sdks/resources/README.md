...
* [list](#list) - listResources resources
...
<!-- UsageSnippet language="typescript" operationID="listResources" method="get" path="/rpc/resources.list" -->
```typescript
...
async function run() {
  const result = await gram.resources.list();
...
async function run() {
  const res = await resourcesList(gram);
  if (res.ok) {
...
import {
  ...
  invalidateListResources,
  invalidateAllListResources,
} from "@gram/client/react-query/resourcesList.js";
```
...
**Promise\<[components.ListResourcesResult](../../models/components/listresourcesresult.md)\>**
