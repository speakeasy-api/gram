...
- [ ] **Step 4: Wire into `ingestRequest`** in `impl.go`: replace `McpInventory: nil, McpInventoryCollected: nil` in the `prompt.submitted` payload with
  ...
  **Important:** `ingestRequest` returns early with `noneResult()` when `prompt == ""` (line ~150). A tools-only request with no text must still record inventory. Move the inventory computation above that early return and, when `prompt == ""` but `len(mcpInventory) > 0`, send the hook event as `session.updated` with `Data.Prompt = nil` instead of `prompt.submitted`: the hooks path upserts the inventory before it switches on the event type, so the row is written, while an empty `prompt.submitted` would run the quarantine gate and prompt scan against nothing and record an empty prompt as a telemetry row. Leave `McpInventoryCollected` nil — the proxy sees one request's tools, not the session's configured servers, and that flag is what licenses the hooks path to cache the list as the session's enforcement state.
...
- [ ] **Step 6: Wire** in `ingestResponse` after the ingest call succeeds:
  ...
  Also run it when the assistant turn was **dropped as a duplicate** of a native hook stream? Yes — a native device stream carries no per-call output tool-call id, so only the proxy produces these rows and the two channels never double-count. What does repeat is LiteLLM itself: the response guardrail fires on sampled chunks of a streamed call and again on retries, each time with the accumulated `tool_calls`. Remember the recorded tool-call ids on the call-cache record (`callcache.Record.ProvenanceToolCallIDs`), skip ids already recorded, and store the updated record after writing. A cache miss records unconditionally.
...
- Modify: `server/internal/telemetry/repo/queries.sql.go` — `ListShadowMCPInventoryUsage` folds the per-trace rows that `listShadowMCPInventoryTraceUsage` returns (each already carrying `max(hook_source)`) into one usage row per canonical URL in Go. Add `Sources []string` to `ShadowMCPInventoryUsageRow` and collect each trace's `HookSource` into it during that fold (trimmed, empty dropped, de-duplicated, sorted). No SQL change is needed.
