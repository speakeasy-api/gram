...
The Go side shipped as designed across PRs 1-5. The PostHog and Slack side
diverged from the plan above in four ways. This section records what actually
exists, so the design above can be read as the reasoning and this as the state.

### Channels were renamed, and AICP got its own significant channel

The legacy channels were renamed to make it explicit that they carry SDK
signals, and AICP moved to its own pair:

| Design                    | As built               | Carries                   |
| ------------------------- | ---------------------- | ------------------------- |
| `#ops-significant-events` | `#ops-sdk-sig-events`  | SDK significant events    |
| `#ops-all-events`         | `#ops-sdk-events`      | SDK firehose              |
| —                         | `#ops-aicp-sig-events` | AICP significant events   |
| `#ops-aicp-events`        | `#ops-aicp-events`     | AICP firehose (unchanged) |
| —                         | `#ops-aicp-dev-events` | All non-prod AICP events  |

Everything the design routed to `#ops-significant-events` now routes to
`#ops-aicp-sig-events`. `#ops-sdk-*` carries no `gram_activity` at all.

### Destinations as built

Production, PostHog project 10122, Slack integration 57009:

| Destination                   | Channel                | Fires on                                                       |
| ----------------------------- | ---------------------- | -------------------------------------------------------------- |
| `AICP - significant activity` | `#ops-aicp-sig-events` | `gram_activity` where `activity` is one of the six significant |
| `AICP - activity firehose`    | `#ops-aicp-events`     | every `gram_activity`                                          |
| `AICP - subscription changes` | `#ops-aicp-sig-events` | `gram_subscription_changed`                                    |
| `AICP - feature requests`     | `#ops-aicp-sig-events` | `feature_requested`                                            |
| `AICP - enterprise gate`      | `#ops-aicp-sig-events` | `enterprise_gate_viewed`                                       |

Non-prod, PostHog project 96574:

| Destination         | Channel                | Fires on                                   |
| ------------------- | ---------------------- | ------------------------------------------ |
| `AICP dev firehose` | `#ops-aicp-dev-events` | all four event types above, in one channel |

The dev destination sets `filter_test_accounts: false`. This inverts the
production setting deliberately: the test-account filter excludes
`%speakeasyapi.dev%` and `%speakeasy.com%` person emails, and every non-prod
event is generated by a Speakeasy developer. Left at `true` it would produce a
destination that is green, error-free, and permanently silent.

Environment separation is by PostHog project, not by event property. Server-side
events carry no `environment` or `$host` property, so a destination cannot filter
dev traffic out by property — correctness depends on `POSTHOG_API_KEY` pointing
at the right project. Note also that a local stack emits nothing at all: with no
`POSTHOG_API_KEY` set, `posthog.New` returns a disabled client and every capture
is dropped. Only deployed environments produce `gram_activity`.

### Branding: username and icon override

The stock `template-slack` hog passes `icon_emoji`. Every destination here
replaces it with `icon_url`, which requires a custom hog body and a matching
`inputs_schema` entry, and the `chat:write.customize` scope. Messages post as
`Control Plane Ops` for AICP and `SDK Ops` for SDK, with the Speakeasy icon.

Two API behaviours are worth knowing before editing these:

- Sending `inputs` without `inputs_schema` silently drops any key the stored
  schema does not know about. Always send both together.
- Editing `hog`, `inputs` or `inputs_schema` on an _enabled_ destination creates
  a draft rather than going live. Publishing is two calls: preview with no
  `confirm` to obtain a token, then `confirm: true` with that token. A partial
  update touching only `enabled` bypasses this entirely.

### Admin deep-link button

AICP destinations carry an "Open org in Admin" button to
`https://<ADMIN_HOST>/organizations/{slug}`, where `<ADMIN_HOST>` is the
internal Gram admin host. The slug is resolved with a chained fallback across
`organization_slug` and `org_slug` because the three carried-over events use
different property names, ending in `''` so the URL is always syntactically
valid. An empty `url` makes Slack reject the entire message, not just the
button — the same constraint the appendix records for `dashboard_url`.

The button is omitted from the dev destination: the admin host serves production
only, so a dev slug would either 404 or open an unrelated real organization.

The SDK channels do not have this button. The SDK admin has no per-organization
route to link to.
