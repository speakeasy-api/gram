...
This guide will walk you through connecting Mistral to a [Gram-hosted MCP server](/blog/release-gram-beta) using the example Acme Todo API. You'll learn how to set up the connection, configure the custom connector, and test it with natural language prompts to manage your todos.
...
You now have Mistral connected to a Gram-hosted MCP server built from the Acme Todo API. Ready to build your own? [Try Gram today](/product/gram) and turn any OpenAPI into agent-ready tools your models can use safely.