...
This guide will show you how to connect n8n to a [Gram-hosted MCP server](/blog/release-gram-beta) using the example Push Advisor API from the [Gram concepts guide](/mcp/core-concepts). You'll learn how to set up the connection, test it, and use natural language to perform vibe checks before running automated deployments.
