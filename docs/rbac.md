...
directory_group:018f8d7b-58d7-7cc4-bb16-9f8c6b99a001
directory_attribute:<base64url-key>:<base64url-value>
```
...
Directory principals assign grants to identity-provider groups and attributes instead of individual users or roles. `directory_group:<uuid>` is a synced directory group. `directory_attribute:<key>:<value>` is a custom attribute pair such as `department=Engineering` or `job_title=I.T Admins`; the key and value are base64url-encoded so they cannot collide with URN separators.

The current RBAC implementation supports `user`, `role`, `directory_group`, and `directory_attribute` principals. We expect to migrate the current API key system into RBAC eventually, which would introduce an `api_key` principal.

A request's effective grants are normally loaded from:

- the authenticated user principal, such as `user:user_01abc`
- the user's assigned role principals, such as `role:global:<role-uuid>` or `role:organization:<role-uuid>`
- any directory group and identity-provider attribute principals linked to that user

This lets us give most access through roles, grant extra scopes to a department or job title, and still allow direct user grants when needed. Directory mappings are Gram-local supplemental grants. They do not replace WorkOS or SCIM role assignment.
...
| Create, update, or delete directory group/attribute permission mappings                  | `org:admin`                                                                                                  | Organization ID                                                       | Directory mappings assign grants to IdP groups and attributes. They are Gram-local and do not change WorkOS or SCIM role assignment.                                                                                                                                                                                                                                                                                                                                                                                        |
| Open Access / RBAC page                                                                  | `org:read` OR `org:admin`                                                                                    | Organization ID                                                       | Viewing access state can be read-only; role, grant, member-role, directory-mapping, and challenge-resolution actions are `org:admin`.                                                                                                                                                                                                                                                                                                                                                                                       |
...
Before handlers run, the auth middleware prepares an auth context. RBAC grant loading is handled by `authz.Engine.PrepareContext`. For an eligible request, the engine reads the auth context, builds principal URNs for the user, their assigned roles, and any linked directory groups or identity-provider attributes, loads the matching rows from `principal_grants`, and stores those grants back on the request context.
