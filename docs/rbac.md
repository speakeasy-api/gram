...
| Open Secure pages: Risk Overview, Risk Policies, Approval Requests, Detection Rules                   | `org:admin`                                                                                                  | Organization ID                                                       | Checked as the internal `risk_policy:read`, which `org:admin` satisfies by expansion. The scope is not assignable — only the shared demo org holds it directly, so its read-only sessions can browse the Secure pages. Every risk mutation still checks `org:admin`. `risk_policy:evaluate` and `risk_policy:bypass` are policy runtime/request-flow scopes, not dashboard page grants.                                                                                                                                     |
