...
- `logs:*` -> `logs` (the `resource_id` is the organization id)
- `root` -> `*`
...
and `logs` selectors can use:

- `actor_department`
- `actor_group`
- `actor_role`

The `logs` dimensions are different in kind from the `mcp` ones: they do not narrow the resource being read, they narrow the population whose activity may be read. A handler proves the caller holds `logs:read` with a dimensionless check — which a narrowed grant satisfies — and then applies the narrowing to the rows it returns. A handler that cannot filter its rows by actor asks for the scope with strict selector matching instead, so a narrowed grant fails the check rather than being served data it cannot scope.
...
| Read observability data on any Observe page                                              | `logs:read`, unrestricted or narrowed                                                                        | Organization ID                                                       | Held in addition to the `org:admin` / `project:read` gate the page already applies. Narrowing a grant with an `actor_department`, `actor_group`, or `actor_role` selector restricts what the holder sees: searchLogs, searchChats, listToolUsageTraces, listHooksTraces, listSessions, and telemetry.query return only that population's activity plus the holder's own. Every other telemetry read requires the scope unrestricted and returns 403 for a narrowed grant rather than over-serving it. A caller holding no `logs:read` at all sees only their own activity on the scoped reads, and is refused on the rest. Actor dimensions resolve through current directory profiles and role assignments, not the identity snapshot on each event, so someone who changes department takes their history with them. |
