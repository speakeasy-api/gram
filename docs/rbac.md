...
| Open Secure pages: Risk Overview, Risk Policies, Approval Requests, Detection Rules                   | `risk_policy:read` OR `org:admin`                                                                            | Organization ID                                                       | Read-only viewing of policies, detection rules, findings, and the risk dashboards. `org:admin` satisfies `risk_policy:read` by expansion, so existing admins are unaffected and no system role grants it by default. `risk_policy:evaluate` and `risk_policy:bypass` are policy runtime/request-flow scopes, not dashboard page grants.                                                                                                                                                                                     |
| Create, edit, or delete a risk policy, detection rule, or exclusion; act on a finding                 | `org:admin`                                                                                                  | Organization ID                                                       | Every risk mutation, plus the authoring tools (`compileExpr`, `testDetectionRule`, the `suggest*` endpoints), false-positive marking, and bypass approve/deny/revoke, stays `org:admin`. `risk_policy:read` grants none of these.                                                                                                                                                                                                                                                                                           |
