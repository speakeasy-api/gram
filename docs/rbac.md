...
const (
	...
	ScopeRoleWrite     Scope = "role:write"
...
`role:write` is a narrow access-management scope for delegating role administration without granting full org admin. A role holding only `role:write` can open the Access page (create/edit/delete roles, assign roles to members, resolve authorization challenges) and the IDP & SSO page (launch the WorkOS admin portal to configure identity-provider group-to-role mappings). It does **not** imply `org:read` or `org:admin`, so it grants no access to Observe (Costs, Agent Sessions, Tool Logs), Secure (Risk), billing, API keys, org logs, webhooks, audit logs, or any project. `org:admin` satisfies `role:write` via scope expansion, so full org admins keep every access-management capability without a separate grant. This is the "control org roles but not see cost/risk/sessions" role.
...
`admin` does not need `role:write` in its explicit grant list because `org:admin` satisfies it via scope expansion. `role:write` is intended for **custom** roles that should administer access (roles, member-role assignments, and identity-provider group-to-role mappings) without seeing Observe, Secure, billing, or any project data.
...
| Create/update/delete roles, assign roles to members, resolve authorization challenges                 | `role:write`                                                                                                 | Organization ID                                                       | The dedicated access-management scope. `org:admin` satisfies it via scope expansion, so admins are unaffected. Holding it alone grants no other org access.                                                                                                                                                                                                                                                                                                                                                                 |
| Open Access / RBAC page                                                                               | `org:read` OR `org:admin` OR `role:write`                                                                    | Organization ID                                                       | Viewing access state can be read-only (`org:read`); role, grant, member-role, and challenge-resolution actions require `role:write` (or `org:admin`, which expands to it).                                                                                                                                                                                                                                                                                                                                                  |
...
| Open IDP & SSO page and configure identity-provider group-to-role mappings                            | `org:read` OR `org:admin` OR `role:write`                                                                    | Organization ID                                                       | Launching the WorkOS admin portal (`organizations.generateWorkOSAdminPortalLink`, SSO / Directory Sync intents) requires `role:write` (or `org:admin`, which expands to it). This is where Okta/directory groups are mapped to Speakeasy roles.                                                                                                                                                                                                                                                                             |
...
Use the narrowest scope that describes the operation. GET and list handlers usually need a `*:read` scope. Mutations usually need `*:write`. Runtime MCP usage needs `mcp:connect`, while role and RBAC management need `role:write` (which `org:admin` satisfies via expansion).
