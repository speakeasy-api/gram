...
| View the Access Hub: trusted platforms, the machines allowed under each, assigned agents | `workload:read` OR `workload:write`                                                                          | Unrestricted (organization-wide)                                      | The trust policy is configured for the organization as a whole, so the grant names no individual resource. Not a member default: `workload:read` alone discloses which machines the organization recognises.                                                                                                                                                                                                                                                                                                                                                                                |
| Trust or stop trusting a platform, allow or withdraw a machine, assign its agent         | `workload:write`                                                                                             | Unrestricted (organization-wide)                                      | Admitting a subject is itself the grant of machine access, and assigning an agent hands a machine that agent's authority without requiring anything on the agent (decided on AIM-291). An admin system-role default; members need an explicit grant through a custom role.                                                                                                                                                                                                                                                                                                                  |
