...
| Open Skills / CLIs                                                                                    | `project:read` when Skills is disabled; `skill:read` when enabled | Project ID                                                            | The org-level `skills` product feature controls both the page and navigation gate. Loading or failed feature checks degrade to the disabled Coming Soon surface and its `project:read` gate. Enabling Skills provisions wildcard `skill:read` for Member and wildcard `skill:read`/`skill:write` for Admin when RBAC is already active; disabling Skills leaves those grants intact.                        |
