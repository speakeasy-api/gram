...
| Run the onboarding wizard or edit onboarding answers (`/onboarding`, `/onboarding/settings`) | `org:admin`                                                                                                  | Organization ID                                                       | Behind the `gram-new-onboarding` PostHog flag. Both pages are page-gated on `org:admin`; `onboarding.saveStack`, `onboarding.selectUseCase` and `onboarding.verifyStep` require `org:admin` and are audited against the organization. `onboarding.get`, `onboarding.listReferenceData` and `onboarding.getUseCaseStatus` accept `org:read`, so members can see onboarding state and the product pages can read use-case coverage.                                                                                                                                                           |
