...
`telemetry:read` protects the Observe dashboard surface (Costs, MCP & Tools Insights, Employee Enrollment, Agent Sessions, and Tool Logs). It is granted to `admin` but **not** to `member`. The Observe nav group stays visible to any project member (like the Secure section); opening a page without `telemetry:read` renders the page-level "Access restricted" notice. It is a dedicated scope rather than reusing `environment:read` so the two can be granted independently.
...
| Open the Observe section: Costs, MCP & Tools Insights, Employee Enrollment, Agent Sessions, Tool Logs | `telemetry:read`                            | Project ID                                                            | Every Observe page is gated on `telemetry:read` at the page level (basic members do not hold it). The Observe nav group and command-palette entries stay visible to project members; opening a page without the scope renders the page-level "Access restricted" notice (the Secure-section pattern).                                                                                                            |
