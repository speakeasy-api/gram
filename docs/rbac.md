...
`org:manage_roles` is a narrow access-management scope for delegating role administration without granting full org admin. A role holding only `org:manage_roles` can open the Access page (create/edit/delete roles, assign roles to members, resolve authorization challenges) and the IDP & SSO page (launch the WorkOS admin portal to configure identity-provider group-to-role mappings). It does **not** imply `org:read` or `org:admin`, so it grants no access to Observe (Costs, Agent Sessions, Tool Logs), Secure (Risk), billing, API keys, org logs, webhooks, audit logs, or any project. `org:admin` satisfies `org:manage_roles` via scope expansion, so full org admins keep every access-management capability without a separate grant. This is the "control org roles but not see cost/risk/sessions" role.
...
`admin` does not need `org:manage_roles` in its explicit grant list because `org:admin` satisfies it via scope expansion. `org:manage_roles` is intended for **custom** roles that should administer access (roles, member-role assignments, and identity-provider group-to-role mappings) without seeing Observe, Secure, billing, or any project data.
...
| Invite members to the organization                                                                    | `org:admin`                                                                                                  | Organization ID                                                       | Inviting/removing WorkOS members stays full org administration.                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Create/update/delete roles, assign roles to members, resolve authorization challenges                 | `org:manage_roles`                                                                                           | Organization ID                                                       | The dedicated access-management scope. `org:admin` satisfies it via scope expansion, so admins are unaffected. Holding it alone grants no other org access.                                                                                                                                                                                                                                                                                                                                                                 |
| Open Access / RBAC page                                                                               | `org:read` OR `org:admin` OR `org:manage_roles`                                                              | Organization ID                                                       | Viewing access state can be read-only (`org:read`); role, grant, member-role, and challenge-resolution actions require `org:manage_roles` (or `org:admin`, which expands to it).                                                                                                                                                                                                                                                                                                                                            |
...
| View organization domains, org logs, webhooks, audit logs, device agent                               | `org:read` OR `org:admin`                                                                                    | Organization ID                                                       | Mutating settings on these pages, including domains, forwarding, webhooks, and device-agent token generation, requires `org:admin`.                                                                                                                                                                                                                                                                                                                                                                                         |
| Open IDP & SSO page and configure identity-provider group-to-role mappings                            | `org:read` OR `org:admin` OR `org:manage_roles`                                                              | Organization ID                                                       | Launching the WorkOS admin portal (`organizations.generateWorkOSAdminPortalLink`, SSO / Directory Sync intents) requires `org:manage_roles` (or `org:admin`, which expands to it). This is where Okta/directory groups are mapped to Speakeasy roles.                                                                                                                                                                                                                                                                       |
...
| Open an individual agent session transcript (`chat.load`)                                             | `chat:read` (or owner)                                                                                       | Chat session                                                          | Reaching the Agent Sessions list first requires `org:admin` (row above). Anyone can always open sessions they own (owner-bypass: `chat.user_id == caller`), with no `chat:read` grant. Opening another user's session requires an explicit unrestricted `chat:read` — not a default of any system role, so even admins must be granted it. Every open is recorded in the audit log (`chat_session:access`).                                                                                                                 |
...
Use the narrowest scope that describes the operation. GET and list handlers usually need a `*:read` scope. Mutations usually need `*:write`. Runtime MCP usage needs `mcp:connect`, while role and RBAC management need `org:manage_roles` (which `org:admin` satisfies via expansion).
