...
| Open an Environment detail page                                                     | `project:read`                              | Project ID                                                            | Adding, editing, deleting, or filling variables requires `environment:write` (the environment scope family is independent from `project:*` because env values include secrets). Environment-specific clone checks use `environment:read` for the source and `environment:write` for the destination.                               |
