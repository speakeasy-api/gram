...
| View mapped Slack accounts on a person’s Accounts & devices page                         | `org:read` to open the identity page; exact caller ID or `org:admin` for the Slack read                      | Organization ID and exact Gram user ID                                | Requires the `claude_tag_support` product feature, a browser session and active caller/subject organization memberships. No email fallback. Employees see only their own mappings, with contact-admin guidance. Admin Review links open the exact membership; all corrections require `org:admin` and explicit confirmation. Inactive people are reviewed through the org table. Mappings grant no extra permissions.                                                                                                                                                                                           |
