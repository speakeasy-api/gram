...
`telemetry:read` protects the Observe dashboard surface (Costs, MCP & Tools Insights, Employee Enrollment, Agent Sessions, and Tool Logs). It is granted to `admin` but **not** to `member`, so basic members do not see the Observe section. It is a dedicated scope rather than reusing `environment:read` so the two can be granted independently.
...
| Open the Observe section: Costs, MCP & Tools Insights, Employee Enrollment, Agent Sessions, Tool Logs | `telemetry:read`                            | Project ID                                                            | The whole Observe nav group and every Observe page are gated on `telemetry:read`, which basic members do not hold. The group (label included) is hidden in the sidebar and command palette, and direct navigation renders the page-level Unauthorized fallback.                                                                                                                                                  |
