...
`chat:read` protects agent session transcripts (`chat.load`). Only admins hold it (it grants read access to every member's sessions). Members hold no `chat:read` grant; they can always read sessions they own — the `chat.load` handler bypasses the scope check when the caller owns the session (`chat.user_id == caller`), and the session list filters to the caller's own sessions when they lack `chat:read`. Reading another user's session requires the unrestricted `chat:read` grant.
...
| Open an individual agent session transcript (`chat.load`)                           | `chat:read` (or owner)                      | Chat session                                                          | Members can always open sessions they own (owner-bypass: `chat.user_id == caller`), with no `chat:read` grant. Opening another user's session requires an unrestricted `chat:read` (admins only). Every open is recorded in the audit log (`chat_session:access`). |
