...
| See the global billing status banners (failed payment, chat spend cap reached)                        | `org:read` OR `org:admin`                                                                                    | Organization ID                                                       | The banners ride the page header, so they appear on every organization and project page. Both reads — the Stripe subscription and `chat.creditUsage` — sit below a pay-as-you-go tier check and an `org:read` gate, so a member without it issues no request at all. Their actions are `org:admin`: without it the banner shows an ask-an-admin note instead of the Stripe portal button or the spend cap link.                                                                                                             |
| Read chat credit usage and the monthly chat spend cap (`chat.creditUsage`)                            | `org:read` OR `org:admin`                                                                                    | Organization ID                                                       | Enforced server-side against the active organization; the dashboard usage meters, the spend cap section, and the cap banner all read it. Setting the cap (`usage.setSpendCap`) is `org:admin`.                                                                                                                                                                                                                                                                                                                              |
