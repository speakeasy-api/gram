...
- directory sync users, including deprovisioning members whose directory
  user is deactivated in WorkOS but still has live organization access

The entrypoint is `server/cmd/workos-backfill`.

## Directory sync deactivations
...
concurrent live event (for example a re-add) wins over the snapshot.

The preflight reports these as `directory_user_rows` (local row changes) and
`directory_deprovision_rows` (members that WOULD lose access), with per-user
`directory_deprovision` entries in the planned change details. Review that
list before confirming a write.

Limitation: the script talks to Postgres only and cannot invalidate the
user-info cache the way the event pipeline does. A deprovisioned member's
cached org access lingers until the cache TTL expires (minutes, not hours).
...
Prod writes clear **two** independent gates. `--confirm-prod=prod` only covers
connecting to prod; the write itself always prompts for `backfill` on stdin,
and `--auto-approve` deliberately does not skip it for prod. Run the command
interactively, or supply the second confirmation on stdin:

```sh
echo backfill | mise backfill:workos --phase=all --environment=prod --cloudsql-proxy --dry-run=false --confirm-prod=prod
```

Non-prod writes prompt for `backfill` too, but there `--auto-approve` skips it.
...
The script sets `lock_timeout=5s` and `statement_timeout` (default `30m`, see
`--statement-timeout`) for DB sessions.
