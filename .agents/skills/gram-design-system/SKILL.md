...
`@speakeasy-api/moonshine` was internalized into `client/dashboard/src/components/ui`. There is no longer any external npm dependency, **and no `components/ui/moonshine` subtree or barrel** — the vendored components were flattened into the single flat `components/ui/` directory. Never `import` from `"@speakeasy-api/moonshine"` or `"@/components/ui/moonshine"`; both are denied by lint (`no-restricted-imports` in `client/dashboard/.oxlintrc.json`).

Everything is imported **per file** — `import { Button } from "@/components/ui/button"` — never from a barrel. There is exactly **one implementation per UI pattern**; always import the winner below, never reintroduce a losing implementation, never hand-roll a duplicate.

## Component catalog — what to import from where

### Vendored-origin components (now flat in `@/components/ui/<name>`)

| Component                                                                                                                                           | File                    | Notes                                                                                                                                                           |
| --------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Button`                                                                                                                                            | `button.tsx`            | All buttons — never a raw `<button className="...">`.                                                                                                           |
| `Badge`                                                                                                                                             | `badge.tsx`             | All badges, including `ReleaseStageBadge`'s primitive.                                                                                                          |
| `Alert`                                                                                                                                             | `alert.tsx`             | Inline alert/callout banners.                                                                                                                                   |
| `Input`                                                                                                                                             | `input.tsx`             | Plain text inputs.                                                                                                                                              |
| `Link`                                                                                                                                              | `link.tsx`              | In-copy link styling (adds an icon; heavy for inline links inside prose — see the `frontend` skill's Navigation section for when to use a plain `<a>` instead). |
| `Table`, `Column`                                                                                                                                   | `table/`                | All dashboard data tables — see the `frontend` skill's Tables section for the declarative/compound API split.                                                   |
| `DropdownMenu`, `DropdownMenuContent`, `DropdownMenuItem`, `DropdownMenuLabel`, `DropdownMenuGroup`, `DropdownMenuTrigger`, `DropdownMenuSeparator` | `dropdown-menu.tsx`     | All dropdown menus.                                                                                                                                             |
| `Stack`, `Grid`                                                                                                                                     | `stack.tsx`, `grid.tsx` | Responsive flex/grid layout primitives.                                                                                                                         |
| `CodeSnippet`                                                                                                                                       | `code-snippet/`         | Syntax-highlighted code block.                                                                                                                                  |
| `Key`, `KeyHint`                                                                                                                                    | `key-hint.tsx`          | Keyboard-key primitives (`Kbd` in `kbd.tsx` wraps these).                                                                                                       |
| `ResizablePanel`                                                                                                                                    | `resizable-panel.tsx`   |                                                                                                                                                                 |
| `ThemeSwitcher`                                                                                                                                     | `theme-switcher.tsx`    | Light/dark toggle. Theme state is `@/contexts/Theme` (`ThemeProvider`) + `useTheme` from `@/contexts/theme-context`.                                            |

There is **no `Icon` export** — it was retired during consolidation. Import icons statically from `lucide-react`. Icon names resolved at runtime from data go through `DynamicIcon` in `@/components/ui/dynamic-icon`. `cn` is imported from `@/lib/utils`.

### shadcn-derived components (`@/components/ui/<name>`)
...
| `Card`, `MiniCard`, `DashboardCard` | `card.tsx`, `card-mini.tsx`, `dashboard-card.tsx` | Pick the variant that matches the surface; don't hand-roll a bordered `<div>`. `Card`'s `icon` prop adds the dot-pattern sidebar the old `DotCard` carried. |
...
- **Squared corners.** The theme collapses the entire Tailwind radius scale to `0px` (`--radius-xs` through `--radius-4xl` in `components/ui/styles/global.css`). Never add a `rounded-*` utility to work around this — the only sanctioned exception is `rounded-full`, reserved for dots and avatars (circles).
...
- **No `export const Thing = Object.assign(Root, {...})`.** That exact form trips `react/only-export-components`, and disabling the rule is not an option. Attach members by property mutation instead (`Root.Item = Item; export { Root as Thing }`). Note that TypeScript only supports this expando-property assignment on function declarations and const-bound function expressions — for a `React.forwardRef(...)` result you must keep `Object.assign` and export via a separate `export { X as Thing }` statement, which the rule accepts.
- **No React context in a file that also exports a component.** Same rule, same reason: put the `createContext` call in its own module (see `contexts/theme-context.ts`, with the provider component in `contexts/Theme.tsx`).
