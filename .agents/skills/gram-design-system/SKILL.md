...
There is **no `Icon` export** — it was retired during consolidation. Import icons statically from `lucide-react`. Icon names resolved at runtime from data go through `DynamicIcon` in `@/components/ui/dynamic-icon`.
...
- **No React context in a file that also exports a component.** Same rule, same reason: put the `createContext` call in its own module (see `components/ui/moonshine/context/config-context.ts`).
