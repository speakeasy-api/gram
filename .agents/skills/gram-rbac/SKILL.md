...
**Organization provisioning.** `authz.Provisioner` seeds both built-in roles and their grants for every organization created through Gram. `ProvisionOrganizationAdmin` performs that seed and assigns the first user to `SystemRoleAdmin` in one transaction. The `identity` leaf package never imports `authz`. Local login-time membership sync injects `authz.Provisioner` as `identity.SystemRoleSeeder` and copies the current WorkOS membership role slugs into `organization_role_assignments` so mock-WorkOS / dev-idp user switches still get RBAC without events. Staging and prod leave that seeder nil; WorkOS organization event reconciliation calls `SeedSystemRoleGrantsTx` for organizations discovered through WorkOS.
