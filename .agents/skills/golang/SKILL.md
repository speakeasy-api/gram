...
Go code that sends transactional email (`server/internal/email` templates, `Variables()`, `AddToAudience()`, `email.NewService`, capture-client tests) is covered by the `transactional-email` skill. Activate it for any change there.
