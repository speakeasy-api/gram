// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        (unknown)
// source: gram/risk/v1/prompt_injection_analysis.proto

package riskv1

import (
	_ "github.com/speakeasy-api/gram/infra/gen/gcp/pubsub/v1"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type PromptInjectionAnalysis struct {
	state                             protoimpl.MessageState               `protogen:"opaque.v1"`
	xxx_hidden_RequestId              *string                              `protobuf:"bytes,1,opt,name=request_id,json=requestId"`
	xxx_hidden_ChatMessageId          *string                              `protobuf:"bytes,2,opt,name=chat_message_id,json=chatMessageId"`
	xxx_hidden_ProjectId              *string                              `protobuf:"bytes,3,opt,name=project_id,json=projectId"`
	xxx_hidden_OrganizationId         *string                              `protobuf:"bytes,4,opt,name=organization_id,json=organizationId"`
	xxx_hidden_RiskPolicyId           *string                              `protobuf:"bytes,5,opt,name=risk_policy_id,json=riskPolicyId"`
	xxx_hidden_RiskPolicyVersion      int64                                `protobuf:"varint,6,opt,name=risk_policy_version,json=riskPolicyVersion"`
	xxx_hidden_CreatedAt              *string                              `protobuf:"bytes,7,opt,name=created_at,json=createdAt"`
	xxx_hidden_Content                *string                              `protobuf:"bytes,8,opt,name=content"`
	xxx_hidden_UserId                 *string                              `protobuf:"bytes,9,opt,name=user_id,json=userId"`
	xxx_hidden_L1Enabled              bool                                 `protobuf:"varint,10,opt,name=l1_enabled,json=l1Enabled"`
	xxx_hidden_MessageType            *string                              `protobuf:"bytes,11,opt,name=message_type,json=messageType"`
	xxx_hidden_Body                   *string                              `protobuf:"bytes,12,opt,name=body"`
	xxx_hidden_ToolName               *string                              `protobuf:"bytes,13,opt,name=tool_name,json=toolName"`
	xxx_hidden_ToolCalls              *[]*PromptInjectionAnalysis_ToolCall `protobuf:"bytes,14,rep,name=tool_calls,json=toolCalls"`
	xxx_hidden_PriorUserRequest       *string                              `protobuf:"bytes,15,opt,name=prior_user_request,json=priorUserRequest"`
	xxx_hidden_RecentUntrustedContent *string                              `protobuf:"bytes,16,opt,name=recent_untrusted_content,json=recentUntrustedContent"`
	XXX_raceDetectHookData            protoimpl.RaceDetectHookData
	XXX_presence                      [1]uint32
	unknownFields                     protoimpl.UnknownFields
	sizeCache                         protoimpl.SizeCache
}

func (x *PromptInjectionAnalysis) Reset() {
	*x = PromptInjectionAnalysis{}
	mi := &file_gram_risk_v1_prompt_injection_analysis_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PromptInjectionAnalysis) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PromptInjectionAnalysis) ProtoMessage() {}

func (x *PromptInjectionAnalysis) ProtoReflect() protoreflect.Message {
	mi := &file_gram_risk_v1_prompt_injection_analysis_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *PromptInjectionAnalysis) GetRequestId() string {
	if x != nil {
		if x.xxx_hidden_RequestId != nil {
			return *x.xxx_hidden_RequestId
		}
		return ""
	}
	return ""
}

func (x *PromptInjectionAnalysis) GetChatMessageId() string {
	if x != nil {
		if x.xxx_hidden_ChatMessageId != nil {
			return *x.xxx_hidden_ChatMessageId
		}
		return ""
	}
	return ""
}

func (x *PromptInjectionAnalysis) GetProjectId() string {
	if x != nil {
		if x.xxx_hidden_ProjectId != nil {
			return *x.xxx_hidden_ProjectId
		}
		return ""
	}
	return ""
}

func (x *PromptInjectionAnalysis) GetOrganizationId() string {
	if x != nil {
		if x.xxx_hidden_OrganizationId != nil {
			return *x.xxx_hidden_OrganizationId
		}
		return ""
	}
	return ""
}

func (x *PromptInjectionAnalysis) GetRiskPolicyId() string {
	if x != nil {
		if x.xxx_hidden_RiskPolicyId != nil {
			return *x.xxx_hidden_RiskPolicyId
		}
		return ""
	}
	return ""
}

func (x *PromptInjectionAnalysis) GetRiskPolicyVersion() int64 {
	if x != nil {
		return x.xxx_hidden_RiskPolicyVersion
	}
	return 0
}

func (x *PromptInjectionAnalysis) GetCreatedAt() string {
	if x != nil {
		if x.xxx_hidden_CreatedAt != nil {
			return *x.xxx_hidden_CreatedAt
		}
		return ""
	}
	return ""
}

func (x *PromptInjectionAnalysis) GetContent() string {
	if x != nil {
		if x.xxx_hidden_Content != nil {
			return *x.xxx_hidden_Content
		}
		return ""
	}
	return ""
}

func (x *PromptInjectionAnalysis) GetUserId() string {
	if x != nil {
		if x.xxx_hidden_UserId != nil {
			return *x.xxx_hidden_UserId
		}
		return ""
	}
	return ""
}

func (x *PromptInjectionAnalysis) GetL1Enabled() bool {
	if x != nil {
		return x.xxx_hidden_L1Enabled
	}
	return false
}

func (x *PromptInjectionAnalysis) GetMessageType() string {
	if x != nil {
		if x.xxx_hidden_MessageType != nil {
			return *x.xxx_hidden_MessageType
		}
		return ""
	}
	return ""
}

func (x *PromptInjectionAnalysis) GetBody() string {
	if x != nil {
		if x.xxx_hidden_Body != nil {
			return *x.xxx_hidden_Body
		}
		return ""
	}
	return ""
}

func (x *PromptInjectionAnalysis) GetToolName() string {
	if x != nil {
		if x.xxx_hidden_ToolName != nil {
			return *x.xxx_hidden_ToolName
		}
		return ""
	}
	return ""
}

func (x *PromptInjectionAnalysis) GetToolCalls() []*PromptInjectionAnalysis_ToolCall {
	if x != nil {
		if x.xxx_hidden_ToolCalls != nil {
			return *x.xxx_hidden_ToolCalls
		}
	}
	return nil
}

func (x *PromptInjectionAnalysis) GetPriorUserRequest() string {
	if x != nil {
		if x.xxx_hidden_PriorUserRequest != nil {
			return *x.xxx_hidden_PriorUserRequest
		}
		return ""
	}
	return ""
}

func (x *PromptInjectionAnalysis) GetRecentUntrustedContent() string {
	if x != nil {
		if x.xxx_hidden_RecentUntrustedContent != nil {
			return *x.xxx_hidden_RecentUntrustedContent
		}
		return ""
	}
	return ""
}

func (x *PromptInjectionAnalysis) SetRequestId(v string) {
	x.xxx_hidden_RequestId = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 0, 16)
}

func (x *PromptInjectionAnalysis) SetChatMessageId(v string) {
	x.xxx_hidden_ChatMessageId = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 1, 16)
}

func (x *PromptInjectionAnalysis) SetProjectId(v string) {
	x.xxx_hidden_ProjectId = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 2, 16)
}

func (x *PromptInjectionAnalysis) SetOrganizationId(v string) {
	x.xxx_hidden_OrganizationId = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 3, 16)
}

func (x *PromptInjectionAnalysis) SetRiskPolicyId(v string) {
	x.xxx_hidden_RiskPolicyId = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 4, 16)
}

func (x *PromptInjectionAnalysis) SetRiskPolicyVersion(v int64) {
	x.xxx_hidden_RiskPolicyVersion = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 5, 16)
}

func (x *PromptInjectionAnalysis) SetCreatedAt(v string) {
	x.xxx_hidden_CreatedAt = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 6, 16)
}

func (x *PromptInjectionAnalysis) SetContent(v string) {
	x.xxx_hidden_Content = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 7, 16)
}

func (x *PromptInjectionAnalysis) SetUserId(v string) {
	x.xxx_hidden_UserId = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 8, 16)
}

func (x *PromptInjectionAnalysis) SetL1Enabled(v bool) {
	x.xxx_hidden_L1Enabled = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 9, 16)
}

func (x *PromptInjectionAnalysis) SetMessageType(v string) {
	x.xxx_hidden_MessageType = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 10, 16)
}

func (x *PromptInjectionAnalysis) SetBody(v string) {
	x.xxx_hidden_Body = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 11, 16)
}

func (x *PromptInjectionAnalysis) SetToolName(v string) {
	x.xxx_hidden_ToolName = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 12, 16)
}

func (x *PromptInjectionAnalysis) SetToolCalls(v []*PromptInjectionAnalysis_ToolCall) {
	x.xxx_hidden_ToolCalls = &v
}

func (x *PromptInjectionAnalysis) SetPriorUserRequest(v string) {
	x.xxx_hidden_PriorUserRequest = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 14, 16)
}

func (x *PromptInjectionAnalysis) SetRecentUntrustedContent(v string) {
	x.xxx_hidden_RecentUntrustedContent = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 15, 16)
}

func (x *PromptInjectionAnalysis) HasRequestId() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 0)
}

func (x *PromptInjectionAnalysis) HasChatMessageId() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 1)
}

func (x *PromptInjectionAnalysis) HasProjectId() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 2)
}

func (x *PromptInjectionAnalysis) HasOrganizationId() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 3)
}

func (x *PromptInjectionAnalysis) HasRiskPolicyId() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 4)
}

func (x *PromptInjectionAnalysis) HasRiskPolicyVersion() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 5)
}

func (x *PromptInjectionAnalysis) HasCreatedAt() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 6)
}

func (x *PromptInjectionAnalysis) HasContent() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 7)
}

func (x *PromptInjectionAnalysis) HasUserId() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 8)
}

func (x *PromptInjectionAnalysis) HasL1Enabled() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 9)
}

func (x *PromptInjectionAnalysis) HasMessageType() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 10)
}

func (x *PromptInjectionAnalysis) HasBody() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 11)
}

func (x *PromptInjectionAnalysis) HasToolName() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 12)
}

func (x *PromptInjectionAnalysis) HasPriorUserRequest() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 14)
}

func (x *PromptInjectionAnalysis) HasRecentUntrustedContent() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 15)
}

func (x *PromptInjectionAnalysis) ClearRequestId() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 0)
	x.xxx_hidden_RequestId = nil
}

func (x *PromptInjectionAnalysis) ClearChatMessageId() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 1)
	x.xxx_hidden_ChatMessageId = nil
}

func (x *PromptInjectionAnalysis) ClearProjectId() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 2)
	x.xxx_hidden_ProjectId = nil
}

func (x *PromptInjectionAnalysis) ClearOrganizationId() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 3)
	x.xxx_hidden_OrganizationId = nil
}

func (x *PromptInjectionAnalysis) ClearRiskPolicyId() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 4)
	x.xxx_hidden_RiskPolicyId = nil
}

func (x *PromptInjectionAnalysis) ClearRiskPolicyVersion() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 5)
	x.xxx_hidden_RiskPolicyVersion = 0
}

func (x *PromptInjectionAnalysis) ClearCreatedAt() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 6)
	x.xxx_hidden_CreatedAt = nil
}

func (x *PromptInjectionAnalysis) ClearContent() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 7)
	x.xxx_hidden_Content = nil
}

func (x *PromptInjectionAnalysis) ClearUserId() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 8)
	x.xxx_hidden_UserId = nil
}

func (x *PromptInjectionAnalysis) ClearL1Enabled() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 9)
	x.xxx_hidden_L1Enabled = false
}

func (x *PromptInjectionAnalysis) ClearMessageType() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 10)
	x.xxx_hidden_MessageType = nil
}

func (x *PromptInjectionAnalysis) ClearBody() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 11)
	x.xxx_hidden_Body = nil
}

func (x *PromptInjectionAnalysis) ClearToolName() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 12)
	x.xxx_hidden_ToolName = nil
}

func (x *PromptInjectionAnalysis) ClearPriorUserRequest() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 14)
	x.xxx_hidden_PriorUserRequest = nil
}

func (x *PromptInjectionAnalysis) ClearRecentUntrustedContent() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 15)
	x.xxx_hidden_RecentUntrustedContent = nil
}

type PromptInjectionAnalysis_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	RequestId         *string
	ChatMessageId     *string
	ProjectId         *string
	OrganizationId    *string
	RiskPolicyId      *string
	RiskPolicyVersion *int64
	CreatedAt         *string
	Content           *string
	UserId            *string
	L1Enabled         *bool
	MessageType       *string
	Body              *string
	ToolName          *string
	ToolCalls         []*PromptInjectionAnalysis_ToolCall
	// Bounded causal evidence captured when the event is read. These fields are
	// untrusted content and are never included in the published finding match.
	PriorUserRequest       *string
	RecentUntrustedContent *string
}

func (b0 PromptInjectionAnalysis_builder) Build() *PromptInjectionAnalysis {
	m0 := &PromptInjectionAnalysis{}
	b, x := &b0, m0
	_, _ = b, x
	if b.RequestId != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 0, 16)
		x.xxx_hidden_RequestId = b.RequestId
	}
	if b.ChatMessageId != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 1, 16)
		x.xxx_hidden_ChatMessageId = b.ChatMessageId
	}
	if b.ProjectId != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 2, 16)
		x.xxx_hidden_ProjectId = b.ProjectId
	}
	if b.OrganizationId != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 3, 16)
		x.xxx_hidden_OrganizationId = b.OrganizationId
	}
	if b.RiskPolicyId != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 4, 16)
		x.xxx_hidden_RiskPolicyId = b.RiskPolicyId
	}
	if b.RiskPolicyVersion != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 5, 16)
		x.xxx_hidden_RiskPolicyVersion = *b.RiskPolicyVersion
	}
	if b.CreatedAt != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 6, 16)
		x.xxx_hidden_CreatedAt = b.CreatedAt
	}
	if b.Content != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 7, 16)
		x.xxx_hidden_Content = b.Content
	}
	if b.UserId != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 8, 16)
		x.xxx_hidden_UserId = b.UserId
	}
	if b.L1Enabled != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 9, 16)
		x.xxx_hidden_L1Enabled = *b.L1Enabled
	}
	if b.MessageType != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 10, 16)
		x.xxx_hidden_MessageType = b.MessageType
	}
	if b.Body != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 11, 16)
		x.xxx_hidden_Body = b.Body
	}
	if b.ToolName != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 12, 16)
		x.xxx_hidden_ToolName = b.ToolName
	}
	x.xxx_hidden_ToolCalls = &b.ToolCalls
	if b.PriorUserRequest != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 14, 16)
		x.xxx_hidden_PriorUserRequest = b.PriorUserRequest
	}
	if b.RecentUntrustedContent != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 15, 16)
		x.xxx_hidden_RecentUntrustedContent = b.RecentUntrustedContent
	}
	return m0
}

type PromptInjectionAnalysis_ToolCall struct {
	state                  protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Name        *string                `protobuf:"bytes,1,opt,name=name"`
	xxx_hidden_Arguments   *string                `protobuf:"bytes,2,opt,name=arguments"`
	XXX_raceDetectHookData protoimpl.RaceDetectHookData
	XXX_presence           [1]uint32
	unknownFields          protoimpl.UnknownFields
	sizeCache              protoimpl.SizeCache
}

func (x *PromptInjectionAnalysis_ToolCall) Reset() {
	*x = PromptInjectionAnalysis_ToolCall{}
	mi := &file_gram_risk_v1_prompt_injection_analysis_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PromptInjectionAnalysis_ToolCall) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PromptInjectionAnalysis_ToolCall) ProtoMessage() {}

func (x *PromptInjectionAnalysis_ToolCall) ProtoReflect() protoreflect.Message {
	mi := &file_gram_risk_v1_prompt_injection_analysis_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *PromptInjectionAnalysis_ToolCall) GetName() string {
	if x != nil {
		if x.xxx_hidden_Name != nil {
			return *x.xxx_hidden_Name
		}
		return ""
	}
	return ""
}

func (x *PromptInjectionAnalysis_ToolCall) GetArguments() string {
	if x != nil {
		if x.xxx_hidden_Arguments != nil {
			return *x.xxx_hidden_Arguments
		}
		return ""
	}
	return ""
}

func (x *PromptInjectionAnalysis_ToolCall) SetName(v string) {
	x.xxx_hidden_Name = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 0, 2)
}

func (x *PromptInjectionAnalysis_ToolCall) SetArguments(v string) {
	x.xxx_hidden_Arguments = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 1, 2)
}

func (x *PromptInjectionAnalysis_ToolCall) HasName() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 0)
}

func (x *PromptInjectionAnalysis_ToolCall) HasArguments() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 1)
}

func (x *PromptInjectionAnalysis_ToolCall) ClearName() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 0)
	x.xxx_hidden_Name = nil
}

func (x *PromptInjectionAnalysis_ToolCall) ClearArguments() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 1)
	x.xxx_hidden_Arguments = nil
}

type PromptInjectionAnalysis_ToolCall_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	Name      *string
	Arguments *string
}

func (b0 PromptInjectionAnalysis_ToolCall_builder) Build() *PromptInjectionAnalysis_ToolCall {
	m0 := &PromptInjectionAnalysis_ToolCall{}
	b, x := &b0, m0
	_, _ = b, x
	if b.Name != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 0, 2)
		x.xxx_hidden_Name = b.Name
	}
	if b.Arguments != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 1, 2)
		x.xxx_hidden_Arguments = b.Arguments
	}
	return m0
}

var File_gram_risk_v1_prompt_injection_analysis_proto protoreflect.FileDescriptor

const file_gram_risk_v1_prompt_injection_analysis_proto_rawDesc = "" +
	"\n" +
	",gram/risk/v1/prompt_injection_analysis.proto\x12\fgram.risk.v1\x1a\x1bgcp/pubsub/v1/options.proto\"\xc4\x05\n" +
	"\x17PromptInjectionAnalysis\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12&\n" +
	"\x0fchat_message_id\x18\x02 \x01(\tR\rchatMessageId\x12\x1d\n" +
	"\n" +
	"project_id\x18\x03 \x01(\tR\tprojectId\x12'\n" +
	"\x0forganization_id\x18\x04 \x01(\tR\x0eorganizationId\x12$\n" +
	"\x0erisk_policy_id\x18\x05 \x01(\tR\friskPolicyId\x12.\n" +
	"\x13risk_policy_version\x18\x06 \x01(\x03R\x11riskPolicyVersion\x12\x1d\n" +
	"\n" +
	"created_at\x18\a \x01(\tR\tcreatedAt\x12\x18\n" +
	"\acontent\x18\b \x01(\tR\acontent\x12\x17\n" +
	"\auser_id\x18\t \x01(\tR\x06userId\x12\x1d\n" +
	"\n" +
	"l1_enabled\x18\n" +
	" \x01(\bR\tl1Enabled\x12!\n" +
	"\fmessage_type\x18\v \x01(\tR\vmessageType\x12\x12\n" +
	"\x04body\x18\f \x01(\tR\x04body\x12\x1b\n" +
	"\ttool_name\x18\r \x01(\tR\btoolName\x12M\n" +
	"\n" +
	"tool_calls\x18\x0e \x03(\v2..gram.risk.v1.PromptInjectionAnalysis.ToolCallR\ttoolCalls\x12,\n" +
	"\x12prior_user_request\x18\x0f \x01(\tR\x10priorUserRequest\x128\n" +
	"\x18recent_untrusted_content\x18\x10 \x01(\tR\x16recentUntrustedContent\x1a<\n" +
	"\bToolCall\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12\x1c\n" +
	"\targuments\x18\x02 \x01(\tR\targuments:\n" +
	"\x8a\xb5\x18\x06\x12\x04\b\x80\xf5$B=Z;github.com/speakeasy-api/gram/infra/gen/gram/risk/v1;riskv1b\beditionsp\xe9\a"

var file_gram_risk_v1_prompt_injection_analysis_proto_msgTypes = make([]protoimpl.MessageInfo, 2)
var file_gram_risk_v1_prompt_injection_analysis_proto_goTypes = []any{
	(*PromptInjectionAnalysis)(nil),          // 0: gram.risk.v1.PromptInjectionAnalysis
	(*PromptInjectionAnalysis_ToolCall)(nil), // 1: gram.risk.v1.PromptInjectionAnalysis.ToolCall
}
var file_gram_risk_v1_prompt_injection_analysis_proto_depIdxs = []int32{
	1, // 0: gram.risk.v1.PromptInjectionAnalysis.tool_calls:type_name -> gram.risk.v1.PromptInjectionAnalysis.ToolCall
	1, // [1:1] is the sub-list for method output_type
	1, // [1:1] is the sub-list for method input_type
	1, // [1:1] is the sub-list for extension type_name
	1, // [1:1] is the sub-list for extension extendee
	0, // [0:1] is the sub-list for field type_name
}

func init() { file_gram_risk_v1_prompt_injection_analysis_proto_init() }
func file_gram_risk_v1_prompt_injection_analysis_proto_init() {
	if File_gram_risk_v1_prompt_injection_analysis_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_gram_risk_v1_prompt_injection_analysis_proto_rawDesc), len(file_gram_risk_v1_prompt_injection_analysis_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   2,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_gram_risk_v1_prompt_injection_analysis_proto_goTypes,
		DependencyIndexes: file_gram_risk_v1_prompt_injection_analysis_proto_depIdxs,
		MessageInfos:      file_gram_risk_v1_prompt_injection_analysis_proto_msgTypes,
	}.Build()
	File_gram_risk_v1_prompt_injection_analysis_proto = out.File
	file_gram_risk_v1_prompt_injection_analysis_proto_goTypes = nil
	file_gram_risk_v1_prompt_injection_analysis_proto_depIdxs = nil
}
