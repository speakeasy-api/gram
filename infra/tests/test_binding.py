...
def test_binds_matching_pair() -> None:
    ...
    assert binding.message_descriptor.full_name == "gram.ping.v2.Message"
    assert binding.subscription_descriptor.full_name == "gram.ping.v2.Processor"
    assert binding.subscription_options.topic == "gram.ping.v2.Message"
...
def test_mismatched_pair_raises(monkeypatch) -> None:
    ...
    with pytest.raises(ValueError, match=r"declares topic 'gram\.ping\.v2\.Message'"):
