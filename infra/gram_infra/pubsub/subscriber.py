...
class _PortalScheduler(Scheduler):
    ...
    def schedule(self, callback, *args, **kwargs) -> None:
        ...
        try:
            ...
            # The portal is no longer running (event loop torn down). Nack so
            # the broker redelivers rather than dropping the message.
            message.nack()
    ...
    def shutdown(self, await_msg_callbacks: bool = True):
        ...
        try:
            ...
            # Event loop already gone; nothing left we can safely drain.
            return []
...
class Subscriber(Generic[M]):
    ...
    async def _dispatch(self, message, callback: MessageCallback[M]) -> None:
        ...
        try:
        ...
        except BaseException as exc:  # noqa: BLE001 - mirror Go's recover()
            ...
            # message is simply left in flight and redelivery recovers it (this
            # mirrors Go's ctx-cancel behavior).
            if isinstance(exc, anyio.get_cancelled_exc_class()):
                raise
            # The callback raised — either a deliberate nack signal or an
            # unexpected error (Go recovers from *any* panic here, including
            # BaseExceptions like SystemExit, so one bad message can't tear down
