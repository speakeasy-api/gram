    ...
    def _reconcile_subscription(self, sub_id: str, topic_id: str, options) -> None:
        ...
        if options.HasField("retry_policy"):
            request["retry_policy"] = {
                "minimum_backoff": _to_timedelta(options.retry_policy.minimum_backoff),
                "maximum_backoff": _to_timedelta(options.retry_policy.maximum_backoff),
            }
...
def _duration_to_seconds(duration: Duration) -> int:
    """Round a Duration to whole seconds (half up), clamping non-positive to 0.
