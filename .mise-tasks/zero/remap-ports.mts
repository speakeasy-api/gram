 ...
 * is therefore re-emitted. Hand-pinned values otherwise remain untouched;
 * the only non-template exception is a narrowly enumerated migration for
 * numeric aliases generated by older versions of this task.
 */
...
/**
 * Before template-valued `_PORT` declarations were recognized as aliases,
 * remap-ports randomized them as if they were listeners. Preserve mode cannot
 * distinguish that generated number from a hand-written number, so migrations
 * must be limited to aliases known to have been emitted by the old behavior.
 */
const LEGACY_NUMERIC_PORT_ALIASES = new Map([
  ["CLICKHOUSE_READ_NATIVE_PORT", "{{env.CLICKHOUSE_NATIVE_PORT}}"],
]);
  ...
  for (const portEnvVar of portEnvVars) {
    ...
    for (const [key, value] of findDependentEnvVars(config.env, portEnvVar)) {
      if (preserve && key in existing) {
        ...
        const legacyNumericAlias = isLegacyNumericPortAlias(key, pinned, value);
        if (!generatedTemplateChanged && !legacyNumericAlias) continue;
        if (!refreshed.has(key)) {
          refreshed.add(key);
          // stderr on purpose: stdout is the declarations themselves when the
          // caller asked for them on `-`.
          const reason = legacyNumericAlias
            ? "replacing a legacy numeric port alias with its source template"
...
function isLegacyNumericPortAlias(
  key: string,
  pinned: unknown,
  current: string,
): boolean {
  if (LEGACY_NUMERIC_PORT_ALIASES.get(key) !== current) return false;

  const port =
    typeof pinned === "number"
      ? pinned
      : typeof pinned === "string" && /^\d+$/.test(pinned)
        ? Number(pinned)
        : NaN;
  return (
