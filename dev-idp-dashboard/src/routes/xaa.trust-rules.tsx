...
import {
  useCreateXaaTrustRule,
  useDeleteXaaTrustRule,
  useUpdateXaaTrustRule,
  useXaaResources,
  useXaaTrustRules,
} from "@/hooks/use-xaa";

export const Route = createFileRoute("/xaa/trust-rules")({
  component: TrustRulesPage,
});

function TrustRulesPage() {
  const { data, isLoading } = useXaaTrustRules();
  const resources = useXaaResources();
  const create = useCreateXaaTrustRule();
  const update = useUpdateXaaTrustRule();
  const remove = useDeleteXaaTrustRule();
