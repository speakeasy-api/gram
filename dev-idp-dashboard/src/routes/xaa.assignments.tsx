...
import {
  useCreateXaaAssignment,
  useDeleteXaaAssignment,
  useXaaApps,
  useXaaAssignments,
  useXaaResources,
} from "@/hooks/use-xaa";

export const Route = createFileRoute("/xaa/assignments")({
  component: AssignmentsPage,
});

function AssignmentsPage() {
  const { data, isLoading } = useXaaAssignments();
  const apps = useXaaApps();
  const resources = useXaaResources();
  const users = useUsers();
  const create = useCreateXaaAssignment();
  const remove = useDeleteXaaAssignment();
