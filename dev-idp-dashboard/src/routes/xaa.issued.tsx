...
import { Cell, OrDash, Row, Section, Table } from "@/components/xaa/Section";
...
export const Route = createFileRoute("/xaa/issued")({
  component: IssuedPage,
});

function IssuedPage() {
  const { data, isLoading } = useXaaIssuedGrants();
  const apps = useXaaApps();
  const resources = useXaaResources();
