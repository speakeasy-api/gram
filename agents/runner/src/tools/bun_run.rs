...
const WORK_DIR: &str = "/tmp/bun-sandbox";
...
Working directory is /tmp/bun-sandbox; `playwright-core` is preinstalled. \
...
impl Tool for BunRunTool {
    ...
    async fn invoke(
        ...
        let file_path = format!("{WORK_DIR}/{filename}");
        ...
        cmd.arg("run").arg(&file_path).current_dir(WORK_DIR);
        cmd.env("NO_COLOR", "1");
