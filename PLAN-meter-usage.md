...
- **Bounded full-generation builder implemented; full-volume acceptance remains open.** Keep the raw ledger authoritative. `chrepo.Queries.RebuildUsageSummaries` reads `FINAL` through the parameterized canonical facet source, adaptively builds tenant/family/time slices, and atomically exchanges the complete staging table with `billing_meter_daily_summaries`. Atlas manages serving, staging, and source-view schemas.
...
- Order summaries by organization, family, reading kind, facet, day, series kind, series key, and measurement metadata. Keep monthly physical partitions; logical day/hour jobs are not physical partitions. Publish one global coverage marker atomically with the complete generation, including empty coverage. A query-window cap is not a retention TTL.
...
- First publish yesterday after 01:00 UTC for the expected one-hour retry window. Continue using `FINAL`: this allowance does not prove merges finished. The hourly rebuild revisits old days for late-arrival repair. Yesterday stays live until successful publication, not merely until a clock boundary.
...
- Every pass recomputes all retained complete days. This repairs late originals and adjustments without a dirty-event queue or ingestion redesign; it is not dirty-only incremental maintenance. Chunks observe source data at different times, not a shared ingestion snapshot. Later passes repair arrivals missed by an earlier chunk.
- One hourly, queue-scoped Temporal workflow invokes one rebuild activity; overlap is skipped. The activity and demo seed share the PostgreSQL session advisory lock `hashtext('gram-demo-seed')`. Temporal actions/month are approximately 1,440 per namespace before retries (720 starts plus 720 activities), fixed rather than per reading or tenant.
- All builder queries inherit a 512 MiB cap and bounded execution/read/thread settings. Resource-limited raw slices are bisected at half-open occurrence-time boundaries. Failed attempts are discarded before retry; successful partials are combined per tenant with sum quantities/counts and max labels. No top-six pruning happens during builds.
- The authorized local experiment uses a 4 GiB Docker container and a 3 GiB ClickHouse server cap through worktree-only overrides; the default development server budget remains 1 GiB.
- Only if full passes become too expensive: introduce durable dirty generations and targeted maintenance. Correct pre-ACK change tracking, source visibility, concurrent-arrival races, and publication fencing must be designed before that optimization. Never introduce per-reading Temporal calls.
- Demo seeding writes only scoped raw facts while holding the shared lock, then invokes the same bounded builder with the existing 60-second seed deadline. Seed code does not write published summaries or introduce tenant publication metadata.
- Publication uses one [`EXCHANGE TABLES`](https://clickhouse.com/docs/sql-reference/statements/exchange) operation, supported on Atomic and Shared database engines; unsupported engines fail before staging mutations. Local success is not proof of Cloud replica visibility or infrastructure permissions.
...
- The builder uses the worker's writer principal. Infrastructure owners must provision and review permissions for raw/source reads, work-table INSERT/TRUNCATE, and atomic EXCHANGE before deployment. Docker/CI use their existing XML-defined application principal; Atlas provisions a SQL principal in its sandbox only. Neither is Cloud provisioning, and publication privileges must not be granted to the API reader.
...
- `server/clickhouse/schema.sql` and both Atlas-generated migration flavors: parameterized canonical facet source, complete-generation staging, partial and attempt tables, and removal of the monolithic native refresh. Keep published migrations intact and generate forward changes through Atlas.
...
- `server/internal/metering/ch_writer.go` remains unchanged. Background registration/activity code schedules bounded full-generation repair hourly; it does not add per-reading work or dirty-generation state.
...
- Authorized memory experiment: Docker's active container limit is 4,294,967,296 bytes, ClickHouse reports a 3,221,225,472-byte server cap, and both native source/refresh definitions enforce 536,870,912 bytes (512 MiB). The two resource overrides live in ignored `mise.local.toml`; XML extraction without the override still returns the 1 GiB default.
- Restart exposed an init-script failure: `CREATE USER IF NOT EXISTS gram` tried to write to Docker's read-only XML access storage. Moved SQL creation/grants into Atlas's sandbox baseline; Docker/CI retain their existing XML principal. Real container startup, bootstrap replay, and both Atlas migration-generation environments succeeded. The generated 512 MiB forward migration applied locally, and the ordinary demo publisher completed.
- Exactly one large-fixture refresh, with retries disabled and a 180-second deadline, failed after 3.332 seconds at the 512 MiB query cap (attempted 514.94 MiB). It read 2,145,926 rows / 338,151,016 bytes and published no rows. Sampled query high-water memory reached 536,785,412 bytes; sampled server-tracked memory reached 810,246,027 bytes, not an RSS or complete server-peak measurement.
- Build EXPLAIN selected all 3,376 source parts; primary-key pruning reported 3,376 / 27,547 granules. The fixture still has exactly 103,000,000 physical rows and 3,376 parts, with refresh and source merges stopped. No retry, additional fixture load, five-reader benchmark, full-retention pass, or replica claim followed this failure. Evidence: ignored `.playwright-cli/meter-summary-512m-refresh.json`. Step 3 remains open.
- Fixture benchmark correction: the prior database-name substitution also rewrote quoted `gram.*` meter identifiers inside the fixture summary view. The earlier 512 MiB refresh is not a production-equivalent benchmark. Corrected only those fixture-view literals; ledger rows are unchanged.
- Corrected bounded build: one synthetic tenant's June 12 storage slice, with all facets and `FINAL`, selected 13/3,376 parts and 2,712/27,547 total granules. It completed in 1.836 seconds, reading 2,965,265 rows / 459,668,042 bytes and producing 20,839 summary rows through `FORMAT Null`. The HTTP summary reported 204,273,855 bytes of query memory. This proves one slice, not publication, worst-case bounds, or reader acceptance.
- Corrected full-source comparison, with a 20-second execution limit and 512 MiB query cap, failed after 1.843 seconds while reading column `id`, attempting 514.95 MiB. It read 898,930 rows / 141,203,662 bytes. These were direct source SELECTs, not native refresh publication tests.
- A more fragmented July 2 tenant-day (171 parts) still exceeded 512 MiB. Subdivision is necessary: explicit organization/meter/occurrence-hour predicates pruned to 9 parts and 15 granules. The complete 13-facet storage query for that hour completed in 275 ms, reading 122,880 rows / 19,218,586 bytes and producing 14,265 partial summary rows; HTTP query memory was 50,875,244 bytes. A single-facet daily query also failed, so reducing facets alone is not sufficient.
- The successful June 12 daily build conserved the raw `FINAL` quantity (1,606,778,133) and reading count (322,222), with 20,000 distinct billing users. These synthetic values are correctness evidence, not customer billing data.
- Bounded publication smoke: the actual Go builder over native TLS completed the preserved 103-million-physical-row fixture in 323.778 seconds. It published 1,288,851 rows including exactly one global marker, and left staging/parts/attempt tables empty. Raw input remains unchanged. An earlier 180-second attempt stopped before publication; the final run used a ten-minute outer deadline, unchanged 512 MiB per-query cap, and six-hour initial slices with adaptive subdivision.
- Native repository reader smoke after publication: 15/15 three-calendar-month billing-user requests at five readers, p95 372.611 ms, 175 returned daily cells. The fixture covers about 25 days, not representative three-month retained traffic; this is not replica acceptance.
- Separate June 12 conservation: raw `FINAL` and published summaries both report usage quantity 1,606,778,133 / 321,900 readings and adjustment quantity zero / 322 readings. The first smoke assertion mistakenly compared usage-only count with the combined count; the independent grouped raw/summary reconciliation resolved it without changing application results.
- Validation: 18 focused reporting/background tests passed; 14 tagged demo-seed safety tests passed. The two rebuild regressions were rerun successfully after preserving DateTime64 nanoseconds in bound parameters and publication markers and switching initial scopes to six hours.
- The generated forward migration applied locally. Server lint passed with the existing deprecated `exhaustruct` warning. The restarted worker registered the hourly queue-scoped schedule; explicitly triggering it against the ordinary local database completed the real workflow/activity/publication path in 6.476 seconds.
- Standalone production-shaped period-ranking SQL pruned to 5/12 parts and 65/166 granules in the recorded EXPLAIN. With reader thread/block/spill settings, it read 538,615 rows / 31,190,735 bytes in 114.637 ms and reported 11,936,777 bytes of query memory. These counters describe that ranking-stage SQL, not all queries in an API request.
- Evidence is retained in ignored `.playwright-cli/meter-bounded-{rebuild-probe,publication}.json`; the temporary Go smoke command was removed. No new raw fixtures were loaded. Full-build peak RSS/cumulative scan counters, representative full-window retention, and separate-replica acceptance remain unproven; step 3 remains open.
