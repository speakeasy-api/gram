...
- **Planned, not implemented in the application. Native-first evaluation approved.** Keep the raw ledger authoritative. A ClickHouse refreshable materialized view can read `FINAL`, build independent daily facet summaries, and atomically replace its entire serving result without Temporal. Benchmark this full-retained-history rebuild before choosing targeted day maintenance. Atlas manages serving/view schemas and generated migrations.
...
- Initialize derived summaries from facts already present in the new ledger. This is not legacy-history reconstruction or new raw readings. Update demo seeding to publish/rebuild its own synthetic daily summaries through the same path.
