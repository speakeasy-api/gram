...
server/internal/devidp/                ← everything dev-idp lives under this tree
  ...
  gen/                                 ← Goa codegen output, NESTED here (not under server/gen/)
    http/openapi3.yaml      — embedded by embed.go below
  ...
  embed.go                  — //go:embed gen/http/openapi3.yaml so the dashboard
                              and dev-idp `/about` endpoint can serve it
...
- **Encapsulation.** The dev-idp is a self-contained sub-binary. Its
  generated handlers, openapi doc, and types have no consumers outside
...
OpenAPI embedding follows the same nesting. `server/main.go` cannot embed
`server/internal/devidp/gen/http/openapi3.yaml` because `//go:embed`
cannot escape its package — but `server/internal/devidp/embed.go` can,
and exposes the bytes through a small accessor the dashboard or a
`devIdp.openapi` RPC can serve.
