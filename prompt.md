...
This branch carries a design document alongside its reference implementation. The following files are the **planning artifacts** — they ship no code:

- `prompt.md` — this file: framing, goals, structure, process
- `spike.md` — design body: definitions, schemas, flows, mgmt API references
- `spike.notion.md` — Notion-rendered spike
- `project.md` — milestones and tickets
- `clarifications.md` — resolved clarifications and open questions
- `idp-design.md` — Milestone #0b mock IDP design
- `plan-notes.md` — scratch planning notes
- `diagrams/*.mermaid` — sequence and state diagrams
- `schemas/*` — proposed Postgres SDL, Redis structures, JWT shape
- `skill-drafts/*.md` — drafts of agent skills

Implementation commits sit on top of these files; they never edit them.

## Commit hygiene for planning artifacts

All planning-file history on this branch is collapsed into a single commit with subject `chore: planning artifacts`. **Any further change to a planning file MUST be committed as a `fixup!` against that commit** so it autosquashes back into place during rebase. Do not mix planning edits into an implementation commit, and do not add a fresh planning commit on top.

To commit planning changes:

```sh
PLAN_SHA=$(git log --grep='^chore: planning artifacts' --format=%H -1)
git add <planning files>
git commit --fixup="$PLAN_SHA"
```

Before opening a PR (or any rebase against this branch):

```sh
git rebase -i --autosquash "$(git merge-base main HEAD)"
```

The result: one planning commit at the base of the branch, with implementation commits cleanly stacked above.
...
1. Enable Gram user sessions to connect to remote OAuth provider sessions even when those sessions must acquire a token for an external server. **This is THE fundamental product goal**, though we are bundling in a number of architectural improvements to our OAuth pathways. Currently, the same Gram component plays both roles: AS to the MCP client _and_ OAuth client to the remote. These responsibilities must be split.

2. Decouple user sessions from remote sessions at the data-model layer. That is: sessions established between Gram and MCP clients are distinct from sessions between Gram and remote OAuth providers.
   - **User sessions** = signed Gram access tokens (JWT) plus refresh tokens stored in Redis attached to the session.
   - **Remote sessions** = stored in Redis on separate documents, keyed on `(session_id, user_session_issuer_id)` rather than the access token value. Each document holds the access token and the refresh token in a structure that lets them have entirely separate TTLs.
...
4. Remove a particular security vulnerability from Gram OAuth: today, consent can be bypassed when a session already exists. The `/authorize` endpoint must verify that the user has previously consented for this `user_session_issuer` to access **all** of its `remote_session_tokens`. Only on a positive match may consent be skipped.
...
9. There is no longer any directory just called `oauth`. Every directory must be qualified by what kind of OAuth concern it carries: user-session, remote-session, or some other concept entirely. OAuth is not a single concept in our system — we treat authentication and authorization in many ways and should stop mushing them together.

10. Use the chat-sessions JWT manager to sign and issue user-session access tokens, with the authenticated identity embedded as the JWT `sub`.
    ...
    - `aud` for user-session tokens is the toolset slug. This lets validators reject chat-session JWTs presented as user-session tokens and vice versa.
    - When we document the JWT schema in `/schemas/`, we will clearly document every URN.

11. The `user_session_issuer` package never generates its own session IDs. Every method that creates a session must **accept** a session ID externally. When consumed from the MCP handler, the injected value is the `mcp_session_id`. The package itself stays agnostic about where the ID comes from — this preserves a clean seam for non-MCP consumers in the future and keeps the MCP/session-ID coupling at the call site, not in the package.
...
- **`spike.md`**: generalized design documentation. Includes:
  ...
  - **definitions section up front** — first general OAuth terms, then _our_ terms. Notable ones:
    ...
    - **chain vs interactive modes** for `user_session_issuer`. There can be multiple `remote_oauth_issuers` per `user_session_issuer`. After issuing a user session, Gram completes the OAuth authn challenges mandated by each remote issuer.
      - **chain**: redirect through each subsequent authn challenge from our callback, build the entire session, then redirect back to the client callback for the final token exchange. Consent must still be prompted somewhere in the request stream — there is just no intermediate "click each server" UI.
      - **interactive**: issue a user session, then render a UX where the user clicks each remote OAuth server to authenticate.
...
- **`project.md`**: tickets defining the scope of work. Tracks all follow-on PRs and milestones. Milestones (roughly):
  ...
  - **Milestone #1 — Management APIs for Resources**: land all Postgres schemas from spike §4.1 and expose CRUD for the new tables through Goa-designed `/rpc/<service>.<method>` endpoints (spike §6.1 / §6.2). Includes audit logging and RBAC scope coverage.
  - **Milestone #2**: instrument `x/mcp` with user sessions in three modes — authenticated, anonymous, passthrough.
  - **Milestone #3**: instrument `x/mcp` with remote sessions in **chain** authn challenge mode (no intermediate UI redirect chain — but consent must still be prompted somewhere).
  - **Milestone #4 — URGENT: optional `user_session_issuer` support on `/mcp`**: opt-in `/mcp` runtime gating on a configured `user_session_issuer`. Lands the `toolsets.user_session_issuer_id` migration alongside the runtime check so the column is meaningful from day one. Supersedes the old "toolsets wiring" milestone.
  - **Milestone #5**: instrument `x/mcp` with passthrough authentication. (Same concept as `passthrough mode` in the definitions.)
  - **Milestone #6**: instrument `x/mcp` with remote sessions in **interactive** ("multi-plexing") mode.
  - **Milestone #7**: migrate all servers on the current `external_oauth_provider` model to `user_session_issuer` with **passthrough mode**.
  - **Milestone #8**: migrate all servers on the current `oauth_proxy_server` model to the new `user_session_issuer`. This requires porting `oauth_proxy_providers` → `user_session_issuers`.
  - **Milestone #9**: add a mode that requests stale remote-session information through URL-mode elicitation. The URL goes to the same authn challenge screen as `interactive` mode.
  - **Milestone #10**: grant remote sessions to other principals (out of initial scope; placeholder).

- **`/diagrams/`**: a directory with one mermaid diagram per pertinent flow. Should include:
  - `mcp-handler.mermaid`
  - `user-session-authn-challenge.mermaid`
  - `remote-session-authn-challenge.mermaid`
  - `unified-authn-challenge.mermaid`
