...
- `token_endpoint_auth_methods_supported` — `["client_secret_basic", "client_secret_post", "none"]` (real MCP clients in the wild use `none` + PKCE; advertising it lets them pass the AS-metadata check before DCR)
...
**Side effects**: writes to `user_session_clients` (§4.1). Emits structured logs for registration attempts and results, including issuer/client metadata safe for operations, but never `client_secret` or `client_secret_hash`.
