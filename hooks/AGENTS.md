...
- Treat `copilot-cli` and `vscode-copilot` as distinct providers. Keep their registrations separate so overlapping hook files do not fire callbacks twice, but share canonical relay logic; never share a codec.
