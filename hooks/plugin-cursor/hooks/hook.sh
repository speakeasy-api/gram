...
gram_hooks_normalized_event_type() {
  local native="$1"
  case "cursor:$native" in
    cursor:sessionStart) printf 'session.started' ;;
    cursor:beforeSubmitPrompt) printf 'prompt.submitted' ;;
    cursor:afterAgentResponse) printf 'assistant.responded' ;;
    cursor:afterAgentThought) printf 'assistant.thought' ;;
    cursor:preToolUse) printf 'tool.requested' ;;
    cursor:postToolUse) printf 'tool.completed' ;;
    cursor:postToolUseFailure) printf 'tool.failed' ;;
    cursor:beforeMCPExecution) printf 'tool.requested' ;;
    cursor:afterMCPExecution) printf 'tool.completed' ;;
    cursor:stop) printf 'usage.reported' ;;
    *) printf '%s' "$native" ;;
  esac
}
...
gram_hooks_build_canonical_payload() {
  ...
  if [ "cursor" != "codex" ] && [ -n "$(gram_hooks_skill_name "$payload")" ]; then
...
if [ "cursor" = "cursor" ] && [ "$native_event" != "beforeSubmitPrompt" ]; then
  ...
  case "$native_event" in
    preToolUse | postToolUse | postToolUseFailure)
      ...
      case "$cursor_tool_name" in
        MCP:*)
          gram_hooks_provider_response "cursor" "$native_event" '{}'
          exit 0
          ;;
      esac
      ;;
...
if [ "$post_status" -eq 78 ]; then
  if [ "cursor" = "claude" ] && [ "$native_event" = "UserPromptSubmit" ]; then
...
if [ "$post_status" -eq 79 ] && [ "cursor" = "claude" ] && [ "$native_event" = "UserPromptSubmit" ]; then
  gram_hooks_emit_login_nudge "$provider_payload" "$script_dir"
  exit 0
fi
...
if [ "$http_code" -ge 200 ] 2>/dev/null && [ "$http_code" -lt 300 ] 2>/dev/null; then
  ...
  gram_hooks_provider_response "cursor" "$native_event" "$body"
  exit 0
fi
