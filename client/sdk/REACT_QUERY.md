...
```tsx
import { useGetRole } from "@gram/client/react-query/accessGetRole.js";

export function Example() {
  const { data, error, status } = useGetRole({
    id: "<id>",
  });
...
import { invalidateGetRole, invalidateAllGetRole } from "@gram/client/react-query/accessGetRole.js";
      ...
      onSubmit={(e) => {
        ...
        mutate(formData, {
          onSuccess: () => {
            ...
            invalidateGetRole(queryClient, /* ... arguments ... */);
            // OR, invalidate all cache entries for the query targets:
            invalidateAllGetRole(queryClient);
          },
        });
      }}
    >
...
import { GramCore } from "@gram/client";
import { GramProvider } from "@gram/client/react-query";
import { useGetRoleSuspense } from "@gram/client/react-query/accessGetRole.js";
...
import { GramCore } from "@gram/client";
import { prefetchGetRole } from "@gram/client/react-query/accessGetRole.js";

export default async function Page() {
  const queryClient = new QueryClient();
  const gram = new GramCore();

  await prefetchGetRole(queryClient, gram, {
    id: "<id>",
  });
