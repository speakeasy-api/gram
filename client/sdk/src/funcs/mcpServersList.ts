...
export function mcpServersList(
  ...
  Result<
    components.ListMcpServersResult,
    | errors.ServiceError
    | GramError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        z.optional(operations.ListMcpServersRequest$outboundSchema),
        value,
      ),
  ...
  const query = encodeFormQuery({
    ...
    "tunnelled_mcp_server_id": payload?.tunnelled_mcp_server_id,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ListMcpServersResult$inboundSchema),
    M.jsonErr(
