 ...
 */
export function riskResultsCluster(
  client: GramCore,
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        z.optional(operations.ClusterRiskResultsRequest$outboundSchema),
        value,
      ),
  ...
  const path = pathToFunc("/rpc/risk.results.cluster")();

  const query = encodeFormQuery({
    "include_rule": payload?.include_rule,
    "limit": payload?.limit,
  ...
  const context = {
    ...
    operationID: "clusterRiskResults",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ListRiskFindingClustersResult$inboundSchema),
    M.jsonErr(
