 ...
 */
export function remoteMcpUpdateServer(
  client: GramCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.UpdateRemoteMcpServerRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.UpdateServerForm, { explode: true });

  const path = pathToFunc("/rpc/remoteMcp.updateServer")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.RemoteMcpServer$inboundSchema),
    M.jsonErr(
