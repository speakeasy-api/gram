 ...
 */
export function hooksHooksNumberVscodeCopilot(
  client: GramCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.HooksNumberVscodeCopilotRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.VSCodeCopilotHookPayload, {
    explode: true,
  });

  const path = pathToFunc("/rpc/hooks.vscode")();
  ...
  const headers = new Headers(compactMap({
    ...
    "Gram-User-Email-Source": encodeSimple(
      "Gram-User-Email-Source",
      payload["Gram-User-Email-Source"],
      { explode: false, charEncoding: "none" },
    ),
  }));
  ...
  const context = {
    ...
    operationID: "hooks#vscodeCopilot",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.VSCodeCopilotHookResult$inboundSchema),
    M.jsonErr(
