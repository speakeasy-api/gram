 ...
 */
export function assistantsKickoffMessage(
  client: GramCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.KickoffAssistantMessageRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.KickoffMessageRequestBody, {
    explode: true,
  });

  const path = pathToFunc("/rpc/assistants.kickoffMessage")();
  ...
  const context = {
    ...
    operationID: "kickoffAssistantMessage",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.SendMessageResult$inboundSchema),
    M.jsonErr(
