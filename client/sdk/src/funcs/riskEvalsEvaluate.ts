...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.EvaluatePromptGuardrailRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.EvaluatePromptGuardrailRequestBody, {
    explode: true,
  });

  const path = pathToFunc("/rpc/riskEvals.evaluate")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.PromptGuardrailEvalResult$inboundSchema),
    M.jsonErr(
