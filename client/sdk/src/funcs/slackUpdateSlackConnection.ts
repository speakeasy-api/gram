 ...
 */
export function slackUpdateSlackConnection(
  client: GramCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateSlackConnectionRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.UpdateSlackConnectionRequestBody, {
    explode: true,
  });

  const path = pathToFunc("/rpc/slack.updateConnection")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.GetSlackConnectionResult$inboundSchema),
    M.jsonErr(
