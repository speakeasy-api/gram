...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.UpdateSpendRuleRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.UpdateSpendRuleRequestBody, {
    explode: true,
  });

  const path = pathToFunc("/rpc/spendrules.updateRule")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.SpendRule$inboundSchema),
    M.jsonErr(
