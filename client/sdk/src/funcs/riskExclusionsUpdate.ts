 ...
 */
export function riskExclusionsUpdate(
  client: GramCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.UpdateRiskExclusionRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.UpdateRiskExclusionRequestBody, {
    explode: true,
  });

  const path = pathToFunc("/rpc/risk.exclusions.update")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.RiskExclusion$inboundSchema),
    M.jsonErr(
