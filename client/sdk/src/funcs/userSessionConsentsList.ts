 ...
 */
export function userSessionConsentsList(
  client: GramCore,
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        z.optional(operations.ListUserSessionConsentsRequest$outboundSchema),
        value,
      ),
  ...
  const path = pathToFunc("/rpc/userSessionConsents.list")();

  const query = encodeFormQuery({
    "cursor": payload?.cursor,
    "limit": payload?.limit,
    "subject_urn": payload?.subject_urn,
    "user_session_client_id": payload?.user_session_client_id,
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListUserSessionConsentsResponse$inboundSchema, {
      key: "Result",
    }),
    M.jsonErr(
