 ...
 */
export function spendRulesRulesPreview(
  client: GramCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.PreviewSpendRuleRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.PreviewSpendRuleRequestBody, {
    explode: true,
  });

  const path = pathToFunc("/rpc/spendrules.previewRule")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.PreviewSpendRuleResult$inboundSchema),
    M.jsonErr(
