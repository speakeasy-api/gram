...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.GetMCPServerDetailsRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/rpc/mcpRegistries.getServerDetails")();

  const query = encodeFormQuery({
    "registry_id": payload.registry_id,
    "server_specifier": payload.server_specifier,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ExternalMCPServer$inboundSchema),
    M.jsonErr(
