 ...
 */
export function tunnelledMcpRotateServerKey(
  client: GramCore,
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        operations.RotateTunnelledMcpServerKeyRequest$outboundSchema,
        value,
      ),
  ...
  const body = encodeJSON("body", payload.RotateTunnelledMcpServerKeyForm, {
    explode: true,
  });

  const path = pathToFunc("/rpc/tunnelledMcp.rotateServerKey")();
  ...
  const context = {
    ...
    operationID: "rotateTunnelledMcpServerKey",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.RotateTunnelledMcpServerKeyResult$inboundSchema),
    M.jsonErr(
