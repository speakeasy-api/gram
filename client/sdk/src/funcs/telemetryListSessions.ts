...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.ListSessionsRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.ListSessionsPayload, {
    explode: true,
  });

  const path = pathToFunc("/rpc/telemetry.listSessions")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ListSessionsResult$inboundSchema),
    M.jsonErr(
