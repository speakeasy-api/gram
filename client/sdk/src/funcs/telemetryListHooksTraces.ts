 ...
 */
export function telemetryListHooksTraces(
  client: GramCore,
  ...
  Result<
    components.ListHooksTracesResult,
    | errors.ServiceError
    | GramError
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.ListHooksTracesRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.ListHooksTracesPayload, {
    explode: true,
  });

  const path = pathToFunc("/rpc/telemetry.listHooksTraces")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ListHooksTracesResult$inboundSchema),
    M.jsonErr(
