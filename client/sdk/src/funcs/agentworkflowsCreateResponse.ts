 ...
 */
export function agentworkflowsCreateResponse(
  client: GramCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.CreateResponseRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.WorkflowAgentRequest, {
    explode: true,
  });

  const path = pathToFunc("/rpc/workflows.createResponse")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.WorkflowAgentResponseOutput$inboundSchema),
    M.jsonErr(
