...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdatePackageRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.UpdatePackageForm, { explode: true });

  const path = pathToFunc("/rpc/packages.update")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.UpdatePackageResponse$inboundSchema),
    M.json(304, operations.UpdatePackageResponse$inboundSchema),
    M.jsonErr(
