...
| `reason`    | _string_                                                                                 | :heavy_minus_sign: | Why the rule isn't supported when `supported` is false.                                |
| `supported` | _boolean_                                                                                | :heavy_check_mark: | False when the rule has no text-only detector (e.g. `shadow_mcp`, `destructive_tool`). |
