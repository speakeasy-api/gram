...
| `enabled`        | _boolean_                                                                                     | :heavy_check_mark: | Whether forwarding is currently active.                                        |
| `endpointUrl`    | _string_                                                                                      | :heavy_check_mark: | URL each OTEL payload is POSTed to. Empty string when no config is set.        |
...
| `id`             | _string_                                                                                      | :heavy_minus_sign: | Config ID. Omitted when no config is set for the organization.                 |
| `organizationId` | _string_                                                                                      | :heavy_check_mark: | Organization the config belongs to.                                            |
