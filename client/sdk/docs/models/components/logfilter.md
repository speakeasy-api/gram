...
| `values`   | _string_[]                                                 | :heavy_minus_sign: | Values to compare against. Pass one value for single-value operators (eq, not_eq, contains) and multiple for 'in'. Ignored for 'exists' and 'not_exists'. |              |
