...
async function run() {
  ...
  if (res.ok) {
    ...
    console.log(
      "organizationsGenerateWorkOSAdminPortalLink failed:",
      res.error,
    );
  }
}
...
async function run() {
  const result = await gram.organizations.sendEnterpriseAdminOnboardingEmail({
    sendEnterpriseAdminOnboardingEmailRequestBody: {
      recipients: ["Talon_Bauch99@yahoo.com", "Garret_King9@hotmail.com"],
    },
  });
...
async function run() {
  ...
  if (res.ok) {
    ...
    console.log(
      "organizationsSendEnterpriseAdminOnboardingEmail failed:",
      res.error,
    );
  }
}
