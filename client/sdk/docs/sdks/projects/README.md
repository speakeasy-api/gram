...
async function run() {
  await gram.projects.setOrganizationWhitelist({
...
```typescript
import { GramCore } from "@gram/client/core.js";
import { projectsSetOrganizationWhitelist } from "@gram/client/funcs/projectsSetOrganizationWhitelist.js";
...
async function run() {
  const res = await projectsSetOrganizationWhitelist(gram, {
    setOrganizationWhitelistRequestBody: {
      organizationId: "<id>",
      whitelisted: true,
    },
