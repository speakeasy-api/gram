...
export class ClientSDK {
  constructor(options = {}) {
    ...
    options = __classPrivateFieldGet(this, _ClientSDK_hooks, "f").sdkInit(
      options,
    );
    ...
    if (
      !__classPrivateFieldGet(this, _ClientSDK_logger, "f") &&
      env().GRAM_DEBUG
    ) {
  ...
  _createRequest(context, conf, options) {
    ...
    let input;
    try {
      input = __classPrivateFieldGet(
        this,
        _ClientSDK_hooks,
        "f",
      ).beforeCreateRequest(context, {
        url: reqURL,
        options: {
          ...fetchOptions,
          body: conf.body ?? null,
    ...
    return retry(
      async () => {
        ...
        if (timeoutMs != null && timeoutMs > 0) {
          ...
          const combined =
            combineSignals(cloned.signal, timeoutSignal) ?? timeoutSignal;
          attempt = new Request(cloned, { signal: combined });
        }
        const req = await __classPrivateFieldGet(
          this,
          _ClientSDK_hooks,
          "f",
        ).beforeRequest(context, attempt);
        await logRequest(
          __classPrivateFieldGet(this, _ClientSDK_logger, "f"),
          req,
        ).catch((e) =>
          __classPrivateFieldGet(this, _ClientSDK_logger, "f")?.log(
            "Failed to log request:",
            e,
          ),
        );
        let response = await __classPrivateFieldGet(
          this,
          _ClientSDK_httpClient,
          "f",
        ).request(req);
        try {
          if (isErrorStatusCode(response.status)) {
            const result = await __classPrivateFieldGet(
              ...
              _ClientSDK_hooks,
              "f",
            ).afterError(context, response, null);
            if (result.error) {
              throw result.error;
            }
            response = result.response || response;
          } else {
            response = await __classPrivateFieldGet(
              this,
              _ClientSDK_hooks,
              "f",
...
((_ClientSDK_httpClient = new WeakMap()),
  (_ClientSDK_hooks = new WeakMap()),
  (_ClientSDK_logger = new WeakMap()));
