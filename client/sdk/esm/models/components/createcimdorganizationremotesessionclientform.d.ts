import * as z from "zod/v4-mini";
/**
 * Form for an org admin to register a standalone remote_session_client in Client ID Metadata Document (CIMD) mode under an existing issuer, with no user_session_issuer attachments. Gram generates the client_id and hosts the metadata document; the issuer must advertise client_id_metadata_document_supported.
 */
export type CreateCimdOrganizationRemoteSessionClientForm = {
  /**
   * Optional upstream OAuth audience to send on the authorize redirect and token exchange.
   */
  audience?: string | undefined;
  /**
   * Owning project id for the new client; the project must belong to the caller's organization. Omit to inherit a project-specific issuer's project, or to create an organization-level client (no project, attachable by every project) under an organization-level issuer.
   */
  projectId?: string | undefined;
  /**
   * The owning remote_session_issuer id; must belong to the caller's organization and advertise client_id_metadata_document_supported.
   */
  remoteSessionIssuerId: string;
  /**
   * Explicit upstream OAuth scopes the dance should request for this client. Omit to fall back to the issuer's scopes_supported.
   */
  scope?: Array<string> | undefined;
};
/** @internal */
export type CreateCimdOrganizationRemoteSessionClientForm$Outbound = {
  audience?: string | undefined;
  project_id?: string | undefined;
  remote_session_issuer_id: string;
  scope?: Array<string> | undefined;
};
/** @internal */
export declare const CreateCimdOrganizationRemoteSessionClientForm$outboundSchema: z.ZodMiniType<
  CreateCimdOrganizationRemoteSessionClientForm$Outbound,
  CreateCimdOrganizationRemoteSessionClientForm
>;
export declare function createCimdOrganizationRemoteSessionClientFormToJSON(
  createCimdOrganizationRemoteSessionClientForm: CreateCimdOrganizationRemoteSessionClientForm,
): string;
//# sourceMappingURL=createcimdorganizationremotesessionclientform.d.ts.map
