/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
import * as z from "zod/v4-mini";
import { remap as remap$ } from "../../lib/primitives.js";
import { safeParse } from "../../lib/schemas.js";
export const AuthzChallengeOperation = {
    Require: "require",
    RequireAny: "require_any",
    Filter: "filter",
};
export const AuthzChallengeOutcome = {
    Allow: "allow",
    Deny: "deny",
    Error: "error",
};
/**
 * Kind of principal.
 */
export const AuthzChallengePrincipalType = {
    User: "user",
    ApiKey: "api_key",
    Assistant: "assistant",
};
export const AuthzChallengeReason = {
    GrantMatched: "grant_matched",
    NoGrants: "no_grants",
    ScopeUnsatisfied: "scope_unsatisfied",
    DenyGrant: "deny_grant",
    InvalidCheck: "invalid_check",
    RbacSkippedApikey: "rbac_skipped_apikey",
    DevOverride: "dev_override",
};
/**
 * How the challenge was resolved.
 */
export const AuthzChallengeResolutionType = {
    RoleAssigned: "role_assigned",
    Dismissed: "dismissed",
};
/** @internal */
export const AuthzChallengeOperation$inboundSchema = z.enum(AuthzChallengeOperation);
/** @internal */
export const AuthzChallengeOutcome$inboundSchema = z.enum(AuthzChallengeOutcome);
/** @internal */
export const AuthzChallengePrincipalType$inboundSchema = z.enum(AuthzChallengePrincipalType);
/** @internal */
export const AuthzChallengeReason$inboundSchema = z.enum(AuthzChallengeReason);
/** @internal */
export const AuthzChallengeResolutionType$inboundSchema = z.enum(AuthzChallengeResolutionType);
/** @internal */
export const AuthzChallenge$inboundSchema = z.pipe(z.object({
    evaluated_grant_count: z.int(),
    id: z.string(),
    matched_grant_count: z.int(),
    operation: AuthzChallengeOperation$inboundSchema,
    organization_id: z.string(),
    outcome: AuthzChallengeOutcome$inboundSchema,
    photo_url: z.optional(z.string()),
    principal_type: AuthzChallengePrincipalType$inboundSchema,
    principal_urn: z.string(),
    project_id: z.optional(z.string()),
    reason: AuthzChallengeReason$inboundSchema,
    resolution_role_slug: z.optional(z.string()),
    resolution_type: z.optional(AuthzChallengeResolutionType$inboundSchema),
    resolved_at: z.optional(z.pipe(z.iso.datetime({ offset: true }), z.transform(v => new Date(v)))),
    resolved_by: z.optional(z.string()),
    resource_id: z.optional(z.string()),
    resource_kind: z.optional(z.string()),
    role_slugs: z.array(z.string()),
    scope: z.string(),
    timestamp: z.pipe(z.iso.datetime({ offset: true }), z.transform(v => new Date(v))),
    user_email: z.optional(z.string()),
}), z.transform((v) => {
    return remap$(v, {
        "evaluated_grant_count": "evaluatedGrantCount",
        "matched_grant_count": "matchedGrantCount",
        "organization_id": "organizationId",
        "photo_url": "photoUrl",
        "principal_type": "principalType",
        "principal_urn": "principalUrn",
        "project_id": "projectId",
        "resolution_role_slug": "resolutionRoleSlug",
        "resolution_type": "resolutionType",
        "resolved_at": "resolvedAt",
        "resolved_by": "resolvedBy",
        "resource_id": "resourceId",
        "resource_kind": "resourceKind",
        "role_slugs": "roleSlugs",
        "user_email": "userEmail",
    });
}));
export function authzChallengeFromJSON(jsonString) {
    return safeParse(jsonString, (x) => AuthzChallenge$inboundSchema.parse(JSON.parse(x)), `Failed to parse 'AuthzChallenge' from JSON`);
}
//# sourceMappingURL=authzchallenge.js.map