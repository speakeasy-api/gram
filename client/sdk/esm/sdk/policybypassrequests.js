/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
import { riskPolicyBypassRequestsApprove } from "../funcs/riskPolicyBypassRequestsApprove.js";
import { riskPolicyBypassRequestsCreate } from "../funcs/riskPolicyBypassRequestsCreate.js";
import { riskPolicyBypassRequestsDeny } from "../funcs/riskPolicyBypassRequestsDeny.js";
import { riskPolicyBypassRequestsList } from "../funcs/riskPolicyBypassRequestsList.js";
import { riskPolicyBypassRequestsRevoke } from "../funcs/riskPolicyBypassRequestsRevoke.js";
import { ClientSDK } from "../lib/sdks.js";
import { unwrapAsync } from "../types/fp.js";
export class PolicyBypassRequests extends ClientSDK {
    /**
     * approveRiskPolicyBypassRequest risk
     *
     * @remarks
     * Approve a risk policy bypass request for the requested policy target.
     */
    async approve(request, security, options) {
        return unwrapAsync(riskPolicyBypassRequestsApprove(this, request, security, options));
    }
    /**
     * createRiskPolicyBypassRequest risk
     *
     * @remarks
     * Create or refresh a risk policy bypass request from a signed request URL token.
     */
    async create(request, security, options) {
        return unwrapAsync(riskPolicyBypassRequestsCreate(this, request, security, options));
    }
    /**
     * denyRiskPolicyBypassRequest risk
     *
     * @remarks
     * Deny a risk policy bypass request, updating workflow state.
     */
    async deny(request, security, options) {
        return unwrapAsync(riskPolicyBypassRequestsDeny(this, request, security, options));
    }
    /**
     * listRiskPolicyBypassRequests risk
     *
     * @remarks
     * List current risk policy bypass request workflow records.
     */
    async list(request, security, options) {
        return unwrapAsync(riskPolicyBypassRequestsList(this, request, security, options));
    }
    /**
     * revokeRiskPolicyBypassRequest risk
     *
     * @remarks
     * Revoke a previously approved risk policy bypass request.
     */
    async revoke(request, security, options) {
        return unwrapAsync(riskPolicyBypassRequestsRevoke(this, request, security, options));
    }
}
//# sourceMappingURL=policybypassrequests.js.map