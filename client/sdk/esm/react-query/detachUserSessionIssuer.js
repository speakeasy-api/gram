/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
import { useMutation, } from "@tanstack/react-query";
import { remoteSessionClientsDetachUserSessionIssuer } from "../funcs/remoteSessionClientsDetachUserSessionIssuer.js";
import { combineSignals } from "../lib/primitives.js";
import { unwrapAsync } from "../types/fp.js";
import { useGramContext } from "./_context.js";
/**
 * detachUserSessionIssuer remoteSessionClients
 *
 * @remarks
 * Detach a user_session_issuer from a remote_session_client by removing the binding from the join table. A no-op when the binding does not exist.
 */
export function useDetachUserSessionIssuerMutation(options) {
    const client = useGramContext();
    return useMutation({
        ...buildDetachUserSessionIssuerMutation(client, options),
        ...options,
    });
}
export function mutationKeyDetachUserSessionIssuer() {
    return ["@gram/client", "remoteSessionClients", "detachUserSessionIssuer"];
}
export function buildDetachUserSessionIssuerMutation(client$, hookOptions) {
    return {
        mutationKey: mutationKeyDetachUserSessionIssuer(),
        mutationFn: function detachUserSessionIssuerMutationFn({ request, security, options, }) {
            const mergedOptions = {
                ...hookOptions,
                ...options,
                fetchOptions: {
                    ...hookOptions?.fetchOptions,
                    ...options?.fetchOptions,
                    signal: combineSignals(hookOptions?.fetchOptions?.signal, options?.fetchOptions?.signal),
                },
            };
            return unwrapAsync(remoteSessionClientsDetachUserSessionIssuer(client$, request, security, mergedOptions));
        },
    };
}
//# sourceMappingURL=detachUserSessionIssuer.js.map