/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
import { useMutation, } from "@tanstack/react-query";
import { remoteSessionIssuersDiscover } from "../funcs/remoteSessionIssuersDiscover.js";
import { combineSignals } from "../lib/primitives.js";
import { unwrapAsync } from "../types/fp.js";
import { useGramContext } from "./_context.js";
/**
 * discoverRemoteSessionIssuer remoteSessionIssuers
 *
 * @remarks
 * Hit an upstream issuer's RFC 8414 .well-known/oauth-authorization-server document and return a draft suitable for createRemoteSessionIssuer. No persistence.
 */
export function useDiscoverRemoteSessionIssuerMutation(options) {
    const client = useGramContext();
    return useMutation({
        ...buildDiscoverRemoteSessionIssuerMutation(client, options),
        ...options,
    });
}
export function mutationKeyDiscoverRemoteSessionIssuer() {
    return ["@gram/client", "remoteSessionIssuers", "discover"];
}
export function buildDiscoverRemoteSessionIssuerMutation(client$, hookOptions) {
    return {
        mutationKey: mutationKeyDiscoverRemoteSessionIssuer(),
        mutationFn: function discoverRemoteSessionIssuerMutationFn({ request, security, options, }) {
            const mergedOptions = {
                ...hookOptions,
                ...options,
                fetchOptions: {
                    ...hookOptions?.fetchOptions,
                    ...options?.fetchOptions,
                    signal: combineSignals(hookOptions?.fetchOptions?.signal, options?.fetchOptions?.signal),
                },
            };
            return unwrapAsync(remoteSessionIssuersDiscover(client$, request, security, mergedOptions));
        },
    };
}
//# sourceMappingURL=discoverRemoteSessionIssuer.js.map