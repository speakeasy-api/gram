/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
import { useMutation, } from "@tanstack/react-query";
import { remoteSessionClientsUpdate } from "../funcs/remoteSessionClientsUpdate.js";
import { combineSignals } from "../lib/primitives.js";
import { unwrapAsync } from "../types/fp.js";
import { useGramContext } from "./_context.js";
/**
 * updateRemoteSessionClient remoteSessionClients
 *
 * @remarks
 * Rotate the client_secret or change the non-issuer settings on an existing remote_session_client. Issuer attachments are managed via attachUserSessionIssuer / detachUserSessionIssuer.
 */
export function useUpdateRemoteSessionClientMutation(options) {
    const client = useGramContext();
    return useMutation({
        ...buildUpdateRemoteSessionClientMutation(client, options),
        ...options,
    });
}
export function mutationKeyUpdateRemoteSessionClient() {
    return ["@gram/client", "remoteSessionClients", "update"];
}
export function buildUpdateRemoteSessionClientMutation(client$, hookOptions) {
    return {
        mutationKey: mutationKeyUpdateRemoteSessionClient(),
        mutationFn: function updateRemoteSessionClientMutationFn({ request, security, options, }) {
            const mergedOptions = {
                ...hookOptions,
                ...options,
                fetchOptions: {
                    ...hookOptions?.fetchOptions,
                    ...options?.fetchOptions,
                    signal: combineSignals(hookOptions?.fetchOptions?.signal, options?.fetchOptions?.signal),
                },
            };
            return unwrapAsync(remoteSessionClientsUpdate(client$, request, security, mergedOptions));
        },
    };
}
//# sourceMappingURL=updateRemoteSessionClient.js.map