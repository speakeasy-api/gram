...
import { ServeChatAttachmentRequest$outboundSchema, ServeChatAttachmentResponse$inboundSchema, } from "../models/operations/servechatattachment.js";
 ...
 */
export function assetsServeChatAttachment(client, request, security, options) {
    return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
    const parsed = safeParse(request, (value) => z.parse(ServeChatAttachmentRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
        return [parsed, { status: "invalid" }];
    }
    const payload = parsed.value;
    const body = null;
    const path = pathToFunc("/rpc/assets.serveChatAttachment")();
    ...
    const requestSecurity = resolveSecurity([
    ...
    ], [
        {
            ...
            type: "http:bearer",
            value: security?.chatSessionsTokenHeaderGramChatSession,
        },
    ]);
    const context = {
    ...
    const [result] = await M.match(M.stream(200, ServeChatAttachmentResponse$inboundSchema, {
...
//# sourceMappingURL=assetsServeChatAttachment.js.map