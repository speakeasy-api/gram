...
import { DownloadPluginPackageRequest$outboundSchema, DownloadPluginPackageResponse$inboundSchema, } from "../models/operations/downloadpluginpackage.js";
 ...
 */
export function pluginsDownloadPluginPackage(client, request, security, options) {
    return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
    const parsed = safeParse(request, (value) => z.parse(DownloadPluginPackageRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result] = await M.match(M.stream(200, DownloadPluginPackageResponse$inboundSchema, {
