...
import { TestDetectionRuleResult$inboundSchema, } from "../models/components/testdetectionruleresult.js";
import { ServiceError$inboundSchema, } from "../models/errors/serviceerror.js";
import { TestDetectionRuleRequest$outboundSchema, } from "../models/operations/testdetectionrule.js";
 ...
 */
export function riskRulesTest(client, request, security, options) {
    return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
    const parsed = safeParse(request, (value) => z.parse(TestDetectionRuleRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
        return [parsed, { status: "invalid" }];
    }
    const payload = parsed.value;
    const body = encodeJSON("body", payload.TestDetectionRuleRequestBody, {
        explode: true,
    });
    const path = pathToFunc("/rpc/risk.testRule")();
    ...
    const [result] = await M.match(M.json(200, TestDetectionRuleResult$inboundSchema), M.jsonErr([400, 401, 403, 404, 409, 415, 422], ServiceError$inboundSchema), M.jsonErr([500, 502], ServiceError$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, req, { extraFields: responseFields });
    if (!result.ok) {
        return [result, { status: "complete", request: req, response }];
    }
    return [result, { status: "complete", request: req, response }];
}
//# sourceMappingURL=riskRulesTest.js.map