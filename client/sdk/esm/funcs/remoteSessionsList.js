...
import { ListRemoteSessionsRequest$outboundSchema, ListRemoteSessionsResponse$inboundSchema, } from "../models/operations/listremotesessions.js";
 ...
 */
export function remoteSessionsList(client, request, security, options) {
    return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
    const parsed = safeParse(request, (value) => z.parse(z.optional(ListRemoteSessionsRequest$outboundSchema), value), "Input validation failed");
    if (!parsed.ok) {
        return [haltIterator(parsed), { status: "invalid" }];
    }
    const payload = parsed.value;
    const body = null;
    const path = pathToFunc("/rpc/remoteSessions.list")();
    ...
    const [result, raw] = await M.match(M.json(200, ListRemoteSessionsResponse$inboundSchema, { key: "Result" }), M.jsonErr([400, 401, 403, 404, 409, 415, 422], ServiceError$inboundSchema), M.jsonErr([500, 502], ServiceError$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, req, { extraFields: responseFields });
    if (!result.ok) {
    ...
    const nextFunc = (responseData) => {
        ...
        const nextVal = () => remoteSessionsList(client, {
            ...request,
            cursor: nextCursor,
        }, security, options);
...
//# sourceMappingURL=remoteSessionsList.js.map