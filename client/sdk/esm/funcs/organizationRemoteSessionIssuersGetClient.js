 ...
 */
export function organizationRemoteSessionIssuersGetClient(client, request, security, options) {
    return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
    const parsed = safeParse(request, (value) => z.parse(operations.GetOrganizationRemoteSessionClientRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
        return [parsed, { status: "invalid" }];
    }
    const payload = parsed.value;
    const body = null;
    const path = pathToFunc("/rpc/organizationRemoteSessionIssuers.getClient")();
...
//# sourceMappingURL=organizationRemoteSessionIssuersGetClient.js.map