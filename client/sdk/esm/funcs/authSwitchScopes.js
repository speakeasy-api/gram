...
import { SwitchAuthScopesRequest$outboundSchema, SwitchAuthScopesResponse$inboundSchema, } from "../models/operations/switchauthscopes.js";
 ...
 */
export function authSwitchScopes(client, request, security, options) {
    return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
    const parsed = safeParse(request, (value) => z.parse(z.optional(SwitchAuthScopesRequest$outboundSchema), value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result] = await M.match(M.nil(200, z.optional(SwitchAuthScopesResponse$inboundSchema), {
        hdrs: true,
...
//# sourceMappingURL=authSwitchScopes.js.map