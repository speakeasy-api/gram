 ...
 */
export function organizationRemoteSessionIssuersCreateClient(client, request, security, options) {
    return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
    const parsed = safeParse(request, (value) => z.parse(operations.CreateOrganizationRemoteSessionClientRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
        return [parsed, { status: "invalid" }];
    }
    const payload = parsed.value;
    const body = encodeJSON("body", payload.CreateOrganizationRemoteSessionClientForm, { explode: true });
    const path = pathToFunc("/rpc/organizationRemoteSessionIssuers.createClient")();
...
//# sourceMappingURL=organizationRemoteSessionIssuersCreateClient.js.map