...
import { DownloadObservabilityPluginRequest$outboundSchema, DownloadObservabilityPluginResponse$inboundSchema, } from "../models/operations/downloadobservabilityplugin.js";
 ...
 */
export function pluginsDownloadObservabilityPlugin(client, request, security, options) {
    return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
    const parsed = safeParse(request, (value) => z.parse(DownloadObservabilityPluginRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result] = await M.match(M.stream(200, DownloadObservabilityPluginResponse$inboundSchema, {
