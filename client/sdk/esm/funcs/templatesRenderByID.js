...
import { RenderTemplateResult$inboundSchema, } from "../models/components/rendertemplateresult.js";
import { ServiceError$inboundSchema, } from "../models/errors/serviceerror.js";
import { RenderTemplateByIDRequest$outboundSchema, } from "../models/operations/rendertemplatebyid.js";
 ...
 */
export function templatesRenderByID(client, request, security, options) {
    return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
    const parsed = safeParse(request, (value) => z.parse(RenderTemplateByIDRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
        return [parsed, { status: "invalid" }];
    }
    const payload = parsed.value;
    const body = encodeJSON("body", payload.RenderTemplateByIDRequestBody, {
        explode: true,
    });
    ...
    const [result] = await M.match(M.json(200, RenderTemplateResult$inboundSchema), M.jsonErr([400, 401, 403, 404, 409, 415, 422], ServiceError$inboundSchema), M.jsonErr([500, 502], ServiceError$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, req, { extraFields: responseFields });
    if (!result.ok) {
        return [result, { status: "complete", request: req, response }];
    }
    return [result, { status: "complete", request: req, response }];
}
//# sourceMappingURL=templatesRenderByID.js.map