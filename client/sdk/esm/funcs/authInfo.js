...
import { SessionInfoRequest$outboundSchema, SessionInfoResponse$inboundSchema, } from "../models/operations/sessioninfo.js";
 ...
 */
export function authInfo(client, request, security, options) {
    return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
    const parsed = safeParse(request, (value) => z.parse(z.optional(SessionInfoRequest$outboundSchema), value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result] = await M.match(M.json(200, SessionInfoResponse$inboundSchema, {
        hdrs: true,
        key: "Result",
