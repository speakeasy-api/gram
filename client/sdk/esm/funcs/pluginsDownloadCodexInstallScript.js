...
import { DownloadCodexInstallScriptRequest$outboundSchema, DownloadCodexInstallScriptResponse$inboundSchema, } from "../models/operations/downloadcodexinstallscript.js";
 ...
 */
export function pluginsDownloadCodexInstallScript(client, request, security, options) {
    return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
    const parsed = safeParse(request, (value) => z.parse(z.optional(DownloadCodexInstallScriptRequest$outboundSchema), value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result] = await M.match(M.stream(200, DownloadCodexInstallScriptResponse$inboundSchema, {
