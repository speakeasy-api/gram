...
import { IngestHookResult$inboundSchema, } from "../models/components/ingesthookresult.js";
import { ServiceError$inboundSchema, } from "../models/errors/serviceerror.js";
import { IngestHookEventRequest$outboundSchema, } from "../models/operations/ingesthookevent.js";
 ...
 */
export function hooksIngest(client, request, security, options) {
    return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
    const parsed = safeParse(request, (value) => z.parse(IngestHookEventRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result] = await M.match(M.json(200, IngestHookResult$inboundSchema), M.jsonErr([400, 401, 403, 404, 409, 415, 422], ServiceError$inboundSchema), M.jsonErr([500, 502], ServiceError$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, req, { extraFields: responseFields });
    if (!result.ok) {
        return [result, { status: "complete", request: req, response }];
    }
    return [result, { status: "complete", request: req, response }];
}
//# sourceMappingURL=hooksIngest.js.map