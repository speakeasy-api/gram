 ...
 */
export function slackSlackLogin(client, request, security, options) {
  return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
  const parsed = safeParse(
    request,
    (value) => operations.SlackLoginRequest$outboundSchema.parse(value),
  ...
  const body = null;
  const pathParams = {
    project_slug: encodeSimple("project_slug", payload.project_slug, {
      explode: false,
  ...
  const requestSecurity = resolveSecurity([
    {
      ...
      type: "apiKey:query",
      value: security?.projectSlugQueryProjectSlug,
    },
    {
  ...
  const [result] = await M.match(
    M.nil(307, operations.SlackLoginResponse$inboundSchema.optional(), {
      hdrs: true,
    }),
    M.jsonErr(
...
//# sourceMappingURL=slackSlackLogin.js.map
