...
import { ProtectedResourceMetadataDiscovery$inboundSchema, } from "../models/components/protectedresourcemetadatadiscovery.js";
import { ServiceError$inboundSchema, } from "../models/errors/serviceerror.js";
import { DiscoverRemoteMcpProtectedResourceMetadataRequest$outboundSchema, } from "../models/operations/discoverremotemcpprotectedresourcemetadata.js";
 ...
 */
export function remoteMcpDiscoverProtectedResourceMetadata(client, request, security, options) {
    return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
    const parsed = safeParse(request, (value) => z.parse(DiscoverRemoteMcpProtectedResourceMetadataRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
        return [parsed, { status: "invalid" }];
    }
    const payload = parsed.value;
    const body = encodeJSON("body", payload.DiscoverProtectedResourceMetadataRequestBody, { explode: true });
    ...
    const [result] = await M.match(M.json(200, ProtectedResourceMetadataDiscovery$inboundSchema), M.jsonErr([400, 401, 403, 404, 409, 415, 422], ServiceError$inboundSchema), M.jsonErr([500, 502], ServiceError$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, req, { extraFields: responseFields });
    if (!result.ok) {
        return [result, { status: "complete", request: req, response }];
    }
    return [result, { status: "complete", request: req, response }];
}
//# sourceMappingURL=remoteMcpDiscoverProtectedResourceMetadata.js.map