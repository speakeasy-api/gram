...
import { ListRemoteSessionIssuersRequest$outboundSchema, ListRemoteSessionIssuersResponse$inboundSchema, } from "../models/operations/listremotesessionissuers.js";
 ...
 */
export function remoteSessionIssuersList(client, request, security, options) {
    return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
    const parsed = safeParse(request, (value) => z.parse(z.optional(ListRemoteSessionIssuersRequest$outboundSchema), value), "Input validation failed");
    if (!parsed.ok) {
        return [haltIterator(parsed), { status: "invalid" }];
    }
    const payload = parsed.value;
    const body = null;
    const path = pathToFunc("/rpc/remoteSessionIssuers.list")();
    ...
    const [result, raw] = await M.match(M.json(200, ListRemoteSessionIssuersResponse$inboundSchema, {
        key: "Result",
    ...
    const nextFunc = (responseData) => {
        ...
        const nextVal = () => remoteSessionIssuersList(client, {
            ...request,
            cursor: nextCursor,
        }, security, options);
...
//# sourceMappingURL=remoteSessionIssuersList.js.map