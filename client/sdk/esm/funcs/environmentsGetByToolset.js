...
import { GetToolsetEnvironmentRequest$outboundSchema, } from "../models/operations/gettoolsetenvironment.js";
 ...
 */
export function environmentsGetByToolset(client, request, security, options) {
    return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
    const parsed = safeParse(request, (value) => z.parse(GetToolsetEnvironmentRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
        return [parsed, { status: "invalid" }];
    }
    const payload = parsed.value;
    const body = null;
    const path = pathToFunc("/rpc/environments.getToolsetEnvironment")();
...
//# sourceMappingURL=environmentsGetByToolset.js.map