...
import { ServeFunctionRequest$outboundSchema, ServeFunctionResponse$inboundSchema, } from "../models/operations/servefunction.js";
 ...
 */
export function assetsServeFunction(client, request, security, options) {
    return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
    const parsed = safeParse(request, (value) => z.parse(ServeFunctionRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result] = await M.match(M.stream(200, ServeFunctionResponse$inboundSchema, {
...
//# sourceMappingURL=assetsServeFunction.js.map