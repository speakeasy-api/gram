 ...
 */
export function teamsListInvites(client, request, security, options) {
  return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.ListTeamInvitesRequest$outboundSchema, value),
  ...
  const body = null;
  const path = pathToFunc("/rpc/teams.listInvites")();
  ...
  const [result] = await M.match(
    M.json(200, components.ListInvitesResult$inboundSchema),
    M.jsonErr(
...
//# sourceMappingURL=teamsListInvites.js.map
