 ...
 */
export function riskListPolicies(client, request, security, options) {
    return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
    const parsed = safeParse(request, (value) => z.parse(z.optional(operations.ListRiskPoliciesRequest$outboundSchema), value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result] = await M.match(M.json(200, components.ListRiskPoliciesResult$inboundSchema), M.jsonErr([400, 401, 403, 404, 409, 415, 422], errors.ServiceError$inboundSchema), M.jsonErr([500, 502], errors.ServiceError$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, req, { extraFields: responseFields });
    if (!result.ok) {
