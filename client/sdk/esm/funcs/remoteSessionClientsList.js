...
import { ListRemoteSessionClientsRequest$outboundSchema, ListRemoteSessionClientsResponse$inboundSchema, } from "../models/operations/listremotesessionclients.js";
 ...
 */
export function remoteSessionClientsList(client, request, security, options) {
    return new APIPromise($do(client, request, security, options));
}
async function $do(client, request, security, options) {
    const parsed = safeParse(request, (value) => z.parse(z.optional(ListRemoteSessionClientsRequest$outboundSchema), value), "Input validation failed");
    if (!parsed.ok) {
        return [haltIterator(parsed), { status: "invalid" }];
    }
    const payload = parsed.value;
    const body = null;
    const path = pathToFunc("/rpc/remoteSessionClients.list")();
    const query = encodeFormQuery({
        "cursor": payload?.cursor,
        "limit": payload?.limit,
        "remote_session_issuer_id": payload?.remote_session_issuer_id,
    ...
    const [result, raw] = await M.match(M.json(200, ListRemoteSessionClientsResponse$inboundSchema, {
        key: "Result",
    ...
    const nextFunc = (responseData) => {
        ...
        const nextVal = () => remoteSessionClientsList(client, {
            ...request,
            cursor: nextCursor,
        }, security, options);
...
//# sourceMappingURL=remoteSessionClientsList.js.map