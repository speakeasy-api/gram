...
// The directory handoff: the SCIM endpoint and bearer token a platform operator
// takes out of the WorkOS dashboard and stores against one organization, so
// that organization's own onboarding can configure Okta from stored values
// instead of sending its administrator through a portal.
//
// The token is write-only here and everywhere else. It goes out on the save and
// never comes back: the read carries a fingerprint, which is enough to tell one
// stored token from another and useless for anything else.
//
// The server half of this is not on the branch yet. The three paths below and
// every field name in this block are the contract's shape, and this is the one
// place that spells them, so adapting to the RPC as generated is an edit here
// and nowhere else.
export type DirectoryHandoff = {
  organization_id: string;
  scim_base_url: string;
  token_fingerprint: string;
  // Filled from WorkOS on the read when the directory is found, so both are
  // absent until the directory exists rather than meaning it does not.
  workos_directory_id?: string;
  workos_directory_state?: string;
  set_by: string;
  updated_at: string;
};

// Which WorkOS environment this server's API key belongs to. Read off the key's
// own prefix by the server, so it says which dashboard environment the operator
// has to be in before creating anything. `unknown` where the server cannot tell.
export const WORKOS_ENVIRONMENTS = [
  "development",
  "production",
  "unknown",
] as const;

export type WorkosEnvironment = (typeof WORKOS_ENVIRONMENTS)[number];

function asWorkosEnvironment(value: unknown): WorkosEnvironment {
  return WORKOS_ENVIRONMENTS.includes(value as WorkosEnvironment)
    ? (value as WorkosEnvironment)
    : "unknown";
}

export type DirectoryHandoffResult = {
  // Absent and null both mean nothing is stored. Goa omits an unset attribute
  // rather than sending null, and the contract writes it as null, so the two
  // are read as one thing here instead of at every call site.
  handoff?: DirectoryHandoff | null;
  // Answered whether or not a handoff is stored: the operator needs to know
  // which environment to open before there is anything to store.
  workos_environment: WorkosEnvironment;
};

export async function getOrganizationDirectoryHandoff(
  organizationID: string,
): Promise<DirectoryHandoffResult> {
  const qs = toSearchParams({ organization_id: organizationID });
  const result = await gramAdminFetch<DirectoryHandoffResult>(
    `/admin/organization.directoryHandoff?${qs}`,
    // The read asks WorkOS for the live directory state, and a stale answer
    // would report a directory as unlinked after it linked.
    { cache: "no-store" },
  );
  // Narrowed rather than trusted: an environment this build has no name for
  // reads as unknown, which the section already draws, instead of reaching the
  // copy as a word the operator has never seen.
  return {
    ...result,
    workos_environment: asWorkosEnvironment(result.workos_environment),
  };
}

export type SetDirectoryHandoffRequest = {
  organization_id: string;
  scim_base_url: string;
  // Write-only. Nothing may read this back, store it, or put it in a cache: the
  // request is the last place in this app the token exists.
  scim_token: string;
};

export function setOrganizationDirectoryHandoff(
  body: SetDirectoryHandoffRequest,
): Promise<DirectoryHandoff> {
  return gramAdminMutation<DirectoryHandoff>(
    "/admin/organization.setDirectoryHandoff",
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
...
export function clearOrganizationDirectoryHandoff(
  ...
  return gramAdminSend("/admin/organization.clearDirectoryHandoff", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
