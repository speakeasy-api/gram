...
// The smallest text drawn in a tone is badge text at 12px/500, and the trial
// callout's is not large either, so the AA threshold is the 4.5:1 one rather
// than 3:1.
...
describe("tone", () => {
  it.each(TONES)("%s is a non-empty class string", (name) => {
    expect(tone[name].trim()).not.toBe("");
  });

  // Both schemes, because a tone that reads in one is not a tone that reads.
  // Every consumer inherits whatever this allows, which is why the check lives
  // beside the tones rather than beside any one badge or callout.
  it.each(TONES)("%s clears AA against its own light background", (name) => {
  ...
  it.each(TONES)("%s clears AA against its own dark background", (name) => {
    const classes = tone[name];
    expect(
  ...
  // The colours travel to anything that wants the tone, so a typographic
  // decision cannot travel with them: on the trial callout `uppercase` would
  // shout a whole sentence. The badge adds its own.
  it.each(TONES)("%s carries colour only", (name) => {
    expect(tone[name]).not.toContain("uppercase");
  });

  // The one place these deviate from the drawing, pinned by value. Every other
  // test here is satisfied by any colour that clears AA, so without this the
  // deviation can be retuned away and the reason for it lost.
  it("keeps the warning foreground darkened from the drawing's own token", () => {
    expect(tone.warning).toContain("text-[hsl(21_70%_37%)]");
  });
