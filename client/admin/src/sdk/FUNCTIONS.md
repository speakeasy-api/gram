...
import { adminAdminLogout } from "@gram/admin-client/funcs/adminAdminLogout.js";
...
async function run() {
  const res = await adminAdminLogout(gram);
  if (res.ok) {
    const { value: result } = res;
    
  } else {
    console.log("adminAdminLogout failed:", res.error);
  }
}
