...
function sortToolsByMethod(tools: Tool[]): Tool[] {
  const httpTools = tools.filter((t) => t.type === "http") as (Tool & {
    type: "http";
  })[];
...
function groupTools(tools: Tool[]): ToolGroup[] {
  ...
  tools.forEach((tool) => {
    if (
      tool.type === "prompt" &&
      tool.kind === PromptTemplateKind.HigherOrderTool
    ) {
      higherOrderTools.push(tool);
    } else if (tool.type === "http" && tool.packageName) {
      // Group HTTP tools from packages
      const existing = packageMap.get(tool.packageName) || [];
      packageMap.set(tool.packageName, [...existing, tool]);
    } else if (tool.type === "function") {
      // Group all function tools together
      functionTools.push(tool);
    } else {
      // Everything else (HTTP tools without packages, etc.)
...
function ToolRow({
  ...
  const isDisabled =
    tool.type === "prompt" && tool.kind === PromptTemplateKind.HigherOrderTool;
  ...
  const actions = [
    ...
    {
      label: "Remove",
      onClick: () => onCheckboxChange(!isSelected),
      icon: "trash" as const,
      destructive: true,
    },
  ];
...
function ToolGroupHeader({
  ...
  return (
    <button
      ...
      aria-label={`${isExpanded ? "Collapse" : "Expand"} ${displayTitle} group`}
      className={cn(
    ...
    >
      <div className="flex gap-4 items-center">
        ...
        <p className="text-sm leading-6 text-foreground">{displayTitle}</p>
      </div>
      <ChevronDown
        className={cn(
...
export function ToolList({
  ...
  // Helper to get the identifier for a tool based on mode
  const getToolIdentifier = (tool: Tool): string => {
    if (selectionMode === "add" && tool.type === "http") {
      return tool.type === "http" ? tool.toolUrn : "";
    }
        ...
        if (
          ...
          const isCurrentlySelected =
            selectionMode === "add"
              ...
              : selectedForRemoval.has(tool.canonicalName);
