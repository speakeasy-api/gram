 ...
 *
 *   - `## Enable Box AI {#enable-box-ai}` becomes a heading carrying
 *     `id="enable-box-ai"`, so the in-document links guides use to
 *     cross-reference their own sections resolve instead of dangling.
 ...
 */
export function remarkSetupGuide(): (tree: unknown) => void {
  return (tree) => {
    visitSetupGuideNode(tree as MarkdownNode);
  };
}

function visitSetupGuideNode(node: MarkdownNode): void {
...
function liftHeadingId(heading: MarkdownNode): void {
