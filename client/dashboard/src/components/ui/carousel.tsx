...
} & Omit<CarouselProps, "onItemChange">;
...
function Carousel({
  ...
  const scrollPrev = React.useCallback(() => {
    api?.scrollPrev();
    onItemChange?.(api?.selectedScrollSnap() ?? 0);
  }, [api, onItemChange]);
  ...
  return (
    <CarouselContext.Provider
      value={{
        ...
        canScrollPrev,
        canScrollNext,
        numItems: api?.slideNodes().length ?? 0,
...
function CarouselDots({ className, ...props }: React.ComponentProps<"div">) {
  const { activeIndex, numItems, setActiveIndex } = useCarousel();
