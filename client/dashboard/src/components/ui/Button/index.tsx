...
interface BaseButtonProps extends Attributes {
  asChild?: boolean;
...
export interface ButtonProps
  extends
    BaseButtonProps,
    // The base props deliberately drop `style` and the button-specific
    // attributes; app code needs both.
    Omit<React.ButtonHTMLAttributes<HTMLButtonElement>, keyof BaseButtonProps> {
  /** Renders a leading icon (or trailing, with `iconAfter`) by design-system name. */
  icon?: IconName;
  /** Moves `icon` to the right of the label. */
  iconAfter?: boolean;
  /** Wraps the button in a tooltip. Survives `disabled`, unlike a bare trigger. */
 ...
 *
 * Wraps the base button with the conveniences the dashboard relies on: a
 * by-name `icon`, a `tooltip` that still shows while disabled, and `href`
 * for link-styled actions.
 */
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ icon, iconAfter = false, tooltip, href, children, ...props }, ref) => {
    ...
    const button = (
      <BaseButton ref={ref} {...props} {...(href ? { asChild: true } : {})}>
    ...
    return (
      <Tooltip>
        {/* The span keeps the trigger interactive when the button is disabled. */}
