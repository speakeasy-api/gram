...
// The staleness window matches the poll: these banners remount on every
// navigation, and stale data is re-read on mount, so a shorter window would
// make the cadence a floor nobody observes.
...
describe("PaygCapReachedBanners", () => {
  ...
  it("reports reached caps during an active enterprise trial", () => {
  ...
  it("keeps a post-checkout Stripe trial read-only", () => {
    subscriptionState({
      data: subscription({
        status: "trialing",
        trialEnd: new Date("2026-08-25T00:00:00.000Z"),
      }),
    });

    render(<PaygCapReachedBanners />);

    expect(screen.getByText(/reached its trial allowance/i)).toBeTruthy();
    expect(
  ...
  it("keeps cap actions read-only until Stripe state is known", () => {
    subscriptionState();
  ...
  it("does not tell a member to raise a trial cap", () => {
    mocks.productTier.mockReturnValue("enterprise");
    mocks.session.mockReturnValue({
      trial: {
        startedAt: new Date(Date.now() - 24 * 60 * 60 * 1000),
    ...
    expect(
      screen.getByText(/read-only until pay as you go begins/i),
    ).toBeTruthy();
    expect(
      screen.queryByText(/ask an organization admin to raise/i),
    ).toBeNull();
  });
  ...
  // to do with, and its control is read-only — so the banner would name the
  // wrong cause and offer an action that can't end the pause.
  it("says nothing about a disabled key that is over its cap", () => {
    inferenceCapsState([
      cap({
        keyType: "chat",
        creditsUsed: 150,
  ...
  // The keys are turned off one at a time, so a disabled one must not take its
  // enabled peer's notice down with it.
  it("still reports an enabled cap alongside a disabled one", () => {
    inferenceCapsState([
      cap({ keyType: "chat", disabled: true }),
    ...
    expect(capBanners()).toHaveLength(1);
    expect(screen.getByText("Security inference cap reached")).toBeTruthy();
  ...
  // ever appears is a poll. A tab nobody is looking at can't show one, and a
  // remount on every navigation must not read ahead of the cadence.
  it("polls a visible tab, leaves a hidden one alone, and holds the cadence across remounts", () => {
    render(<PaygCapReachedBanners />);
