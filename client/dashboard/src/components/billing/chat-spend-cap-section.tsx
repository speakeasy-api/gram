 ...
 * running, and in both cases there is no pay-as-you-go bill yet — so an active
 * trial gets the cap shown as locked rather than editable, and the section
 * touches neither the query nor the mutation until the trial is over.
 */
type SpendCapMode = "editable" | "trial-locked" | "hidden";
...
export function ChatSpendCapSection(): JSX.Element | null {
  ...
  return (
    <Page.Section>
      ...
      <Page.Section.Body>
        {mode === "trial-locked" ? <TrialLockedSpendCap /> : <PaygSpendCap />}
      </Page.Section.Body>
    </Page.Section>
  );
}

// Nothing here is writable, so the section never mounts the mutation for a
// trialing organization: there is no cap to set until checkout creates the
// pay-as-you-go bill the cap applies to.
function TrialLockedSpendCap(): JSX.Element {
  return (
    <Stack gap={2} className="max-w-md">
      ...
      />
      <Text muted small>
        Your trial has no spend cap. The monthly chat spend cap starts when pay
        as you go begins, and you can set it here once checkout is complete.
      </Text>
    </Stack>
  );
}
