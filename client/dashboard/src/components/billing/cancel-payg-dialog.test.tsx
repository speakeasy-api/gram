...
vi.mock("@gram/client/react-query/cancelStripeSubscription.js", () => ({
  useCancelStripeSubscriptionMutation: (options?: { onSettled?: () => void }) =>
...
/** Fails it: the mutation settles, but the per-call success never runs. */
function fails() {
  mocks.mutate.mockImplementation(() => {
    settle();
  });
...
describe("CancelPaygDialog", () => {
  ...
  // Stripe can apply the cancellation and the call still fail afterwards, so a
  // failure is no proof the subscription is untouched. Leaving the stale copy
  // cached would show a plan state Stripe has already moved on from.
  it("refreshes the cached subscription after a failed cancellation", () => {
