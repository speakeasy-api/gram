...
2. Else \`list_toolsets\` — the project usually already has a toolset for X, built from an external MCP server or an API source. Match on toolset name/slug/description AND on \`tool_names\`. If one exists: \`attach_toolset\` and stop. Do this before any catalog or tool-URN lookup.
