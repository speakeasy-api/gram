...
4. **Show the Slack app guide — only if the bot doesn't already exist.** Before calling, \`list_environments\` and check \`populated_entry_names\` on the assistant's env. If \`SLACK_BOT_TOKEN\` is already there, the user has already created and installed the app — skip this step entirely and continue to the self-handshake (step 7). Otherwise, \`show_slack_app_guide\` with the \`webhook_url\` from step 3. The component builds the manifest itself — it pulls the assistant's name, derives bot scopes from the slack platform tools you attached in step 1, and derives \`bot_events\` from the slack trigger you created in step 3. Don't pass \`bot_scopes\` or \`bot_events\` unless you need to add a scope the catalog doesn't cover. The user clicks through, picks a workspace, and creates the app — Slack verifies the webhook URL against Gram on the spot.
...
2. Only if \`SLACK_BOT_TOKEN\` is NOT yet populated on the assistant's env (check \`list_environments\` → \`populated_entry_names\`): \`show_slack_app_guide\` without a \`webhook_url\`. The component still derives bot scopes from the attached slack toolset; event subscriptions are left empty (Slack doesn't accept them without a verified URL). If the bot token is already populated, skip the guide — the app exists.
...
- **URL length.** The pre-filled manifest is encoded into the deep link URL. The built-in builder already keeps it lean (no icons, no long description, no workflow steps). If you pass huge \`bot_scopes\`/\`bot_events\` overrides and the link ever gets rejected, drop the overrides and let derivation handle it.
- **search.all needs a user token.** The \`search_messages_and_files\` platform tool calls Slack's \`search.all\`, which Slack only authorizes via a user token (\`search:read\` is a user-token scope). The manifest builder skips it intentionally; if you attach that tool, the user must also generate a user token from **OAuth & Permissions → User Token Scopes → search:read** and store it as \`SLACK_USER_TOKEN\`.
