...
1. **Propose the setup.** \`propose_slack_setup\` with preselected groups derived from the user's stated goal. Examples (shape, not content): an @-mention responder typically gets \`send\` + \`mentions\`; a research-and-post bot gets \`send\` + \`read\` + \`mentions\`; a reaction-driven workflow gets \`react\` + \`reactions\`. The card produces a per-assistant Slack toolset (already attached) and, if any events were chosen, a slack trigger with its \`webhook_url\`. **This tool BLOCKS** until the user submits or skips — never call other tools in parallel with it. If the call returns an error about a toolset-name collision (another project toolset already uses the default name), propose a more distinctive name that hints at what this assistant does, OR ask the user one short question for a name, then re-call with \`toolset_name\` set. Do not show the card a second time before resolving the name.
