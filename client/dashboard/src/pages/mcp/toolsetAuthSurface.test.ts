...
describe("canConfigureExternalOAuth", () => {
  ...
  it("allows external OAuth for a public server with an OAuth authorization code flow", () => {
    expect(canConfigureExternalOAuth(eligibleToolset, "public", false)).toBe(
      true,
    );
  });
  ...
  it("allows external OAuth when an attached external MCP source requires it", () => {
    ...
    expect(canConfigureExternalOAuth(externalMcpToolset, "public", true)).toBe(
      true,
    );
  });

  it("does not expose external OAuth for disabled or private servers", () => {
    expect(canConfigureExternalOAuth(eligibleToolset, "disabled", true)).toBe(
      false,
    );
    expect(canConfigureExternalOAuth(eligibleToolset, "private", true)).toBe(
      false,
    );
    expect(canConfigureExternalOAuth(eligibleToolset, undefined, true)).toBe(
      false,
    );
  });
