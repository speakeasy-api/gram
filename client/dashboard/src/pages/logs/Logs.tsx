...
export default function LogsPage() {
  ...
  const sources = useMemo(() => {
    return (
      toolsetsData?.toolsets
        ?.map((t) => ({ slug: t.slug, id: t.id, toolUrns: t.toolUrns }))
        .filter(Boolean) || []
    );
  }, [toolsetsData]);
  ...
  const selectedToolUrns = useMemo(() => {
    ...
    const selectedToolset = sources.find(
      (s) => s.slug === filters.sourceFilter,
    );
  ...
  const listToolLogsQuery = useListToolLogs(
    {
      ...
      status:
        (filters.statusFilter as "success" | "failure" | undefined) ||
        undefined,
                            ...
                            >
                              ...
                              <TableCell className="py-4">
                                <div className="flex items-center justify-center">
                                  <StatusIcon
                                    isSuccess={isSuccessfulCall(log)}
                                  />
                                </div>
                              </TableCell>
                              <TableCell className="text-muted-foreground font-mono py-4">
