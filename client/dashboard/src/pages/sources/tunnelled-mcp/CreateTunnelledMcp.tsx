...
import { mcpServerRouteParam, tunnelledMcpRouteParam } from "@/lib/sources";
...
import { useCreateTunnelledMcpSource } from "./hooks";
import { TunnelledMcpSetupTabs } from "./TunnelledMCPDetails";
...
export default function CreateTunnelledMcp(): JSX.Element {
  return (
    <Page>
      <Page.Header>
...
function CreateTunnelledMcpForm() {
  const routes = useRoutes();
  const createSource = useCreateTunnelledMcpSource();
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setCreated(result);
      toast.success("Tunnelled MCP server added");
    } catch (error) {
      const message =
        error instanceof Error
          ? error.message
          : "Failed to add tunnelled MCP server";
      toast.error(message);
    }
  };
      ...
      <div className="max-w-4xl">
        <Stack gap={3} className="mb-8">
          <Stack direction="horizontal" gap={3} align="center">
            ...
            <Heading variant="h3">Tunnelled MCP server added</Heading>
          </Stack>
          <Type muted>
        ...
        <Stack gap={6}>
          ...
          <TunnelledMcpSetupTabs
            ...
            serverName={created.tunnelledMcpServer.name}
          />
            ...
            <Button
              ...
              onClick={() =>
                routes.sources.source.goTo(
                  "tunnelledmcp",
                  tunnelledMcpRouteParam(created.tunnelledMcpServer),
                )
              }
            >
    ...
    <div className="max-w-2xl">
      <Stack gap={3} className="mb-8">
        <Stack direction="horizontal" gap={3} align="center">
          ...
          <Heading variant="h3">Add a tunnelled MCP server</Heading>
        </Stack>
        <Type muted>
      ...
      >
        <Stack gap={4}>
          <Stack gap={1}>
            <label
              htmlFor="tunnelled-mcp-name"
              className="text-sm leading-none font-medium"
            >
              Display name
            </label>
            <Input
              id="tunnelled-mcp-name"
              autoFocus
              ...
              aria-describedby={
                validationError ? "tunnelled-mcp-name-error" : undefined
              }
            />
            {validationError && (
              <div
                id="tunnelled-mcp-name-error"
                role="alert"
