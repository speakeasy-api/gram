...
export default function Killswitches(): JSX.Element {
  ...
  const capabilitiesQuery = useKillswitchCapabilities(security);
  ...
  return (
    <main className="mx-auto w-full max-w-[1270px] space-y-6 px-4 py-6 sm:px-8 sm:py-8">
      ...
      {editorOpen && editorCatalogLoading && (
        <p role="status" className="text-muted-foreground text-sm">
          Loading the Killswitch editor…
        </p>
      )}
      {editorOpen && editorCatalogError && (
        <Alert variant="error">
          <AlertTitle>Unable to load the Killswitch editor</AlertTitle>
          <AlertDescription>
            {editorCatalogError.message}
            <div className="mt-2">
              <Button
                size="sm"
                variant="secondary"
                onClick={retryEditorCatalog}
              >
                Try again
              </Button>
            </div>
      ...
      {!editorCatalogLoading && !editorCatalogError && (
