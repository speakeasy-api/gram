...
/** Splits text into nodes, wrapping each matched [start,end) byte range in a
 *  <mark>. Offsets are byte offsets from the engine; fine for the ASCII content
 *  these previews carry. */
...
/** A live preview that evaluates the detection expression against a sample
 *  message in the browser — the same engine that runs in production — and shows
 *  the verdict plus the exact spans it matched. This is the capability the old
 *  JS type-checker could not provide: real evaluation with byte-accurate spans. */
