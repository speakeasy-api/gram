...
export function ConnectIdpStep({
  ...
  const needsDomain =
    onboardingStatus?.domainVerified === false &&
  ...
  const handleConnect = () => {
    if (!provider || needsDomain) return;
    ...
    generatePortalLink.mutate(
      {
        request: {
          generateWorkOSAdminPortalLinkRequestBody: {
            ...
            // WorkOS open its own provider picker so all providers work.
          },
        },
      },
      {
  ...
  return (
    <StepContainer
      ...
      isLoading={isLoading}
      canContinue={!!selectedProvider && !needsDomain}
    >
