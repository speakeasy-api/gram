...
vi.mock("./assignee-picker", () => ({
  AssigneePicker: (
    ...
    <button disabled={props.disabled} onClick={() => props.onChange(undefined)}>
...
it("does not display mixed legacy card owners", () => {
  const allTasks = [
...
it("ignores mixed legacy owners and keeps workstream assignment explicit", () => {
  const member = {
    kind: "user" as const,
    userId: "member",
    name: "Team member",
