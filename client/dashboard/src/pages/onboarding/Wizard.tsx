...
const CliSetupStep = ({
  ...
  const commands = [
    ...
    { label: "Connect your session", command: `gram login ${cliState.sessionToken}` },
    { label: "Build your functions", command: "gram build ./functions/ -o output.zip" },
    { label: "Upload to Gram", command: "gram upload --type function --location output.zip" },
  ];
    ...
    <>
      ...
      <Stack gap={4}>
        ...
        {cliState.deploymentStatus !== "none" && (
          <Stack gap={2}>
            <Type small className={cn(
              "font-medium",
              cliState.deploymentStatus === "processing" && "text-primary",
              cliState.deploymentStatus === "complete" && "text-success-foreground",
              cliState.deploymentStatus === "error" && "text-destructive"
            )}>
              {cliState.deploymentStatus === "processing" && "⏳ Deployment in progress..."}
              {cliState.deploymentStatus === "complete" && "✓ Deployment complete!"}
      ...
      >
        ...
        <div className="p-4 font-mono text-sm space-y-1 min-h-[300px] max-h-[400px] overflow-y-auto">
          {cliState.logs.map((log) => {
            const shouldShowLoading = log.loading && typedCommands.has(log.id.replace('-status', '-cmd'));
