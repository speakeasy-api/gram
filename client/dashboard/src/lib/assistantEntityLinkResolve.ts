import type { ResolvedLink } from "@/elements";

/**
 * Scheme the Project Assistant emits to reference an in-app entity, e.g.
 * `gram:risk_policy/<id>`, `gram:chat/<chat-id>`, or `gram:skill/<id>`.
 * Elements stays agnostic to this scheme; the resolver below (and the
 * assistant's system prompt) own it.
 */
const SCHEME = "gram:";
...
export function resolveEntityLink(
  ...
  switch (type) {
    ...
    case "skill":
      return newTab(`${proj}/skills/${enc(rest)}`);
    case "source":
      return id2
