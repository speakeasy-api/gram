...
/**
 * One MCP server and the Okta resource connection recorded for its upstream, with the derived Cross App Access state and the exact values the administrator enters in the identity provider console. Servers that share an upstream resource share one Okta resource connection.
 */
export type OktaResourceConnectionServer = {
  /**
...
export const OktaResourceConnectionServerState$inboundSchema: z.ZodMiniEnum<
  typeof OktaResourceConnectionServerState
> = z.enum(OktaResourceConnectionServerState);

/** @internal */
export const OktaResourceConnectionServer$inboundSchema: z.ZodMiniType<
...
> = z.pipe(
  z.object({
    ...
    state: OktaResourceConnectionServerState$inboundSchema,
  }),
  z.transform((v) => {
    return remap$(v, {
...
export function oktaResourceConnectionServerFromJSON(
  jsonString: string,
): SafeParseResult<OktaResourceConnectionServer, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => OktaResourceConnectionServer$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'OktaResourceConnectionServer' from JSON`,
  );
}
