...
import {
  ...
  ChatAnalysisSettings$inboundSchema,
} from "../models/components/chatanalysissettings.js";
...
import {
  ...
  UpsertChatAnalysisSettingsRequest$outboundSchema,
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(UpsertChatAnalysisSettingsRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.UpsertWorkUnitsSettingsRequestBody, {
    explode: true,
  });

  const path = pathToFunc("/rpc/adminChatAnalysis.upsertWorkUnitsSettings")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, ChatAnalysisSettings$inboundSchema),
