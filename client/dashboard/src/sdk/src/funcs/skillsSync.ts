...
import {
  SyncSkillsResult,
  SyncSkillsResult$inboundSchema,
} from "../models/components/syncskillsresult.js";
...
import {
  SyncSkillsRequest,
  SyncSkillsRequest$outboundSchema,
  SyncSkillsSecurity,
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(SyncSkillsRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.SyncSkillsRequestBody, {
    explode: true,
  });

  const path = pathToFunc("/rpc/skills.sync")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, SyncSkillsResult$inboundSchema),
