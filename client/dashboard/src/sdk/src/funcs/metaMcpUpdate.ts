...
import {
  MetaMcpServer,
  MetaMcpServer$inboundSchema,
} from "../models/components/metamcpserver.js";
...
import {
  ...
  UpdateMetaMcpServerRequest$outboundSchema,
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(UpdateMetaMcpServerRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.UpdateMetaMcpServerForm, {
    explode: true,
  });

  const path = pathToFunc("/rpc/metaMcp.update")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, MetaMcpServer$inboundSchema),
