...
import { Skill, Skill$inboundSchema } from "../models/components/skill.js";
...
import {
  UpdateSkillRequest,
  UpdateSkillRequest$outboundSchema,
  UpdateSkillSecurity,
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(UpdateSkillRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.UpdateSkillRequestBody, {
    explode: true,
  });

  const path = pathToFunc("/rpc/skills.update")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, Skill$inboundSchema),
