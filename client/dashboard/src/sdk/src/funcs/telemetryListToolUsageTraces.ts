...
import {
  ...
  ListToolUsageTracesResult$inboundSchema,
} from "../models/components/listtoolusagetracesresult.js";
...
import {
  ...
  ListToolUsageTracesRequest$outboundSchema,
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(ListToolUsageTracesRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.ListToolUsageTracesPayload, {
    explode: true,
  });

  const path = pathToFunc("/rpc/telemetry.listToolUsageTraces")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, ListToolUsageTracesResult$inboundSchema),
