...
import {
  ...
  ListIdentityProviderConnectionApplicationsResult$inboundSchema,
} from "../models/components/listidentityproviderconnectionapplicationsresult.js";
...
import {
  ...
  ListIdentityProviderConnectionApplicationsRequest$outboundSchema,
...
> {
  ...
  const path = pathToFunc(
    "/rpc/identityProviderConnections.listApplications",
  )();

  const query = encodeFormQuery({
    "id": payload.id,
    "include_removed": payload.include_removed,
  });
  ...
  const context = {
    ...
    operationID: "listIdentityProviderConnectionApplications",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, ListIdentityProviderConnectionApplicationsResult$inboundSchema),
