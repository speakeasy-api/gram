...
import {
  ...
  CreatePromptTemplateResult$inboundSchema,
} from "../models/components/createprompttemplateresult.js";
...
import {
  ...
  CreateTemplateRequest$outboundSchema,
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(CreateTemplateRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.CreatePromptTemplateForm, {
    explode: true,
  });

  const path = pathToFunc("/rpc/templates.create")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, CreatePromptTemplateResult$inboundSchema),
