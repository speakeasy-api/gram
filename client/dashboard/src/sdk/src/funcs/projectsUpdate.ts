...
import {
  UpdateProjectResult,
  UpdateProjectResult$inboundSchema,
} from "../models/components/updateprojectresult.js";
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(UpdateProjectRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.UpdateProjectForm, { explode: true });

  const path = pathToFunc("/rpc/projects.update")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, UpdateProjectResult$inboundSchema),
