...
import {
  SessionInfoRequest,
  SessionInfoRequest$outboundSchema,
  SessionInfoResponse,
  SessionInfoResponse$inboundSchema,
  SessionInfoSecurity,
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(z.optional(SessionInfoRequest$outboundSchema), value),
  ...
  const path = pathToFunc("/rpc/auth.info")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, SessionInfoResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
