...
import {
  ListPluginsResult,
  ListPluginsResult$inboundSchema,
} from "../models/components/listpluginsresult.js";
...
import {
  ListPluginsRequest,
  ListPluginsRequest$outboundSchema,
  ListPluginsSecurity,
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(z.optional(ListPluginsRequest$outboundSchema), value),
  ...
  const path = pathToFunc("/rpc/plugins.listPlugins")();

  const query = encodeFormQuery({
    "principal_urns": payload?.principal_urns,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, ListPluginsResult$inboundSchema),
