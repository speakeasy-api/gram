...
import {
  ...
  UpsertPlatformAiScanTargetRequest$outboundSchema,
 ...
 */
export function platformAiScanTargetsUpsert(
  client: GramCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(UpsertPlatformAiScanTargetRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.UpsertRequestBody2, {
    explode: true,
  });

  const path = pathToFunc("/rpc/platformAiScanTargets.upsert")();
  ...
  const context = {
    ...
    operationID: "upsertPlatformAiScanTarget",
    oAuth2Scopes: null,
