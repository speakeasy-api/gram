...
import {
  ...
  SetAIToolDecisionResult$inboundSchema,
} from "../models/components/setaitooldecisionresult.js";
...
import {
  ...
  SetAIToolDecisionRequest$outboundSchema,
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(SetAIToolDecisionRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.SetAIToolDecisionRequestBody, {
    explode: true,
  });

  const path = pathToFunc("/rpc/access.setAIToolDecision")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, SetAIToolDecisionResult$inboundSchema),
