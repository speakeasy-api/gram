...
const SANITIZED_AI_ACCESS_DENIED_ERROR = new Error(AI_ACCESS_DENIED);

// Tenant-authored denial notes are display content, never telemetry content.
// Return a stable fixed error for the two console/Datadog call sites.
export const sanitizeStreamErrorForTelemetry = (error: unknown): unknown =>
  findAIAccessDenial(error) ? SANITIZED_AI_ACCESS_DENIED_ERROR : error;
