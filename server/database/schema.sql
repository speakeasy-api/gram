...
-- Chat sessions for session-scoped OAuth credentials
-- Used by Elements and Playground to persist OAuth tokens across requests
CREATE TABLE IF NOT EXISTS chat_sessions (
  id uuid NOT NULL DEFAULT generate_uuidv7(),
  project_id uuid NOT NULL,
  organization_id TEXT NOT NULL,

  -- For Elements: external user identifier
  external_user_id TEXT,
  -- For Elements: embed origin for postMessage security
  embed_origin TEXT,
  ...
  CONSTRAINT chat_sessions_project_id_fkey FOREIGN KEY (project_id) REFERENCES projects (id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS chat_sessions_project_id_idx ON chat_sessions (project_id) WHERE deleted IS FALSE;

-- Chat session OAuth credentials
-- Stores encrypted OAuth tokens scoped to a specific chat session and toolset
CREATE TABLE IF NOT EXISTS chat_session_credentials (
  id uuid NOT NULL DEFAULT generate_uuidv7(),
  ...
  CONSTRAINT chat_session_credentials_pkey PRIMARY KEY (id),
  CONSTRAINT chat_session_credentials_session_fkey FOREIGN KEY (chat_session_id) REFERENCES chat_sessions (id) ON DELETE CASCADE,
  CONSTRAINT chat_session_credentials_project_fkey FOREIGN KEY (project_id) REFERENCES projects (id) ON DELETE CASCADE,
  CONSTRAINT chat_session_credentials_toolset_fkey FOREIGN KEY (toolset_id) REFERENCES toolsets (id) ON DELETE CASCADE
);

CREATE UNIQUE INDEX IF NOT EXISTS chat_session_credentials_session_toolset_key
ON chat_session_credentials (chat_session_id, toolset_id);
