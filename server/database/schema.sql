...
-- OTEL forwarding destinations: each row is one customer endpoint that
-- receives a copy of inbound OTLP payloads. An org or project can configure
-- multiple destinations (e.g., a Honeycomb endpoint + a Datadog endpoint).
-- name distinguishes destinations within a scope; project_id is nullable to
-- allow org-wide destinations alongside per-project ones.
...
-- Unique name per org-wide destination set
CREATE UNIQUE INDEX IF NOT EXISTS otel_forwarding_configs_org_name_key
  ON otel_forwarding_configs (organization_id, name)
  WHERE project_id IS NULL AND deleted IS FALSE;

-- Unique name per project-scoped destination set
CREATE UNIQUE INDEX IF NOT EXISTS otel_forwarding_configs_org_project_name_key
  ON otel_forwarding_configs (organization_id, project_id, name)
  WHERE project_id IS NOT NULL AND deleted IS FALSE;

-- Helps the drain workflow's "list enabled destinations for org" lookup.
CREATE INDEX IF NOT EXISTS otel_forwarding_configs_org_enabled_idx
  ON otel_forwarding_configs (organization_id)
  WHERE enabled IS TRUE AND deleted IS FALSE;

-- Append-only queue of raw OTLP payloads awaiting forwarding to the
-- per-org customer endpoint. payload is BYTEA so we can pass through
-- protobuf bodies verbatim without re-encoding. A separate
-- otel_forwarding_deliveries row tracks per-attempt status; this table is
-- write-once.
CREATE TABLE IF NOT EXISTS otel_forwarding_outbox (
  id BIGINT NOT NULL GENERATED BY DEFAULT AS IDENTITY,
  organization_id TEXT NOT NULL,

  -- "logs" or "metrics" — corresponds to the inbound /rpc/hooks.otel/v1/<kind>
  -- path so the worker logs/metrics can break things down by signal type.
  kind TEXT NOT NULL,
  content_type TEXT NOT NULL,
  ...
  CONSTRAINT otel_forwarding_outbox_pkey PRIMARY KEY (id),
  CONSTRAINT otel_forwarding_outbox_organization_id_fkey
    FOREIGN KEY (organization_id) REFERENCES organization_metadata(id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS otel_forwarding_outbox_organization_id_id_idx
ON otel_forwarding_outbox (organization_id, id);

-- Per (outbox row, destination) delivery tracking. One row per outbox
-- entry × destination pair the drain workflow has touched. UPDATE-heavy:
-- each retry rewrites the row, so we tune for HOT updates with a low
-- fillfactor + aggressive autovacuum.
CREATE TABLE IF NOT EXISTS otel_forwarding_deliveries (
  ...
  CONSTRAINT otel_forwarding_deliveries_pkey PRIMARY KEY (outbox_id, destination_id),
  CONSTRAINT otel_forwarding_deliveries_outbox_id_fkey
    FOREIGN KEY (outbox_id) REFERENCES otel_forwarding_outbox(id) ON DELETE CASCADE,
  CONSTRAINT otel_forwarding_deliveries_destination_id_fkey
    FOREIGN KEY (destination_id) REFERENCES otel_forwarding_configs(id) ON DELETE CASCADE
) WITH (
  fillfactor = 80,
...
CREATE INDEX IF NOT EXISTS otel_forwarding_deliveries_pending_idx
ON otel_forwarding_deliveries (outbox_id, destination_id) WHERE processed_at IS NULL AND dead_lettered IS FALSE;
