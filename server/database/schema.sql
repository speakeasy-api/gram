...
-- scoped: they describe the products, vendor plans, techniques and
-- capabilities the platform knows how to cover. The server seeds them from
-- its Go catalog at startup; nothing edits them per organization. Every row
-- carries a stable slug that application code refers to.
CREATE TABLE IF NOT EXISTS onboarding_products (
  id uuid NOT NULL DEFAULT generate_uuidv7(),
  slug TEXT NOT NULL CHECK (slug <> '' AND CHAR_LENGTH(slug) <= 64),
  name TEXT NOT NULL,
  -- Vendor that sells the product (anthropic, openai, cursor, github, ...).
  -- Plans are matched to a product through this value.
...
-- Which plans an admin can declare for a product. Products without rows here
-- (open-source harnesses, LiteLLM) have no plan.
CREATE TABLE IF NOT EXISTS onboarding_product_plans (
  product_id uuid NOT NULL,
  plan_id uuid NOT NULL,
  created_at timestamptz NOT NULL DEFAULT clock_timestamp(),

  CONSTRAINT onboarding_product_plans_pkey PRIMARY KEY (product_id, plan_id),
  CONSTRAINT onboarding_product_plans_product_id_fkey FOREIGN KEY (product_id) REFERENCES onboarding_products (id) ON DELETE CASCADE,
  CONSTRAINT onboarding_product_plans_plan_id_fkey FOREIGN KEY (plan_id) REFERENCES onboarding_plans (id) ON DELETE CASCADE
);
...
-- available only on the listed plans; a pair with no rows is available on
-- every plan.
...
CREATE TABLE IF NOT EXISTS organization_onboarding_answers (
  ...
  -- security or mcp-gateway. NULL until the admin answers.
  use_case TEXT,
...
-- Products the admin selected, each with the declared vendor plan. plan_id is
-- NULL only for products that have no plans.
CREATE TABLE IF NOT EXISTS organization_onboarding_products (
  ...
  CONSTRAINT organization_onboarding_products_plan_id_fkey FOREIGN KEY (plan_id) REFERENCES onboarding_plans (id) ON DELETE SET NULL
);
...
CREATE UNIQUE INDEX IF NOT EXISTS projects_organization_id_id_key
...
CREATE TABLE IF NOT EXISTS skill_observations (
  ...
  efficacy_enqueued_at timestamptz,
...
CREATE INDEX IF NOT EXISTS skill_observations_pending_efficacy_idx
ON skill_observations (project_id, seen_at, id)
WHERE reconciled_at IS NOT NULL AND efficacy_enqueued_at IS NULL AND session_id IS NOT NULL AND skill_version_id IS NOT NULL;
...
CREATE TABLE IF NOT EXISTS skill_efficacy_settings (
  organization_id TEXT NOT NULL,
  ...
  CONSTRAINT skill_efficacy_settings_pkey PRIMARY KEY (organization_id),
...
CREATE TABLE IF NOT EXISTS skill_efficacy_evaluations (
  ...
  CONSTRAINT skill_efficacy_evaluations_pkey PRIMARY KEY (id),
  CONSTRAINT skill_efficacy_evaluations_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES organization_metadata (id) ON DELETE CASCADE,
  CONSTRAINT skill_efficacy_evaluations_project_id_fkey FOREIGN KEY (project_id) REFERENCES projects (id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS skill_efficacy_evaluations_pending_idx
ON skill_efficacy_evaluations (project_id, observed_at DESC, id DESC)
WHERE state = 'pending';

CREATE INDEX IF NOT EXISTS skill_efficacy_evaluations_recovered_pending_idx
ON skill_efficacy_evaluations (project_id, observed_at DESC, id DESC)
WHERE state = 'pending' AND reserved_on IS NOT NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS api_keys_organization_id_project_id_id_key
ON api_keys (organization_id, project_id, id);

CREATE TABLE IF NOT EXISTS litellm_instances (
  id uuid NOT NULL DEFAULT generate_uuidv7(),
  organization_id TEXT NOT NULL,
  project_id uuid NOT NULL,
  ...
  CONSTRAINT litellm_instances_pkey PRIMARY KEY (id),
  CONSTRAINT litellm_instances_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES organization_metadata (id) ON DELETE CASCADE,
  CONSTRAINT litellm_instances_project_tenant_fkey FOREIGN KEY (organization_id, project_id) REFERENCES projects (organization_id, id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX IF NOT EXISTS litellm_instances_api_key_tenant_key
ON litellm_instances (organization_id, project_id, api_key_id);
...
-- Provider values are constrained by the subtype discriminator, not here.
-- last_error carries a typed reason, never a raw body.
...
ON identity_provider_connections (organization_id, provider)
WHERE deleted IS FALSE;

-- Composite FK target so managed rows pin the connection to one org.
CREATE UNIQUE INDEX IF NOT EXISTS identity_provider_connections_organization_id_id_key
ON identity_provider_connections (organization_id, id);
...
CREATE TABLE IF NOT EXISTS external_keys (
  ...
  -- this key; its mutation guards refuse edits to marked rows.
  ...
  CONSTRAINT external_keys_identity_provider_connection_tenant_fkey FOREIGN KEY (organization_id, identity_provider_connection_id) REFERENCES identity_provider_connections (organization_id, id),
  -- organization_id is nullable and the composite FK is MATCH SIMPLE, so a
  -- NULL organization_id would skip the tenant check; require it when marked.
  CONSTRAINT external_keys_identity_provider_connection_id_check CHECK (
    identity_provider_connection_id IS NULL
...
CREATE TABLE IF NOT EXISTS json_web_key_sets (
  ...
  name TEXT NOT NULL,
  -- Managed-by marker: set when an identity provider connection provisioned
  -- this set; its mutation guards refuse edits to marked rows.
  identity_provider_connection_id uuid,
  created_at timestamptz NOT NULL DEFAULT clock_timestamp(),
  updated_at timestamptz NOT NULL DEFAULT clock_timestamp(),
  deleted_at timestamptz,
...
CREATE TABLE IF NOT EXISTS remote_session_clients (
  ...
  CONSTRAINT remote_session_clients_identity_provider_connection_tenant_fkey FOREIGN KEY (organization_id, identity_provider_connection_id) REFERENCES identity_provider_connections (organization_id, id),
  -- Same MATCH SIMPLE gap as json_web_key_set_id below: require the org when marked.
  CONSTRAINT remote_session_clients_identity_provider_connection_id_check CHECK (
    identity_provider_connection_id IS NULL
    OR organization_id IS NOT NULL
  ),
...
CREATE INDEX IF NOT EXISTS remote_session_clients_identity_provider_connection_idx
...
-- FK cascades while managed-by markers are NO ACTION: a hard delete of a
-- marked connection is blocked by design; app code soft-deletes.
CREATE TABLE IF NOT EXISTS okta_identity_provider_connections (
  ...
  -- Display-only, admin-entered. The exchange path must never read these; the
...
ON mcp_endpoints (project_id, id);
...
CREATE TABLE IF NOT EXISTS assistant_mcp_oauth_clients (
  ...
  -- are required, and retaining orphaned encrypted credentials is unsafe.
...
-- DEPRECATED: switching over to `publish_outbox` and `publish_outbox_dead_letters` for new messages.
...
-- without the relay learning anything about that component.
--
-- Rows are DELETEd once published, so steady state is near-empty. That is what
...
-- Catalog registrations persist the desired Platform-owned component stack for
-- one canonical catalog entry. They do not store provider credentials, URLs,
-- or arbitrary setup state.
CREATE TABLE IF NOT EXISTS platform_mcp_catalog_registrations (
  id uuid NOT NULL DEFAULT generate_uuidv7(),
  organization_id TEXT NOT NULL,
  project_id uuid NOT NULL,
  source_kind TEXT NOT NULL,
  ...
  CONSTRAINT platform_mcp_catalog_registrations_status_check CHECK (status <> ''),
  CONSTRAINT platform_mcp_catalog_registrations_organization_id_fkey
    FOREIGN KEY (organization_id) REFERENCES organization_metadata (id) ON DELETE CASCADE,
  CONSTRAINT platform_mcp_catalog_registrations_organization_project_fkey
    FOREIGN KEY (organization_id, project_id) REFERENCES projects (organization_id, id) ON DELETE CASCADE,
  CONSTRAINT platform_mcp_catalog_registrations_organization_connection_fkey
    FOREIGN KEY (organization_id, connection_id) REFERENCES platform_mcp_connections (organization_id, id) ON DELETE CASCADE,
  -- Components are normally soft-deleted. A hard delete must first detach the
...
-- Operation receipts retain a safe result projection for 24-hour idempotency.
CREATE TABLE IF NOT EXISTS platform_mcp_operation_receipts (
  id uuid NOT NULL DEFAULT generate_uuidv7(),
  organization_id TEXT NOT NULL,
  project_id uuid NOT NULL,
  ...
  CONSTRAINT platform_mcp_operation_receipts_status_check CHECK (status <> ''),
  CONSTRAINT platform_mcp_operation_receipts_organization_id_fkey
    FOREIGN KEY (organization_id) REFERENCES organization_metadata (id) ON DELETE CASCADE,
  CONSTRAINT platform_mcp_operation_receipts_organization_project_fkey
    FOREIGN KEY (organization_id, project_id) REFERENCES projects (organization_id, id) ON DELETE CASCADE,
  CONSTRAINT platform_mcp_operation_receipts_organization_connection_fkey
    FOREIGN KEY (organization_id, connection_id) REFERENCES platform_mcp_connections (organization_id, id) ON DELETE CASCADE,
  CONSTRAINT platform_mcp_operation_receipts_project_registration_fkey
...
CREATE TABLE IF NOT EXISTS platform_mcp_setup_handoffs (
  id uuid NOT NULL DEFAULT generate_uuidv7(),
  organization_id TEXT NOT NULL,
  project_id uuid NOT NULL,
  ...
  CONSTRAINT platform_mcp_setup_handoffs_organization_id_fkey
    FOREIGN KEY (organization_id) REFERENCES organization_metadata (id) ON DELETE CASCADE,
  CONSTRAINT platform_mcp_setup_handoffs_organization_project_fkey
    FOREIGN KEY (organization_id, project_id) REFERENCES projects (organization_id, id) ON DELETE CASCADE,
  CONSTRAINT platform_mcp_setup_handoffs_organization_connection_fkey
    FOREIGN KEY (organization_id, connection_id) REFERENCES platform_mcp_connections (organization_id, id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS platform_mcp_readiness (
  ...
  CONSTRAINT platform_mcp_readiness_organization_id_fkey
    FOREIGN KEY (organization_id) REFERENCES organization_metadata (id) ON DELETE CASCADE,
  CONSTRAINT platform_mcp_readiness_organization_project_fkey
    FOREIGN KEY (organization_id, project_id) REFERENCES projects (organization_id, id) ON DELETE CASCADE,
  CONSTRAINT platform_mcp_readiness_organization_connection_fkey
    FOREIGN KEY (organization_id, connection_id) REFERENCES platform_mcp_connections (organization_id, id) ON DELETE CASCADE,
