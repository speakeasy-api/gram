...
CREATE TABLE IF NOT EXISTS deployments_external_mcps (
  ...
  slug TEXT NOT NULL CHECK (slug <> '' AND slug ~ '^[a-z0-9-]+$'),
  ...
  CONSTRAINT deployments_external_mcps_deployment_id_fkey FOREIGN KEY (deployment_id) REFERENCES deployments(id) ON DELETE SET NULL,
  CONSTRAINT deployments_external_mcps_registry_id_fkey FOREIGN KEY (registry_id) REFERENCES mcp_registries(id) ON DELETE SET NULL
);

CREATE INDEX IF NOT EXISTS deployments_external_mcps_deployment_id_idx
ON deployments_external_mcps (deployment_id)
WHERE deleted IS FALSE;

CREATE UNIQUE INDEX IF NOT EXISTS deployments_external_mcps_deployment_id_slug_key
ON deployments_external_mcps (deployment_id, slug)
WHERE deleted IS FALSE;
