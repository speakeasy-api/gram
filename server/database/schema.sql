...
CREATE TABLE IF NOT EXISTS policy_access_requests (
  id uuid NOT NULL DEFAULT generate_uuidv7(),
  organization_id TEXT NOT NULL,
  project_id uuid NOT NULL,

  -- The risk policy that blocked the caller, plus an optional generic target.
  -- target_key is a stable canonical key for deduplication. target_dimensions
  -- carries selector dimensions such as {"server_url":"mcp.example.com"}.
  ...
  -- Who got blocked and asked for access.
  requester_user_id TEXT NOT NULL DEFAULT '',
  requester_email TEXT NOT NULL DEFAULT '',
  note TEXT NOT NULL DEFAULT '',

  -- Lifecycle: requested -> approved | denied.
  status TEXT NOT NULL DEFAULT 'requested',

  -- Decision. granted_principal_urns are the principals the bypass was granted
  -- to on approval, e.g. user:<id> or role:<id>.
  ...
  CONSTRAINT policy_access_requests_pkey PRIMARY KEY (id),
  CONSTRAINT policy_access_requests_target_dimensions_check CHECK (jsonb_typeof(target_dimensions) = 'object'),
  CONSTRAINT policy_access_requests_status_check CHECK (status IN ('requested', 'approved', 'denied')),
  CONSTRAINT policy_access_requests_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES organization_metadata (id) ON DELETE CASCADE,
  CONSTRAINT policy_access_requests_project_id_fkey FOREIGN KEY (project_id) REFERENCES projects (id) ON DELETE CASCADE
);

COMMENT ON TABLE policy_access_requests IS 'Risk-policy access request workflow. A block records a request here; an admin approves by granting risk_policy:bypass to a role.';

CREATE UNIQUE INDEX IF NOT EXISTS policy_access_requests_current_key
ON policy_access_requests (organization_id, requester_user_id, policy_id, target_key);

CREATE INDEX IF NOT EXISTS policy_access_requests_org_status_idx
ON policy_access_requests (organization_id, status);
