...
-- Peered organizations: super org grants sub org access to its resources
CREATE TABLE IF NOT EXISTS peered_organizations (
  id uuid NOT NULL DEFAULT generate_uuidv7(),
  ...
  CONSTRAINT peered_organizations_pkey PRIMARY KEY (id),
  CONSTRAINT peered_organizations_super_fkey FOREIGN KEY (super_organization_id) REFERENCES organization_metadata(id) ON DELETE CASCADE,
  CONSTRAINT peered_organizations_sub_fkey FOREIGN KEY (sub_organization_id) REFERENCES organization_metadata(id) ON DELETE CASCADE
);

CREATE UNIQUE INDEX IF NOT EXISTS peered_organizations_super_sub_key
ON peered_organizations (super_organization_id, sub_organization_id);

CREATE INDEX IF NOT EXISTS peered_organizations_sub_organization_id_idx
ON peered_organizations (sub_organization_id);

-- Grants for organizations to access private MCP registries
CREATE TABLE IF NOT EXISTS mcp_registry_grants (
  id uuid NOT NULL DEFAULT generate_uuidv7(),
  registry_id uuid NOT NULL,
  organization_id TEXT NOT NULL,
  ...
  CONSTRAINT mcp_registry_grants_pkey PRIMARY KEY (id),
  CONSTRAINT mcp_registry_grants_registry_fkey FOREIGN KEY (registry_id) REFERENCES mcp_registries(id) ON DELETE CASCADE,
  CONSTRAINT mcp_registry_grants_org_fkey FOREIGN KEY (organization_id) REFERENCES organization_metadata(id) ON DELETE CASCADE
);

CREATE UNIQUE INDEX IF NOT EXISTS mcp_registry_grants_registry_org_key
ON mcp_registry_grants (registry_id, organization_id);