...
-- Per-row delivery tracking. One row per outbox entry the drain workflow
-- has touched. UPDATE-heavy: each retry rewrites the row, so we tune for
-- HOT updates with a low fillfactor + aggressive autovacuum.
CREATE TABLE IF NOT EXISTS otel_forwarding_deliveries (
  ...
  CONSTRAINT otel_forwarding_deliveries_pkey PRIMARY KEY (outbox_id),
...
ON otel_forwarding_deliveries (outbox_id) WHERE processed_at IS NULL AND dead_lettered IS FALSE;
