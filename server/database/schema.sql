...
-- used for admin reporting and downstream analytics.
CREATE TABLE IF NOT EXISTS billing_cycle_usage (
  id uuid NOT NULL DEFAULT generate_uuidv7(),
  -- Nullable so the permanent usage record survives organization deletion.
  ...
  -- changing during the late-arrival window until the cycle is finalized.
  ...
  -- sum must never become part of the permanent usage record.
...
-- day and cumulative USD snapshot preserve the financial record independently
