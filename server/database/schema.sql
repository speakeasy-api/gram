...
-- per-org customer endpoint. Anything received under /rpc/hooks.otel/ is
-- stored verbatim — we make no attempt to categorise by signal type or
-- subpath. payload is BYTEA so protobuf bodies pass through without
-- re-encoding. A separate otel_forwarding_deliveries row tracks
-- per-attempt status; this table is write-once.
