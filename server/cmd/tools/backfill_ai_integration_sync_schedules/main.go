...
// backfill for ai_integration_syncs schedule discriminators.
//
// Run manually after deploying the compatible application code:
//
//	GRAM_DATABASE_URL=postgres://USER:PASS@HOST:5432/gram \
//	  go run ./server/cmd/tools/backfill_ai_integration_sync_schedules
//
// The update is idempotent. A successful rerun reports zero updated rows.
package main
...
func run() error {
	...
	updated, err := repo.New(pool).BackfillSyncSchedules(ctx)
	if err != nil {
		return fmt.Errorf("backfill sync schedules: %w", err)
	}

	if _, err := fmt.Fprintf(os.Stdout, "updated primary sync rows: %d\n", updated); err != nil {
