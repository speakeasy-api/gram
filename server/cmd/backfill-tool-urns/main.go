...
func backfillHTTPToolDefinitions(ctx context.Context, conn *pgx.Conn) error {
	...
	updateQuery := `UPDATE gram.http_tool_definitions SET tool_urn = $1 WHERE id = $2`
	var updated int
...
func backfillPromptTemplates(ctx context.Context, conn *pgx.Conn) error {
	...
	updateQuery := `UPDATE gram.prompt_templates SET tool_urn = $1 WHERE id = $2`
	var updated int
