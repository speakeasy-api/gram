...
type orgExpectation struct {
	...
	directoryUsers       []directoryUserRecord
	...
	directoryUserChanges changeCounts
	deprovisionChanges   changeCounts
...
type report struct {
	...
	directoryUserRows  changeCounts
	deprovisionRows    changeCounts
}
...
func planOrganization(ctx context.Context, db *pgxpool.Pool, workosClient *workos.Client, org workos.Organization) (orgExpectation, error) {
	...
	planLogger := slog.New(slog.DiscardHandler)
	directoryUsers, err := listDirectoryUserRecords(ctx, planLogger, workosClient, org.ID)
	if err != nil {
		return zero, err
	}
	...
	directoryUserChanges, deprovisionChanges, directoryDetails, err := classifyDirectoryUserChanges(ctx, db, gramOrgID, skipped, directoryUsers)
	if err != nil {
		return zero, err
	}
	...
	changeDetails = append(changeDetails, directoryDetails...)
	...
	return orgExpectation{
		...
		directoryUserChanges: directoryUserChanges,
		deprovisionChanges:   deprovisionChanges,
...
func classifyAssignmentChanges(ctx context.Context, db *pgxpool.Pool, organizationID string, skipped bool, roles []workos.Role, users map[string]workos.User, members []workos.Member) (changeCounts, error) {
	...
	activeByMembership, err := activeAssignmentURNs(ctx, db, organizationID, membershipIDs)
	if err != nil {
		return changeCounts{}, err
	}
	// Slug -> URN resolution is deterministic per organization, and members
	// overwhelmingly share role sets, so resolve each distinct set once.
	urnCache := map[string]map[string]string{}
	...
	for _, member := range members {
		...
		expectedSlugs, err := availableMemberRoleSlugs(ctx, db, organizationID, roles, member.RoleSlugs)
		if err != nil {
			return changeCounts{}, err
		}
		cacheKey := slugSetKey(expectedSlugs)
		expectedURNs, cached := urnCache[cacheKey]
		if !cached {
			expectedURNs, err = expectedAssignmentURNs(ctx, db, organizationID, expectedSlugs)
			if err != nil {
				return changeCounts{}, err
			}
			urnCache[cacheKey] = expectedURNs
		}
		activeURNs := activeByMembership[member.ID]
		missingUserAssignments, err := countAssignmentsMissingUser(ctx, db, organizationID, member.ID, gramUserID)
		if err != nil {
			return changeCounts{}, err
		}

		// A slug with no URN yet belongs to a role this same run will create
		// earlier in the write phase, so its assignment is still planned work.
		// Dropping it here would understate the totals the operator approves.
		resolvedSlugs := make(map[string]bool, len(expectedURNs))
		for _, slug := range expectedURNs {
		...
		if len(expectedURNs) == 0 && plannedNew == 0 {
			if len(activeURNs) > 0 {
				counts.Delete += len(activeURNs)
			} else {
				counts.Noop++
			}
			continue
		}
		if missingUserAssignments > 0 {
			counts.Update += missingUserAssignments
		}
		// Compare role URN sets rather than counts so a replacement reports as
		// one create plus one delete instead of hiding as a noop.
		counts.Create += plannedNew
		unchanged := 0
		for roleURN := range expectedURNs {
			if activeURNs[roleURN] {
				unchanged++
			} else {
				counts.Create++
			}
		}
		if stale := len(activeURNs) - unchanged; stale > 0 {
			counts.Delete += stale
		}
		if settled := unchanged - missingUserAssignments; settled > 0 {
			counts.Noop += settled
		}
	}
	return counts, nil
...
// slugSetKey builds a stable cache key for a set of role slugs.
func slugSetKey(slugs []string) string {
	sorted := append([]string(nil), slugs...)
	sort.Strings(sorted)
	return strings.Join(sorted, "\x00")
}

// availableMemberRoleSlugs filters a membership's WorkOS role slugs down to
// those the write phase can resolve locally. SyncUserOrganizationRoleAssignments
// silently drops slugs with no matching organization or global role, so the
// plan must not count assignments for them.
func availableMemberRoleSlugs(ctx context.Context, db *pgxpool.Pool, organizationID string, roles []workos.Role, roleSlugs []string) ([]string, error) {
	available := make([]string, 0, len(roleSlugs))
	for _, slug := range roleSlugs {
		ok, err := plannedAssignmentRoleAvailable(ctx, db, organizationID, roles, slug)
		if err != nil {
			return nil, err
		}
		if ok {
...
func countAssignmentsMissingUser(ctx context.Context, db *pgxpool.Pool, organizationID, membershipID, gramUserID string) (int, error) {
...
WHERE organization_id = $1
  ...
  AND (user_id IS NULL OR user_id <> $3)`, organizationID, membershipID, gramUserID).Scan(&missingUserCount); err != nil {
		return 0, fmt.Errorf("count role assignments missing user for membership %q: %w", membershipID, err)
	}
	return missingUserCount, nil
}

// expectedAssignmentURNs resolves role slugs to the role URNs
// SyncUserOrganizationRoleAssignments would write for them, mirroring its
// input_role_urns CTE, and maps each back to its slug for reporting. Comparing
// URNs rather than slugs keeps an organization role distinct from a global
// role that happens to share its slug — a slug-only set lets one stand in for
// the other.
func expectedAssignmentURNs(ctx context.Context, db *pgxpool.Pool, organizationID string, slugs []string) (map[string]string, error) {
	...
	rows, err := db.Query(ctx, `
SELECT 'role:organization:' || id::text, workos_slug
FROM organization_roles
WHERE organization_id = $1
...
UNION
SELECT 'role:global:' || id::text, workos_slug
FROM global_roles
WHERE workos_slug = ANY($2::text[])
  AND deleted IS FALSE
  AND workos_deleted IS FALSE`, organizationID, slugs)
	if err != nil {
		return nil, fmt.Errorf("resolve expected role urns: %w", err)
	}
	defer rows.Close()

	for rows.Next() {
		var roleURN, slug string
		if err := rows.Scan(&roleURN, &slug); err != nil {
			return nil, fmt.Errorf("scan expected role urn: %w", err)
		}
		byURN[roleURN] = slug
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate expected role urns: %w", err)
	}

	return byURN, nil
}

// activeAssignmentURNs returns the role URNs backing each membership's live
// assignments. role_urn is stored directly, so this is the same identity the
// write path compares on.
func activeAssignmentURNs(ctx context.Context, db *pgxpool.Pool, organizationID string, membershipIDs []string) (map[string]map[string]bool, error) {
	byMembership := map[string]map[string]bool{}
	if len(membershipIDs) == 0 {
		return byMembership, nil
	}

	rows, err := db.Query(ctx, `
SELECT workos_membership_id, role_urn
FROM organization_role_assignments
WHERE organization_id = $1
  AND workos_membership_id = ANY($2::text[])
  AND deleted_at IS NULL`, organizationID, membershipIDs)
	if err != nil {
		return nil, fmt.Errorf("query active role assignment urns: %w", err)
	}
	defer rows.Close()

	for rows.Next() {
		var membershipID, roleURN string
		if err := rows.Scan(&membershipID, &roleURN); err != nil {
			return nil, fmt.Errorf("scan role assignment urn: %w", err)
		}
		if byMembership[membershipID] == nil {
			byMembership[membershipID] = map[string]bool{}
		}
		byMembership[membershipID][roleURN] = true
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate role assignment urns: %w", err)
	}
...
WHERE organization_id = $1
  ...
  AND deleted_at IS NULL`, organizationID, member.ID)
		...
		rows.Close()
		if activeAssignments < len(expectedSlugs) {
			for _, slug := range expectedSlugs[activeAssignments:] {
				details = append(details, roleAssignmentCreateDetail(organizationID, member, gramUserID, slug))
			}
		}
	}
	return details, nil
...
func runOrganizationBackfill(ctx context.Context, logger *slog.Logger, db *pgxpool.Pool, workosClient Client, opts options, orgs []orgExpectation) report {
	rep := report{
		...
		directoryUserRows:  changeCounts{Create: 0, Update: 0, Noop: 0, Delete: 0, StaleSkip: 0},
		deprovisionRows:    changeCounts{Create: 0, Update: 0, Noop: 0, Delete: 0, StaleSkip: 0},
	}
	...
	for i, org := range orgs {
		...
		if err := backfill.Do(ctx, BackfillWorkOSOrganizationParams{WorkOSOrganizationID: org.workosOrgID}); err != nil {
			...
			rep.directoryUserRows = rep.directoryUserRows.Add(org.directoryUserChanges)
			rep.deprovisionRows = rep.deprovisionRows.Add(org.deprovisionChanges)
...
func validateOrganization(ctx context.Context, db *pgxpool.Pool, org orgExpectation) error {
	...
	expectedAssignments := make(map[string][]string, len(org.members))
	for _, member := range org.members {
		...
		for _, slug := range member.RoleSlugs {
			roleExists, err := activeAssignmentRoleExists(ctx, db, gramOrgID, slug)
			if err != nil {
				return err
			}
			if roleExists {
				expectedAssignments[member.ID] = append(expectedAssignments[member.ID], slug)
			}
		}
	}
	...
	if err := requireRoleAssignments(ctx, db, gramOrgID, expectedAssignments); err != nil {
		return err
	}
	if err := validateDirectoryUsers(ctx, db, gramOrgID, org.directoryUsers); err != nil {
		return err
	}
...
// requireRoleAssignments checks that every membership holds a live assignment
// for each role WorkOS says it should have. Asserting on role URNs rather than
// on the mere existence of a row is what catches a replacement the write phase
// failed to apply — a member left on the old role still has an assignment, and
// a global role sharing an organization role's slug must not stand in for it.
// Extra local assignments are not failures here: global-role sync runs as its
// own phase and can legitimately add rows this snapshot does not describe.
func requireRoleAssignments(ctx context.Context, db *pgxpool.Pool, orgID string, expected map[string][]string) error {
	if len(expected) == 0 {
		return nil
	}

	membershipIDs := make([]string, 0, len(expected))
	for membershipID := range expected {
		membershipIDs = append(membershipIDs, membershipID)
	}
	sort.Strings(membershipIDs)

	actual, err := activeAssignmentURNs(ctx, db, orgID, membershipIDs)
	if err != nil {
		return err
	}

	for _, membershipID := range membershipIDs {
		expectedURNs, err := expectedAssignmentURNs(ctx, db, orgID, expected[membershipID])
		if err != nil {
			return err
		}
		roleURNs := make([]string, 0, len(expectedURNs))
		for roleURN := range expectedURNs {
			roleURNs = append(roleURNs, roleURN)
		}
		sort.Strings(roleURNs)
		for _, roleURN := range roleURNs {
			if !actual[membershipID][roleURN] {
				return fmt.Errorf("missing role assignment %q (%s) for membership %q", expectedURNs[roleURN], roleURN, membershipID)
			}
		}
	}
	return nil
...
func printOrganizationPlan(orgs []orgExpectation) {
	...
	directoryUserChanges := changeCounts{Create: 0, Update: 0, Noop: 0, Delete: 0, StaleSkip: 0}
	deprovisionChanges := changeCounts{Create: 0, Update: 0, Noop: 0, Delete: 0, StaleSkip: 0}
	var directoryUsers int
	...
	for _, org := range orgs {
		...
		directoryUserChanges = directoryUserChanges.Add(org.directoryUserChanges)
		deprovisionChanges = deprovisionChanges.Add(org.deprovisionChanges)
		for _, role := range org.roles {
			if role.Type == "OrganizationRole" {
				roles++
			}
		...
		directoryUsers += len(org.directoryUsers)
	...
	fmt.Printf("  expected_directory_users: %d\n", directoryUsers)
	...
	printChangeCounts("  directory_user_rows", directoryUserChanges)
	printChangeCounts("  directory_deprovision_rows", deprovisionChanges)
	printSamples(orgs)
...
func plannedOrganizationMutation(org orgExpectation) int {
	return org.orgChanges.Mutating() +
		...
		org.directoryUserChanges.Mutating() +
		org.deprovisionChanges.Mutating()
}
...
func roleAssignmentCreateDetail(organizationID string, member workos.Member, gramUserID string, roleSlug string) changeDetail {
	return changeDetail{
		Entity: "role_assignment",
		ID:     member.ID,
		Action: "create",
		Fields: []fieldChange{
			...
			{Name: "user_id", Before: "<missing>", After: gramUserID},
			{Name: "workos_membership_id", Before: "<missing>", After: member.ID},
			{Name: "workos_slug", Before: "<missing>", After: roleSlug},
		},
	}
}
...
func printReport(title string, rep report) {
	...
	if reportHasRowOutcomes(rep) {
		...
		printChangeCounts("  directory_user_rows", rep.directoryUserRows)
		printChangeCounts("  directory_deprovision_rows", rep.deprovisionRows)
	}
}
...
func reportHasRowOutcomes(rep report) bool {
	return rep.organizationRows != (changeCounts{Create: 0, Update: 0, Noop: 0, Delete: 0, StaleSkip: 0}) ||
		...
		rep.directoryUserRows != (changeCounts{Create: 0, Update: 0, Noop: 0, Delete: 0, StaleSkip: 0}) ||
		rep.deprovisionRows != (changeCounts{Create: 0, Update: 0, Noop: 0, Delete: 0, StaleSkip: 0})
}
...
func confirmWrite(opts options, summary string) error {
	...
	fmt.Printf("  DB changes run with lock_timeout=5s and statement_timeout=%s.\n", opts.statementTimeout)
