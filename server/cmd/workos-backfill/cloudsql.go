...
func grantCloudSQLIAMUserAccess(ctx context.Context, db dbExecer, user string, mode cloudSQLAccessMode) error {
	...
	case cloudSQLAccessModeWrite:
		// DELETE is scoped to the single table that needs it: revoking a
		// de-provisioned role's permissions hard-deletes from principal_grants,
		// while every other write in the backfill is a soft delete.
		grantsTable := pgx.Identifier{"gram", "principal_grants"}.Sanitize()
		statements = append(statements,
			...
			fmt.Sprintf("GRANT DELETE ON %s TO %s", grantsTable, grantee),
		)
	default:
