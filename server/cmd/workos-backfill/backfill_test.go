...
func TestBackfillWorkOSOrganization_RepairsMissingMembershipID(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	conn := newBackfillTestConn(t, "workos_backfill_repair_missing_membership_id")
	logger := testenv.NewLogger(t)

	const organizationID = "gram_org_backfill_missing_membership_id"
	const workosOrgID = "org_01JBACKFILLMISSINGMEMID"
	const workosUserID = "user_01JBACKFILLMISSINGMEMID"
	const gramUserID = "gram_user_01JBACKFILLMISSINGMEMID"
	const membershipID = "om_01JBACKFILLMISSINGMEMID"

	seedLinkedWorkOSOrganization(t, ctx, conn, organizationID, workosOrgID)
	_, err := usersrepo.New(conn).UpsertUser(ctx, usersrepo.UpsertUserParams{
		ID:          gramUserID,
		Email:       "missing-membership-id@example.com",
		DisplayName: "Missing Membership ID",
		PhotoUrl:    conv.ToPGTextEmpty(""),
		Admin:       false,
	})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	err = orgrepo.New(conn).SetOrganizationRelationshipWorkOSCursor(ctx, orgrepo.SetOrganizationRelationshipWorkOSCursorParams{
		OrganizationID:    organizationID,
		UserID:            conv.ToPGText(gramUserID),
		WorkosUpdatedAt:   pgtype.Timestamptz{Time: time.Time{}, Valid: false},
		WorkosLastEventID: conv.ToPGText("event_01JNEWER"),
	...
	workosClient.UpsertOrganization(workos.Organization{
		ID:         workosOrgID,
		Name:       "Backfill Missing Membership ID",
		ExternalID: "",
		CreatedAt:  "2026-05-07T11:00:00Z",
		UpdatedAt:  "2026-05-07T11:00:00Z",
	})
	...
	workosClient.UpsertOrganizationMembership(workos.Member{
		...
		UserID:         workosUserID,
		OrganizationID: workosOrgID,
		Organization:   "Backfill Missing Membership ID",
		RoleSlugs:      nil,
		Status:         "active",
		CreatedAt:      "2026-05-07T11:05:00Z",
		UpdatedAt:      "2026-05-07T11:05:00Z",
...
func TestBackfillWorkOSOrganization_KeepsRevocationNewerThanSnapshot(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	conn := newBackfillTestConn(t, "workos_backfill_keeps_newer_revocation")
	logger := testenv.NewLogger(t)

	const organizationID = "gram_org_backfill_keeps_newer_revocation"
	const workosOrgID = "org_01JBACKFILLKEEPSREVOCATION"
	const workosUserID = "user_01JBACKFILLKEEPSREVOCATION"
	const gramUserID = "gram_user_01JBACKFILLKEEPSREVOCATION"
	const membershipID = "om_01JBACKFILLKEEPSREVOCATION"
	const roleSlug = "org-member"
	...
	_, err := usersrepo.New(conn).UpsertUser(ctx, usersrepo.UpsertUserParams{
		ID:          gramUserID,
		Email:       "keeps-revocation@example.com",
		DisplayName: "Keeps Revocation",
		PhotoUrl:    conv.ToPGTextEmpty(""),
		Admin:       false,
	})
	require.NoError(t, err)
	err = orgrepo.New(conn).SyncUserOrganizationRoleAssignments(ctx, orgrepo.SyncUserOrganizationRoleAssignmentsParams{
		...
		WorkosMembershipID: conv.ToPGText(membershipID),
		WorkosUpdatedAt:    conv.ToPGTimestamptz(time.Date(2026, 5, 7, 11, 0, 0, 0, time.UTC)),
		WorkosLastEventID:  conv.ToPGText("event_01REVOKESETUP"),
	})
	require.NoError(t, err)
	// The revocation lands after the snapshot's membership updated_at, so it
	// represents an event the snapshot cannot have observed.
	err = orgrepo.New(conn).SyncUserOrganizationRoleAssignments(ctx, orgrepo.SyncUserOrganizationRoleAssignmentsParams{
		OrganizationID:     organizationID,
		WorkosUserID:       workosUserID,
		WorkosRoleSlugs:    []string{},
		UserID:             conv.ToPGText(gramUserID),
		WorkosMembershipID: conv.ToPGText(membershipID),
		WorkosUpdatedAt:    conv.ToPGTimestamptz(time.Date(2026, 5, 7, 14, 0, 0, 0, time.UTC)),
		WorkosLastEventID:  conv.ToPGText("event_02REVOKESETUP"),
	})
	require.NoError(t, err)
	...
	workosClient := newWorkOSSnapshotClient(t, ctx,
		workos.Organization{
			ID:         workosOrgID,
			Name:       "Backfill Keeps Newer Revocation",
			ExternalID: "",
			CreatedAt:  "2026-05-07T11:00:00Z",
			UpdatedAt:  "2026-05-07T11:00:00Z",
		},
		[]workos.Role{{
			ID:          "role_01JKEEPSREVOCATION",
			Name:        "Member",
			Slug:        roleSlug,
			Description: "",
			Type:        "OrganizationRole",
		...
		[]workos.Member{{
			...
			UserID:         workosUserID,
			OrganizationID: workosOrgID,
			Organization:   "Backfill Keeps Newer Revocation",
			RoleSlugs:      []string{roleSlug},
			Status:         "active",
			CreatedAt:      "2026-05-07T13:00:00Z",
			UpdatedAt:      "2026-05-07T13:00:00Z",
	...
	require.NoError(t, err)
	for _, assignment := range assignments {
		require.True(t, assignment.DeletedAt.Valid, "snapshot must not reinstate a revocation newer than itself")
	}
}
...
func seedDeactivationFixture(t *testing.T, ctx context.Context, conn *pgxpool.Pool, organizationID, workosOrgID, workosUserID, gramUserID, membershipID, directoryUserID, email, roleSlug string) {
	t.Helper()

	seedLinkedWorkOSOrganization(t, ctx, conn, organizationID, workosOrgID)
	seedOrganizationRoleWithCursor(t, ctx, conn, organizationID, roleSlug, "Member", "")

	seededAt := time.Date(2026, 5, 7, 10, 0, 0, 0, time.UTC)
	_, err := usersrepo.New(conn).UpsertSyncedUser(ctx, usersrepo.UpsertSyncedUserParams{
		...
		WorkosCreatedAt: conv.ToPGTimestamptz(seededAt),
		WorkosUpdatedAt: conv.ToPGTimestamptz(seededAt),
	})
	require.NoError(t, err)
...
func TestBackfillWorkOSOrganization_DeactivatedDirectoryUserDeprovisionsAccess(t *testing.T) {
	t.Parallel()

	ctx := t.Context()
	conn := newBackfillTestConn(t, "workos_backfill_directory_deactivation")
	logger := testenv.NewLogger(t)

	const organizationID = "gram_org_backfill_dir_deactivation"
	const workosOrgID = "org_01JBACKFILLDIRDEACT"
	const workosUserID = "user_01JBACKFILLDIRDEACT"
	const gramUserID = "gram_user_01JBACKFILLDIRDEACT"
	const membershipID = "om_01JBACKFILLDIRDEACT"
	const directoryID = "directory_01JBACKFILLDIRDEACT"
	const directoryUserID = "directory_user_01JBACKFILLDIRDEACT"
	const email = "deactivated@example.com"
	const roleSlug = "org-member"

	seedDeactivationFixture(t, ctx, conn, organizationID, workosOrgID, workosUserID, gramUserID, membershipID, directoryUserID, email, roleSlug)

	workosClient := workos.NewStubClient()
	workosClient.UpsertOrganization(workos.Organization{
		ID:         workosOrgID,
		Name:       "Backfill Directory Deactivation",
		ExternalID: "",
	...
	backfill := NewBackfillWorkOSOrganization(logger, conn, workosClient)
	...
	for _, assignment := range assignments {
		require.True(t, assignment.DeletedAt.Valid)
	}

	require.NoError(t, validateDirectoryUsers(ctx, conn, organizationID, []directoryUserRecord{{
...
func TestBackfillWorkOSOrganization_RelationshipNewerThanSnapshotNotDeprovisioned(t *testing.T) {
	...
	ctx := t.Context()
	conn := newBackfillTestConn(t, "workos_backfill_directory_postdated_relationship")
	logger := testenv.NewLogger(t)

	const organizationID = "gram_org_backfill_dir_postdated"
	const workosOrgID = "org_01JBACKFILLDIRPOSTDATED"
	const workosUserID = "user_01JBACKFILLDIRPOSTDATED"
	const gramUserID = "gram_user_01JBACKFILLDIRPOSTDATED"
	const membershipID = "om_01JBACKFILLDIRPOSTDATED"
	const directoryID = "directory_01JBACKFILLDIRPOSTDATED"
	const directoryUserID = "directory_user_01JBACKFILLDIRPOSTDATED"
	const email = "postdated@example.com"
	const roleSlug = "org-member"

	seedDeactivationFixture(t, ctx, conn, organizationID, workosOrgID, workosUserID, gramUserID, membershipID, directoryUserID, email, roleSlug)

	// A concurrent membership event stamped after the snapshot fetch must win
	// over the snapshot's deactivation: re-stamp the relationship into the
	// future relative to the backfill's snapshotAt (time.Now at Do time).
	...
	err := orgrepo.New(conn).UpsertWorkOSMembership(ctx, orgrepo.UpsertWorkOSMembershipParams{
		...
		WorkosUserID:       conv.ToPGText(workosUserID),
		WorkosMembershipID: conv.ToPGText(membershipID),
		WorkosUpdatedAt:    conv.ToPGTimestamptz(futureAt),
		WorkosLastEventID:  conv.ToPGText(""),
	})
	require.NoError(t, err)
	...
	workosClient.UpsertOrganization(workos.Organization{
		ID:         workosOrgID,
		Name:       "Backfill Directory Postdated",
		ExternalID: "",
		CreatedAt:  "2026-05-07T09:00:00Z",
		UpdatedAt:  "2026-05-07T09:00:00Z",
	})
