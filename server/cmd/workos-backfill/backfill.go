...
func (b *BackfillWorkOSOrganization) Do(ctx context.Context, params BackfillWorkOSOrganizationParams) error {
	...
	if err != nil {
		return oops.E(oops.CodeUnexpected, err, "list WorkOS organization users").LogError(ctx, logger)
	}
	...
	directoryUsers, err := listDirectoryUserRecords(ctx, logger, b.workos, params.WorkOSOrganizationID)
	if err != nil {
	...
	for _, directoryUser := range directoryUsers {
		if err := backfillDirectoryUser(ctx, logger, tx, org.ID, directoryUser, snapshotAt); err != nil {
			return err
		}
	}
...
func backfillOrganizationRoles(ctx context.Context, logger *slog.Logger, dbtx pgx.Tx, organizationID string, roles []workos.Role) error {
	...
	for _, localRole := range localRoles {
		...
		// Organization-role grants are keyed by the role's table id, not its
...
func backfillOrganizationMember(ctx context.Context, dbtx pgx.Tx, organizationID string, parsed backfillWorkOSMember, gramUserID string) error {
	...
	// Guard on the timestamp alone. The backfill has no event ID to offer, so
	// feeding the cursor's event ID here would compare "" against it and skip
	// every row an event has ever touched — including the revoked assignments
	// this phase exists to reconcile. Comparing timestamps keeps the useful
	// half of the guard: a revocation newer than the snapshot still wins, an
	// older one loses to WorkOS.
	if !shouldProcessEvent(nil, assignmentCursor.updatedAt, "", parsed.updatedAt) {
		return nil
	}
...
func latestAssignmentCursor(ctx context.Context, repo *orgrepo.Queries, organizationID, workosUserID string) (membershipCursor, error) {
	...
	// Soft-deleted assignments count toward the cursor: a revocation is the
	// newest local truth, and ignoring it would let an older snapshot reinstate
	// the role it just removed. Callers must guard on the timestamp rather than
	// the event ID — see backfillOrganizationMember.
