// Code generated by goa v3.25.3, DO NOT EDIT.
//
// HTTP request path constructors for the slackDirectoryConnections service.
//
// Command:
// $ goa gen github.com/speakeasy-api/gram/server/design

package client

// ListSlackDirectoryConnectionsPath returns the URL path to the slackDirectoryConnections service list HTTP endpoint.
func ListSlackDirectoryConnectionsPath() string {
	return "/rpc/slackDirectoryConnections.list"
}

// SyncSlackDirectoryConnectionsPath returns the URL path to the slackDirectoryConnections service sync HTTP endpoint.
func SyncSlackDirectoryConnectionsPath() string {
	return "/rpc/slackDirectoryConnections.sync"
}

// ListMembersSlackDirectoryConnectionsPath returns the URL path to the slackDirectoryConnections service listMembers HTTP endpoint.
func ListMembersSlackDirectoryConnectionsPath() string {
	return "/rpc/slackDirectoryConnections.listMembers"
}

// ListPersonAccountsSlackDirectoryConnectionsPath returns the URL path to the slackDirectoryConnections service listPersonAccounts HTTP endpoint.
func ListPersonAccountsSlackDirectoryConnectionsPath() string {
	return "/rpc/slackDirectoryConnections.listPersonAccounts"
}

// GetMemberSlackDirectoryConnectionsPath returns the URL path to the slackDirectoryConnections service getMember HTTP endpoint.
func GetMemberSlackDirectoryConnectionsPath() string {
	return "/rpc/slackDirectoryConnections.getMember"
}

// SetMappingSlackDirectoryConnectionsPath returns the URL path to the slackDirectoryConnections service setMapping HTTP endpoint.
func SetMappingSlackDirectoryConnectionsPath() string {
	return "/rpc/slackDirectoryConnections.setMapping"
}

// BeginSlackDirectoryConnectionsPath returns the URL path to the slackDirectoryConnections service begin HTTP endpoint.
func BeginSlackDirectoryConnectionsPath() string {
	return "/rpc/slackDirectoryConnections.begin"
}

// DisconnectSlackDirectoryConnectionsPath returns the URL path to the slackDirectoryConnections service disconnect HTTP endpoint.
func DisconnectSlackDirectoryConnectionsPath() string {
	return "/rpc/slackDirectoryConnections.disconnect"
}
