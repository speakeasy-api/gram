...
func New(
	...
	return &Server{
		Mounts: []*MountPoint{
			...
			{"QueryChatTurns", "POST", "/rpc/telemetry.queryChatTurns"},
		...
		QueryChatTurns:            NewQueryChatTurnsHandler(e.QueryChatTurns, mux, decoder, encoder, errhandler, formatter),
...
func (s *Server) Use(m func(http.Handler) http.Handler) {
	...
	s.Query = m(s.Query)
	s.QueryChatTurns = m(s.QueryChatTurns)
...
func Mount(mux goahttp.Muxer, h *Server) {
	...
	MountQueryChatTurnsHandler(mux, h.QueryChatTurns)
...
// MountQueryChatTurnsHandler configures the mux to serve the "telemetry"
// service "queryChatTurns" endpoint.
func MountQueryChatTurnsHandler(mux goahttp.Muxer, h http.Handler) {
	f, ok := h.(http.HandlerFunc)
	if !ok {
		f = func(w http.ResponseWriter, r *http.Request) {
			h.ServeHTTP(w, r)
	...
	mux.Handle("POST", "/rpc/telemetry.queryChatTurns", f)
}

// NewQueryChatTurnsHandler creates a HTTP handler which loads the HTTP request
// and calls the "telemetry" service "queryChatTurns" endpoint.
func NewQueryChatTurnsHandler(
	endpoint goa.Endpoint,
	mux goahttp.Muxer,
	decoder func(*http.Request) goahttp.Decoder,
	encoder func(context.Context, http.ResponseWriter) goahttp.Encoder,
	...
	var (
		decodeRequest  = DecodeQueryChatTurnsRequest(mux, decoder)
		encodeResponse = EncodeQueryChatTurnsResponse(encoder)
		encodeError    = EncodeQueryChatTurnsError(encoder, formatter)
	)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithValue(r.Context(), goahttp.AcceptTypeKey, r.Header.Get("Accept"))
		ctx = context.WithValue(ctx, goa.MethodKey, "queryChatTurns")
		ctx = context.WithValue(ctx, goa.ServiceKey, "telemetry")
		payload, err := decodeRequest(r)
		if err != nil {
			if err := encodeError(ctx, w, err); err != nil && errhandler != nil {
