// Code generated by goa v3.25.3, DO NOT EDIT.
//
// organizations client HTTP transport
//
// Command:
// $ goa gen github.com/speakeasy-api/gram/server/design

package client

import (
	"context"
	"net/http"

	goahttp "goa.design/goa/v3/http"
	goa "goa.design/goa/v3/pkg"
)

// Client lists the organizations service endpoint HTTP clients.
type Client struct {
	// SendInvite Doer is the HTTP client used to make requests to the sendInvite
	// endpoint.
	SendInviteDoer goahttp.Doer

	// RevokeInvite Doer is the HTTP client used to make requests to the
	// revokeInvite endpoint.
	RevokeInviteDoer goahttp.Doer

	// ListInvites Doer is the HTTP client used to make requests to the listInvites
	// endpoint.
	ListInvitesDoer goahttp.Doer

	// GetInviteByID Doer is the HTTP client used to make requests to the
	// getInviteByID endpoint.
	GetInviteByIDDoer goahttp.Doer

	// GetInviteByToken Doer is the HTTP client used to make requests to the
	// getInviteByToken endpoint.
	GetInviteByTokenDoer goahttp.Doer

	// ListUsers Doer is the HTTP client used to make requests to the listUsers
	// endpoint.
	ListUsersDoer goahttp.Doer

	// RemoveUser Doer is the HTTP client used to make requests to the removeUser
	// endpoint.
	RemoveUserDoer goahttp.Doer

	// RestoreResponseBody controls whether the response bodies are reset after
	// decoding so they can be read again.
	RestoreResponseBody bool

	scheme  string
	host    string
	encoder func(*http.Request) goahttp.Encoder
	decoder func(*http.Response) goahttp.Decoder
}

// NewClient instantiates HTTP clients for all the organizations service
// servers.
func NewClient(
	scheme string,
	host string,
	doer goahttp.Doer,
	enc func(*http.Request) goahttp.Encoder,
	dec func(*http.Response) goahttp.Decoder,
	restoreBody bool,
) *Client {
	return &Client{
		SendInviteDoer:       doer,
		RevokeInviteDoer:     doer,
		ListInvitesDoer:      doer,
		GetInviteByIDDoer:    doer,
		GetInviteByTokenDoer: doer,
		ListUsersDoer:        doer,
		RemoveUserDoer:       doer,
		RestoreResponseBody:  restoreBody,
		scheme:               scheme,
		host:                 host,
		decoder:              dec,
		encoder:              enc,
	}
}

// SendInvite returns an endpoint that makes HTTP requests to the organizations
// service sendInvite server.
func (c *Client) SendInvite() goa.Endpoint {
	var (
		encodeRequest  = EncodeSendInviteRequest(c.encoder)
		decodeResponse = DecodeSendInviteResponse(c.decoder, c.RestoreResponseBody)
	)
	return func(ctx context.Context, v any) (any, error) {
		req, err := c.BuildSendInviteRequest(ctx, v)
		if err != nil {
			return nil, err
		}
		err = encodeRequest(req, v)
		if err != nil {
			return nil, err
		}
		resp, err := c.SendInviteDoer.Do(req)
		if err != nil {
			return nil, goahttp.ErrRequestError("organizations", "sendInvite", err)
		}
		return decodeResponse(resp)
	}
}

// RevokeInvite returns an endpoint that makes HTTP requests to the
// organizations service revokeInvite server.
func (c *Client) RevokeInvite() goa.Endpoint {
	var (
		encodeRequest  = EncodeRevokeInviteRequest(c.encoder)
		decodeResponse = DecodeRevokeInviteResponse(c.decoder, c.RestoreResponseBody)
	)
	return func(ctx context.Context, v any) (any, error) {
		req, err := c.BuildRevokeInviteRequest(ctx, v)
		if err != nil {
			return nil, err
		}
		err = encodeRequest(req, v)
		if err != nil {
			return nil, err
		}
		resp, err := c.RevokeInviteDoer.Do(req)
		if err != nil {
			return nil, goahttp.ErrRequestError("organizations", "revokeInvite", err)
		}
		return decodeResponse(resp)
	}
}

// ListInvites returns an endpoint that makes HTTP requests to the
// organizations service listInvites server.
func (c *Client) ListInvites() goa.Endpoint {
	var (
		encodeRequest  = EncodeListInvitesRequest(c.encoder)
		decodeResponse = DecodeListInvitesResponse(c.decoder, c.RestoreResponseBody)
	)
	return func(ctx context.Context, v any) (any, error) {
		req, err := c.BuildListInvitesRequest(ctx, v)
		if err != nil {
			return nil, err
		}
		err = encodeRequest(req, v)
		if err != nil {
			return nil, err
		}
		resp, err := c.ListInvitesDoer.Do(req)
		if err != nil {
			return nil, goahttp.ErrRequestError("organizations", "listInvites", err)
		}
		return decodeResponse(resp)
	}
}

// GetInviteByID returns an endpoint that makes HTTP requests to the
// organizations service getInviteByID server.
func (c *Client) GetInviteByID() goa.Endpoint {
	var (
		encodeRequest  = EncodeGetInviteByIDRequest(c.encoder)
		decodeResponse = DecodeGetInviteByIDResponse(c.decoder, c.RestoreResponseBody)
	)
	return func(ctx context.Context, v any) (any, error) {
		req, err := c.BuildGetInviteByIDRequest(ctx, v)
		if err != nil {
			return nil, err
		}
		err = encodeRequest(req, v)
		if err != nil {
			return nil, err
		}
		resp, err := c.GetInviteByIDDoer.Do(req)
		if err != nil {
			return nil, goahttp.ErrRequestError("organizations", "getInviteByID", err)
		}
		return decodeResponse(resp)
	}
}

// GetInviteByToken returns an endpoint that makes HTTP requests to the
// organizations service getInviteByToken server.
func (c *Client) GetInviteByToken() goa.Endpoint {
	var (
		encodeRequest  = EncodeGetInviteByTokenRequest(c.encoder)
		decodeResponse = DecodeGetInviteByTokenResponse(c.decoder, c.RestoreResponseBody)
	)
	return func(ctx context.Context, v any) (any, error) {
		req, err := c.BuildGetInviteByTokenRequest(ctx, v)
		if err != nil {
			return nil, err
		}
		err = encodeRequest(req, v)
		if err != nil {
			return nil, err
		}
		resp, err := c.GetInviteByTokenDoer.Do(req)
		if err != nil {
			return nil, goahttp.ErrRequestError("organizations", "getInviteByToken", err)
		}
		return decodeResponse(resp)
	}
}

// ListUsers returns an endpoint that makes HTTP requests to the organizations
// service listUsers server.
func (c *Client) ListUsers() goa.Endpoint {
	var (
		encodeRequest  = EncodeListUsersRequest(c.encoder)
		decodeResponse = DecodeListUsersResponse(c.decoder, c.RestoreResponseBody)
	)
	return func(ctx context.Context, v any) (any, error) {
		req, err := c.BuildListUsersRequest(ctx, v)
		if err != nil {
			return nil, err
		}
		err = encodeRequest(req, v)
		if err != nil {
			return nil, err
		}
		resp, err := c.ListUsersDoer.Do(req)
		if err != nil {
			return nil, goahttp.ErrRequestError("organizations", "listUsers", err)
		}
		return decodeResponse(resp)
	}
}

// RemoveUser returns an endpoint that makes HTTP requests to the organizations
// service removeUser server.
func (c *Client) RemoveUser() goa.Endpoint {
	var (
		encodeRequest  = EncodeRemoveUserRequest(c.encoder)
		decodeResponse = DecodeRemoveUserResponse(c.decoder, c.RestoreResponseBody)
	)
	return func(ctx context.Context, v any) (any, error) {
		req, err := c.BuildRemoveUserRequest(ctx, v)
		if err != nil {
			return nil, err
		}
		err = encodeRequest(req, v)
		if err != nil {
			return nil, err
		}
		resp, err := c.RemoveUserDoer.Do(req)
		if err != nil {
			return nil, goahttp.ErrRequestError("organizations", "removeUser", err)
		}
		return decodeResponse(resp)
	}
}
