...
func riskUsage() {
	...
	fmt.Fprintln(os.Stderr, `    suggest-custom-detection-rule: Suggest a custom detection rule (rule_id, title, description, regex, severity) or, with target `+"`"+`exclusion`+"`"+`, an exclusion rule (match_type, match_value, filters) from a natural-language prompt. Calls the configured LLM with a JSON-schema constrained response so the dashboard can prefill the create form.`)
...
func riskSuggestCustomDetectionRuleUsage() {
	...
	// Description
	fmt.Fprintln(os.Stderr)
	fmt.Fprintln(os.Stderr, `Suggest a custom detection rule (rule_id, title, description, regex, severity) or, with target `+"`"+`exclusion`+"`"+`, an exclusion rule (match_type, match_value, filters) from a natural-language prompt. Calls the configured LLM with a JSON-schema constrained response so the dashboard can prefill the create form.`)
