...
// MountCompileCelHandler configures the mux to serve the "risk" service
// "compileCel" endpoint.
func MountCompileCelHandler(mux goahttp.Muxer, h http.Handler) {
	f, ok := h.(http.HandlerFunc)
	if !ok {
		f = func(w http.ResponseWriter, r *http.Request) {
			h.ServeHTTP(w, r)
	...
	mux.Handle("GET", "/rpc/risk.compileCel", f)
}

// NewCompileCelHandler creates a HTTP handler which loads the HTTP request and
// calls the "risk" service "compileCel" endpoint.
func NewCompileCelHandler(
	endpoint goa.Endpoint,
	mux goahttp.Muxer,
	decoder func(*http.Request) goahttp.Decoder,
	encoder func(context.Context, http.ResponseWriter) goahttp.Encoder,
	...
	var (
		decodeRequest  = DecodeCompileCelRequest(mux, decoder)
		encodeResponse = EncodeCompileCelResponse(encoder)
		encodeError    = EncodeCompileCelError(encoder, formatter)
	)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithValue(r.Context(), goahttp.AcceptTypeKey, r.Header.Get("Accept"))
		ctx = context.WithValue(ctx, goa.MethodKey, "compileCel")
		ctx = context.WithValue(ctx, goa.ServiceKey, "risk")
		payload, err := decodeRequest(r)
		if err != nil {
			if err := encodeError(ctx, w, err); err != nil && errhandler != nil {
