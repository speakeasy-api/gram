...
type Client struct {
	...
	GetSetupDocsEndpoint     goa.Endpoint
}

// NewClient initializes a "mcpRegistries" service client given the endpoints.
func NewClient(clearCache, listRegistries, listCatalog, getServerDetails, getSetupDocs goa.Endpoint) *Client {
	return &Client{
		...
		GetSetupDocsEndpoint:     getSetupDocs,
	}
}
...
// GetSetupDocs calls the "getSetupDocs" endpoint of the "mcpRegistries"
// service.
// GetSetupDocs may return the following errors:
//   - "unauthorized" (type *goa.ServiceError): unauthorized access
//   - "forbidden" (type *goa.ServiceError): permission denied
//   - "bad_request" (type *goa.ServiceError): request is invalid
//   - "not_found" (type *goa.ServiceError): resource not found
...
//   - "gateway_error" (type *goa.ServiceError): an unexpected error occurred
//   - error: internal error
func (c *Client) GetSetupDocs(ctx context.Context, p *GetSetupDocsPayload) (res *GetSetupDocsResult, err error) {
	var ires any
	ires, err = c.GetSetupDocsEndpoint(ctx, p)
	if err != nil {
		return
	}
	return ires.(*GetSetupDocsResult), nil
}
