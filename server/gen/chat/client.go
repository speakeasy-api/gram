...
// RenameChat calls the "renameChat" endpoint of the "chat" service.
// RenameChat may return the following errors:
//   - "unauthorized" (type *goa.ServiceError): unauthorized access
//   - "forbidden" (type *goa.ServiceError): permission denied
//   - "bad_request" (type *goa.ServiceError): request is invalid
//   - "not_found" (type *goa.ServiceError): resource not found
...
//   - "gateway_error" (type *goa.ServiceError): an unexpected error occurred
//   - error: internal error
func (c *Client) RenameChat(ctx context.Context, p *RenameChatPayload) (res *RenameChatResult, err error) {
	var ires any
	ires, err = c.RenameChatEndpoint(ctx, p)
	if err != nil {
		return
	}
	return ires.(*RenameChatResult), nil
}
