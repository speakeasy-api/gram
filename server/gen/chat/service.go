...
type LoadChatPayload struct {
	...
	Limit int
	// Keyset cursor: return messages with `seq` strictly less than this value
	// (older messages), newest first within the page. Use the `seq` of the oldest
	// message you currently hold to load the previous page. Ignored when
	// `risk_only` is set.
	BeforeSeq *int64
	// Keyset cursor: return messages with `seq` strictly greater than this value
	// (newer messages), oldest first within the page. Use the `seq` of the newest
	// message you currently hold to load the next page. Ignored when `risk_only`
	// is set.
	AfterSeq *int64
