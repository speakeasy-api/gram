// Code generated by goa v3.25.3, DO NOT EDIT.
//
// chat endpoints
//
// Command:
// $ goa gen github.com/speakeasy-api/gram/server/design

package chat

import (
	"context"

	goa "goa.design/goa/v3/pkg"
	"goa.design/goa/v3/security"
)

// Endpoints wraps the "chat" service endpoints.
type Endpoints struct {
	ListChats                  goa.Endpoint
	GetAssistantSessionSummary goa.Endpoint
	GetWorkUnitsTrend          goa.Endpoint
	LoadChat                   goa.Endpoint
	GenerateTitle              goa.Endpoint
	CreditUsage                goa.Endpoint
	DeleteChat                 goa.Endpoint
	SetPinned                  goa.Endpoint
	Summarize                  goa.Endpoint
	SummarizeToolCall          goa.Endpoint
	SubmitFeedback             goa.Endpoint
	ListSources                goa.Endpoint
	ListSessionLinks           goa.Endpoint
}

// NewEndpoints wraps the methods of the "chat" service with endpoints.
func NewEndpoints(s Service) *Endpoints {
	// Casting service to Auther interface
	a := s.(Auther)
	return &Endpoints{
		ListChats:                  NewListChatsEndpoint(s, a.APIKeyAuth, a.JWTAuth),
		GetAssistantSessionSummary: NewGetAssistantSessionSummaryEndpoint(s, a.APIKeyAuth),
		GetWorkUnitsTrend:          NewGetWorkUnitsTrendEndpoint(s, a.APIKeyAuth),
		LoadChat:                   NewLoadChatEndpoint(s, a.APIKeyAuth, a.JWTAuth),
		GenerateTitle:              NewGenerateTitleEndpoint(s, a.APIKeyAuth, a.JWTAuth),
		CreditUsage:                NewCreditUsageEndpoint(s, a.APIKeyAuth),
		DeleteChat:                 NewDeleteChatEndpoint(s, a.APIKeyAuth),
		SetPinned:                  NewSetPinnedEndpoint(s, a.APIKeyAuth),
		Summarize:                  NewSummarizeEndpoint(s, a.APIKeyAuth),
		SummarizeToolCall:          NewSummarizeToolCallEndpoint(s, a.APIKeyAuth),
		SubmitFeedback:             NewSubmitFeedbackEndpoint(s, a.APIKeyAuth, a.JWTAuth),
		ListSources:                NewListSourcesEndpoint(s, a.APIKeyAuth, a.JWTAuth),
		ListSessionLinks:           NewListSessionLinksEndpoint(s, a.APIKeyAuth, a.JWTAuth),
	}
}

// Use applies the given middleware to all the "chat" service endpoints.
func (e *Endpoints) Use(m func(goa.Endpoint) goa.Endpoint) {
	e.ListChats = m(e.ListChats)
	e.GetAssistantSessionSummary = m(e.GetAssistantSessionSummary)
	e.GetWorkUnitsTrend = m(e.GetWorkUnitsTrend)
	e.LoadChat = m(e.LoadChat)
	e.GenerateTitle = m(e.GenerateTitle)
	e.CreditUsage = m(e.CreditUsage)
	e.DeleteChat = m(e.DeleteChat)
	e.SetPinned = m(e.SetPinned)
	e.Summarize = m(e.Summarize)
	e.SummarizeToolCall = m(e.SummarizeToolCall)
	e.SubmitFeedback = m(e.SubmitFeedback)
	e.ListSources = m(e.ListSources)
	e.ListSessionLinks = m(e.ListSessionLinks)
}

// NewListChatsEndpoint returns an endpoint function that calls the method
// "listChats" of service "chat".
func NewListChatsEndpoint(s Service, authAPIKeyFn security.AuthAPIKeyFunc, authJWTFn security.AuthJWTFunc) goa.Endpoint {
	return func(ctx context.Context, req any) (any, error) {
		p := req.(*ListChatsPayload)
		var err error
		sc := security.APIKeyScheme{
			Name:           "session",
			Scopes:         []string{},
			RequiredScopes: []string{},
		}
		var key string
		if p.SessionToken != nil {
			key = *p.SessionToken
		}
		ctx, err = authAPIKeyFn(ctx, key, &sc)
		if err == nil {
			sc := security.APIKeyScheme{
				Name:           "project_slug",
				Scopes:         []string{},
				RequiredScopes: []string{},
			}
			var key string
			if p.ProjectSlugInput != nil {
				key = *p.ProjectSlugInput
			}
			ctx, err = authAPIKeyFn(ctx, key, &sc)
		}
		if err != nil {
			sc := security.JWTScheme{
				Name:           "chat_sessions_token",
				Scopes:         []string{},
				RequiredScopes: []string{},
			}
			var token string
			if p.ChatSessionsToken != nil {
				token = *p.ChatSessionsToken
			}
			ctx, err = authJWTFn(ctx, token, &sc)
		}
		if err != nil {
			return nil, err
		}
		return s.ListChats(ctx, p)
	}
}

// NewGetAssistantSessionSummaryEndpoint returns an endpoint function that
// calls the method "getAssistantSessionSummary" of service "chat".
func NewGetAssistantSessionSummaryEndpoint(s Service, authAPIKeyFn security.AuthAPIKeyFunc) goa.Endpoint {
	return func(ctx context.Context, req any) (any, error) {
		p := req.(*GetAssistantSessionSummaryPayload)
		var err error
		sc := security.APIKeyScheme{
			Name:           "session",
			Scopes:         []string{},
			RequiredScopes: []string{},
		}
		var key string
		if p.SessionToken != nil {
			key = *p.SessionToken
		}
		ctx, err = authAPIKeyFn(ctx, key, &sc)
		if err == nil {
			sc := security.APIKeyScheme{
				Name:           "project_slug",
				Scopes:         []string{},
				RequiredScopes: []string{},
			}
			var key string
			if p.ProjectSlugInput != nil {
				key = *p.ProjectSlugInput
			}
			ctx, err = authAPIKeyFn(ctx, key, &sc)
		}
		if err != nil {
			return nil, err
		}
		return s.GetAssistantSessionSummary(ctx, p)
	}
}

// NewGetWorkUnitsTrendEndpoint returns an endpoint function that calls the
// method "getWorkUnitsTrend" of service "chat".
func NewGetWorkUnitsTrendEndpoint(s Service, authAPIKeyFn security.AuthAPIKeyFunc) goa.Endpoint {
	return func(ctx context.Context, req any) (any, error) {
		p := req.(*GetWorkUnitsTrendPayload)
		var err error
		sc := security.APIKeyScheme{
			Name:           "session",
			Scopes:         []string{},
			RequiredScopes: []string{},
		}
		var key string
		if p.SessionToken != nil {
			key = *p.SessionToken
		}
		ctx, err = authAPIKeyFn(ctx, key, &sc)
		if err == nil {
			sc := security.APIKeyScheme{
				Name:           "project_slug",
				Scopes:         []string{},
				RequiredScopes: []string{},
			}
			var key string
			if p.ProjectSlugInput != nil {
				key = *p.ProjectSlugInput
			}
			ctx, err = authAPIKeyFn(ctx, key, &sc)
		}
		if err != nil {
			return nil, err
		}
		return s.GetWorkUnitsTrend(ctx, p)
	}
}

// NewLoadChatEndpoint returns an endpoint function that calls the method
// "loadChat" of service "chat".
func NewLoadChatEndpoint(s Service, authAPIKeyFn security.AuthAPIKeyFunc, authJWTFn security.AuthJWTFunc) goa.Endpoint {
	return func(ctx context.Context, req any) (any, error) {
		p := req.(*LoadChatPayload)
		var err error
		sc := security.APIKeyScheme{
			Name:           "session",
			Scopes:         []string{},
			RequiredScopes: []string{},
		}
		var key string
		if p.SessionToken != nil {
			key = *p.SessionToken
		}
		ctx, err = authAPIKeyFn(ctx, key, &sc)
		if err == nil {
			sc := security.APIKeyScheme{
				Name:           "project_slug",
				Scopes:         []string{},
				RequiredScopes: []string{},
			}
			var key string
			if p.ProjectSlugInput != nil {
				key = *p.ProjectSlugInput
			}
			ctx, err = authAPIKeyFn(ctx, key, &sc)
		}
		if err != nil {
			sc := security.JWTScheme{
				Name:           "chat_sessions_token",
				Scopes:         []string{},
				RequiredScopes: []string{},
			}
			var token string
			if p.ChatSessionsToken != nil {
				token = *p.ChatSessionsToken
			}
			ctx, err = authJWTFn(ctx, token, &sc)
		}
		if err != nil {
			sc := security.APIKeyScheme{
				Name:           "apikey",
				Scopes:         []string{"consumer", "producer", "chat", "hooks", "agent", "agent_user"},
				RequiredScopes: []string{"producer"},
			}
			var key string
			if p.ApikeyToken != nil {
				key = *p.ApikeyToken
			}
			ctx, err = authAPIKeyFn(ctx, key, &sc)
			if err == nil {
				sc := security.APIKeyScheme{
					Name:           "project_slug",
					Scopes:         []string{},
					RequiredScopes: []string{"producer"},
				}
				var key string
				if p.ProjectSlugInput != nil {
					key = *p.ProjectSlugInput
				}
				ctx, err = authAPIKeyFn(ctx, key, &sc)
			}
		}
		if err != nil {
			return nil, err
		}
		return s.LoadChat(ctx, p)
	}
}

// NewGenerateTitleEndpoint returns an endpoint function that calls the method
// "generateTitle" of service "chat".
func NewGenerateTitleEndpoint(s Service, authAPIKeyFn security.AuthAPIKeyFunc, authJWTFn security.AuthJWTFunc) goa.Endpoint {
	return func(ctx context.Context, req any) (any, error) {
		p := req.(*GenerateTitlePayload)
		var err error
		sc := security.APIKeyScheme{
			Name:           "session",
			Scopes:         []string{},
			RequiredScopes: []string{},
		}
		var key string
		if p.SessionToken != nil {
			key = *p.SessionToken
		}
		ctx, err = authAPIKeyFn(ctx, key, &sc)
		if err == nil {
			sc := security.APIKeyScheme{
				Name:           "project_slug",
				Scopes:         []string{},
				RequiredScopes: []string{},
			}
			var key string
			if p.ProjectSlugInput != nil {
				key = *p.ProjectSlugInput
			}
			ctx, err = authAPIKeyFn(ctx, key, &sc)
		}
		if err != nil {
			sc := security.JWTScheme{
				Name:           "chat_sessions_token",
				Scopes:         []string{},
				RequiredScopes: []string{},
			}
			var token string
			if p.ChatSessionsToken != nil {
				token = *p.ChatSessionsToken
			}
			ctx, err = authJWTFn(ctx, token, &sc)
		}
		if err != nil {
			return nil, err
		}
		return s.GenerateTitle(ctx, p)
	}
}

// NewCreditUsageEndpoint returns an endpoint function that calls the method
// "creditUsage" of service "chat".
func NewCreditUsageEndpoint(s Service, authAPIKeyFn security.AuthAPIKeyFunc) goa.Endpoint {
	return func(ctx context.Context, req any) (any, error) {
		p := req.(*CreditUsagePayload)
		var err error
		sc := security.APIKeyScheme{
			Name:           "session",
			Scopes:         []string{},
			RequiredScopes: []string{},
		}
		var key string
		if p.SessionToken != nil {
			key = *p.SessionToken
		}
		ctx, err = authAPIKeyFn(ctx, key, &sc)
		if err != nil {
			return nil, err
		}
		return s.CreditUsage(ctx, p)
	}
}

// NewDeleteChatEndpoint returns an endpoint function that calls the method
// "deleteChat" of service "chat".
func NewDeleteChatEndpoint(s Service, authAPIKeyFn security.AuthAPIKeyFunc) goa.Endpoint {
	return func(ctx context.Context, req any) (any, error) {
		p := req.(*DeleteChatPayload)
		var err error
		sc := security.APIKeyScheme{
			Name:           "session",
			Scopes:         []string{},
			RequiredScopes: []string{},
		}
		var key string
		if p.SessionToken != nil {
			key = *p.SessionToken
		}
		ctx, err = authAPIKeyFn(ctx, key, &sc)
		if err == nil {
			sc := security.APIKeyScheme{
				Name:           "project_slug",
				Scopes:         []string{},
				RequiredScopes: []string{},
			}
			var key string
			if p.ProjectSlugInput != nil {
				key = *p.ProjectSlugInput
			}
			ctx, err = authAPIKeyFn(ctx, key, &sc)
		}
		if err != nil {
			return nil, err
		}
		return nil, s.DeleteChat(ctx, p)
	}
}

// NewSetPinnedEndpoint returns an endpoint function that calls the method
// "setPinned" of service "chat".
func NewSetPinnedEndpoint(s Service, authAPIKeyFn security.AuthAPIKeyFunc) goa.Endpoint {
	return func(ctx context.Context, req any) (any, error) {
		p := req.(*SetPinnedPayload)
		var err error
		sc := security.APIKeyScheme{
			Name:           "session",
			Scopes:         []string{},
			RequiredScopes: []string{},
		}
		var key string
		if p.SessionToken != nil {
			key = *p.SessionToken
		}
		ctx, err = authAPIKeyFn(ctx, key, &sc)
		if err == nil {
			sc := security.APIKeyScheme{
				Name:           "project_slug",
				Scopes:         []string{},
				RequiredScopes: []string{},
			}
			var key string
			if p.ProjectSlugInput != nil {
				key = *p.ProjectSlugInput
			}
			ctx, err = authAPIKeyFn(ctx, key, &sc)
		}
		if err != nil {
			return nil, err
		}
		return nil, s.SetPinned(ctx, p)
	}
}

// NewSummarizeEndpoint returns an endpoint function that calls the method
// "summarize" of service "chat".
func NewSummarizeEndpoint(s Service, authAPIKeyFn security.AuthAPIKeyFunc) goa.Endpoint {
	return func(ctx context.Context, req any) (any, error) {
		p := req.(*SummarizePayload)
		var err error
		sc := security.APIKeyScheme{
			Name:           "session",
			Scopes:         []string{},
			RequiredScopes: []string{},
		}
		var key string
		if p.SessionToken != nil {
			key = *p.SessionToken
		}
		ctx, err = authAPIKeyFn(ctx, key, &sc)
		if err == nil {
			sc := security.APIKeyScheme{
				Name:           "project_slug",
				Scopes:         []string{},
				RequiredScopes: []string{},
			}
			var key string
			if p.ProjectSlugInput != nil {
				key = *p.ProjectSlugInput
			}
			ctx, err = authAPIKeyFn(ctx, key, &sc)
		}
		if err != nil {
			return nil, err
		}
		return s.Summarize(ctx, p)
	}
}

// NewSummarizeToolCallEndpoint returns an endpoint function that calls the
// method "summarizeToolCall" of service "chat".
func NewSummarizeToolCallEndpoint(s Service, authAPIKeyFn security.AuthAPIKeyFunc) goa.Endpoint {
	return func(ctx context.Context, req any) (any, error) {
		p := req.(*SummarizeToolCallPayload)
		var err error
		sc := security.APIKeyScheme{
			Name:           "session",
			Scopes:         []string{},
			RequiredScopes: []string{},
		}
		var key string
		if p.SessionToken != nil {
			key = *p.SessionToken
		}
		ctx, err = authAPIKeyFn(ctx, key, &sc)
		if err == nil {
			sc := security.APIKeyScheme{
				Name:           "project_slug",
				Scopes:         []string{},
				RequiredScopes: []string{},
			}
			var key string
			if p.ProjectSlugInput != nil {
				key = *p.ProjectSlugInput
			}
			ctx, err = authAPIKeyFn(ctx, key, &sc)
		}
		if err != nil {
			return nil, err
		}
		return s.SummarizeToolCall(ctx, p)
	}
}

// NewSubmitFeedbackEndpoint returns an endpoint function that calls the method
// "submitFeedback" of service "chat".
func NewSubmitFeedbackEndpoint(s Service, authAPIKeyFn security.AuthAPIKeyFunc, authJWTFn security.AuthJWTFunc) goa.Endpoint {
	return func(ctx context.Context, req any) (any, error) {
		p := req.(*SubmitFeedbackPayload)
		var err error
		sc := security.APIKeyScheme{
			Name:           "session",
			Scopes:         []string{},
			RequiredScopes: []string{},
		}
		var key string
		if p.SessionToken != nil {
			key = *p.SessionToken
		}
		ctx, err = authAPIKeyFn(ctx, key, &sc)
		if err == nil {
			sc := security.APIKeyScheme{
				Name:           "project_slug",
				Scopes:         []string{},
				RequiredScopes: []string{},
			}
			var key string
			if p.ProjectSlugInput != nil {
				key = *p.ProjectSlugInput
			}
			ctx, err = authAPIKeyFn(ctx, key, &sc)
		}
		if err != nil {
			sc := security.JWTScheme{
				Name:           "chat_sessions_token",
				Scopes:         []string{},
				RequiredScopes: []string{},
			}
			var token string
			if p.ChatSessionsToken != nil {
				token = *p.ChatSessionsToken
			}
			ctx, err = authJWTFn(ctx, token, &sc)
		}
		if err != nil {
			return nil, err
		}
		return s.SubmitFeedback(ctx, p)
	}
}

// NewListSourcesEndpoint returns an endpoint function that calls the method
// "listSources" of service "chat".
func NewListSourcesEndpoint(s Service, authAPIKeyFn security.AuthAPIKeyFunc, authJWTFn security.AuthJWTFunc) goa.Endpoint {
	return func(ctx context.Context, req any) (any, error) {
		p := req.(*ListSourcesPayload)
		var err error
		sc := security.APIKeyScheme{
			Name:           "session",
			Scopes:         []string{},
			RequiredScopes: []string{},
		}
		var key string
		if p.SessionToken != nil {
			key = *p.SessionToken
		}
		ctx, err = authAPIKeyFn(ctx, key, &sc)
		if err == nil {
			sc := security.APIKeyScheme{
				Name:           "project_slug",
				Scopes:         []string{},
				RequiredScopes: []string{},
			}
			var key string
			if p.ProjectSlugInput != nil {
				key = *p.ProjectSlugInput
			}
			ctx, err = authAPIKeyFn(ctx, key, &sc)
		}
		if err != nil {
			sc := security.JWTScheme{
				Name:           "chat_sessions_token",
				Scopes:         []string{},
				RequiredScopes: []string{},
			}
			var token string
			if p.ChatSessionsToken != nil {
				token = *p.ChatSessionsToken
			}
			ctx, err = authJWTFn(ctx, token, &sc)
		}
		if err != nil {
			return nil, err
		}
		return s.ListSources(ctx, p)
	}
}

// NewListSessionLinksEndpoint returns an endpoint function that calls the
// method "listSessionLinks" of service "chat".
func NewListSessionLinksEndpoint(s Service, authAPIKeyFn security.AuthAPIKeyFunc, authJWTFn security.AuthJWTFunc) goa.Endpoint {
	return func(ctx context.Context, req any) (any, error) {
		p := req.(*ListSessionLinksPayload)
		var err error
		sc := security.APIKeyScheme{
			Name:           "session",
			Scopes:         []string{},
			RequiredScopes: []string{},
		}
		var key string
		if p.SessionToken != nil {
			key = *p.SessionToken
		}
		ctx, err = authAPIKeyFn(ctx, key, &sc)
		if err == nil {
			sc := security.APIKeyScheme{
				Name:           "project_slug",
				Scopes:         []string{},
				RequiredScopes: []string{},
			}
			var key string
			if p.ProjectSlugInput != nil {
				key = *p.ProjectSlugInput
			}
			ctx, err = authAPIKeyFn(ctx, key, &sc)
		}
		if err != nil {
			sc := security.JWTScheme{
				Name:           "chat_sessions_token",
				Scopes:         []string{},
				RequiredScopes: []string{},
			}
			var token string
			if p.ChatSessionsToken != nil {
				token = *p.ChatSessionsToken
			}
			ctx, err = authJWTFn(ctx, token, &sc)
		}
		if err != nil {
			return nil, err
		}
		return s.ListSessionLinks(ctx, p)
	}
}
