...
CREATE TABLE "external_mcp_tool_definitions" (
  ...
  "deleted" boolean NOT NULL GENERATED ALWAYS AS (deleted_at IS NOT NULL) STORED,
  PRIMARY KEY ("id"),
  CONSTRAINT "external_mcp_tool_definitions_deployment_external_mcp_id_fkey" FOREIGN KEY ("deployment_external_mcp_id") REFERENCES "deployments_external_mcps" ("id") ON UPDATE NO ACTION ON DELETE SET NULL,
...
CREATE INDEX "external_mcp_tool_definitions_deployment_external_mcp_id_idx" ON "external_mcp_tool_definitions" ("deployment_external_mcp_id") WHERE (deleted IS FALSE);
