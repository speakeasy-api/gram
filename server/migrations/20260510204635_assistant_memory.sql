...
CREATE TABLE "assistant_memories" (
  ...
  PRIMARY KEY ("id"),
  CONSTRAINT "assistant_memories_assistant_id_fkey" FOREIGN KEY ("assistant_id") REFERENCES "assistants" ("id") ON UPDATE NO ACTION ON DELETE SET NULL,
  CONSTRAINT "assistant_memories_project_id_fkey" FOREIGN KEY ("project_id") REFERENCES "projects" ("id") ON UPDATE NO ACTION ON DELETE SET NULL,
...
CREATE INDEX "assistant_memories_assistant_active" ON "assistant_memories" ("assistant_id", "created_at" DESC) WHERE (deleted IS FALSE);
-- Create index "assistant_memories_embedding_hnsw" to table: "assistant_memories"
CREATE INDEX "assistant_memories_embedding_hnsw" ON "assistant_memories" USING hnsw ("embedding" halfvec_cosine_ops) WHERE ((deleted IS FALSE) AND (superseded_at IS NULL));
-- Create index "assistant_memories_tags_gin" to table: "assistant_memories"
CREATE INDEX "assistant_memories_tags_gin" ON "assistant_memories" USING gin ("tags") WHERE (deleted IS FALSE);
