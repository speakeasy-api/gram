...
CREATE TABLE "okta_resource_connections" (
  ...
  CONSTRAINT "okta_resource_connections_connection_tenant_fkey" FOREIGN KEY ("organization_id", "identity_provider_connection_id") REFERENCES "okta_identity_provider_connections" ("organization_id", "identity_provider_connection_id") ON UPDATE NO ACTION ON DELETE CASCADE,
  CONSTRAINT "okta_resource_connections_okta_application_fkey" FOREIGN KEY ("organization_id", "identity_provider_connection_id", "okta_application_id") REFERENCES "okta_applications" ("organization_id", "identity_provider_connection_id", "okta_app_id") ON UPDATE NO ACTION ON DELETE SET NULL,
  ...
  CONSTRAINT "okta_resource_connections_audience_check" CHECK (btrim(audience) <> ''::text),
  CONSTRAINT "okta_resource_connections_okta_application_id_check" CHECK ((okta_application_id IS NULL) OR (btrim(okta_application_id) <> ''::text)),
  CONSTRAINT "okta_resource_connections_resource_check" CHECK (btrim(resource) <> ''::text)
);
