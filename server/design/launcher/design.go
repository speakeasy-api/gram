...
var LauncherContext = Type("LauncherContext", func() {
	...
	Attribute("route", String, "The current dashboard route path.")
})

var LauncherCandidate = Type("LauncherCandidate", func() {
	Description("One item the palette could open or act on.")

	Attribute("id", String, "Caller-chosen stable identifier echoed back in the judgment.")
	Attribute("kind", String, "The candidate kind, such as page, mcp_server, plugin or marketplace.")
	...
	Attribute("verbs", ArrayOf(String), "Verbs the caller may run on this candidate, such as open, enable, disable or publish.")
...
var LauncherJudgment = Type("LauncherJudgment", func() {
	...
	Attribute("disabled", Boolean, "True when no intent service is configured; the other fields are then absent.")
