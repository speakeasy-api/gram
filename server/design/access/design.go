...
var SelectorModel = Type("Selector", func() {
	...
	Attribute("resource_kind", String, func() {
		...
		Enum("project", "mcp", "org", "environment", "skill", "risk_policy", "chat", "agent", "logs", "*")
	})
	...
	Attribute("actor_department", String, func() {
		Description("Directory department of the people whose activity this grant covers (logs scopes only).")
	})
	Attribute("actor_group", String, func() {
		Description("Directory group name of the people whose activity this grant covers (logs scopes only).")
	})
	Attribute("actor_role", String, func() {
		Description("Identity-provider role slug of the people whose activity this grant covers (logs scopes only).")
	})
})
...
var ScopeModel = Type("ScopeDefinition", func() {
	...
	Attribute("resource_type", String, func() {
		...
		Enum("org", "project", "mcp", "environment", "skill", "risk_policy", "chat", "agent", "logs")
	})
...
var RequestAccessForm = Type("RequestAccessForm", func() {
	...
	Attribute("scope", String, func() {
		...
		Enum("org:read", "org:admin", "project:read", "project:write", "mcp:read", "mcp:write", "mcp:connect", "environment:read", "environment:write", "skill:read", "skill:write", "risk_policy:evaluate", "risk_policy:bypass", "chat:read", "chat:write", "logs:read", "agent:read", "agent:write", "agent:authorize", "agent:transfer")
	})
