...
var ChatOverview = Type("ChatOverview", func() {
	...
	Attribute("risk_findings_count", Int, "Number of distinct risk findings recorded against messages in this chat (deduped by source/rule/match; project-scoped, found=true, excluding excluded and false-positive results). Only populated by endpoints that join risk data; absent elsewhere.")
	Attribute("low_risk_findings_count", Int, "Number of distinct active findings (same dedup as risk_findings_count) whose policy severity is low (score < 4.0). Only populated by endpoints that join risk data.")
	Attribute("medium_risk_findings_count", Int, "Number of distinct active findings whose policy severity is medium (4.0 <= score < 7.0). Only populated by endpoints that join risk data.")
	Attribute("high_risk_findings_count", Int, "Number of distinct active findings whose policy severity is high or critical (score >= 7.0). Only populated by endpoints that join risk data.")
