...
var _ = Service("admin", func() {
	...
	Method("listOrganizations", func() {
		...
		Payload(func() {
			...
			Attribute("min_members", Int64, "Inclusive minimum active member count, from 0 through 9223372036854775807. JavaScript clients must use a decimal string or bigint above Number.MAX_SAFE_INTEGER.", func() { Minimum(0) })
			Attribute("max_members", Int64, "Inclusive maximum active member count, from 0 through 9223372036854775807. Must be at least min_members. JavaScript clients must use a decimal string or bigint above Number.MAX_SAFE_INTEGER.", func() { Minimum(0) })
			Attribute("disabled_only", Boolean, "When supplied, overrides both include_disabled and disabled_states: true selects only disabled organizations (even for exact ID searches); false does not restrict status. Omitted preserves legacy behavior, including the exact ID exception.")
