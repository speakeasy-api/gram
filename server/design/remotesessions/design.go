...
var _ = Service("remoteSessions", func() {
	Description("Operator visibility into remote_sessions Gram is holding on a subject's behalf. Read + revoke; sessions are written by /mcp/{slug}/remote_login_callback and the silent-refresh path. access_token_encrypted and refresh_token_encrypted are never returned.")
...
var RemoteSession = Type("RemoteSession", func() {
	...
	Description("A remote_session record — Gram's upstream OAuth session for a (subject, remote_session_client) pair. access_token_encrypted and refresh_token_encrypted are never returned.")
