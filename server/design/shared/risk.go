...
var RiskResult = Type("RiskResult", func() {
	...
	Attribute("source", String, "Detection source (e.g. gitleaks). Stable, lowercase identifier of the scanner that produced this finding.")
	Attribute("rule_id", String, "The matched rule identifier, in lowercase kebab-case. The pair (source, rule_id) is the stable composite key consumers should use to recognize a finding type; the same id never carries a source prefix.")
	Attribute("description", String, "Human-readable, source-agnostic description of the finding. Never echoes the matched value and never leaks internal validator detail. Safe to display verbatim.")
	Attribute("match", String, "The matched secret or sensitive data. Treat as sensitive; do not surface unredacted in public contracts.")
