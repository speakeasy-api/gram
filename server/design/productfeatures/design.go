...
var _ = Service("features", func() {
	...
	Method("getProductFeatures", func() {
		...
		Result(func() {
			...
			Attribute("admin_mcp_enabled", Boolean, "Whether the organization is eligible for the Gram Admin MCP capability")
	...
	Method("setProductFeature", func() {
		...
		Payload(func() {
			Attribute("feature_name", String, "Name of the feature to update", func() {
				MaxLength(60)
				Enum("logs", "tool_io_logs", "session_capture", "authz_challenge_logging", "webhooks", "sso", "scim", "hooks_browser_login", "hooks_fail_open", "custom_model_keys", "skills", "skill_capture_metadata_only", "ai_platform_push_integrations", "admin_mcp")
			})
