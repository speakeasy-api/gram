...
var ProductFeaturesResult = ResultType("application/vnd.gram.product-features", func() {
	...
	Attributes(func() {
		...
		Required("logs_enabled", "tool_io_logs_enabled", "session_capture_enabled", "skills_capture_enabled", "block_shadow_mcp_enabled")
	})
})
	...
	Method("setProductFeature", func() {
		...
		Payload(func() {
			Attribute("feature_name", String, "Name of the feature to update", func() {
				MaxLength(60)
				Enum("logs", "tool_io_logs", "session_capture", "skills_capture", "block_shadow_mcp")
			})
