...
var _ = Service("features", func() {
	...
	Method("getProductFeatures", func() {
		...
		Result(func() {
			...
			Required("logs_enabled", "tool_io_logs_enabled", "session_capture_enabled", "authz_challenge_logging_enabled", "webhooks", "sso_enabled", "scim_enabled", "observability_mode_enabled", "hooks_browser_login_enabled", "hooks_fail_open_enabled", "custom_model_keys_enabled")
		})
	...
	Method("setProductFeature", func() {
		...
		Payload(func() {
			Attribute("feature_name", String, "Name of the feature to update", func() {
				MaxLength(60)
				Enum("logs", "tool_io_logs", "session_capture", "authz_challenge_logging", "webhooks", "sso", "scim", "observability_mode", "hooks_browser_login", "hooks_fail_open", "custom_model_keys")
			})
