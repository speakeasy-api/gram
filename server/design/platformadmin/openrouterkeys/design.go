...
// completions. It exposes the encryption state of each key alongside the
// enable/disable and encrypt-at-rest remediation actions. Key material never
// appears in any payload or result.
...
var AdminKey = Type("AdminOpenRouterKey", func() {
	...
	Required("organization_id", "organization_name", "organization_slug", "gram_account_type", "key_type", "monthly_credits", "disabled", "encryption_status", "created_at", "updated_at")
	...
	Attribute("encryption_status", String, "Storage state of the key material.", func() {
		Enum("plaintext", "encrypted_with_plaintext", "encrypted")
	})
...
var _ = Service("adminOpenRouterKeys", func() {
	...
	Method("listKeys", func() {
		Description("List every organization's platform OpenRouter keys with their encryption state. Requires platform admin.")
	...
	Method("encryptKey", func() {
		Description("Encrypt an organization's stored OpenRouter key at rest: writes the encrypted copy, verifies it decrypts, then clears the plaintext column. Idempotent. Requires platform admin.")

		Payload(func() {
			security.SessionPayload()
			Attribute("organization_id", String, "Organization that owns the key.")
			Attribute("key_type", String, "Key type to encrypt.", func() { Enum("chat", "internal") })
			Required("organization_id", "key_type")
			// Named explicitly: the encrypt, disable, and enable payloads are
			// structurally identical and Goa deduplicates request bodies by
			// shape, which would give the other methods this method's SDK
			// type name.
			Meta("openapi:typename", "EncryptOpenRouterKeyRequestBody")
		})

		Result(AdminKey)

		HTTP(func() {
			POST("/rpc/adminOpenRouterKeys.encryptKey")
			security.SessionHeader()
			Response(StatusOK)
		})

		Meta("openapi:operationId", "encryptAdminOpenRouterKey")
		Meta("openapi:extension:x-speakeasy-name-override", "encryptKey")
		Meta("openapi:extension:x-speakeasy-react-hook", `{"name": "EncryptAdminOpenRouterKey"}`)
	})
	...
	Method("disableKey", func() {
		...
		Payload(func() {
			...
			Required("organization_id", "key_type")
			// Named explicitly to avoid Goa's request-body dedupe; see
			// encryptKey.
