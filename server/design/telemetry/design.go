...
var _ = Service("telemetry", func() {
	...
	Method("listEventLog", func() {
		...
		// Org-scoped: the feed spans every project in the caller's
		// organization.
		Security(security.Session)
		...
		HTTP(func() {
			POST("/rpc/telemetry.listEventLog")
			security.SessionHeader()
			Response(StatusOK)
		})
	...
	Method("getEventVolume", func() {
		...
		HTTP(func() {
			POST("/rpc/telemetry.getEventVolume")
			security.SessionHeader()
			Response(StatusOK)
		})
	...
	Method("getEventFacets", func() {
		...
		HTTP(func() {
			POST("/rpc/telemetry.getEventFacets")
			security.SessionHeader()
			Response(StatusOK)
		})
...
var EventLogEntryType = Type("EventLogEntry", func() {
	...
	Attribute("record_id", String, "Stable dedup id assigned at the ingest edge")
