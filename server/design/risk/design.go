...
var _ = Service("risk", func() {
	...
	Method("createRiskPolicy", func() {
		...
		Payload(func() {
			...
			Attribute("judge_temperature", Float64, "For prompt_based policies: Sampling temperature for the LLM judge. Defaults to a low value for deterministic verdicts.")
			Attribute("judge_fail_open", Boolean, "For prompt_based policies: When the LLM judge errors or times out: true allows the message (fail-open), false blocks it (fail-closed). Defaults to fail-open.")
	...
	Method("updateRiskPolicy", func() {
		...
		Payload(func() {
			...
			Attribute("judge_temperature", Float64, "For prompt_based policies: Sampling temperature for the LLM judge. Defaults to a low value for deterministic verdicts. Omit to preserve the current value.")
			Attribute("judge_fail_open", Boolean, "For prompt_based policies: When the LLM judge errors or times out: true allows the message (fail-open), false blocks it (fail-closed). Defaults to fail-open. Omit to preserve the current value.")
	...
	Method("evaluatePromptGuardrail", func() {
		...
		Payload(func() {
			...
			Attribute("judge_temperature", Float64, "Sampling temperature for the LLM judge. Defaults to a low value for deterministic verdicts.")
			Attribute("judge_fail_open", Boolean, "When the LLM judge errors or times out: true allows the message (fail-open), false blocks it (fail-closed). Defaults to fail-open.")
