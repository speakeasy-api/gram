...
func (s *Service) dialMetaMember(
	...
	case member.remoteServerID.Valid:
		...
		return func(context.Context) (*proxy.Proxy, error) {
			...
			p := s.remoteProxyManager.Build(logger, &remoteServer, member.serverID.String(), headers, member.visibility, gate.organizationID, gate.projectID.String(), upstreamToken, "", gate.toolSelection)
	...
	case member.tunneledServerID.Valid:
		...
		return func(ctx context.Context) (*proxy.Proxy, error) {
			p, berr := s.tunnelManager.buildProxy(ctx, affinity, logger, gate.projectID, gate.organizationID, &serverRow, upstreamToken, "", gate.toolSelection)
			if berr != nil {
