...
func TestServePublic_RBAC_ToolsList_DisabledRBACAllowsAll(t *testing.T) {
	...
	authzEngine := authz.NewEngine(ti.logger, ti.conn, authz.NewNoopChallengePublisher(ti.logger), authztest.RBACAlwaysDisabled, authztest.ChallengeLoggingAlwaysDisabled, workos.NewStubClient())
