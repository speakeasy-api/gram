...
func (s *Service) ServeConsentAction(w http.ResponseWriter, r *http.Request, endpoint *ResolvedMcpEndpoint) error {
	...
	// Delegation retry is an authorization action, not a downstream card action.
	// It uses only the already validated challenge identity and original issuer/client binding.
	if r.PostForm.Get("action") == "retry_delegation" {
