...
func TestConsentTemplateValidationStates(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "rejected", card: remoteSessionCard{Connected: true, Rejected: true, CanRefresh: true, ValidatedAt: "2026-09-08T12:00:00Z", ValidatedAgo: "just now", ValidationReason: "Rejected by linear", ConnectedAs: "grant-owner@example.com · Acme", TokenActive: true, TokenExpiresIn: "1 hour", RefreshExpiresIn: "29 days", AuthorizationExpiresIn: "365 days", CanValidate: true}, contains: []string{`data-validation="rejected" >Rejected by linear — reconnect to continue</span >`, `data-connect-link > Reconnect`, `aria-label="Disconnect example-issuer"`, `Authenticated as <span class="-mx-0.5 px-0.5 py-px box-decoration-clone" data-identity-text>grant-owner@example.com · Acme`, `data-refresh-link`}, notContains: []string{"Verified", "data-card-status", "data-validation-reason", "data-validate-link", "data-token-line", "Lapses in", "Access ends in", "Auto refresh on."}},
		{name: "inactive", card: remoteSessionCard{Connected: true, Inactive: true, CanRefresh: true, ValidatedAt: "2026-09-08T12:00:00Z", ValidatedAgo: "just now", ValidationReason: "Inactive at linear-idp", ConnectedAs: "grant-owner@example.com · Acme", TokenActive: true, TokenExpiresIn: "1 hour", RefreshExpiresIn: "29 days", AuthorizationExpiresIn: "365 days", CanValidate: true}, contains: []string{`data-validation="inactive" >Inactive at linear-idp — reconnect to continue</span >`, `data-connect-link > Reconnect`, `aria-label="Disconnect example-issuer"`, `Authenticated as <span class="-mx-0.5 px-0.5 py-px box-decoration-clone" data-identity-text>grant-owner@example.com · Acme`, `data-refresh-link`}, notContains: []string{"Verified", `data-validation="rejected"`, "data-card-status", "data-validate-link", "data-validation-reason", "data-token-line", "Lapses in", "Access ends in", "Auto refresh on."}},
		...
		{name: "expired keeps the account", card: remoteSessionCard{Expired: true, ConnectedAs: "grant-owner@example.com", TokenActive: true, AuthorizationExpiresIn: "365 days"}, contains: []string{"Expired", `Authenticated as <span class="-mx-0.5 px-0.5 py-px box-decoration-clone" data-identity-text>grant-owner@example.com`, `data-connect-link > Reconnect`}, notContains: []string{"data-token-line", "Access ends in"}},
