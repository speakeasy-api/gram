...
func TestConsentTemplateValidationStates(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "rejected", card: remoteSessionCard{Connected: true, Rejected: true, CanRefresh: true, ValidatedAt: "2026-09-08T12:00:00Z", ValidatedAgo: "just now", ValidationReason: "Rejected by linear", ConnectedAs: "grant-owner@example.com · Acme", TokenActive: true, TokenExpiresIn: "1 hour", RefreshExpiresIn: "29 days", AuthorizationExpiresIn: "365 days", CanValidate: true}, contains: []string{`data-validation="rejected" >Rejected by linear — reconnect to continue</span >`, `data-connect-link > Reconnect`, `aria-label="Disconnect example-issuer"`, `Authenticated as <span class="-mx-0.5 rounded-sm px-0.5 py-px box-decoration-clone" data-identity-text>grant-owner@example.com · Acme`, `data-refresh-link`}, notContains: []string{"Verified", "data-card-status", "data-validation-reason", "data-validate-link", "data-token-line", "Lapses in", "Access ends in", "Auto refresh on."}},
		{name: "inactive", card: remoteSessionCard{Connected: true, Inactive: true, CanRefresh: true, ValidatedAt: "2026-09-08T12:00:00Z", ValidatedAgo: "just now", ValidationReason: "Inactive at linear-idp", ConnectedAs: "grant-owner@example.com · Acme", TokenActive: true, TokenExpiresIn: "1 hour", RefreshExpiresIn: "29 days", AuthorizationExpiresIn: "365 days", CanValidate: true}, contains: []string{`data-validation="inactive" >Inactive at linear-idp — reconnect to continue</span >`, `data-connect-link > Reconnect`, `aria-label="Disconnect example-issuer"`, `Authenticated as <span class="-mx-0.5 rounded-sm px-0.5 py-px box-decoration-clone" data-identity-text>grant-owner@example.com · Acme`, `data-refresh-link`}, notContains: []string{"Verified", `data-validation="rejected"`, "data-card-status", "data-validate-link", "data-validation-reason", "data-token-line", "Lapses in", "Access ends in", "Auto refresh on."}},
		...
		{name: "expired keeps the account", card: remoteSessionCard{Expired: true, ConnectedAs: "grant-owner@example.com", TokenActive: true, AuthorizationExpiresIn: "365 days"}, contains: []string{"Expired", `Authenticated as <span class="-mx-0.5 rounded-sm px-0.5 py-px box-decoration-clone" data-identity-text>grant-owner@example.com`, `data-connect-link > Reconnect`}, notContains: []string{"data-token-line", "Access ends in"}},
		...
		{name: "account context chips", card: remoteSessionCard{Connected: true, Verified: true, ConnectedAs: "Grant Owner · grant-owner@example.com", AccountChips: []string{"Acme Docs", "Acme Team", "octocat"}, CanValidate: true}, contains: []string{`Authenticated as <span class="-mx-0.5 rounded-sm px-0.5 py-px box-decoration-clone" data-identity-text>Grant Owner · grant-owner@example.com</span></span >`, `data-account-context ><span class="break-words">Acme Docs</span><span class="break-words">&nbsp;·&nbsp;Acme Team</span><span class="break-words">&nbsp;·&nbsp;octocat</span></span >`}, notContains: []string{"whitespace-nowrap", `<span class="break-words"> ·`, `<span class="break-words">·`}},
		{name: "identity caveat", card: remoteSessionCard{Connected: true, Verified: true, ConnectedAs: "Grant Owner · grant-owner@example.com", IdentityCaveat: "Shown from userinfo. The ID token was rejected.", CanValidate: true}, contains: []string{`Authenticated as <span class="-mx-0.5 rounded-sm px-0.5 py-px box-decoration-clone" data-identity-text>Grant Owner · grant-owner@example.com</span>`, `has-[[data-caveat-trigger]:hover]:[&_[data-identity-text]]:bg-destructive/20`, `data-caveat-trigger`, `data-identity-caveat`, `role="tooltip"`, `group-hover/caveat:block group-focus-within/caveat:block" >Shown from userinfo. The ID token was rejected.</span`}, notContains: []string{"data-account-context"}},
