...
func TestGrantRoutesToUpstream(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "remote query trailing slash trimmed as at mint", resource: upstream + "?tenant=a/", upstream: upstream + "?tenant=a", tunneled: false, want: true},
		{name: "remote encoded slash stays distinct", resource: upstream + "%2F", upstream: upstream, tunneled: false, want: false},
