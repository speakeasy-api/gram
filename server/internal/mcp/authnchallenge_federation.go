...
func (s *Service) prepareFederatedLogin(w http.ResponseWriter, r *http.Request, endpoint *ResolvedMcpEndpoint, state *AuthnChallengeState) (*url.URL, error) {
	return s.prepareBoundFederatedLogin(w, r, endpoint, state, "")
}

// retryHuman is supplied only by the validated, consumed consent retry action.
func (s *Service) prepareBoundFederatedLogin(w http.ResponseWriter, r *http.Request, endpoint *ResolvedMcpEndpoint, state *AuthnChallengeState, retryHuman string) (*url.URL, error) {
	ctx := r.Context()
