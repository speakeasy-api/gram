...
func TestDeploymentsService_ListDeployments_Success(t *testing.T) {
	...
	require.GreaterOrEqual(t, foundDeployment.OpenapiAssetCount, int64(1), "asset count should be at least 1")
	require.GreaterOrEqual(t, foundDeployment.OpenapiToolCount, int64(0), "tool count should be at least 0")
}
	...
	for _, item := range result.Items {
		for _, dep := range deployments {
			if item.ID == dep.Deployment.ID {
				...
				require.GreaterOrEqual(t, item.OpenapiAssetCount, int64(1), "asset count should be at least 1 for deployment %s", dep.Deployment.ID)
				require.GreaterOrEqual(t, item.OpenapiToolCount, int64(0), "tool count should be at least 0 for deployment %s", dep.Deployment.ID)
			}
		}
	}
