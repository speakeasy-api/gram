...
func (q *Queries) ListActorSpendForRules(ctx context.Context, projectIDs []string, timeStart, timeEnd int64) ([]ActorSpendRow, error) {
	...
	sb := sq.Select("user_email").
		...
		Where("user_email != ''"). //nolint:glint // norawuseremailfilter: fold-neutral emptiness check; spend enforcement identity semantics pending a product decision (DNO-857 tail)
		GroupBy("user_email")      //nolint:glint // norawuseremailfilter: actor rows keyed by literal email; casing collapsed in Go after read - enforcement fold pending decision (DNO-857 tail)
...
func (q *Queries) ListActorWindowSpendForRulesByEmail(
	...
	sb := sq.Select("user_email").
		...
		Where("time_bucket >= toStartOfMinute(fromUnixTimestamp64Nano(?))", timeStart).
		Where("time_bucket <= toStartOfMinute(fromUnixTimestamp64Nano(?))", timeEnd).
		Where("lowerUTF8(user_email) = ?", email). //nolint:glint // norawuseremailfilter: spend ENFORCEMENT matches the literal actor email deliberately - folding changes blocking semantics and needs its own decision (DNO-857 tail)
		GroupBy("user_email")                      //nolint:glint // norawuseremailfilter: actor rows keyed by literal email; casing collapsed in Go after read - enforcement fold pending decision (DNO-857 tail)
