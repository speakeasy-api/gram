...
func TestEnqueuePersistsDeterministicReadings(t *testing.T) {
	...
	require.Equal(t, string(definition.ID), stored.GetMeterId())
	require.Equal(t, definition.Version, stored.GetMeterVersion())
	require.Equal(t, string(definition.Unit), stored.GetUnit())
	require.Equal(t, string(definition.MeasurementMethod), stored.GetMeasurementMethod())
