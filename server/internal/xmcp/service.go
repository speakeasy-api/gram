...
func Attach(mux goahttp.Muxer, service *Service, metadataService *mcpmetadata.Service) {
	...
	o11y.AttachHandler(mux, http.MethodPost, "/x/mcp/{mcpSlug}/connect/frozen-v2", oops.ErrHandle(service.logger, service.handleOAuthConsent).ServeHTTP)
...
func AttachAuthenticationHost(host *mcp.AuthenticationHost, service *Service) {
	...
	handle(http.MethodPost, "/x/mcp/{mcpSlug}/connect/frozen-v2", service.handleOAuthConsent)
...
// handleOAuthConsentAction adapts the chi
// /x/mcp/{mcpSlug}/connect/remote-session route to
// mcp.Service.ServeConsentAction.
func (s *Service) handleOAuthConsentAction(w http.ResponseWriter, r *http.Request) error {
	endpoint, err := s.resolveOAuthEndpoint(r)
	if err != nil {
		return err
	}
