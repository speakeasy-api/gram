...
func TestToolsCallClickHouseLogInterceptor_NoAuthContextSkips(t *testing.T) {
	...
	logger := testenv.NewLogger(t)
	// NewStub is hard-wired disabled so any Log call would be a no-op; the
	// test only asserts the interceptor short-circuits without panic.
	interceptor := xmcp.NewToolsCallClickHouseLogInterceptor(telemetry.NewStub(logger), "server-id", logger)
