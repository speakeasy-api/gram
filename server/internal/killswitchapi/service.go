...
func (s *Service) List(ctx context.Context, payload *gen.ListPayload) (*gen.KillswitchListResult, error) {
	...
	if result.NextCursor != nil {
		encoded, encodeErr := encodeCursor(listCursor{Version: 2, CreatedAt: result.NextCursor.CreatedAt.Format(time.RFC3339Nano), ID: string(result.NextCursor.ID), Filter: filter})
		if encodeErr != nil {
...
func decodeCursor(value, filter string) (listCursor, error) {
	...
	if err := json.Unmarshal(data, &cursor); err != nil || cursor.Version != 2 || cursor.Filter != filter || cursor.CreatedAt == "" || cursor.ID == "" {
		return listCursor{}, errors.New("cursor is invalid")
	}
	return cursor, nil
}
