...
Cursor setup is organization-level. Anthropic and Codex compliance imports also require `external_organization_id`: Anthropic uses the Claude organization ID, while Codex cost import uses the OpenAI API organization ID (`org-*`). Today each integration attaches to the organization's first-created project automatically. A new config's time-kind schedules start at the epoch watermark, so the first poll covers the initial lookback (24 hours); replacing a key resets the provider-named schedule's watermark to one lookback ago. Either way `next_poll_after` is in the past, so the config is due immediately.
...
Credentials are verified before they are stored. Any upsert that would start polling on an unanswered credential — a new or changed API key or external scope, or re-enabling an integration — probes every sync schedule the provider runs with the cheapest read that schedule already makes (the upper-bound or first-page fetch, narrowed to one record). A 401/403/404 is the provider refusing the configuration: when every schedule is refused the save fails with the provider's own message and nothing is written, so no config generation exists and the poll loop never starts. Providers entitle their feeds separately (Anthropic's Compliance API and Admin Analytics reports are distinct grants), so a partial refusal still saves; the refused schedules are recorded and paused immediately, reaching the same state repeated poll rejections would after `AutoPauseAfterRejectedPolls`, without the polls. Anything else a probe sees — an outage, a timeout, a 429 — says nothing about the credentials and never blocks a save. Probes are bounded by `credentialProbeTimeout` and skipped for saves that change no credential (the enabled toggle off, a billing mode edit), which reuse an answer the provider already gave.

Provider failures do not fail the poll activity. When a poll ends in a provider rejection or a provider outage and that outcome is recorded on the schedule row, `PollAIData` returns success to Temporal: the failure is durable in Postgres, shown in the dashboard, and the schedule owns the retry cadence (backoff, plus auto-pause for rejections), so re-running the activity cannot change the provider's answer. Unrecorded failures and failures that are Gram's own still fail the activity, which keeps activity-failure alerting pointed at unexpected errors instead of at one misconfigured integration's schedules.
...
Cursor windows are non-overlapping on success. Cursor includes both request bounds, so each request starts at `poll_watermark_at + 1ms` and ends at the activity's `endTime`. On success, `poll_watermark_at` advances to `endTime`, `next_poll_after` advances to `endTime + 1h`, and failure metadata is cleared. On the final failed `SyncAIIntegrationUsage` activity attempt, `poll_watermark_at` is left unchanged, the final error is recorded, and `next_poll_after` is rescheduled with failure backoff: the schedule's base interval doubled per consecutive final failure, bounded by a ceiling of a few hours, anchored on the later of the attempt's `endTime` and the recording time. A success resets the streak and the base cadence. A child workflow can wait behind the dedicated activity task queue; while it is still open, later coordinators skip the same provider/org via the stable workflow ID.
...
ClickHouse and Postgres are not updated atomically. If ClickHouse insert succeeds but the success sync-state update fails before a retry advances `poll_watermark_at`, the same window can be re-inserted. Rows carry provider-specific event hashes — `cursor.event_hash`, `claude_chat.event_hash`, `codex.compliance.event_hash` — but ClickHouse enforces no uniqueness, so a hash only prevents double counting where a writer checks it. The Codex compliance COSTS importer does: it drops events already ingested for the project before writing (`telemetry.LogBulkDeduped`), which makes a re-inserted window a no-op for that feed. The Cursor and Claude Chat importers do **not**, so re-inserting one of their windows still double counts, and consumers needing exact-once sums from those two must dedupe by `(gram_project_id, <provider event hash>)` themselves. If the final activity attempt fails before inserting, the failure is recorded and only `next_poll_after` advances so that provider/org does not block later work.
...
Codex cost data comes from OpenAI's Compliance Logs Platform `COSTS` files. The poller lists `GET /v1/compliance/{organizations|workspaces}/{id}/logs?event_type=COSTS&after=<watermark>`, downloads each returned JSONL file, verifies `file_sha256`, and writes one telemetry row per cost event. The feed mixes product surfaces: only events with `payload.product == "codex"` land under `codex:usage:metrics` (the URN the codex.cost.usd stream and ClickHouse agent-usage predicates match on); ChatGPT, Work, and unknown surfaces land under `chatgpt:usage:metrics`, which the summary MVs admit as agent usage; Work rows carry the `chatgpt-work` hook source (ChatGPT and unknown surfaces carry `chatgpt`) so the per-product split survives summarization — hook_source is a summary GROUP BY dimension while `codex.compliance.product` (the exact raw surface, still stamped on every row) is not, and the summaries outlive the raw-row TTL. Codex rows meter cost only: their token counts are moved off the `gen_ai.usage.*` keys at ingest because the agent-usage predicates would sum them on top of the Codex OTEL stream (the token source of truth), double counting metering for orgs running both feeds. The raw counts are preserved under `codex.compliance.{input,cached_input,output,total}_tokens`, which nothing sums — deliberately, because the COSTS feed also covers Codex surfaces the device OTEL stream never sees (cloud-delegated tasks, GitHub code review; the surface is in `codex.compliance.surface`/`client`), so a later pass can partition metering by surface and promote the counts for OTEL-invisible usage instead of re-deriving them. Non-Codex rows keep `gen_ai.usage.*` token counts since the compliance feed is ChatGPT/Work's only usage source. Each row stores `codex.compliance.event_hash`, derived from the upstream `event_id`, and the importer enforces it: the feed delivers the same `event_id` in more than one log file, so before writing a page it drops events already ingested for the project. An event with no `timestamp` falls back to its payload `day`/`hour` bucket rather than the log file's end time, so that repeated deliveries of one event agree on their event time and the dedupe lookup can find the copy already written. Upstream `CREDITS` billing values are converted to `gen_ai.usage.cost` dollars at `$0.04` per credit while retaining the raw `codex.compliance.cost_unit`. The poll watermark advances to the API's `last_end_time`; empty pages leave the watermark unchanged but still schedule the next attempt from the current poll completion time. The first Codex poll looks back 30 days (`codexComplianceInitialLookback`) rather than the shared 24-hour default: COSTS files are hourly per-user aggregates cheap to backfill, and an org's Codex history usually predates the key being configured.

ChatGPT conversation content comes from the same Compliance Logs Platform, but under a different provider (`chatgpt_compliance`) because the scope differs: `CONVERSATION_MESSAGE` files are served per ChatGPT workspace (`GET /v1/compliance/workspaces/{workspace-uuid}/logs`), not per API organization, so the config's `external_organization_id` holds a workspace UUID. The importer reuses the COSTS poller machinery (time-window watermark, `file_sha256` verification, 30-day initial lookback) but sinks into the external chat tables like the Anthropic compliance import: one `UpsertExternalChat` per conversation (title, external id, resolved user) and one `chat_messages` row per user/assistant message event, deduplicated by `(chat_id, external_message_id)` so replayed windows are idempotent. Messages carry source `chatgpt` and the feed's `author.client_type` (e.g. `desktop_web`) in `user_agent`. The old stateful `/workspaces/{id}/conversations` endpoint is deprecated upstream (410) — the log-file feed is the only supported conversation surface.
