...
Codex cost data comes from OpenAI's Compliance Logs Platform `COSTS` files. The poller lists `GET /v1/compliance/{organizations|workspaces}/{id}/logs?event_type=COSTS&after=<watermark>`, downloads each returned JSONL file, verifies `file_sha256`, and writes one `codex:usage:metrics` row per cost event. Each row stores `codex.compliance.event_hash`, derived from the upstream `event_id`, as the current provider-specific dedup key. Upstream `CREDITS` billing values are converted to `gen_ai.usage.cost` dollars at `$0.04` per credit while retaining the raw `codex.compliance.cost_unit`. The poll watermark advances to the API's `last_end_time`; empty pages leave the watermark unchanged but still schedule the next attempt from the current poll completion time.
