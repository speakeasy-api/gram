...
func newTestPackagesService(t *testing.T) (context.Context, *testInstance) {
	t.Helper()
	...
	svc := packages.NewService(logger, tracerProvider, conn, sessionManager, access.NewManager(logger, conn, accesstest.AlwaysEnabledFeatureChecker{}, workos.NewStubClient(), cache.NoopCache))
...
func withExactAccessGrants(t *testing.T, ctx context.Context, conn *pgxpool.Pool, grants ...access.Grant) context.Context {
	...
	principal := urn.NewPrincipal(urn.PrincipalTypeRole, "packages-rbac-grants-"+uuid.NewString())
