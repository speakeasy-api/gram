...
func newTestService(t *testing.T) (context.Context, *testInstance) {
	...
	svc := mcpslugs.NewService(logger, tracerProvider, conn, sessionManager, authz.NewEngine(logger, conn, authztest.RBACAlwaysEnabled, workos.NewStubClient(), cache.NoopCache))
...
func withExactAuthzGrants(t *testing.T, ctx context.Context, conn *pgxpool.Pool, grants ...authz.Grant) context.Context {
	...
	principal := urn.NewPrincipal(urn.PrincipalTypeRole, "mcpslugs-rbac-grants-"+uuid.NewString())
...
// seedMcpFrontend creates a remote_mcp_server + mcp_frontend row directly
// through the generated repos so slug tests have a valid mcp_frontend_id FK
// without depending on the mcpfrontends service package.
func seedMcpFrontend(t *testing.T, ctx context.Context, conn *pgxpool.Pool, projectID uuid.UUID) uuid.UUID {
	t.Helper()
