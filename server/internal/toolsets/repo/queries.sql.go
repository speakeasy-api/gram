...
RETURNING id, organization_id, project_id, name, slug, description, default_environment_slug, mcp_slug, mcp_is_public, mcp_enabled, tool_selection_mode, custom_domain_id, external_oauth_server_id, oauth_proxy_server_id, parent_toolset_id, editing_mode, current_release_id, predecessor_id, version, history_id, created_at, updated_at, deleted_at, deleted
`
...
// ==============================================================================
func (q *Queries) GetStagingToolset(ctx context.Context, parentToolsetID uuid.NullUUID) (Toolset, error) {
	row := q.db.QueryRow(ctx, getStagingToolset, parentToolsetID)
	var i Toolset
	err := row.Scan(
		&i.ID,
		&i.OrganizationID,
...
const getToolsetVersionByID = `-- name: GetToolsetVersionByID :one
SELECT id, toolset_id, version, tool_urns, resource_urns, predecessor_id, created_at, updated_at, deleted_at, deleted
FROM toolset_versions
WHERE id = $1
  AND deleted IS FALSE
...
func (q *Queries) GetToolsetVersionByID(ctx context.Context, id uuid.UUID) (ToolsetVersion, error) {
	row := q.db.QueryRow(ctx, getToolsetVersionByID, id)
	var i ToolsetVersion
	err := row.Scan(
		&i.ID,
		&i.ToolsetID,
...
const getToolsetWithStagingVersion = `-- name: GetToolsetWithStagingVersion :one
SELECT
  t.id, t.organization_id, t.project_id, t.name, t.slug, t.description, t.default_environment_slug, t.mcp_slug, t.mcp_is_public, t.mcp_enabled, t.tool_selection_mode, t.custom_domain_id, t.external_oauth_server_id, t.oauth_proxy_server_id, t.parent_toolset_id, t.editing_mode, t.current_release_id, t.predecessor_id, t.version, t.history_id, t.created_at, t.updated_at, t.deleted_at, t.deleted,
  s.id as staging_id,
...
type GetToolsetWithStagingVersionParams struct {
	Slug      string
	ProjectID uuid.UUID
}

type GetToolsetWithStagingVersionRow struct {
	ID                     uuid.UUID
	OrganizationID         string
	ProjectID              uuid.UUID
	Name                   string
	...
	DeletedAt              pgtype.Timestamptz
	Deleted                bool
	StagingID              uuid.NullUUID
	StagingSlug            pgtype.Text
}

func (q *Queries) GetToolsetWithStagingVersion(ctx context.Context, arg GetToolsetWithStagingVersionParams) (GetToolsetWithStagingVersionRow, error) {
	row := q.db.QueryRow(ctx, getToolsetWithStagingVersion, arg.Slug, arg.ProjectID)
	var i GetToolsetWithStagingVersionRow
	err := row.Scan(
		&i.ID,
		&i.OrganizationID,
		&i.ProjectID,
...
const updateToolsetCurrentRelease = `-- name: UpdateToolsetCurrentRelease :one
UPDATE toolsets
SET
...
type UpdateToolsetCurrentReleaseParams struct {
	CurrentReleaseID uuid.NullUUID
	ID               uuid.UUID
}

func (q *Queries) UpdateToolsetCurrentRelease(ctx context.Context, arg UpdateToolsetCurrentReleaseParams) (Toolset, error) {
	row := q.db.QueryRow(ctx, updateToolsetCurrentRelease, arg.CurrentReleaseID, arg.ID)
	var i Toolset
	err := row.Scan(
		&i.ID,
		&i.OrganizationID,
...
const updateToolsetEditingMode = `-- name: UpdateToolsetEditingMode :one
UPDATE toolsets
SET
    editing_mode = $1
  , updated_at = clock_timestamp()
...
type UpdateToolsetEditingModeParams struct {
	EditingMode string
	Slug        string
	ProjectID   uuid.UUID
}

func (q *Queries) UpdateToolsetEditingMode(ctx context.Context, arg UpdateToolsetEditingModeParams) (Toolset, error) {
	row := q.db.QueryRow(ctx, updateToolsetEditingMode, arg.EditingMode, arg.Slug, arg.ProjectID)
	var i Toolset
	err := row.Scan(
		&i.ID,
		&i.OrganizationID,
