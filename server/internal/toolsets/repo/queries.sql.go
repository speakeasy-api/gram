...
type GetToolsetByIDParams struct {
	ID        uuid.UUID
	ProjectID uuid.UUID
}

func (q *Queries) GetToolsetByID(ctx context.Context, arg GetToolsetByIDParams) (Toolset, error) {
	row := q.db.QueryRow(ctx, getToolsetByID, arg.ID, arg.ProjectID)
	var i Toolset
	err := row.Scan(
		&i.ID,
		&i.OrganizationID,
