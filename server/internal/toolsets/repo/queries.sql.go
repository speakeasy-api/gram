...
RETURNING id, organization_id, project_id, name, slug, description, default_environment_slug, mcp_slug, mcp_is_public, mcp_enabled, tool_selection_mode, auto_sync_sources, custom_domain_id, external_oauth_server_id, oauth_proxy_server_id, user_session_issuer_id, created_at, updated_at, deleted_at, deleted
`
...
const listToolsetsByAutoSyncSource = `-- name: ListToolsetsByAutoSyncSource :many
SELECT id, organization_id, project_id, name, slug, description, default_environment_slug, mcp_slug, mcp_is_public, mcp_enabled, tool_selection_mode, auto_sync_sources, custom_domain_id, external_oauth_server_id, oauth_proxy_server_id, user_session_issuer_id, created_at, updated_at, deleted_at, deleted
FROM toolsets
WHERE project_id = $1
  AND deleted IS FALSE
  AND auto_sync_sources && $2::TEXT[]
ORDER BY id
`

type ListToolsetsByAutoSyncSourceParams struct {
	ProjectID uuid.UUID
	Sources   []string
}
...
func (q *Queries) ListToolsetsByAutoSyncSource(ctx context.Context, arg ListToolsetsByAutoSyncSourceParams) ([]Toolset, error) {
	rows, err := q.db.Query(ctx, listToolsetsByAutoSyncSource, arg.ProjectID, arg.Sources)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
SET
  ...
  , auto_sync_sources = COALESCE($9::TEXT[], auto_sync_sources)
  , updated_at = clock_timestamp()
