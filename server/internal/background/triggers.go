...
func TriggerWakeWorkflow(ctx workflow.Context, input bgtriggers.ScheduleWorkflowInput) error {
	var a *Activities
	...
	if result.Task != nil {
		if err := workflow.ExecuteActivity(ctx, a.DispatchTrigger, activities.DispatchTriggerInput(result)).Get(ctx, nil); err != nil {
			return err
		}
	}

	return workflow.ExecuteActivity(ctx, a.MarkTriggerFired, activities.MarkTriggerFiredInput{
		TriggerInstanceID: input.TriggerInstanceID,
	}).Get(ctx, nil)
}
