// Package outbox_svix_relay drains the per-organization Svix relay outbox.
//
// Activities here back the RelayOutboxSvixWorkflow defined in the parent
...
type FetchEventsResult struct {
	Events  []*OutboxSvixEvent
	HasMore bool
}

type OutboxSvixEvent struct {
	OutboxID        int64
...
func New(logger *slog.Logger, tracerProvider trace.TracerProvider, db *pgxpool.Pool, svixClient *svix.Svix) *Relay {
	return &Relay{
		logger:      logger,
		tracer:      tracerProvider.Tracer("github.com/speakeasy-api/gram/server/internal/background/activities/outbox_svix_relay"),
		db:          db,
...
func (r *Relay) FetchEvents(ctx context.Context, args FetchEventArgs) (FetchEventsResult, error) {
	...
	results := make([]*OutboxSvixEvent, 0, len(rows))
	for _, row := range rows {
		results = append(results, &OutboxSvixEvent{
			OutboxID:        row.ID,
...
func (r *Relay) FilterNoopEvents(ctx context.Context, events []*OutboxSvixEvent) ([]*OutboxSvixEvent, error) {
	...
	stmt := psql.Insert("outbox_svix_relays").
		Columns("outbox_id", "processed_at", "noop", "attempts", "last_error")

	total := 0
	validEvents := make([]*OutboxSvixEvent, 0, len(events))
...
func (r *Relay) RelayEvents(ctx context.Context, events []*OutboxSvixEvent) error {
	trace.SpanFromContext(ctx).SetAttributes(
		attr.OutboxBatchSize(len(events)),
	)
