...
func presidioClient(t *testing.T) *risk_analysis.PresidioClient {
	...
	return risk_analysis.NewPresidioClient(url, &http.Client{Timeout: 30 * time.Second}) //nolint:forbidigo // Test-only HTTP client.
}
...
func TestCombinedScanners_BothSourcesAppear(t *testing.T) {
	...
	presidioResults, err := client.AnalyzeBatch(t.Context(), []string{content})
	require.NoError(t, err)
...
func TestPresidio_StressBatch(t *testing.T) {
	...
	start := time.Now()
	results, err := client.AnalyzeBatch(t.Context(), messages)
