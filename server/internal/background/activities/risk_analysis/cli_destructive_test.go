...
func TestMatchCLIDestructiveString_CuratedPatterns(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "destructive.shell.rm-rf", input: "rm -rf /tmp/work", fullName: "destructive.shell.rm-rf"},
		{name: "destructive.shell.rm-rf-glob", input: "sudo rm -rf *", fullName: "destructive.shell.rm-rf"}, // rm-rf precedes sudo in declaration order
		...
		{name: "destructive.shell.fork-bomb", input: ":(){ :|: & };:", fullName: "destructive.shell.fork-bomb"},
		{name: "destructive.shell.chmod-recursive", input: "chmod -R 777 /etc", fullName: "destructive.shell.chmod-recursive"},
		{name: "destructive.shell.chown-recursive", input: "chown --recursive root:root /home", fullName: "destructive.shell.chown-recursive"},
		{name: "destructive.shell.sudo", input: "sudo cat /etc/shadow", fullName: "destructive.shell.sudo"},
		{name: "destructive.git.push-force", input: "git push --force origin main", fullName: "destructive.git.push-force"},
		{name: "destructive.git.push-force-shorthand", input: "git push -f origin main", fullName: "destructive.git.push-force"},
		{name: "destructive.git.push-force-with-lease", input: "git push --force-with-lease origin main", fullName: "destructive.git.push-force"},
		{name: "destructive.git.reset-hard", input: "git reset --hard HEAD~3", fullName: "destructive.git.reset-hard"},
		{name: "destructive.git.clean-force", input: "git clean -fd", fullName: "destructive.git.clean-force"},
		{name: "destructive.git.branch-delete-force", input: "git branch -D feature/x", fullName: "destructive.git.branch-delete-force"},
		...
		{name: "destructive.cloud.aws-ec2-terminate", input: "aws ec2 terminate-instances --instance-ids i-1234", fullName: "destructive.cloud.aws-ec2-terminate"},
		{name: "destructive.cloud.aws-s3-rb", input: "aws s3 rb s3://my-bucket --force", fullName: "destructive.cloud.aws-s3-rb"},
		{name: "destructive.cloud.gcloud-projects-delete", input: "gcloud projects delete my-project", fullName: "destructive.cloud.gcloud-projects-delete"},
		{name: "destructive.cloud.kubectl-delete-namespace", input: "kubectl delete ns production", fullName: "destructive.cloud.kubectl-delete-namespace"},
		{name: "destructive.cloud.kubectl-delete-deployment", input: "kubectl delete deployment api", fullName: "destructive.cloud.kubectl-delete-workload"},
	}
...
func TestMatchCLIDestructiveString_DeleteWhereGuard(t *testing.T) {
	...
	if assert.True(t, ok, "DELETE without WHERE must flag") {
		assert.Equal(t, "destructive.database.delete-without-where", unguarded.FullName())
	}
}
...
func TestScanForCLIDestructive_NestedStructures(t *testing.T) {
	...
	t.Run("map value", func(t *testing.T) {
		...
		if assert.True(t, ok) {
			assert.Equal(t, "destructive.shell.rm-rf", matched.FullName())
		}
	})
	...
	t.Run("slice element", func(t *testing.T) {
		...
		// matches when assembled. So this doesn't trip destructive.git.push-force, which
