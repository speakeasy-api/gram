...
func TestSetLogo_CreatesAuditLog(t *testing.T) {
	...
	require.True(t, ok)
	ctx = withAccessGrants(t, ctx, ti.conn, access.Grant{Scope: access.ScopeBuildWrite, Selector: access.ForResource(authCtx.ProjectID.String())})
...
func TestSetLogo_DatabaseErrorProjectNotFound(t *testing.T) {
	...
	ctx = withAccessGrants(t, ctx, ti.conn, access.Grant{Scope: access.ScopeBuildWrite, Selector: access.ForResource(nonExistentProjectID.String())})
