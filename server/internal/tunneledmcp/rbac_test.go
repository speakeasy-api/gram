...
func TestListAndGetServersAllowProjectScopedMCPReadGrant(t *testing.T) {
	...
	require.Len(t, listResult.TunneledMcpServers, 1)
	require.Equal(t, server.ID.String(), listResult.TunneledMcpServers[0].ID)
...
func seedTunneledMcpServer(t *testing.T, ctx context.Context, conn *pgxpool.Pool, projectID uuid.UUID) repo.TunneledMcpServer {
	t.Helper()
