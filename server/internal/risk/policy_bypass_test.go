...
func TestDeleteRiskPolicy_HidesBypassRequests(t *testing.T) {
	t.Parallel()
	ctx, ti := newTestRiskService(t)
	...
	policy, err := ti.service.CreateRiskPolicy(ctx, &gen.CreateRiskPolicyPayload{
		Name: new("Policy With Bypass Request"),
	})
	require.NoError(t, err)

	token := riskPolicyBypassRequestToken(t, authCtx, policy.ID, "https://mcp.example.com/server")
	request, err := ti.service.CreateRiskPolicyBypassRequest(ctx, &gen.CreateRiskPolicyBypassRequestPayload{
		RequestToken: token,
	})
	require.NoError(t, err)
	...
	beforeDelete, err := ti.service.ListRiskPolicyBypassRequests(ctx, &gen.ListRiskPolicyBypassRequestsPayload{})
	require.NoError(t, err)
	require.Len(t, beforeDelete.Requests, 1)

	err = ti.service.DeleteRiskPolicy(ctx, &gen.DeleteRiskPolicyPayload{ID: policy.ID})
	require.NoError(t, err)

	afterDelete, err := ti.service.ListRiskPolicyBypassRequests(ctx, &gen.ListRiskPolicyBypassRequestsPayload{})
	require.NoError(t, err)
	require.Empty(t, afterDelete.Requests)

	_, err = ti.service.GetRiskPolicy(ctx, &gen.GetRiskPolicyPayload{ID: policy.ID})
	require.Error(t, err)
}
