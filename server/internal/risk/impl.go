...
func (s *Service) UpdateRiskPolicy(ctx context.Context, payload *gen.UpdateRiskPolicyPayload) (*types.RiskPolicy, error) {
	...
	var audiencePrincipalURNs []string
	if audienceType == riskPolicyAudienceTargeted {
		if payload.AudiencePrincipalUrns != nil {
...
func (s *Service) DeleteRiskPolicy(ctx context.Context, payload *gen.DeleteRiskPolicyPayload) error {
	...
	if err := repo.New(dbtx).DeleteRiskPolicyBypassRequestsByPolicy(ctx, repo.DeleteRiskPolicyBypassRequestsByPolicyParams{
		RiskPolicyID: id,
		ProjectID:    *authCtx.ProjectID,
	}); err != nil {
	...
	if err := syncRiskPolicyAudienceGrants(ctx, dbtx, authCtx.ActiveOrganizationID, id.String(), riskPolicyAudienceEveryone, nil); err != nil {
...
func policyRowSnapshot(row repo.RiskPolicy) *types.RiskPolicy {
	return policyRowSnapshotWithAudience(row, nil)
}
