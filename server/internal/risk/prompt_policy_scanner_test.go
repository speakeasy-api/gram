...
func TestScanner_PromptBasedPolicyBlocksToolRequest(t *testing.T) {
	...
	scanner, err := risk.NewScanner(testenv.NewLogger(t), testenv.NewTracerProvider(t), testenv.NewMeterProvider(t), ti.conn, newTestCustomRuleAnalyzer(t, ti.conn), nil, nil, judge.Evaluate, promptPoliciesFlag(ctx), testCELEngine(t))
	require.NoError(t, err)
...
func TestScanner_PromptBasedPolicyDisabledWhenFlagOff(t *testing.T) {
	...
	scanner, err := risk.NewScanner(testenv.NewLogger(t), testenv.NewTracerProvider(t), testenv.NewMeterProvider(t), ti.conn, newTestCustomRuleAnalyzer(t, ti.conn), nil, nil, judge.Evaluate, &feature.InMemory{}, testCELEngine(t))
	require.NoError(t, err)
