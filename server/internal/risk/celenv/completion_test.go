...
func TestComplete_NonFieldReceiverOffersNothing(t *testing.T) {
	t.Parallel()
	eng, err := New()
	require.NoError(t, err)

	// kind is a string, not a field — member completion must not offer matchers.
	c := eng.Complete("kind.")
	require.Equal(t, "member", c.Context)
	require.Empty(t, c.Items)
}

func TestComplete_BindDoesNotLeakPastComprehension(t *testing.T) {
	t.Parallel()
	eng, err := New()
	require.NoError(t, err)

	// t is bound only inside exists(...); once it closes, t must not be offered.
	c := eng.Complete(`tool_calls.exists(t, t.name.present()) && `)
	require.Equal(t, "name", c.Context)
	require.NotContains(t, labels(c.Items), "t")
}
