...
func (s *Service) SetSpendCap(ctx context.Context, payload *gen.SetSpendCapPayload) (*gen.SpendCap, error) {
	...
	switch {
	case err == nil:
		return nil, oops.E(oops.CodeConflict, nil, "inference spend caps cannot be changed during an active trial")
	case !errors.Is(err, pgx.ErrNoRows):
		return nil, oops.E(oops.CodeUnexpected, err, "check active trial before setting inference spend cap").LogError(ctx, s.logger)
	}
	...
	case "trialing":
		return nil, oops.E(oops.CodeConflict, nil, "inference spend caps cannot be changed before pay as you go billing starts")
	...
	default:
		return nil, oops.E(oops.CodeConflict, nil, "inference spend caps cannot be changed without an active pay as you go subscription")
	}
	...
	if err := s.keyRefresher.SetOpenRouterSpendCap(
		...
		return nil, oops.E(oops.CodeUnexpected, err, "set inference spend cap").LogError(ctx, s.logger)
	}
