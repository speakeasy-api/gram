...
func TestCancelStripeSubscriptionPropagatesStripeReadFailure(t *testing.T) {
	t.Parallel()

	ti := newStripeCheckoutTestInstance(t)
	configureStripeSubscription(t, ti, false)
	ti.stripe.subscriptionReadError = errors.New("Stripe unavailable")

	_, err := ti.service.CancelStripeSubscription(ti.adminContext(t), &gen.CancelStripeSubscriptionPayload{})
	require.Error(t, err)
	requireOopsCode(t, err, oops.CodeUnavailable)
	// The state the change is validated against never arrived, so nothing is
	// sent to Stripe on the strength of it.
	require.Empty(t, ti.stripe.subscriptionUpdates)
}

// The read succeeds and the write is what fails — the branch a single shared
// error field can never reach, because the read short-circuits first.
func TestStripeSubscriptionLifecyclePropagatesStripeWriteFailure(t *testing.T) {
	t.Parallel()

	for _, operation := range []struct {
		name string
		// The subscription state the operation starts from, and the one it
		// tries to write.
		startsCanceling bool
		writes          bool
		action          audit.Action
		run             func(*testing.T, *stripeCheckoutTestInstance) error
	}{
		{
			name:            "cancel",
			startsCanceling: false,
			writes:          true,
			action:          audit.ActionBillingMetadataCancelStripeSubscription,
			run: func(t *testing.T, ti *stripeCheckoutTestInstance) error {
				t.Helper()
				_, err := ti.service.CancelStripeSubscription(ti.adminContext(t), &gen.CancelStripeSubscriptionPayload{})
				return err
			},
		...
		{
			name:            "resume",
			startsCanceling: true,
			writes:          false,
			action:          audit.ActionBillingMetadataResumeStripeSubscription,
			run: func(t *testing.T, ti *stripeCheckoutTestInstance) error {
				t.Helper()
				_, err := ti.service.ResumeStripeSubscription(ti.adminContext(t), &gen.ResumeStripeSubscriptionPayload{})
				return err
		...
		t.Run(operation.name, func(t *testing.T) {
			...
			ti := newStripeCheckoutTestInstance(t)
			configureStripeSubscription(t, ti, operation.startsCanceling)
			ti.stripe.subscriptionUpdateError = errors.New("Stripe unavailable")
			baseline, err := audittest.AuditLogCountByAction(t.Context(), ti.db, operation.action)
			require.NoError(t, err)

			err = operation.run(t, ti)
			require.Error(t, err)
			requireOopsCode(t, err, oops.CodeUnavailable)

			// The write was reached — this is the failure of the call that
			// changes the subscription, not of the one that reads it.
			require.Equal(t, []stripeclient.SetSubscriptionCancelAtPeriodEndInput{
				{SubscriptionID: "sub_test", CancelAtPeriodEnd: operation.writes},
			}, ti.stripe.subscriptionUpdates)

			// Nothing at Stripe changed, so the audit log must not claim an
			// admin changed it.
			after, err := audittest.AuditLogCountByAction(t.Context(), ti.db, operation.action)
			require.NoError(t, err)
			require.Equal(t, baseline, after)
		})
	}
