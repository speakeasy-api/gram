...
type typedCheckpointDenial struct{}

func (*typedCheckpointDenial) Error() string { return "hosted inference denied during retry" }
...
func TestHostedInferenceDenialPrecedesAllRequestSideEffects(t *testing.T) {
	...
	tests := []struct {
		name string
		call func(*ChatClient) error
	}{
		{name: "completion", call: func(client *ChatClient) error { _, err := client.GetCompletion(t.Context(), completion); return err }},
		{name: "stream", call: func(client *ChatClient) error {
			_, err := client.GetCompletionStream(t.Context(), completion)
			return err
		}},
		{name: "object completion", call: func(client *ChatClient) error {
			_, err := client.GetObjectCompletion(t.Context(), ObjectCompletionRequest{OrgID: "org", ProjectID: projectID, Prompt: "hello"})
			return err
		}},
		{name: "embeddings", call: func(client *ChatClient) error {
...
func TestEmbeddingSDKRetryRechecksHostedInferenceBeforeHTTPAttempt(t *testing.T) {
	...
	_, err = client.CreateEmbeddings(t.Context(), "org", "openai/text-embedding-3-small", []string{"hello"})
	var typedDenial *typedCheckpointDenial
