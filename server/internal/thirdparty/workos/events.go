...
import (
	...
	workosevents "github.com/workos/workos-go/v6/pkg/events"
)
...
func HandleOrganizationEvent(ctx context.Context, logger *slog.Logger, dbtx database.DBTX, event workosevents.Event) error {
	switch event.Event {
	case string(EventKindOrganizationCreated):
		return handleOrganizationCreated(ctx, logger, dbtx, event)
	case string(EventKindOrganizationDeleted):
		return handleOrganizationDeleted(ctx, logger, dbtx, event)
	case string(EventKindOrganizationUpdated):
		return handleOrganizationUpdated(ctx, logger, dbtx, event)
	default:
		logger.Debug("unhandled workos organization event", slog.String("event_type", event.Event))
		return nil
	}
}

func handleOrganizationCreated(ctx context.Context, logger *slog.Logger, dbtx database.DBTX, event workosevents.Event) error {
