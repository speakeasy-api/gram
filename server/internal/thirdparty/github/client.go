...
// appJWT creates a short-lived JWT signed by the app's private key for
// authenticating as the GitHub App itself.
...
func (c *Client) installationToken(ctx context.Context, installationID int64) (string, error) {
	...
	c.tokens[installationID] = cachedToken{
		token:     result.Token,
		expiresAt: result.ExpiresAt.Add(-5 * time.Minute), // refresh 5 min early
	}
	c.mu.Unlock()
...
// doAPI executes an authenticated GitHub API request for the given installation.
