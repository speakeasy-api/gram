...
func renderAuthPreflightScript(cfg GenerateConfig) []byte {
	// In observability mode nothing may block session start, including a
	// broken established auth state — the login attempt still runs, but
	// failures exit 0.
	failureExit := "2"
	if cfg.ObservabilityMode {
		failureExit = "0"
...
exit 0
`, cfg.ServerURL, cfg.ProjectSlug, failureExit, failureExit)
}
