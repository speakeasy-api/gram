...
// module under <config-dir>/plugin/ and lets it register servers and skills at
// startup. The package is therefore a loader module plus a slug-named data
// directory, both extracted into an OpenCode config dir (~/.config/opencode/
// or a repo's .opencode/):
//
...
// .opencode/.
//
// The module speaks both OpenCode plugin APIs. OpenCode 2 loads the default
// export's {id, setup} and registers servers and skills through the mcp and
// skill transforms; OpenCode 1 calls server() for the legacy config hook.
// Neither API resolves @opencode/plugin from a config dir, so the V2
// definition is a plain object rather than Plugin.define. Both paths let user
// config win on name collisions. ${env:VAR} header values are resolved from
// the loader's environment; headers left empty by an unset variable are
// dropped so unauthenticated requests fail at the server rather than send
// malformed credentials. mcp.json stays in V1's shape and is converted here,
// so published packages work unchanged under either version.
...
// Splits SKILL.md into its frontmatter description and body; V2 takes the
// body as content, matching what V1's skills.paths discovery hands the model.
// V2 rejects a present-but-undefined key and disables the whole plugin on any
// transform failure, so description is only set when it is a plain string.
