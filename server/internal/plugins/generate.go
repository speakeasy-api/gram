...
func generateClaudeBasePlugin(files map[string][]byte, cfg GenerateConfig) error {
	...
	files[path.Join(subdir, "hook.sh")] = renderHookScript(cfg.HooksAPIKey, cfg.ServerURL, "claude")
...
func generateCursorBasePlugin(files map[string][]byte, cfg GenerateConfig) error {
	...
	files[path.Join(subdir, "hook.sh")] = renderHookScript(cfg.HooksAPIKey, cfg.ServerURL, "cursor")
...
func renderHookScript(apiKey, serverURL, platform string) []byte {
	endpoint := serverURL + "/hooks/" + platform
	keyPrefix := apiKey
	if len(keyPrefix) > 12 {
