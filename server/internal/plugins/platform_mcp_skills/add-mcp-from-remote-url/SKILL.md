...
6. If this workflow is running in a managed project assistant, call `get_mcp_readiness` without `force` and follow only its persisted, actor-scoped evidence. Present the returned dashboard setup URL when one exists. When that evidence requires provider attachment, ask for explicit confirmation, call `attach_platform_mcp_identity_provider`, present its exact authorization URL, and wait for the user. Never force a provider probe from an assistant.
