...
6. If this workflow is running in a managed project assistant, present the returned dashboard setup URL when one exists. The assistant reads only persisted, actor-scoped readiness evidence and does not force provider probes; it may still attach the discovered identity provider and set client admission with explicit confirmation.
