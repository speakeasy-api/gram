...
8. Route secure setup from the exact readiness evidence:
   - For `upstream_identity_provider_not_configured`, explain that AICP can attach the identity provider it discovers live from the persisted remote server URL. Ask for explicit confirmation, then call `attach_platform_mcp_identity_provider`, present its exact Inspect authorization URL, and wait for the user to use Connect or Authorize.
   ...
   - For any other non-ready state, follow only the exact setup action or URL that state returns — never proceed past a non-ready state, and never invent a next step for it.
