...
func EnsureDefaultPlugin(ctx context.Context, tx pgx.Tx, organizationID string, projectID uuid.UUID) (*EnsureDefaultPluginResult, error) {
	...
	// "default" from before auto-provisioning existed. Promoting it first
	// avoids the org/project/slug unique violation the create below would
	// hit — a conflict the savepoint could unwind but not resolve, since
	// there'd still be no is_default row to re-fetch.
