...
func TestIssuerMetadataRefreshRecord_PinsInstrumentAndDimensions(t *testing.T) {
	...
	m.Record(t.Context(), "idp.example.com", IssuerMetadataRefreshOutcomeTransientFailure)
	...
	metricdatatest.AssertHasAttributes(t, got,
		attr.ServerAddress("idp.example.com"),
...
func TestIssuerMetadataRefreshRecord_NilSafe(t *testing.T) {
	...
	m.Record(t.Context(), "idp.example.com", IssuerMetadataRefreshOutcomeRefreshed)

	empty := &IssuerMetadataRefresh{attempts: nil}
	empty.Record(t.Context(), "idp.example.com", IssuerMetadataRefreshOutcomeRefreshed)
}
