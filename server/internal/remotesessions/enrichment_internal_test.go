...
func TestOverwritableIdentitySources(t *testing.T) {
	...
	require.Equal(t, []string{IdentitySourceIDToken, IdentitySourceIntrospection, IdentitySourceUserinfo}, overwritableIdentitySources(IdentitySourceIDToken))
	require.Equal(t, []string{IdentitySourceIntrospection, IdentitySourceUserinfo}, overwritableIdentitySources(IdentitySourceUserinfo))
	require.Equal(t, []string{IdentitySourceIntrospection}, overwritableIdentitySources(IdentitySourceIntrospection))
	require.Empty(t, overwritableIdentitySources(""), "no identity never moves the typed columns")
	require.Empty(t, overwritableIdentitySources("access_token_jwt"), "an unranked source never moves the typed columns")
}
...
func TestBuildEnrichmentKeepsUserinfoUnderItsOwnKey(t *testing.T) {
	...
	raw, err := buildEnrichment(tok, identity, map[string]interfaceRecord{IdentitySourceUserinfo: {Status: "ok"}})
	...
	raw, err = buildEnrichment(tok, nil, map[string]interfaceRecord{IdentitySourceUserinfo: {Status: "failed", Reason: "no sub"}})
	require.NoError(t, err)
	require.NoError(t, json.Unmarshal(raw, &doc))
