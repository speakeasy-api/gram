...
func TestPreparationCubicReview_CIMDUnknownRequiresConfirmation(t *testing.T) {
	t.Parallel()
	...
	require.Equal(t, []string{preparationJWTGrant}, confirmed.GrantTypes, "do not guess interactive grants for a legacy NULL record")
	require.Equal(t, "cimd_published", confirmed.GrantSource)
}

func TestPreparationCubicReview_ManualRecordsEvidenceWithoutDiscoveryReadiness(t *testing.T) {
	t.Parallel()
	for _, state := range []string{"unsupported_profile", "transient_failure"} {
		t.Run(state, func(t *testing.T) {
			...
			in := remotesessions.PreparationInput{UserSessionIssuerID: user, RemoteSessionIssuerID: issuer, ClientID: client, Resource: "https://resource.example.com/", Mechanism: "manual", ConfirmGrants: []string{preparationJWTGrant}, Scopes: []string{"openid"}}
			result, err := ti.service.PrepareIdentityChaining(ctx, in)
			require.NoError(t, err)
