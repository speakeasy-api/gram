...
func RunRegistrationAdmissionRegression(t *testing.T, pool *pgxpool.Pool) {
	...
	require.Eventually(t, func() bool {
		return pool.Stat().AcquiredConns() == 2
	}, time.Second, time.Millisecond, "queued registrations must not acquire connections")
	...
	for range 2 {
		...
		case <-ctx.Done():
			t.Fatalf("waiting for queued registration workers: %v", ctx.Err())
		}
	}
	cancelActive()
	for range 2 {
		...
		case err := <-done:
			require.ErrorIs(t, err, context.Canceled)
		case <-ctx.Done():
			t.Fatalf("waiting for done registration workers: %v", ctx.Err())
		}
	}
