...
func TestCIMD_OutboundRoundTripAgainstDevIDP(t *testing.T) {
	...
	mgr = remotesessions.NewChallengeManager(testenv.NewLogger(t), testenv.NewTracerProvider(t), testenv.NewMeterProvider(t), ti.conn, enc, policy, nil, ti.redisCache, mustURL(t, gramSrv.URL))
