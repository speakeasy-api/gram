...
func TestFederatedRefreshResponse(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "rate limited overrides invalid grant", status: 429, body: `{"error":"invalid_grant"}`, kind: FederatedRefreshRetryable},
		{name: "rate limited overrides invalid client", status: 429, body: `{"error":"invalid_client"}`, kind: FederatedRefreshRetryable},
