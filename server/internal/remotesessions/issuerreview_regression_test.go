...
func TestIssuerReview_RotationAdoptsEMABoundReplacement(t *testing.T) {
	t.Parallel()
...
func TestIssuerReview_FailedDiscoveryAfterConcurrentEditConflicts(t *testing.T) {
	t.Parallel()
...
func TestIssuerReview_ProjectUserIssuerMutationHidesSiblingBindings(t *testing.T) {
	t.Parallel()
	ctx, ti, in := preparationFixture(t)
	preparationRecordGrants(t, ctx, ti, in.ClientID, []string{preparationJWTGrant})
	prepared, err := ti.service.PrepareIdentityChaining(ctx, in)
