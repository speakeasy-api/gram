...
AND ($1::text = 'unlinked' OR (
 ...
 AND ($2::uuid IS NULL OR EXISTS (SELECT 1 FROM remote_session_clients c WHERE c.id = $2 AND c.remote_session_issuer_id = b.remote_session_issuer_id AND c.deleted IS FALSE AND (c.project_id = b.project_id OR (c.project_id IS NULL AND c.organization_id = b.organization_id))))
))
