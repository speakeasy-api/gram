...
func TestIssuerReviewPartialRefreshUsesOnlyFreshEvidence(t *testing.T) {
	t.Parallel()
	...
	for _, storedIssuer := range []string{upstream.URL, "https://other.example.com"} {
		t.Run(storedIssuer, func(t *testing.T) {
			t.Parallel()
			issuer := repo.RemoteSessionIssuer{Issuer: upstream.URL, Metadata: []byte(`{"issuer":"` + storedIssuer + `","token_endpoint":"https://old.example.com/token","claims_supported":["email"]}`)}
...
func TestIssuerReviewLegacyNullArraysRemainStrictOnNetwork(t *testing.T) {
	t.Parallel()
