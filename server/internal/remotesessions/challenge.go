...
// IsConnected reports whether an active remote_sessions row exists for
// (subject, client). Used to render the "connected" badge on the consent
// card.
func (m *ChallengeManager) IsConnected(
	ctx context.Context,
	subject urn.SessionSubject,
	...
	_, err := remotesessions_repo.New(m.db).GetActiveRemoteSession(ctx, remotesessions_repo.GetActiveRemoteSessionParams{
		SubjectUrn:            subject,
	...
	if err != nil {
		return false, fmt.Errorf("get active remote session: %w", err)
	}
	return true, nil
}
