...
func TestProviderUpdatesRejectInsecureURLs(t *testing.T) {
	t.Parallel()
...
// A provider stored before the URL rules tightened can still be renamed; only
// a URL the update sets is validated.
func TestProviderUpdatesValidateOnlyTheURLsTheySet(t *testing.T) {
	t.Parallel()
	...
	noOrganization := pgtype.Text{String: "", Valid: false}

	projectProviderID := createLegacyProvider(t, ctx, ti, "legacy-project-provider", projectID, organizationID)
	_, err := ti.service.UpdateRemoteSessionIssuer(ctx, &issuersgen.UpdateRemoteSessionIssuerPayload{
		ID:   projectProviderID.String(),
		Name: conv.PtrEmpty("Renamed project provider"),
	})
	require.NoError(t, err)
	_, err = ti.service.UpdateRemoteSessionIssuer(ctx, &issuersgen.UpdateRemoteSessionIssuerPayload{
		ID:            projectProviderID.String(),
		TokenEndpoint: &insecure,
	})
	requireOopsCode(t, err, oops.CodeBadRequest)

	organizationProviderID := createLegacyProvider(t, ctx, ti, "legacy-organization-provider", uuid.NullUUID{UUID: uuid.Nil, Valid: false}, organizationID)
	_, err = ti.service.UpdateIssuer(ctx, &orgissuersgen.UpdateIssuerPayload{
		ID:   organizationProviderID.String(),
		Name: conv.PtrEmpty("Renamed organization provider"),
	})
	require.NoError(t, err)
	_, err = ti.service.UpdateIssuer(ctx, &orgissuersgen.UpdateIssuerPayload{
		ID:     organizationProviderID.String(),
	...
	adminCtx := withAdmin(t, ctx)
	globalProviderID := createLegacyProvider(t, ctx, ti, "legacy-global-provider-"+uuid.NewString(), uuid.NullUUID{UUID: uuid.Nil, Valid: false}, noOrganization)
	_, err = ti.service.UpdateGlobalIssuer(adminCtx, &adminrsgen.UpdateGlobalIssuerPayload{
		ID:   globalProviderID.String(),
		Name: conv.PtrEmpty("Renamed global provider"),
	})
	require.NoError(t, err)
	_, err = ti.service.UpdateGlobalIssuer(adminCtx, &adminrsgen.UpdateGlobalIssuerPayload{
		ID:      globalProviderID.String(),
...
// createLegacyProvider stores a provider whose issuer and endpoints fail
// today's URL validation.
func createLegacyProvider(t *testing.T, ctx context.Context, ti *testInstance, slug string, projectID uuid.NullUUID, organizationID pgtype.Text) uuid.UUID {
	t.Helper()
	provider, err := repo.New(ti.conn).CreateRemoteSessionIssuer(ctx, repo.CreateRemoteSessionIssuerParams{
		ProjectID:                         projectID,
		OrganizationID:                    organizationID,
		Slug:                              slug,
		Issuer:                            "http://legacy.example.com/" + slug,
		AuthorizationEndpoint:             conv.ToPGText("http://legacy.example.com/authorize"),
		TokenEndpoint:                     conv.ToPGText("https://legacy.example.com/token#fragment"),
