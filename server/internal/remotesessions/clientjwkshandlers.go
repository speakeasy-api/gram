...
func (s *Service) settleClientKeySet(
	...
	event := audit.LogRemoteSessionClientJsonWebKeySetAttachmentEvent{
	...
	logKeySetChange := s.auditLogger.LogRemoteSessionClientDetachJsonWebKeySet
	if target.Valid {
		logKeySetChange = s.auditLogger.LogRemoteSessionClientAttachJsonWebKeySet
	}
