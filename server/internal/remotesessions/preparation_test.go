...
func TestPreparationEligibility(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{"jwt only", nil, []string{oauthwire.GrantTypeJWTBearer}, "unsupported_profile"},
		{"profile only", []string{oauthwire.GrantProfileIDJAG}, nil, "incomplete_metadata"},
		{"both", []string{oauthwire.GrantProfileIDJAG}, []string{oauthwire.GrantTypeJWTBearer}, "eligible"},
		{"unrelated", []string{"urn:example:unrelated"}, []string{oauthwire.GrantTypeJWTBearer}, "unsupported_profile"},
		{"removed", []string{}, []string{}, "unsupported_profile"},
		{"not substring", []string{oauthwire.GrantProfileIDJAG + "x"}, []string{oauthwire.GrantTypeJWTBearer}, "unsupported_profile"},
	} {
...
func TestPreparationDCRValidation(t *testing.T) {
	t.Parallel()
	base := preparationDCRResponse{ClientID: "resource-client", ClientSecret: "test-secret", TokenEndpointAuthMethod: "client_secret_basic", GrantTypes: []string{oauthwire.GrantTypeJWTBearer}}
	for _, tt := range []struct {
		name, want string
...
func TestPreparationDCRRejectsUnsupportedConfidentialMethods(t *testing.T) {
	...
	for _, method := range []string{"", "none", "private_key_jwt", "unknown"} {
		t.Run(method, func(t *testing.T) {
			...
			response := preparationDCRResponse{ClientID: "client", ClientSecret: "secret", TokenEndpointAuthMethod: method, GrantTypes: []string{oauthwire.GrantTypeJWTBearer}}
