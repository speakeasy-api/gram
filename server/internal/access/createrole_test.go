...
func TestService_CreateRole(t *testing.T) {
	...
	role, err := ti.service.CreateRole(ctx, &gen.CreateRolePayload{
		...
		Grants: []*gen.RoleGrant{
			{Scope: string(authz.ScopeProjectRead), Selectors: []map[string]string{{"resource_kind": "project", "resource_id": "project-1"}, {"resource_kind": "project", "resource_id": "project-2"}}},
...
func TestService_CreateRole_WorkOSCreateFailure(t *testing.T) {
	...
	_, err := ti.service.CreateRole(ctx, &gen.CreateRolePayload{
		...
		Description: "Can build selected resources",
		Grants: []*gen.RoleGrant{
			{Scope: string(authz.ScopeProjectRead), Selectors: []map[string]string{{"resource_kind": "project", "resource_id": "project-1"}}},
		},
	})
	require.Error(t, err)
