...
func (s *Service) ListScopes(ctx context.Context, _ *gen.ListScopesPayload) (*gen.ListScopesResult, error) {
	...
	return &gen.ListScopesResult{Scopes: []*gen.ScopeDefinition{
		...
		{Slug: string(authz.ScopeToolsExecuteDestructive), Description: "Allow agents to invoke tools annotated as destructive or whose call input matches a curated destructive-command pattern.", ResourceType: "tools"},
	}}, nil
}
