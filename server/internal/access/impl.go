...
func (s *Service) ListScopes(ctx context.Context, _ *gen.ListScopesPayload) (*gen.ListScopesResult, error) {
	...
	scopes := []scopeDefinitionInput{
		...
		{scope: authz.ScopeChatRead, description: "Read and pin every member's agent session transcripts, and reveal secrets flagged in Risk Events.", resourceType: "chat"},
		{scope: authz.ScopeChatWrite, description: "Rename, delete, and give feedback on every member's agent sessions.", resourceType: "chat"},
