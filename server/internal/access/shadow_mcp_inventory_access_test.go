...
func TestService_AllowShadowMCPInventoryServer_CanonicalizesAndIsIdempotent(t *testing.T) {
	...
	require.Equal(t, shadowMCPInventoryAccessAllowed, first.ExplicitAccess)
	require.Equal(t, shadowMCPInventoryAccessAllowed, first.EffectiveAccess)
	require.NotNil(t, first.ExplicitRule)
	require.Equal(t, shadowMCPRuleAllowed, first.ExplicitRule.Disposition)
	require.Equal(t, shadowMCPAccessScopeProject, first.ExplicitRule.AccessScope)
	require.Equal(t, projectID, *first.ExplicitRule.ProjectID)
	require.Equal(t, "full_url", first.ExplicitRule.MatchBreadth)
	require.Equal(t, "https://example.com/mcp", first.ExplicitRule.MatchValue)
	require.Equal(t, "Example MCP", first.ExplicitRule.DisplayName)
	...
	require.NoError(t, err)
	require.NotNil(t, second.ExplicitRule)
	require.Equal(t, first.ExplicitRule.ID, second.ExplicitRule.ID)
...
func TestService_BlockShadowMCPInventoryServer_UpdatesExistingExplicitRule(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, err)
	require.NotNil(t, allowed.ExplicitRule)
	...
	require.NoError(t, err)
	require.Equal(t, shadowMCPInventoryAccessDenied, blocked.ExplicitAccess)
	require.Equal(t, shadowMCPInventoryAccessDenied, blocked.EffectiveAccess)
	require.NotNil(t, blocked.ExplicitRule)
	require.Equal(t, allowed.ExplicitRule.ID, blocked.ExplicitRule.ID)
	require.Equal(t, shadowMCPRuleDenied, blocked.ExplicitRule.Disposition)
...
func TestService_ClearShadowMCPInventoryServerAccess_RemovesExplicitRuleAndPreservesBroaderState(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, err)
	require.Equal(t, shadowMCPInventoryAccessAllowed, allowed.ExplicitAccess)
	require.Equal(t, shadowMCPInventoryAccessDenied, allowed.EffectiveAccess)
	...
	require.NoError(t, err)
	require.Equal(t, shadowMCPInventoryAccessNone, cleared.ExplicitAccess)
	require.Nil(t, cleared.ExplicitRule)
	require.Equal(t, shadowMCPInventoryAccessDenied, cleared.EffectiveAccess)
	require.NotNil(t, cleared.EffectiveRule)
	require.Equal(t, "Broader deny", cleared.EffectiveRule.DisplayName)
	...
	require.NoError(t, err)
	require.Equal(t, shadowMCPInventoryAccessNone, clearedAgain.ExplicitAccess)
