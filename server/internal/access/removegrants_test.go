...
func TestAccessService_RemoveGrants(t *testing.T) {
	t.Parallel()

	t.Run("removes all grants for a principal", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("no-op when principal has no grants", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("unauthorized without auth context", func(t *testing.T) {
		...
		err := ti.service.RemoveGrants(ctxWithoutAuth, &gen.RemoveGrantsPayload{
			SessionToken: nil,
			PrincipalUrn: "user:user_abc",
		})
		require.Error(t, err)
