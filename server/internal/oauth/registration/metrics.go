...
// RecordPostRegistrationCommitFailure notifies recorder, when it supports it,
// that an upstream registration succeeded but its local transaction did not
// commit. The provider may have no portable delete API, so the issued client
// is left behind upstream. The signal survives caller cancellation.
func RecordPostRegistrationCommitFailure(ctx context.Context, recorder Recorder, method Method) {
	if commitRecorder, ok := recorder.(PostRegistrationCommitFailureRecorder); ok {
		commitRecorder.RecordPostRegistrationCommitFailure(context.WithoutCancel(ctx), method)
	}
}
