...
// authenticated customer Gram session. Support, legacy impersonation, and the
// membership-free shared demo organization are deliberately excluded.
func ValidatedGramSessionActingUser(ctx context.Context) (GramSessionActingUser, bool) {
	authCtx, ok := GetAuthContext(ctx)
	if !ok || authCtx == nil || !authCtx.gramSessionValidated ||
		...
		return GramSessionActingUser{}, false
	}
