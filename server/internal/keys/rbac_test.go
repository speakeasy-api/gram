...
func TestKeysService_CreateKey_AllowsOrgAdminGrant(t *testing.T) {
	...
	ctx, ti := newTestKeysService(t)
	ctx = rbactest.WithExactAccessGrants(t, ctx, access.Grant{Scope: access.ScopeOrgAdmin, Selector: access.ForResource(testAuthContext(t, ctx).ActiveOrganizationID)})
