...
func TestReviewedRemoteSessionProviderVerticalSlice(t *testing.T) {
	...
	beforeRefresh, err := adapter.ProbeReadiness(ctx, providerProbeRequest(principal, project.ID, registration))
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.NotEqual(t, beforeRefreshFingerprint, afterRefreshFingerprint, "refresh must invalidate cached readiness")
