...
func registerMCPConnectionSettingsTool(reg *Registrar, service *MCPConnectionSettingsService) {
	addTool(reg, &mcp.Tool{
		...
		Description: "Read one exact MCP server or gateway in an explicit project: stored network mode, visibility, gateway discovery mode, endpoint IDs/slugs/domains/root markers, configured ingress state, and direct plugin memberships. The version changes only with the target's network mode or endpoint ID, slug, and domain, not with ingress observations, root selection, or plugin membership. Ingress DNS/status are observations, not proof of client connectivity; endpoints are not necessarily publisher-selected package URLs. Never returns upstream URLs, credentials, tokens, or other secrets.",
