...
func TestValidateFeedbackInputRejectsSensitiveOrOutOfBoundsValues(t *testing.T) {
	...
	for _, input := range []FeedbackInput{
		{Category: "success", Rating: &rating, ToolName: "get_platform_context", Note: "Helpful", IdempotencyKey: "feedback-1"},
		...
		{Category: "success", Note: strings.Repeat("a", 501), IdempotencyKey: "feedback-7"},
