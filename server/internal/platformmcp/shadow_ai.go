...
var (
	// ErrShadowAIInvalid marks an argument the caller can correct itself, such
	// as a category outside the scan library's vocabulary. It is kept apart
	// from ErrShadowAIUnavailable so a mistyped argument is not reported as the
	// whole feature being switched off: the caller should fix the argument and
	// retry, not conclude Shadow AI is unavailable to this organization.
	ErrShadowAIInvalid     = errors.New("invalid platform mcp shadow ai request")
	ErrShadowAIUnavailable = errors.New("platform mcp shadow ai unavailable")
)
...
func (s *ShadowAIService) ListLibrary(ctx context.Context, principal Principal, input ListAIScanLibraryInput) (ListAIScanLibraryOutput, error) {
	...
	if category != "" && !slices.Contains(aitargets.KnownCategories(), aitargets.Category(category)) {
		return ListAIScanLibraryOutput{}, ErrShadowAIInvalid
	}
