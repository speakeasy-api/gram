...
func TestMCPFromInventoryLabelsOwnershipAndNeverProbesLegacy(t *testing.T) {
	...
	platform := mcpFromInventory(mcpID, projectID, "Project", "project", "Reviewed", "reviewed", "private", true, registrationID, "catalog", "registry", "reviewed/server", "registered", complete, complete, complete, complete, "ready", "checked", "expires", map[uuid.UUID][]MCPDistribution{registrationID: {{PluginID: "plugin", State: "attached", PublicationState: "published"}}})
	...
	incomplete := mcpFromInventory(mcpID, projectID, "Project", "project", "Incomplete", "incomplete", "private", true, registrationID, "catalog", "registry", "reviewed/incomplete", "pending", uuid.NullUUID{UUID: uuid.Nil, Valid: true}, complete, complete, complete, "", "", "", nil)
	...
	dashboard := mcpFromInventory(mcpID, projectID, "Project", "project", "Dashboard", "dashboard", "private", true, uuid.Nil, "", "", "", "", uuid.NullUUID{}, uuid.NullUUID{}, uuid.NullUUID{}, uuid.NullUUID{}, "", "", "", nil)
	...
	legacy := mcpFromInventory(mcpID, projectID, "Project", "project", "Legacy", "legacy", "disabled", false, uuid.Nil, "", "", "", "", uuid.NullUUID{}, uuid.NullUUID{}, uuid.NullUUID{}, uuid.NullUUID{}, "ready", "checked", "expires", nil)
...
func TestMatchInventoryQueryReturnsExactThenBoundedCandidates(t *testing.T) {
	t.Parallel()
	mcps := make([]MCP, 0, 12)
	for range 12 {
	...
	exact := matchInventoryQuery(mcps, "unique")
	require.Len(t, exact, 1)
	require.Equal(t, "unique", exact[0].Slug)

	candidates := matchInventoryQuery(mcps, "example")
	require.Len(t, candidates, 10)

	duplicates := matchInventoryQuery([]MCP{{Name: "Duplicate"}, {Name: "Duplicate"}}, "duplicate")
