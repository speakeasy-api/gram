...
// the user_email recorded on the telemetry rows.
...
// excluded — they cannot be attributed to an actor. The source is bucketed
// hourly; timeStart is floored to the hour by the query, so callers that need
// an exclusive lower bound (e.g. clamping to a rule's evaluated_from) should
// ceil timeStart to the next hour boundary before calling.
//
...
func (q *Queries) ListActorSpend(ctx context.Context, projectIDs []string, timeStart, timeEnd int64) ([]ActorSpendRow, error) {
	...
	for rows.Next() {
		...
		if err = rows.ScanStruct(&row); err != nil {
			return nil, fmt.Errorf("scanning actor spend row: %w", err)
		}
		out = append(out, row)
	}
