...
// GetClaudeTurnAttributionByChatIDsParams contains the parameters for getting
// Claude Code turn usage broken down by the request attribution fields Claude
// emits on api_request events.
type GetClaudeTurnAttributionByChatIDsParams struct {
	GramProjectID string
	ChatIDs       []string
	TimeStart     int64
	TimeEnd       int64

	// RecacheIdleThresholdSeconds marks rows after a long idle gap as re-cache
	// rows. Defaults to 300 seconds, matching Claude prompt-cache TTL behavior.
	RecacheIdleThresholdSeconds int64
	ExcludeRecache              bool
}
...
// ClaudeTurnAttributionRow represents Claude Code api_request usage for one
// attribution bucket within a prompt.id turn.
type ClaudeTurnAttributionRow struct {
	GramChatID          string
	PromptID            string
	QuerySource         string
	SkillName           string
...
type claudeTurnAttributionScanRow struct {
	GramChatID          string   `ch:"gram_chat_id"`
	PromptID            string   `ch:"prompt_id"`
...
func (r claudeTurnAttributionScanRow) row(secondsSincePrev int64, isRecache bool) ClaudeTurnAttributionRow {
	return ClaudeTurnAttributionRow{
		GramChatID:          r.GramChatID,
		PromptID:            r.PromptID,
		QuerySource:         r.QuerySource,
		SkillName:           r.SkillName,
		AgentName:           r.AgentName,
		MCPServerName:       r.MCPServerName,
...
// GetClaudeTurnAttributionByChatIDs retrieves Claude Code api_request usage
// broken down by turn and request attribution dimensions. Re-cache detection is
// computed while reading so it can compare adjacent summarized turns across MV
// insert blocks.
//
//nolint:errcheck,wrapcheck // Replicating SQLC syntax which doesn't comply to this lint rule
func (q *Queries) GetClaudeTurnAttributionByChatIDs(ctx context.Context, arg GetClaudeTurnAttributionByChatIDsParams) (map[string][]ClaudeTurnAttributionRow, error) {
	attributionByChatID := make(map[string][]ClaudeTurnAttributionRow, len(arg.ChatIDs))
	for _, chatID := range arg.ChatIDs {
		attributionByChatID[chatID] = []ClaudeTurnAttributionRow{}
	}
	if len(arg.ChatIDs) == 0 {
		return attributionByChatID, nil
	}

	thresholdSeconds := arg.RecacheIdleThresholdSeconds
	...
	// TimeStart is applied after scanning so re-cache detection can still see
	// the previous turn just before the requested window.
	if arg.TimeEnd > 0 {
		sb = sb.Having("start_time_unix_nano <= ?", arg.TimeEnd)
	}

	query, args, err := sb.ToSql()
	if err != nil {
		return nil, fmt.Errorf("building get Claude turn attribution by chat IDs query: %w", err)
	}
	...
	lastTurnKeyByChatID := make(map[string]string)
	previousTurnStartByChatID := make(map[string]int64)
	secondsSincePrevByTurn := make(map[string]int64)
	isRecacheByTurn := make(map[string]bool)

	for rows.Next() {
		var scanRow claudeTurnAttributionScanRow
		if err = rows.ScanStruct(&scanRow); err != nil {
			return nil, fmt.Errorf("error scanning Claude turn attribution row: %w", err)
		}

		turnKey := scanRow.GramChatID + "\x00" + scanRow.PromptID
		secondsSincePrev, ok := secondsSincePrevByTurn[turnKey]
		isRecache := isRecacheByTurn[turnKey]
		if !ok {
			lastTurnKey := lastTurnKeyByChatID[scanRow.GramChatID]
			if lastTurnKey != turnKey {
				if previousStart := previousTurnStartByChatID[scanRow.GramChatID]; previousStart > 0 {
					secondsSincePrev = max((scanRow.StartTimeUnixNano-previousStart)/int64(time.Second), 0)
					isRecache = secondsSincePrev > thresholdSeconds
				}
				lastTurnKeyByChatID[scanRow.GramChatID] = turnKey
				previousTurnStartByChatID[scanRow.GramChatID] = scanRow.StartTimeUnixNano
			}
			secondsSincePrevByTurn[turnKey] = secondsSincePrev
			isRecacheByTurn[turnKey] = isRecache
		}

		if arg.TimeStart > 0 && scanRow.StartTimeUnixNano < arg.TimeStart {
			continue
		}
		if arg.ExcludeRecache && isRecache {
			continue
		}
		attributionByChatID[scanRow.GramChatID] = append(attributionByChatID[scanRow.GramChatID], scanRow.row(secondsSincePrev, isRecache))
	}
