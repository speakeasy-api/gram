...
// SpendChecker consults the spend-rule circuit for the actor on a hook event
// and renders the deny reason. It owns the whole spend decision — which
// adapters are gated, the circuit lookup, and the reason wording — so the
// stages only translate a blocked verdict into a decision. blocked=false
// covers every non-deny outcome: an ungated adapter, an under-budget actor,
// and the primitive's fail-open error paths.
type SpendChecker interface {
	CheckSpend(ctx context.Context, req *Request, actor Actor, kind string, at time.Time) (auditReason string, blocked bool)
}
...
// surface.
func SpendGatePrompt(spend SpendChecker) func(context.Context, *agenthooks.PromptEvent) (agenthooks.PromptDecision, error) {
	return func(ctx context.Context, ev *agenthooks.PromptEvent) (agenthooks.PromptDecision, error) {
		...
		auditReason, blocked := spend.CheckSpend(ctx, req, ActorFromContext(ctx), "prompt", ev.Time)
		if !blocked {
			return neutral, nil
		}
...
func spendGateToolRequest(ctx context.Context, spend SpendChecker, links BlockPageLinker, kind, toolName string, eventTime time.Time) (agenthooks.ToolPreDecision, error) {
	...
	auditReason, blocked := spend.CheckSpend(ctx, req, actor, kind, eventTime)
	if !blocked {
		return agenthooks.NoDecision(), nil
	}
