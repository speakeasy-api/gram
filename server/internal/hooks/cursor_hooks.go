...
func (s *Service) Cursor(ctx context.Context, payload *gen.CursorPayload) (*gen.CursorHookResult, error) {
	...
	// Record the hook after the deny decision so telemetry can carry the
	// block reason as an attribute.
	// Runs after the deny decision so the ClickHouse entry can carry the
	// block reason as an attribute.
	s.recordCursorHook(ctx, hookEvent, payload, orgID, projectID, actorUserID, blockReason)

	return result, nil
}

func (s *Service) recordCursorHook(ctx context.Context, hookEvent any, payload *gen.CursorPayload, orgID string, projectID string, userID string, blockReason string) {
	...
	go func() {
		...
		if _, ok := hookEvent.(*hookevents.AfterAgentResponse); ok {
