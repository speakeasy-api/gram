...
// (data.mcp_attribution). This file stores those tuples in Redis, keyed by
// (org, request id); the scheduled staged-telemetry sweep joins them against
// the rows parked in telemetry_logs_staging (see server/internal/hooks/otel.go
// for the staging fork and
// server/internal/background/promote_staged_telemetry.go for promotion).
//
// The key is org-scoped, not project-scoped: the plugin authenticates with a
// hooks key plus a client-chosen project slug (GRAM_HOOKS_PROJECT_SLUG,
// default "default"), while the staged OTEL row's project comes from the
// OTEL exporter's own credential. With an org-wide hooks key the two resolve
// to different projects, so a project-scoped key would never match at
// promotion time. Both paths always agree on the org.
...
func (s *Service) captureMCPAttribution(ctx context.Context, payload *gen.IngestPayload, authCtx *contextvalues.AuthContext) {
	...
	for _, entry := range payload.Data.McpAttribution {
		...
		if err := s.enforcer.cache.Set(ctx, telemetry.MCPAttributionTupleKey(authCtx.ActiveOrganizationID, requestID), tuple, telemetry.MCPAttributionTupleTTL); err != nil {
			s.enforcer.logger.WarnContext(ctx, "failed to store MCP attribution tuple",
