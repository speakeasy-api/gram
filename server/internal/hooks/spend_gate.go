...
// (fail-open): a nil gate, an unresolved org/actor identity (same guard as
...
func (s *Service) checkSpendGate(ctx context.Context, ev hookevents.Event) *spendrules.Block {
	...
	if ev.Context.OrganizationID == "" || (ev.Context.User.ID == "" && ev.Context.User.Email == "") {
		return nil
	}

	block, err := s.spendGate.CheckBlocked(ctx, ev.Context.OrganizationID, ev.Context.User.ID, ev.Context.User.Email)
	if err != nil {
