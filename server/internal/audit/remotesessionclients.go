...
type LogRemoteSessionClientDetachMcpServerEvent struct {
	OrganizationID string
	ProjectID      uuid.UUID
...
// LogRemoteSessionClientDetachMcpServer records that an organization admin
// detached a remote_session_client from an MCP server (clearing that server's
// user_session_issuer link). The affected MCP server is captured in metadata so
// the audit trail names the server whose authentication changed.
func (l *Logger) LogRemoteSessionClientDetachMcpServer(ctx context.Context, dbtx repo.DBTX, event LogRemoteSessionClientDetachMcpServerEvent) error {
	action := ActionRemoteSessionClientDetachMcpServer
...
// LogRemoteSessionClientRevokeSessions records a bulk revoke of every session
// minted against a client as a single audit event on the client subject
// (mirroring how a client delete records one event for its cascaded session
// deletes), with the revoked count captured in metadata.
func (l *Logger) LogRemoteSessionClientRevokeSessions(ctx context.Context, dbtx repo.DBTX, event LogRemoteSessionClientRevokeSessionsEvent) error {
	action := ActionRemoteSessionClientRevokeSessions

	metadata, err := marshalAuditPayload(map[string]any{
		"revoked_count": event.RevokedCount,
	})
	if err != nil {
		return fmt.Errorf("marshal %s metadata: %w", action, err)
	}
