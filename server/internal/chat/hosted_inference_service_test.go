...
func (c *checkpointCompletionClient) GetCompletion(ctx context.Context, request openrouter.CompletionRequest) (*openrouter.CompletionResponse, error) {
	c.completionCalls++
	if err := c.checkpoint.Check(ctx, request.OrgID); err != nil {
		return nil, err
	}
	c.providerAttempts++
...
func (c *checkpointCompletionClient) CreateEmbeddings(ctx context.Context, orgID string, _ string, _ []string, _ ...openrouter.EmbeddingOption) ([][]float32, error) {
	if err := c.checkpoint.Check(ctx, orgID); err != nil {
		return nil, err
	}
...
func (c *checkpointCompletionClient) PreflightHostedInference(ctx context.Context, organizationID string) error {
	c.preflightCalls++
	return c.checkpoint.Check(ctx, organizationID)
}
...
func TestHandleCompletionGovernedSessionDenialAndContextWindowPreflight(t *testing.T) {
	t.Parallel()
	...
	for _, includeContextWindow := range []bool{false, true} {
		t.Run(fmt.Sprintf("include context window %t", includeContextWindow), func(t *testing.T) {
			...
			if includeContextWindow {
				require.Equal(t, 1, gate.preflightCalls)
				require.Zero(t, gate.streamCalls, "metadata preflight denies before completion delegation")
			} else {
				require.Zero(t, gate.preflightCalls, "ordinary requests do not add a third evaluation")
				require.Equal(t, 1, gate.streamCalls, "the completion client's common checkpoint owns ordinary preflight")
			}
		})
	}
}
