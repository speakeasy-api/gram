...
func newTestChatServiceWithOptions(t *testing.T, isRBACEnabled authz.IsRBACEnabled, completionClient openrouter.CompletionClient) *chatTestInstance {
	...
	authzEngine := authz.NewEngine(logger, conn, authz.NewNoopChallengePublisher(logger), isRBACEnabled, authztest.ChallengeLoggingAlwaysDisabled, workos.NewStubClient())
