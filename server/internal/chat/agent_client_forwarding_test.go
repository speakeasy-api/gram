...
func (c *forwardingCompletionClient) PreflightHostedInference(ctx context.Context, organizationID string) error {
	c.record(ctx, "PreflightHostedInference:"+organizationID)
	return nil
}
...
func TestAgenticClientGenericMethodsPreserveCallerClassification(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, client.PreflightHostedInference(ctx, "org"))
	...
	require.Equal(t, map[string]int{
		...
		"CreateEmbeddings":             1,
		"ResolveKey":                   1,
		"PreflightHostedInference:org": 1,
	}, underlying.calls)
}

type completionClientWithoutPreflight struct {
	openrouter.CompletionClient
}

func TestAgenticClientPreflightIsOptional(t *testing.T) {
	t.Parallel()

	ctx := t.Context()
	underlying := &forwardingCompletionClient{t: t, wantCtx: ctx, calls: map[string]int{}}
	client := &Client{completionClient: completionClientWithoutPreflight{CompletionClient: underlying}}
	require.NoError(t, client.PreflightHostedInference(ctx, "org"))
	require.Empty(t, underlying.calls)
}
...
func TestChatInferenceExclusionsRemainUnsupported(t *testing.T) {
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			classified, err := classifyChatInference(ctx, tt.authCtx, tt.keySlot)
			require.NoError(t, err)
