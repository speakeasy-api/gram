...
func TestChatMessageWriterMetersStoredTextAndToolCalls(t *testing.T) {
	...
	require.Equal(t, string(definition.ID), messages[0].GetMeterId())
	...
	require.Equal(t, definition.Version, messages[0].GetMeterVersion())
	require.Equal(t, meteringv1.MeterReading_KIND_USAGE, messages[0].GetKind())
	require.Equal(t, string(definition.MeasurementMethod), messages[0].GetMeasurementMethod())
