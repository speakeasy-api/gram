...
func (s *Service) ListTools(ctx context.Context, payload *gen.ListToolsPayload) (res *gen.ListToolsResult, err error) {
	...
	for i, tool := range tools {
		result.Tools[i] = &gen.HTTPToolDefinition{
			...
			Path:             tool.Path,
			HeadersSchema:    conv.FromBytes(tool.HeadersSchema),
			QueriesSchema:    conv.FromBytes(tool.QueriesSchema),
			PathparamsSchema: conv.FromBytes(tool.PathparamsSchema),
			BodySchema:       conv.FromBytes(tool.BodySchema),
