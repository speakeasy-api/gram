...
func TestEngineRequire_requiresAuthContext(t *testing.T) {
	t.Parallel()
	engine := NewEngine(testenv.NewLogger(t), nil, gcp.NewNoopPublisher[*authzv1.Challenge](), staticChallengeLogging(true), workos.NewStubClient())
...
func TestEngineFilter_logsSingleAggregateChallenge(t *testing.T) {
	...
	engine := NewEngine(testenv.NewLogger(t), nil, publisher, staticChallengeLogging(true), workos.NewStubClient())
...
func TestEngineFilter_logsDenyWhenNoMatches(t *testing.T) {
	...
	ctx := GrantsToContext(enterpriseSessionCtxWithOrg(t, "org_"+uuid.NewString()), []Grant{NewGrant(ScopeProjectRead, "proj_other")})
	publisher, published := capturingChallengePublisher(t)
	engine := NewEngine(testenv.NewLogger(t), nil, publisher, staticChallengeLogging(true), workos.NewStubClient())
...
func TestEngineFilter_skipsLogWhenNoChecks(t *testing.T) {
	...
	ctx := GrantsToContext(enterpriseSessionCtxWithOrg(t, "org_"+uuid.NewString()), []Grant{NewGrant(ScopeProjectRead, WildcardResource)})
...
func TestEngineFindMatched_logsSingleAggregateChallenge(t *testing.T) {
	...
	ctx := GrantsToContext(enterpriseSessionCtxWithOrg(t, "org_"+uuid.NewString()), []Grant{NewGrant(ScopeProjectRead, "proj_allowed")})
	publisher, published := capturingChallengePublisher(t)
	engine := NewEngine(testenv.NewLogger(t), nil, publisher, staticChallengeLogging(true), workos.NewStubClient())
	...
	// the returned slice, not in the published event stream.
	require.Len(t, *published, 1)
	message := (*published)[0]
	require.Equal(t, string(authzrepo.OutcomeAllow), message.GetOutcome())
	require.Equal(t, string(authzrepo.ReasonGrantMatched), message.GetReason())
...
func TestCanUseOverride_devPlusAdmin(t *testing.T) {
	t.Parallel()
	engine := NewEngine(testenv.NewLogger(t), nil, gcp.NewNoopPublisher[*authzv1.Challenge](), staticChallengeLogging(true), workos.NewStubClient(), EngineOpts{DevMode: true})
