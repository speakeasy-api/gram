...
func TestEngineRequire_requiresAuthContext(t *testing.T) {
	...
	engine := NewEngine(testenv.NewLogger(t), nil, testinfra.NewClickhouseStub(), staticRBAC(true), workos.NewStubClient(), cache.NoopCache)
...
func TestEngineRequire_skipsWhenRBACFeatureDisabled(t *testing.T) {
	...
	engine := NewEngine(testenv.NewLogger(t), nil, testinfra.NewClickhouseStub(), staticRBAC(false), workos.NewStubClient(), cache.NoopCache)
...
func TestEngineRequire_returnsUnexpectedWhenFeatureCheckFails(t *testing.T) {
	...
	engine := NewEngine(testenv.NewLogger(t), nil, testinfra.NewClickhouseStub(), failingRBAC(errors.New("boom")), workos.NewStubClient(), cache.NoopCache)
...
func TestResolveRoleSlug_cachesEmptyMembershipResult(t *testing.T) {
	...
	engine := NewEngine(testenv.NewLogger(t), conn, testinfra.NewClickhouseStub(), staticRBAC(true), membership, newMapCache())
...
func TestCanUseOverride_devPlusAdmin(t *testing.T) {
	t.Parallel()
	engine := NewEngine(testenv.NewLogger(t), nil, testinfra.NewClickhouseStub(), staticRBAC(false), workos.NewStubClient(), cache.NoopCache, EngineOpts{DevMode: true})
