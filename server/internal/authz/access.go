...
type Check struct {
	...
	// expanded is true for checks generated by scope expansion.
	// Legacy deny-effect grants only apply to the original scope, not expanded
	// parents.
	expanded bool
}
...
func (c Check) expand() []Check {
	checks := []Check{
		...
		{Scope: ScopeRoot, ResourceKind: c.ResourceKind, ResourceID: c.ResourceID, Dimensions: c.Dimensions, selectorMatch: c.selectorMatch, expanded: true},
		// The original scope is not expanded — both allows and legacy deny-effect
		// grants match it.
		{Scope: c.Scope, ResourceKind: c.ResourceKind, ResourceID: c.ResourceID, Dimensions: c.Dimensions, selectorMatch: c.selectorMatch, expanded: false},
	}
	...
	for _, s := range scopeExpansions[c.Scope] {
		checks = append(checks, Check{Scope: s, ResourceKind: c.ResourceKind, ResourceID: c.ResourceID, Dimensions: c.Dimensions, selectorMatch: c.selectorMatch, expanded: true})
	}
	return checks
}
