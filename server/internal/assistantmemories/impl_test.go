...
func newTestHarness(t *testing.T) (*testHarness, context.Context) {
	...
	authzEngine := authz.NewEngine(logger, nil, authz.NewNoopChallengePublisher(logger), authztest.RBACAlwaysDisabled, authztest.ChallengeLoggingAlwaysDisabled, workos.NewStubClient())
...
func TestListAssistantMemories_RBACDenied(t *testing.T) {
	...
	logger := testenv.NewLogger(t)
	h.svc.authz = authz.NewEngine(logger, nil, authz.NewNoopChallengePublisher(logger), authztest.RBACAlwaysEnabled, authztest.ChallengeLoggingAlwaysDisabled, workos.NewStubClient())
