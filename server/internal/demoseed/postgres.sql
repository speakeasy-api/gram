...
BEGIN
  ...
  INSERT INTO otel_destinations (
    ...
    (
      ...
      demo_org, proj_a, 'Datadog', 'https://datadog.example.com/otlp', 'exclude',
      now() - interval '8 days'
    ),
    (
      demo.det_uuid('gram-demo-otel-destination-' || demo_org || '-grafana'),
      demo_org, proj_a, 'Grafana Cloud', 'https://grafana.example.com/otlp', 'include',
      now() - interval '5 days'
    );

  INSERT INTO data_export_routes (
    id, organization_id, project_id, data_source, enabled,
    otel_destination_id, created_at
  ) VALUES
    (
...
E'--- a/SKILL.md\n+++ b/SKILL.md\n@@ -6,4 +6,5 @@\n # Refund handling\n \n 1. Verify the order id and amount with the customer.\n+1a. If the customer pastes a card number, stop and ask them to remove it.\n 2. Use the process_refund tool with the confirmed amount.',
  ...
  IF stray <> 2 THEN
    RAISE EXCEPTION 'demo seed postflight: expected 2 OTEL destinations, found %', stray;
  END IF;
