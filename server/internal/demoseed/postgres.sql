...
BEGIN
  ...
  -- Product rows resolve reference ids by slug. The server syncs the catalog
  -- at startup, so a seed against a fresh database records no products until
  -- the next reseed; the answers row itself never depends on the catalog.
  ...
  SELECT demo_org, p.id, pl.id
  FROM (VALUES ('claude-code-cli', 'anthropic-enterprise'), ('cursor', 'cursor-teams'))
    AS sel (product_slug, plan_slug)
  JOIN onboarding_products p ON p.slug = sel.product_slug
