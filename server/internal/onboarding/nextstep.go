...
// SelectedProvider is one provider an admin picked, with the plan the
// organization is on. The plan applies to every product of the provider.
type SelectedProvider struct {
	Provider string
	// Plan is empty for providers without plans.
	Plan string
}
...
type Answers struct {
	...
	// Products are product slugs, each of a selected provider.
	Products []string
}

// PlanFor returns the plan declared for the product's provider, or "" when
// the provider has none or was not selected.
func (a Answers) PlanFor(product ProductSpec) string {
	for _, sel := range a.Providers {
		if sel.Provider == product.Provider {
			return sel.Plan
		}
	}
	return ""
...
func StepsFor(c Catalog, a Answers) []Step {
	...
	case UseCaseSecurity:
		...
		for _, slug := range a.Products {
			add(instrumentationStep(c, a, slug))
		}
...
// flowing, or false when nothing in the code can observe it on the plan its
// provider is on.
func instrumentationStep(c Catalog, a Answers, slug string) (Step, bool) {
	product, ok := c.Product(slug)
	if !ok {
		return noStep, false
	}
	plan := a.PlanFor(product)
	...
	case ProviderAnthropic:
		...
		if plan == PlanAnthropicEnterprise {
	...
	case ProviderOpenAI:
		...
		default:
			if slices.Contains(openAIOrgPlans, plan) {
...
func costStep(c Catalog, a Answers, slug string) (Step, bool) {
	product, ok := c.Product(slug)
	if !ok {
		return noStep, false
	}
	...
	switch product.Provider {
	case ProviderAnthropic:
		if plan == PlanAnthropicEnterprise && product.Slug != ProductClaudeTag {
			return stepAnthropicAdminAnalytics(), true
		}
		return instrumentationStep(c, a, slug)
	case ProviderOpenAI:
		if slices.Contains(openAIOrgPlans, plan) {
		...
		return instrumentationStep(c, a, slug)
	case ProviderCursor:
		if slices.Contains(cursorOrgPlans, plan) {
...
func stepAnthropicInferenceHooks(c Catalog, a Answers) Step {
	sources := selectedSources(c, a, func(p ProductSpec) bool { return p.Provider == ProviderAnthropic && p.Slug != ProductClaudeTag })
	return Step{
...
func stepOpenAIComplianceImport(c Catalog, a Answers) Step {
	sources := selectedSources(c, a, func(p ProductSpec) bool { return p.Provider == ProviderOpenAI && p.Slug != ProductCodex })
	return Step{
