...
const getSkillEfficacyProjectOrganization = `-- name: GetSkillEfficacyProjectOrganization :one
...
func (q *Queries) GetSkillEfficacyProjectOrganization(ctx context.Context, projectID uuid.UUID) (string, error) {
	row := q.db.QueryRow(ctx, getSkillEfficacyProjectOrganization, projectID)
...
const listSkillEfficacyChatStates = `-- name: ListSkillEfficacyChatStates :many
SELECT id, deleted
FROM chats
WHERE project_id = $1
  AND id = ANY($2::uuid[])
...
type ListSkillEfficacyChatStatesParams struct {
	ProjectID uuid.UUID
	ChatIds   []uuid.UUID
}

type ListSkillEfficacyChatStatesRow struct {
	ID      uuid.UUID
	Deleted bool
}
...
func (q *Queries) ListSkillEfficacyChatStates(ctx context.Context, arg ListSkillEfficacyChatStatesParams) ([]ListSkillEfficacyChatStatesRow, error) {
	rows, err := q.db.Query(ctx, listSkillEfficacyChatStates, arg.ProjectID, arg.ChatIds)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListSkillEfficacyChatStatesRow
	for rows.Next() {
		var i ListSkillEfficacyChatStatesRow
		if err := rows.Scan(&i.ID, &i.Deleted); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
...
const listSkillEfficacyJudgeSkills = `-- name: ListSkillEfficacyJudgeSkills :many
SELECT
  so.skill_id::uuid AS skill_id,
  so.skill_version_id::uuid AS skill_version_id,
  sv.canonical_sha256,
...
WHERE so.project_id = $2
  AND so.session_id = $3::text
  AND so.reconciled_at IS NOT NULL
  AND so.skill_version_id IS NOT NULL
  AND (
...
type ListSkillEfficacyJudgeSkillsParams struct {
	ChatID    uuid.UUID
	ProjectID uuid.UUID
	SessionID string
}

type ListSkillEfficacyJudgeSkillsRow struct {
	SkillID         uuid.UUID
	SkillVersionID  uuid.UUID
	CanonicalSha256 string
	SkillName       string
...
// session alone.
func (q *Queries) ListSkillEfficacyJudgeSkills(ctx context.Context, arg ListSkillEfficacyJudgeSkillsParams) ([]ListSkillEfficacyJudgeSkillsRow, error) {
	rows, err := q.db.Query(ctx, listSkillEfficacyJudgeSkills, arg.ChatID, arg.ProjectID, arg.SessionID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListSkillEfficacyJudgeSkillsRow
	for rows.Next() {
		var i ListSkillEfficacyJudgeSkillsRow
		if err := rows.Scan(
			&i.SkillID,
			&i.SkillVersionID,
			&i.CanonicalSha256,
