...
func TestSkillsFeatureDisabledRejectsEveryEndpoint(t *testing.T) {
	...
	requireOopsCode(t, err, oops.CodeForbidden)
	_, err = ti.service.ApproveSuggestion(ctx, &gen.ApproveSuggestionPayload{ID: suggestion.ID.String(), Content: nil, SessionToken: nil, ApikeyToken: nil, ProjectSlugInput: nil})
	requireOopsCode(t, err, oops.CodeForbidden)
	...
	requireOopsCode(t, err, oops.CodeForbidden)
	err = ti.service.Undistribute(ctx, &gen.UndistributePayload{ID: created.Skill.ID, PluginID: new(uuid.NewString()), SessionToken: nil, ApikeyToken: nil, ProjectSlugInput: nil})
	requireOopsCode(t, err, oops.CodeForbidden)
