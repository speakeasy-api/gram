...
type rejectingUserRequestInterceptor struct {
	calls atomic.Int32
}

func (i *rejectingUserRequestInterceptor) Name() string { return "rejecting-policy" }

func (i *rejectingUserRequestInterceptor) InterceptUserRequest(context.Context, *proxy.UserRequest) error {
	i.calls.Add(1)
	return errors.New("request rejected by policy")
}

func TestToolsCallKillswitchRunsBeforeRejectingUserPolicy(t *testing.T) {
	t.Parallel()
	...
	policy := &rejectingUserRequestInterceptor{}
	...
	p.UserRequestInterceptors = []proxy.UserRequestInterceptor{policy}
	...
	require.Equal(t, int32(1), policy.calls.Load())
