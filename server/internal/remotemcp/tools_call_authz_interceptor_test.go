...
func newAuthzEngineForTest(t *testing.T) *authz.Engine {
	t.Helper()
	return authz.NewEngine(testenv.NewLogger(t), nil, authz.NewNoopChallengePublisher(testenv.NewLogger(t)), authztest.RBACAlwaysEnabled, authztest.ChallengeLoggingAlwaysDisabled, workos.NewStubClient())
}
