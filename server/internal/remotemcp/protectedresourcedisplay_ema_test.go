...
func TestUpdateServer_EMABindingBlocksResourceIdentityChange(t *testing.T) {
	...
	b, err = q.SetEMABinding(ctx, remoterepo.SetEMABindingParams{ID: b.ID, ProjectID: b.ProjectID, OrganizationID: b.OrganizationID, ExpectedGeneration: b.Generation, Generation: b.Generation + 1, RemoteSessionClientID: conv.ToNullUUID(attached.client.ID), State: conv.ToPGText("unknown_grants"), GrantSource: conv.ToPGText("unknown"), RequestedScopes: []string{}})
	require.NoError(t, err)
