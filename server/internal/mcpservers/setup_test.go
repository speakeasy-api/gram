...
func newTestService(t *testing.T) (context.Context, *testInstance) {
	...
	svc := mcpservers.NewService(logger, tracerProvider, conn, sessionManager, authz.NewEngine(logger, conn, authz.NewNoopChallengePublisher(logger), authztest.RBACAlwaysEnabled, authztest.ChallengeLoggingAlwaysDisabled, workos.NewStubClient()), auditLogger, nil, dispositions, false)
...
// seedRemoteMcpServer inserts a remote_mcp_servers row directly through the
// generated repo so we have a valid backend FK for mcp_servers tests.
func seedRemoteMcpServer(t *testing.T, ctx context.Context, conn *pgxpool.Pool, projectID uuid.UUID) uuid.UUID {
	t.Helper()
...
// seedTunneledMcpServer inserts a tunneled_mcp_servers row directly through
