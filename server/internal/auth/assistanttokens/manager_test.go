...
func TestAuthorizeDelegatedUserRevalidatesMembershipAndNeverSubstitutesOwner(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = f.conn.Exec(ctx, `INSERT INTO users (id, email, display_name) VALUES ('assistant-creator', 'creator@example.test', 'Assistant Creator')`)
	require.NoError(t, err)
	_, err = f.conn.Exec(ctx, `UPDATE assistants SET created_by_user_id = 'assistant-creator' WHERE id = $1`, f.assistantID)
	require.NoError(t, err)
	...
	autonomous, err := manager.Generate(GenerateInput{OrgID: "org-test", ProjectID: f.projectID, UserID: "assistant-creator", SessionID: "", AssistantID: f.assistantID, ThreadID: f.threadID, TTL: time.Minute})
	require.NoError(t, err)
