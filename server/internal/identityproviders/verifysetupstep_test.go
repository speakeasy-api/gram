...
func (f *fakeOktaServer) handleToken(w http.ResponseWriter, r *http.Request) {
	...
	mode := f.Mode()
	validScopes := requestedScopes == allScopes || requestedScopes == claimVerificationScopes || requestedScopes == directoryScopes || requestedScopes == applicationScopes || requestedScopes == readScopes || requestedScopes == claimScopes || requestedScopes == "okta.apps.read"
