...
func TestDirectorySetupGuidedLifecycle(t *testing.T) {
	...
	require.Equal(t, []string{"Open the setup portal and configure directory sync for this organization."}, setup.Steps[2].Instructions)
	...
	require.Equal(t, "Okta directory sync is linked and the sign-in provider has received directory groups.", verified.Detail)
	...
	require.Equal(t, "The sign-in provider received 1 directory group(s).", *verified.Evidence.Reads[2].Detail)
	require.Equal(t, "directory_users", verified.Evidence.Reads[3].Resource)
	require.Equal(t, 1, *verified.Evidence.Reads[3].Count)
	require.Equal(t, "The sign-in provider received 1 directory user(s).", *verified.Evidence.Reads[3].Detail)
...
func TestDirectoryVerificationRemainsPendingBeforeFirstGroupPush(t *testing.T) {
	...
	require.Equal(t, "The sign-in provider received 0 directory group(s).", *verified.Evidence.Reads[2].Detail)
	require.Equal(t, "The sign-in provider received 0 directory user(s).", *verified.Evidence.Reads[3].Detail)
	row, err = repo.New(ti.conn).GetIdentityProviderConnectionByOrganization(ctx, ti.orgID)
	require.NoError(t, err)
