...
// the shared *oauthwire.Error; the HTTP handler decides whether
...
func (r *AuthorizationRequest) ValidateRedirectableFields() error {
	if r.ClientID == "" {
		return &oauthwire.Error{Code: "invalid_request", Description: "client_id is required"}
	}
	if r.RedirectURI == "" {
...
func (r *AuthorizationRequest) ValidatePostRedirect() error {
	if !slices.Contains(SupportedResponseTypes, r.ResponseType) {
		return &oauthwire.Error{Code: "unsupported_response_type", Description: fmt.Sprintf("response_type must be one of %v", SupportedResponseTypes)}
	}
	if r.CodeChallenge == "" {
		return &oauthwire.Error{Code: "invalid_request", Description: "code_challenge is required (PKCE mandatory)"}
	}
	if !slices.Contains(SupportedCodeChallengeMethods, r.CodeChallengeMethod) {
		return &oauthwire.Error{Code: "invalid_request", Description: fmt.Sprintf("unsupported code_challenge_method %q", r.CodeChallengeMethod)}
	}
	return nil
}
