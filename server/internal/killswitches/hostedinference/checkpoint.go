...
func NewProductionCheckpoint(db *pgxpool.Pool, registry *killswitches.Registry, timeout time.Duration, meterProvider metric.MeterProvider, logger *slog.Logger) (*Checkpoint, error) {
	evaluation, err := killswitches.NewEvaluator(db, registry, timeout, meterProvider, logger)
	if err != nil {
		return nil, fmt.Errorf("create hosted-inference evaluator: %w", err)
	}
...
func (c *Checkpoint) Check(ctx context.Context, organizationID string) error {
	if c == nil {
		return newInfrastructureUnavailable(errors.New("hosted-inference checkpoint is unavailable"))
	}
...
// MatchedDenialError is a typed matched prescription denial. Error returns the
// exact normalized customer-safe external note.
type MatchedDenialError struct{ externalNote string }

func (e *MatchedDenialError) Error() string        { return e.externalNote }
