...
func (f fakePrincipalAdapter) DeriveCandidates(_ context.Context, org killswitches.OrganizationID, source any) (killswitches.PrincipalCandidateResult, error) {
	...
	return killswitches.NewPrincipalCandidateResult([]killswitches.PrincipalCandidate{{Kind: PrincipalKindUser, Key: killswitches.PrincipalKey(provenance.UserID())}})
}
...
func TestCheckpointDistinguishesMatchedDenialAndInfrastructure(t *testing.T) {
	...
	require.Equal(t, "Paused exactly.", denial.Error())
...
func TestCheckpointClassificationAndIdentityPosture(t *testing.T) {
	...
	for name, build := range map[string]func() (context.Context, string, fakePrincipalAdapter){
		"missing classification": func() (context.Context, string, fakePrincipalAdapter) {
			return t.Context(), "org", fakePrincipalAdapter{active: true}
		},
		"cross tenant": func() (context.Context, string, fakePrincipalAdapter) {
			return governedContext(t, "org-a", "user"), "org-b", fakePrincipalAdapter{active: true}
		},
		"inactive user": func() (context.Context, string, fakePrincipalAdapter) {
			return governedContext(t, "org", "user"), "org", fakePrincipalAdapter{active: false}
		},
		"session-backed JWT cross tenant": func() (context.Context, string, fakePrincipalAdapter) {
			return governedChatContext(t, "org-a", "user"), "org-b", fakePrincipalAdapter{active: true}
		},
		"session-backed JWT inactive user": func() (context.Context, string, fakePrincipalAdapter) {
			return governedChatContext(t, "org", "user"), "org", fakePrincipalAdapter{active: false}
		},
	} {
		t.Run(name, func(t *testing.T) {
			ctx, org, principal := build()
...
func TestClassificationRejectsUnknownAndSubstituteIdentity(t *testing.T) {
	...
	for name, ctx := range map[string]context.Context{
		"api key creator":        contextvalues.SetAuthContext(t.Context(), &contextvalues.AuthContext{ActiveOrganizationID: "org", UserID: "api-key-owner", APIKeyID: "key"}),
		"assistant owner":        contextvalues.SetAuthContext(t.Context(), &contextvalues.AuthContext{ActiveOrganizationID: "org", UserID: "assistant-owner"}),
		"chat session":           contextvalues.SetAuthContext(t.Context(), &contextvalues.AuthContext{ActiveOrganizationID: "org", UserID: "chat-user", SessionID: &chatSessionID}),
		"external identity":      contextvalues.SetAuthContext(t.Context(), &contextvalues.AuthContext{ActiveOrganizationID: "org", ExternalUserID: "external"}),
		"anonymous organization": contextvalues.SetAuthContext(t.Context(), &contextvalues.AuthContext{ActiveOrganizationID: "org"}),
		"shared demo session":    contextvalues.WithValidatedGramSession(t.Context(), &contextvalues.AuthContext{ActiveOrganizationID: constants.DemoOrganizationID, UserID: "user", SessionID: &chatSessionID}, false),
		"absent identity":        t.Context(),
	} {
		t.Run(name, func(t *testing.T) {
			_, err := WithGovernedUser(ctx, CallCategoryUserChatCompletion)
			require.Error(t, err)
		})
	}
}
