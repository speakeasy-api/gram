...
func TestMCPToolExecutionRegistration(t *testing.T) {
	...
	require.ElementsMatch(t, []killswitches.PrincipalKind{PrincipalKindUser, PrincipalKindAgent}, definition.PrincipalKinds)
	...
	agentAdapter, ok := registry.PrincipalAdapter(PrincipalKindAgent)
	require.True(t, ok)
	require.Equal(t, PrincipalKindAgent, agentAdapter.Kind())
