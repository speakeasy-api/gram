...
The `pre_call` callback is the only `ai_access` enforcement checkpoint. It runs before content extraction and provider inference for text, image, tool-only, and textless requests. `BLOCKED` is LiteLLM's acknowledgement that protected work must not continue; there is no separate acknowledgement or cached allow decision. `post_call` remains capture-only. Missing, invalid, stale, cross-tenant, or otherwise unavailable identity/resource state presented to an authenticated callback blocks with a safe unavailable message; revoked or rotated integration keys fail during callback authentication. Evaluator failures follow the registered fail-closed policy. Only a matched prescription returns its selected external note.
