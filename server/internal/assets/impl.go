...
func (s *Service) ServeSkill(ctx context.Context, payload *gen.ServeSkillForm) (*gen.ServeSkillResult, io.ReadCloser, error) {
	authCtx, ok := contextvalues.GetAuthContext(ctx)
	if !ok || authCtx == nil {
		return nil, nil, oops.C(oops.CodeUnauthorized)
	}
	...
	row, err := s.repo.GetSkillAssetURL(ctx, repo.GetSkillAssetURLParams{
		ID:        assetID,
		ProjectID: projectID,
	})
	switch {
	case errors.Is(err, sql.ErrNoRows):
		return nil, nil, oops.C(oops.CodeNotFound)
	case err != nil:
		return nil, nil, oops.E(oops.CodeUnexpected, fmt.Errorf("get skill asset url: %w", err), "error loading asset").Log(ctx, logger)
	}
