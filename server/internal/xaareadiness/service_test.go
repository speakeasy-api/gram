...
import (
	...
	gen "github.com/speakeasy-api/gram/server/gen/xaa_readiness"
...
func list(t *testing.T, ctx context.Context, si *instance, includeAll bool) *gen.ListXaaReadinessResult {
	t.Helper()
	res, err := si.svc.ListReadiness(ctx, &gen.ListReadinessPayload{SessionToken: nil, IncludeAll: includeAll})
	require.NoError(t, err)
	return res
}

func rowFor(t *testing.T, res *gen.ListXaaReadinessResult, serverID uuid.UUID) *gen.XaaServerReadiness {
	t.Helper()
...
func confirm(t *testing.T, ctx context.Context, si *instance, audience string, appID *string, ids ...uuid.UUID) (*gen.ConfirmXaaConnectionsResult, error) {
	t.Helper()
	items := make([]*gen.XaaConnectionConfirmation, 0, len(ids))
	for _, id := range ids {
		items = append(items, &gen.XaaConnectionConfirmation{McpServerID: id.String(), Audience: audience, OktaApplicationID: appID})
	}
	res, err := si.svc.ConfirmConnections(ctx, &gen.ConfirmConnectionsPayload{SessionToken: nil, Connections: items})
	if err != nil {
...
func TestListReadiness_DerivesStatesAndValues(t *testing.T) {
	t.Parallel()
...
func TestListReadiness_ClientBindingStates(t *testing.T) {
	t.Parallel()
	ctx, si := newTestService(t)
	recordAgent(t, ctx, si, "wlp1")
...
func TestListReadiness_UsesAttachedClient(t *testing.T) {
	t.Parallel()
	ctx, si := newTestService(t)
	projectID := createProject(t, ctx, si, si.orgID, "proj")
	issuerID := createResourceIssuer(t, ctx, si, si.orgID, projectID, true)
...
func TestConfirmAndReset(t *testing.T) {
	...
	rowA := rowFor(t, &gen.ListXaaReadinessResult{Servers: confirmed.Servers, PendingCount: 0, TotalCount: 0, UndiscoveredCount: 0, AgentRecorded: true, ConnectionID: nil, DeepLink: nil}, a.serverID)
	...
	reset, err := si.svc.ResetConnection(ctx, &gen.ResetConnectionPayload{SessionToken: nil, McpServerID: a.serverID.String()})
	require.NoError(t, err)
	...
	_, err = si.svc.ResetConnection(ctx, &gen.ResetConnectionPayload{SessionToken: nil, McpServerID: a.serverID.String()})
	...
	requireOopsCode(t, err, oops.CodeBadRequest)
	rows, err := si.q.ListReadinessRows(ctx, repo.ListReadinessRowsParams{OrganizationID: si.orgID, IdentityProviderConnectionID: si.connectionID})
	require.NoError(t, err)
...
func TestConfirm_RejectsRemovedApplication(t *testing.T) {
	...
	_, err = si.svc.ResetConnection(ctx, &gen.ResetConnectionPayload{SessionToken: nil, McpServerID: server.serverID.String()})
	require.NoError(t, err)
	_, err = confirm(t, ctx, si, audience, &appID, server.serverID)
	requireOopsCode(t, err, oops.CodeBadRequest)
...
func TestSharedUpstream_OneRowManyServers(t *testing.T) {
	...
	_, err = si.svc.ResetConnection(ctx, &gen.ResetConnectionPayload{SessionToken: nil, McpServerID: twin.String()})
	requireOopsCode(t, err, oops.CodeFailedPrecondition)
...
func TestReadiness_Guards(t *testing.T) {
	...
	requireOopsCode(t, err, oops.CodeNotFound)
	_, err = si.svc.ConfirmConnections(ctx, &gen.ConfirmConnectionsPayload{SessionToken: nil, Connections: []*gen.XaaConnectionConfirmation{{McpServerID: "nope", Audience: audience, OktaApplicationID: nil}}})
	requireOopsCode(t, err, oops.CodeBadRequest)
	_, err = si.svc.ConfirmConnections(ctx, &gen.ConfirmConnectionsPayload{SessionToken: nil, Connections: []*gen.XaaConnectionConfirmation{nil}})
	requireOopsCode(t, err, oops.CodeBadRequest)
	...
	requireOopsCode(t, err, oops.CodeNotFound)
	_, err = si.svc.ResetConnection(otherCtx, &gen.ResetConnectionPayload{SessionToken: nil, McpServerID: f.serverID.String()})
	requireOopsCode(t, err, oops.CodeNotFound)
	...
	_, err = si.svc.ListReadiness(ctx, &gen.ListReadinessPayload{SessionToken: nil, IncludeAll: false})
	requireOopsCode(t, err, oops.CodeFailedPrecondition)
}
...
func TestConfirm_RequiresVerifiedConnection(t *testing.T) {
	...
	_, err = confirm(t, ctx, si, audience, nil, f.serverID)
	requireOopsCode(t, err, oops.CodeFailedPrecondition)
	_, err = si.svc.ResetConnection(ctx, &gen.ResetConnectionPayload{SessionToken: nil, McpServerID: f.serverID.String()})
	requireOopsCode(t, err, oops.CodeFailedPrecondition)
}

func TestDeleteConnectionReadiness(t *testing.T) {
	t.Parallel()
	ctx, si := newTestService(t)
	recordAgent(t, ctx, si, "wlp1")
	...
	require.NoError(t, xaareadiness.DeleteConnectionReadiness(ctx, si.conn, si.orgID, si.connectionID))
	rows, err = si.q.ListReadinessRows(ctx, repo.ListReadinessRowsParams{OrganizationID: si.orgID, IdentityProviderConnectionID: si.connectionID})
	require.NoError(t, err)
