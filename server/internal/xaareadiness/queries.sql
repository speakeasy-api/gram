...
-- Readiness rows of a connection with the label of the identity provider app
-- instance the administrator picked, when the snapshot still has it.
-- name: ListReadinessRows :many
SELECT
    sqlc.embed(r)
...
-- name: GetReadinessRowForUpdate :one
SELECT *
FROM okta_resource_connections
WHERE organization_id = @organization_id
  AND identity_provider_connection_id = @identity_provider_connection_id
...
-- Revocation deletes the connection's readiness rows outright.
-- name: DeleteReadinessForConnection :execrows
DELETE FROM okta_resource_connections
WHERE organization_id = @organization_id
  AND identity_provider_connection_id = @identity_provider_connection_id;
