...
const confirmConnection = `-- name: ConfirmConnection :one
...
type ConfirmConnectionParams struct {
...
func (q *Queries) ConfirmConnection(ctx context.Context, arg ConfirmConnectionParams) (OktaResourceConnection, error) {
	row := q.db.QueryRow(ctx, confirmConnection,
		arg.OrganizationID,
...
type DeleteReadinessForConnectionParams struct {
	OrganizationID               string
	IdentityProviderConnectionID uuid.UUID
}

// Revocation deletes the connection's readiness rows outright.
func (q *Queries) DeleteReadinessForConnection(ctx context.Context, arg DeleteReadinessForConnectionParams) (int64, error) {
	result, err := q.db.Exec(ctx, deleteReadinessForConnection, arg.OrganizationID, arg.IdentityProviderConnectionID)
	if err != nil {
		return 0, err
	}
...
const getReadinessRowForUpdate = `-- name: GetReadinessRowForUpdate :one
...
type GetReadinessRowForUpdateParams struct {
	OrganizationID               string
	IdentityProviderConnectionID uuid.UUID
	RemoteSessionIssuerID        uuid.UUID
	Resource                     string
...
func (q *Queries) GetReadinessRowForUpdate(ctx context.Context, arg GetReadinessRowForUpdateParams) (OktaResourceConnection, error) {
	row := q.db.QueryRow(ctx, getReadinessRowForUpdate,
		arg.OrganizationID,
		arg.IdentityProviderConnectionID,
		arg.RemoteSessionIssuerID,
		arg.Resource,
...
const listReadinessRows = `-- name: ListReadinessRows :many
SELECT
...
type ListReadinessRowsParams struct {
	OrganizationID               string
	IdentityProviderConnectionID uuid.UUID
}

type ListReadinessRowsRow struct {
...
func (q *Queries) ListReadinessRows(ctx context.Context, arg ListReadinessRowsParams) ([]ListReadinessRowsRow, error) {
	rows, err := q.db.Query(ctx, listReadinessRows, arg.OrganizationID, arg.IdentityProviderConnectionID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListReadinessRowsRow
	for rows.Next() {
		var i ListReadinessRowsRow
		if err := rows.Scan(
...
const resetConnection = `-- name: ResetConnection :one
DELETE FROM okta_resource_connections
WHERE organization_id = $1
  AND identity_provider_connection_id = $2
  AND remote_session_issuer_id = $3
...
type ResetConnectionParams struct {
	OrganizationID               string
	IdentityProviderConnectionID uuid.UUID
	RemoteSessionIssuerID        uuid.UUID
	Resource                     string
...
func (q *Queries) ResetConnection(ctx context.Context, arg ResetConnectionParams) (OktaResourceConnection, error) {
	row := q.db.QueryRow(ctx, resetConnection,
		arg.OrganizationID,
		arg.IdentityProviderConnectionID,
		arg.RemoteSessionIssuerID,
		arg.Resource,
