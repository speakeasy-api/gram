...
import (
	...
	gen "github.com/speakeasy-api/gram/server/gen/http/xaa_readiness/server"
	srv "github.com/speakeasy-api/gram/server/gen/xaa_readiness"
...
func NewService(
	...
	logger = logger.With(attr.SlogComponent("xaareadiness"))
	return &Service{
		tracer:   tracerProvider.Tracer("github.com/speakeasy-api/gram/server/internal/xaareadiness"),
		logger:   logger,
		db:       db,
...
// DeleteConnectionReadiness removes every resource connection recorded for an
// identity provider connection; the revoke path calls it inside its own
// transaction.
func DeleteConnectionReadiness(ctx context.Context, dbtx repo.DBTX, organizationID string, connectionID uuid.UUID) error {
	_, err := repo.New(dbtx).DeleteReadinessForConnection(ctx, repo.DeleteReadinessForConnectionParams{OrganizationID: organizationID, IdentityProviderConnectionID: connectionID})
	if err != nil {
...
func (s *Service) authorize(ctx context.Context, mutation bool) (*contextvalues.AuthContext, *slog.Logger, error) {
	...
	if mutation {
		if mode, byKey := contextvalues.APIKeyAuthorization(ctx); byKey && mode != contextvalues.APIKeyAuthorizationModePrincipal {
			return nil, logger, oops.E(oops.CodeForbidden, nil, "readiness cannot be changed with an API key").LogWarn(ctx, logger)
		}
		if contextvalues.IsSupportSession(ctx) {
			return nil, logger, oops.E(oops.CodeForbidden, nil, "readiness cannot be changed from a support session").LogWarn(ctx, logger)
		}
	}
...
func (s *Service) load(ctx context.Context, logger *slog.Logger, organizationID string) (*snapshot, error) {
	...
	records, err := q.ListReadinessRows(ctx, repo.ListReadinessRowsParams{OrganizationID: organizationID, IdentityProviderConnectionID: connection.ID})
	if err != nil {
...
func (s *Service) ListReadiness(ctx context.Context, payload *srv.ListReadinessPayload) (*srv.ListXaaReadinessResult, error) {
	...
	servers := make([]*srv.XaaServerReadiness, 0, len(snap.rows))
	...
	return &srv.ListXaaReadinessResult{
		Servers:           servers,
...
func parseConfirmations(items []*srv.XaaConnectionConfirmation) ([]confirmation, error) {
...
func (s *Service) ConfirmConnections(ctx context.Context, payload *srv.ConfirmConnectionsPayload) (*srv.ConfirmXaaConnectionsResult, error) {
	...
	out := make([]*srv.XaaServerReadiness, 0, len(items))
	for _, item := range items {
		...
		after, err := q.ConfirmConnection(ctx, repo.ConfirmConnectionParams{
	...
	return &srv.ConfirmXaaConnectionsResult{Servers: out}, nil
}
...
func readinessForUpdate(ctx context.Context, q *repo.Queries, organizationID string, connectionID, issuerID uuid.UUID, resource string) (*record, error) {
	rc, err := q.GetReadinessRowForUpdate(ctx, repo.GetReadinessRowForUpdateParams{OrganizationID: organizationID, IdentityProviderConnectionID: connectionID, RemoteSessionIssuerID: issuerID, Resource: resource})
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, nil
	}
	if err != nil {
...
func (s *Service) ResetConnection(ctx context.Context, payload *srv.ResetConnectionPayload) (*srv.XaaServerReadiness, error) {
	authCtx, logger, err := s.authorize(ctx, true)
	if err != nil {
		return nil, err
	}
	...
	if _, err := q.ResetConnection(ctx, repo.ResetConnectionParams{OrganizationID: authCtx.ActiveOrganizationID, IdentityProviderConnectionID: snap.connection.ID, RemoteSessionIssuerID: server.IssuerID, Resource: resource}); err != nil {
...
func buildRow(snap *snapshot, r row) *srv.XaaServerReadiness {
	out := &srv.XaaServerReadiness{
...
func (s *Service) auditEvent(authCtx *contextvalues.AuthContext, principal urn.Principal, snap *snapshot, r row, before, after *record) audit.LogOktaResourceConnectionEvent {
	...
	return audit.LogOktaResourceConnectionEvent{
		...
		ActorSlug:        nil,
		ReadinessURN:     urn.NewOktaResourceConnection(subject.ID),
		ServerName:       r.server.Name.String,
		ServerSlug:       r.server.Slug.String,
