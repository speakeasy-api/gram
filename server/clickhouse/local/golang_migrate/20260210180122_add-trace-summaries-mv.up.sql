...
PRIMARY KEY (`gram_project_id`, `trace_id`) ORDER BY (`gram_project_id`, `trace_id`) TTL fromUnixTimestamp64Nano(start_time_unix_nano) + toIntervalDay(30) SETTINGS index_granularity = 8192 COMMENT 'Pre-aggregated trace summaries for fast trace-level queries without needing to scan all logs';
