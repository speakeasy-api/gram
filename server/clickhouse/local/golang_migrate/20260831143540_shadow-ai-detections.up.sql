...
CREATE TABLE `ai_detections` (
  ...
  `signal` LowCardinality(String) COMMENT 'What the scan observed: installed or running.',
  `category` LowCardinality(String) COMMENT 'Category of the target at write time: harness or local_model. From the server catalog for targets it knows, as reported otherwise.',
...
CREATE TABLE `ai_scan_receipts` (
  ...
  `match_count` UInt32 COMMENT 'Number of matches the server accepted from this scan. Zero-match scans still post a receipt so coverage is provable.',
  `received_at` DateTime64(9, 'UTC') COMMENT 'When the server received the report.'
) ENGINE = MergeTree
PRIMARY KEY (`organization_id`, `device_serial`, `received_at`) ORDER BY (`organization_id`, `device_serial`, `received_at`) TTL toDateTime(received_at) + toIntervalDay(90) SETTINGS index_granularity = 8192 COMMENT 'Append-only receipts for device-agent AI scans, proving a device scanned even when nothing matched';
