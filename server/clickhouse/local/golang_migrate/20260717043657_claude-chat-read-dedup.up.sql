-- create long-lived Claude Chat event storage
...
-- atomically remove Claude Chat from the row-summing attribute aggregate
...
-- atomically exclude Claude Chat from the legacy row-summing aggregate
ALTER TABLE `metrics_summaries_mv` MODIFY QUERY
SELECT gram_project_id, toStartOfHour(fromUnixTimestamp64Nano(time_unix_nano)) AS time_bucket, min(time_unix_nano) AS first_seen_unix_nano, max(time_unix_nano) AS last_seen_unix_nano, uniqExactIfState(toString(attributes.gen_ai.conversation.id), toString(attributes.gen_ai.conversation.id) != '') AS total_chats, uniqExactIfState(toString(attributes.gen_ai.response.model), toString(attributes.gen_ai.response.model) != '') AS distinct_models, uniqExactIfState(toString(attributes.gen_ai.provider.name), toString(attributes.gen_ai.provider.name) != '') AS distinct_providers, sumIfState(toInt64OrZero(toString(attributes.gen_ai.usage.input_tokens)), toString(attributes.gen_ai.usage.input_tokens) != '') AS total_input_tokens, sumIfState(toInt64OrZero(toString(attributes.gen_ai.usage.output_tokens)), toString(attributes.gen_ai.usage.output_tokens) != '') AS total_output_tokens, sumIfState(toInt64OrZero(toString(attributes.gen_ai.usage.total_tokens)), toString(attributes.gen_ai.usage.total_tokens) != '') AS total_tokens, sumIfState(toInt64OrZero(toString(attributes.gen_ai.usage.cache_read.input_tokens)), toString(attributes.gen_ai.usage.cache_read.input_tokens) != '') AS cache_read_input_tokens, sumIfState(toInt64OrZero(toString(attributes.gen_ai.usage.cache_creation.input_tokens)), toString(attributes.gen_ai.usage.cache_creation.input_tokens) != '') AS cache_creation_input_tokens, sumIfState(toFloat64OrZero(toString(attributes.gen_ai.usage.cost)), toString(attributes.gen_ai.usage.cost) != '') AS total_cost, avgIfState(toFloat64OrZero(toString(attributes.gen_ai.usage.total_tokens)), toString(attributes.gen_ai.conversation.id) != '') AS avg_tokens_per_request, countIfState(toString(attributes.gen_ai.conversation.id) != '') AS total_chat_requests, avgIfState(toFloat64OrZero(toString(attributes.gen_ai.conversation.duration)) * 1000, toString(attributes.gen_ai.conversation.id) != '') AS avg_chat_duration_ms, countIfState(position(toString(attributes.gen_ai.response.finish_reasons), 'stop') > 0) AS finish_reason_stop, countIfState(position(toString(attributes.gen_ai.response.finish_reasons), 'tool_calls') > 0) AS finish_reason_tool_calls, countIfState(startsWith(toString(attributes.gram.tool.urn), 'tools:')) AS total_tool_calls, countIfState(startsWith(toString(attributes.gram.tool.urn), 'tools:') AND (toInt32OrZero(toString(attributes.http.response.status_code)) >= 200) AND (toInt32OrZero(toString(attributes.http.response.status_code)) < 300)) AS tool_call_success, countIfState(startsWith(toString(attributes.gram.tool.urn), 'tools:') AND (toInt32OrZero(toString(attributes.http.response.status_code)) >= 400)) AS tool_call_failure, avgIfState(toFloat64OrZero(toString(attributes.http.server.request.duration)) * 1000, startsWith(toString(attributes.gram.tool.urn), 'tools:')) AS avg_tool_duration_ms, countIfState(evaluation_score_label = 'success') AS chat_resolution_success, countIfState(evaluation_score_label = 'failure') AS chat_resolution_failure, countIfState(evaluation_score_label = 'partial') AS chat_resolution_partial, countIfState(evaluation_score_label = 'abandoned') AS chat_resolution_abandoned, avgIfState(toFloat64OrZero(toString(attributes.gen_ai.evaluation.score.value)), evaluation_score_label != '') AS avg_chat_resolution_score, uniqExactIfState(toString(attributes.gen_ai.conversation.id), (toString(attributes.gen_ai.conversation.id) != '') AND (evaluation_score_label != '')) AS evaluated_chats, uniqExactIfState(toString(attributes.gen_ai.conversation.id), (toString(attributes.gen_ai.conversation.id) != '') AND (evaluation_score_label = 'success')) AS resolved_chats, uniqExactIfState(toString(attributes.gen_ai.conversation.id), (toString(attributes.gen_ai.conversation.id) != '') AND (evaluation_score_label = 'failure')) AS failed_chats, avgIfState(toFloat64OrZero(toString(attributes.gen_ai.conversation.duration)) * 1000, evaluation_score_label = 'success') AS avg_resolution_time_ms, sumMapIfState(map(toString(attributes.gen_ai.response.model), toUInt64(1)), (toString(attributes.gen_ai.conversation.id) != '') AND (toString(attributes.gen_ai.response.model) != '')) AS models, sumMapIfState(map(gram_urn, toUInt64(1)), startsWith(gram_urn, 'tools:')) AS tool_counts, sumMapIfState(map(gram_urn, toUInt64(1)), startsWith(gram_urn, 'tools:') AND (toInt32OrZero(toString(attributes.http.response.status_code)) >= 200) AND (toInt32OrZero(toString(attributes.http.response.status_code)) < 300)) AS tool_success_counts, sumMapIfState(map(gram_urn, toUInt64(1)), startsWith(gram_urn, 'tools:') AND (toInt32OrZero(toString(attributes.http.response.status_code)) >= 400)) AS tool_failure_counts FROM telemetry_logs WHERE NOT startsWith(gram_urn, 'claude_chat:') GROUP BY gram_project_id, time_bucket;

-- create "claude_chat_metrics_mv" view
CREATE MATERIALIZED VIEW `claude_chat_metrics_mv` TO `claude_chat_metrics` AS SELECT gram_project_id, toString(attributes.claude_chat.event_hash) AS event_hash, toStartOfHour(fromUnixTimestamp64Nano(time_unix_nano)) AS time_bucket, observed_time_unix_nano, toString(attributes.user.attributes.department_name) AS department_name, toString(attributes.user.attributes.job_title) AS job_title, toString(attributes.user.attributes.employee_type) AS employee_type, toString(attributes.user.attributes.division_name) AS division_name, toString(attributes.user.attributes.cost_center_name) AS cost_center_name, user_email, toString(attributes.gen_ai.response.model) AS model, hook_source, arraySort(JSONExtract(ifNull(toJSONString(attributes.user.roles), '[]'), 'Array(String)')) AS roles, arraySort(JSONExtract(ifNull(toJSONString(attributes.user.groups), '[]'), 'Array(String)')) AS groups, toInt64OrZero(toString(attributes.gen_ai.usage.input_tokens)) AS total_input_tokens, toInt64OrZero(toString(attributes.gen_ai.usage.output_tokens)) AS total_output_tokens, (toInt64OrZero(toString(attributes.gen_ai.usage.input_tokens)) + toInt64OrZero(toString(attributes.gen_ai.usage.output_tokens))) + toInt64OrZero(toString(attributes.gen_ai.usage.cache_creation.input_tokens)) AS total_tokens, toInt64OrZero(toString(attributes.gen_ai.usage.cache_read.input_tokens)) AS cache_read_input_tokens, toInt64OrZero(toString(attributes.gen_ai.usage.cache_creation.input_tokens)) AS cache_creation_input_tokens, toFloat64OrZero(toString(attributes.gen_ai.usage.cost)) AS total_cost, account_type, provider, billing_mode, '' AS query_source, '' AS skill_name, '' AS agent_name, '' AS mcp_server_name, '' AS mcp_tool_name FROM telemetry_logs WHERE (gram_urn IN ('claude_chat:usage:metrics', 'claude_chat:cost:metrics')) AND (toString(attributes.claude_chat.event_hash) != '');

-- create the aggregate-state-compatible deduplicated read view
CREATE VIEW `deduplicated_attribute_metrics_summaries` (
  ...
  `mcp_server_name` String,
  `mcp_tool_name` String
) AS SELECT gram_project_id, time_bucket, department_name, job_title, employee_type, division_name, cost_center_name, user_email, model, hook_source, roles, groups, total_chats, total_input_tokens, total_output_tokens, total_tokens, cache_read_input_tokens, cache_creation_input_tokens, total_cost, total_tool_calls, unique_tool_calls, account_type, provider, billing_mode, query_source, skill_name, agent_name, mcp_server_name, mcp_tool_name FROM attribute_metrics_summaries WHERE is_active = 1 UNION ALL SELECT gram_project_id, time_bucket, department_name, job_title, employee_type, division_name, cost_center_name, user_email, model, hook_source, roles, groups, uniqExactIfState(toString(''), toUInt8(0)) AS total_chats, sumIfState(total_input_tokens, toUInt8(1)) AS total_input_tokens, sumIfState(total_output_tokens, toUInt8(1)) AS total_output_tokens, sumIfState(total_tokens, toUInt8(1)) AS total_tokens, sumIfState(cache_read_input_tokens, toUInt8(1)) AS cache_read_input_tokens, sumIfState(cache_creation_input_tokens, toUInt8(1)) AS cache_creation_input_tokens, sumIfState(total_cost, toUInt8(1)) AS total_cost, countIfState(toUInt8(0)) AS total_tool_calls, uniqExactIfState(toString(''), toUInt8(0)) AS unique_tool_calls, account_type, provider, billing_mode, query_source, skill_name, agent_name, mcp_server_name, mcp_tool_name FROM claude_chat_metrics FINAL GROUP BY gram_project_id, event_hash, time_bucket, department_name, job_title, employee_type, division_name, cost_center_name, user_email, model, hook_source, roles, groups, account_type, provider, billing_mode, query_source, skill_name, agent_name, mcp_server_name, mcp_tool_name;
