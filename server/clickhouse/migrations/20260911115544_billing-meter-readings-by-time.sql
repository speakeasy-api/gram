...
PRIMARY KEY (`organization_id`, `meter_id`, `occurred_at`) ORDER BY (`organization_id`, `meter_id`, `occurred_at`, `project_id`, `id`) PARTITION BY (toYYYYMM(occurred_at)) SETTINGS index_granularity = 8192 COMMENT 'Time-windowed accepted usage facts and separate adjustments requiring duplicate-safe ingestion';
