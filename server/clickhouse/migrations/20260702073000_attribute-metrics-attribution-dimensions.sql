-- Hand-edited (not atlas's DROP+recreate): add Claude attribution columns as
-- sort-key dimensions to "attribute_metrics_summaries" non-destructively.
-- ADD COLUMN and MODIFY ORDER BY must be one ALTER statement because ClickHouse
-- only lets MODIFY ORDER BY extend the key with columns added in the same ALTER.
ALTER TABLE `attribute_metrics_summaries`
  ...
  MODIFY ORDER BY (`gram_project_id`, `time_bucket`, `department_name`, `job_title`, `employee_type`, `division_name`, `cost_center_name`, `user_email`, `model`, `hook_source`, `roles`, `groups`, `account_type`, `provider`, `billing_mode`, `query_source`, `skill_name`, `agent_name`, `mcp_server_name`, `mcp_tool_name`);
-- Recreate the MV so Claude cost/token rows come from api_request events and
-- attribution dimensions are populated only on those rows.
...
GROUP BY gram_project_id, time_bucket, department_name, job_title, employee_type, division_name, cost_center_name, user_email, model, hook_source, roles, groups, account_type, provider, billing_mode, query_source, skill_name, agent_name, mcp_server_name, mcp_tool_name;
