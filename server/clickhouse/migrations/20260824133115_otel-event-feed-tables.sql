...
CREATE TABLE `otel_logs` (
  `record_id` String COMMENT 'Edge-assigned unique id for the log record (gram.otel.v1.LogRecord.record_id). Pub/Sub delivery is at-least-once, so this is the ReplacingMergeTree dedup key.' CODEC(ZSTD(1)),
  ...
  `time_unix_nano` Int64 COMMENT 'Unix time (ns) when the event occurred. The writer substitutes observed_time_unix_nano (then write time) when the producer sent 0, so this is never epoch zero.' CODEC(Delta(8), ZSTD(1)),
...
PRIMARY KEY (`organization_id`, `time_unix_nano`, `record_id`) ORDER BY (`organization_id`, `time_unix_nano`, `record_id`) PARTITION BY (toYYYYMMDD(fromUnixTimestamp64Nano(time_unix_nano))) TTL fromUnixTimestamp64Nano(time_unix_nano) + toIntervalDay(90) SETTINGS index_granularity = 8192 COMMENT 'Normalized OTel log records teed off the gram.otel.v1.LogRecord topic after transform/enrichment, powering the org-scoped Event Feed. Pub/Sub redelivery is deduped by the sort key via ReplacingMergeTree.';
-- Create "otel_traces" table
CREATE TABLE `otel_traces` (
  `record_id` String COMMENT 'Dedup key derived by the writer as hex(trace_id):hex(span_id). Stable across Pub/Sub redelivery, so ReplacingMergeTree dedups on it.' CODEC(ZSTD(1)),
...
PRIMARY KEY (`organization_id`, `time_unix_nano`, `record_id`) ORDER BY (`organization_id`, `time_unix_nano`, `record_id`) PARTITION BY (toYYYYMMDD(fromUnixTimestamp64Nano(time_unix_nano))) TTL fromUnixTimestamp64Nano(time_unix_nano) + toIntervalDay(90) SETTINGS index_granularity = 8192 COMMENT 'Normalized OTel spans teed off the gram.otel.v1.Span topic after transform/enrichment, powering the org-scoped Event Feed. Pub/Sub redelivery is deduped by the sort key via ReplacingMergeTree.';
