...
-- ============================================================================
-- Tenant-scoped backfill staging for attribute_metrics_summaries.
--
-- Standard ClickHouse Null-table backfill pattern: the Temporal backfill
-- workflow (BackfillAttributeMetricsSummariesWorkflow) replays a tenant's raw
-- telemetry_logs rows through telemetry_logs_backfill_feed (ENGINE = Null, no
-- storage). The attached backfill MV — the same transform as the live MV,
-- minus the time cutoff — rebuilds their summaries into
-- attribute_metrics_summaries_backfill. After the operator validates staging
-- and signals approval, the workflow archives the tenant's live rows, deletes
-- them, and swaps in the staged rows. Pre-commit rollback is just clearing
-- staging; post-commit rollback restores from the archive.
-- ============================================================================

-- Column-for-column clone of telemetry_logs (ordinary + materialized columns,
-- in the same order, so `INSERT INTO ... SELECT * FROM telemetry_logs` maps
-- 1:1 and materialized columns are recomputed at insert). ENGINE = Null stores
-- nothing; inserted blocks only flow through attached backfill MVs. Keep in
-- sync with telemetry_logs when adding columns the summary MVs read.
CREATE TABLE IF NOT EXISTS telemetry_logs_backfill_feed (
...
COMMENT 'Null-engine replay feed for telemetry_logs backfills. Inserts store nothing and only flow through attached backfill materialized views.';

-- Staging target: identical schema to attribute_metrics_summaries so
-- `INSERT INTO attribute_metrics_summaries SELECT * FROM ...backfill` maps
-- 1:1 at commit time and merge semantics match during validation.
CREATE TABLE IF NOT EXISTS attribute_metrics_summaries_backfill (
    gram_project_id UUID,
    time_bucket DateTime('UTC'),
    department_name String,
    job_title String,
...
PRIMARY KEY (gram_project_id, time_bucket, department_name, job_title, employee_type, division_name, cost_center_name, user_email, model, hook_source, roles, groups)
ORDER BY (gram_project_id, time_bucket, department_name, job_title, employee_type, division_name, cost_center_name, user_email, model, hook_source, roles, groups, account_type, provider, billing_mode, query_source, skill_name, agent_name, mcp_server_name, mcp_tool_name)
-- Staging rows are transient (cleared by the workflow); the TTL is a backstop
-- against abandoned runs leaving data behind forever. It is keyed on
-- time_bucket like the live table, so it must match the live 730-day window —
-- a shorter TTL would immediately expire staged historical buckets.
TTL time_bucket + INTERVAL 730 DAY
SETTINGS index_granularity = 8192
COMMENT 'Staging target for tenant-scoped attribute_metrics_summaries backfills. Populated via telemetry_logs_backfill_feed, validated, then swapped into the live table.';

-- Same transform as attribute_metrics_summaries_mv but reading from the
-- Null-engine feed and without the ingest-time cutoff: the backfill workflow
-- controls the replay window by what it inserts into the feed. Keep the
-- SELECT in sync with the live MV above.
CREATE MATERIALIZED VIEW IF NOT EXISTS attribute_metrics_summaries_backfill_mv TO attribute_metrics_summaries_backfill AS
WITH
    (gram_urn = 'claude-code:otel:logs') AS is_claude_otel_row,
    (
        is_claude_otel_row
...
SELECT
    ...
    if(is_claude_api_request, toString(attributes.mcp_server.name), '') AS mcp_server_name,
    if(is_claude_api_request, toString(attributes.mcp_tool.name), '') AS mcp_tool_name
FROM telemetry_logs_backfill_feed
WHERE is_claude_api_request OR is_claude_tool_result OR is_agent_usage_row OR is_agent_tool_call
GROUP BY
    gram_project_id,
    time_bucket,
    department_name,
...
-- Pre-delete snapshots of the live rows each backfill run replaces, keyed by
-- the Temporal workflow run id so a committed backfill can be rolled back.
CREATE TABLE IF NOT EXISTS attribute_metrics_summaries_backfill_archive (
...
ORDER BY (backfill_run_id, gram_project_id, time_bucket)
-- Archives only need to survive long enough to decide a committed backfill was
-- good; 90 days matches the raw-log window that could rebuild them anyway.
TTL archived_at + INTERVAL 90 DAY
SETTINGS index_granularity = 8192
COMMENT 'Pre-delete snapshots of attribute_metrics_summaries rows replaced by tenant backfills, keyed by Temporal workflow run id for post-commit rollback.';
