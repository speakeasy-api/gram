...
CREATE TABLE IF NOT EXISTS telemetry_logs (
    ...
    external_user_id Nullable(String) MATERIALIZED nullIf(toString(attributes.`gram.external_user.id`), '') COMMENT 'External user ID (materialized from attributes.gram.external_user.id).'
) ENGINE = MergeTree
...
-- Indexes for new materialized columns
CREATE INDEX IF NOT EXISTS idx_telemetry_logs_new_deployment_id ON telemetry_logs (deployment_id) TYPE bloom_filter(0.01) GRANULARITY 1;
CREATE INDEX IF NOT EXISTS idx_telemetry_logs_new_function_id ON telemetry_logs (function_id) TYPE bloom_filter(0.01) GRANULARITY 1;
CREATE INDEX IF NOT EXISTS idx_telemetry_logs_new_urn ON telemetry_logs (urn) TYPE bloom_filter(0.01) GRANULARITY 1;
CREATE INDEX IF NOT EXISTS idx_telemetry_logs_new_chat_id ON telemetry_logs (chat_id) TYPE bloom_filter(0.01) GRANULARITY 1;
CREATE INDEX IF NOT EXISTS idx_telemetry_logs_user_id ON telemetry_logs (user_id) TYPE bloom_filter(0.01) GRANULARITY 1;
CREATE INDEX IF NOT EXISTS idx_telemetry_logs_external_user_id ON telemetry_logs (external_user_id) TYPE bloom_filter(0.01) GRANULARITY 1;