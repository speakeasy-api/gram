// Code generated by goa v3.25.3, DO NOT EDIT.
//
// HTTP request path constructors for the mdm service.
//
// Command:
// $ goa gen github.com/speakeasy-api/gram/server/design -o server

package client

// GetInstallScriptMdmPath returns the URL path to the mdm service getInstallScript HTTP endpoint.
func GetInstallScriptMdmPath() string {
	return "/rpc/mdm.getInstallScript"
}

// GenerateMDMScriptMdmPath returns the URL path to the mdm service generateMDMScript HTTP endpoint.
func GenerateMDMScriptMdmPath() string {
	return "/rpc/mdm.generateMDMScript"
}

// PatchClaudeSettingsMdmPath returns the URL path to the mdm service patchClaudeSettings HTTP endpoint.
func PatchClaudeSettingsMdmPath() string {
	return "/rpc/mdm.patchClaudeSettings"
}
