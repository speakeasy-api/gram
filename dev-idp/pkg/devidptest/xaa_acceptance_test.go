...
// an ID-JAG. Nothing about this leg is XAA-specific, which is the point:
...
// TestXAA_LoginToMintToRedeem walks the whole cross-app access flow across
...
func TestXAA_LoginToMintToRedeem(t *testing.T) {
	t.Parallel()

	inst := devidptest.Launch(t, devidptest.LaunchOpts{EnableWorkOS: false, Key: nil})
	ctx := t.Context()

	app := devidptest.CreateXaaApp(t, ctx, inst.Repo, devidptest.XaaAppOpts{
		ClientID: acceptanceClientID, ClientSecret: "", Name: "", Disabled: false,
	})
	resource := devidptest.CreateXaaResource(t, ctx, inst.Repo, devidptest.XaaResourceOpts{
		Slug: acceptanceResourceSlug, Name: "Chat", ResourceIdentifier: acceptanceResourceID,
	})
	devidptest.AssignXaaApp(t, ctx, inst.Repo, app, inst.DefaultUser.ID, resource.ID, "chat.read chat.history")
	devidptest.TrustXaaIssuer(t, ctx, inst.Repo, resource.ID, inst.OAuth21URL, devidptest.XaaTrustRuleOpts{
	...
	status, body := postForm(t, inst.OAuth21URL+"/token", url.Values{
		"grant_type":           {xaa.GrantTypeTokenExchange},
		"requested_token_type": {xaa.TokenTypeIDJAG},
		"audience":             {audience},
		"resource":             {acceptanceResourceID},
		"scope":                {"chat.read"},
		"subject_token":        {idToken},
		"subject_token_type":   {xaa.TokenTypeIDToken},
	...
	require.Equal(t, xaa.TokenTypeIDJAG, minted.IssuedTokenType)
	...
	status, body = postForm(t, audience+"/token", url.Values{
		"grant_type": {xaa.GrantTypeJWTBearer},
...
// TestXAA_DiscoveryAdvertisesBothHalves covers what an MCP client reads
...
func TestXAA_DiscoveryAdvertisesBothHalves(t *testing.T) {
	t.Parallel()

	inst := devidptest.Launch(t, devidptest.LaunchOpts{EnableWorkOS: false, Key: nil})
	devidptest.CreateXaaResource(t, t.Context(), inst.Repo, devidptest.XaaResourceOpts{
		Slug: acceptanceResourceSlug, Name: "Chat", ResourceIdentifier: acceptanceResourceID,
	})
	...
	require.Contains(t, idp.GrantTypesSupported, xaa.GrantTypeTokenExchange)
	require.Contains(t, idp.RequestedTokenTypes, xaa.TokenTypeIDJAG)
	...
	require.Contains(t, resourceAS.GrantTypesSupported, xaa.GrantTypeJWTBearer)
	require.Contains(t, resourceAS.AuthorizationGrantProfilesSupported, xaa.GrantProfileIDJAG)
}
