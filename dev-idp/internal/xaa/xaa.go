// Package xaa carries the wire vocabulary shared by the two halves of the
// cross-app access flow: the IdP that mints an Identity Assertion JWT
// Authorization Grant (ID-JAG) and the resource authorization server that
// redeems one.
//
// The flow is the MCP Enterprise-Managed Authorization profile of
