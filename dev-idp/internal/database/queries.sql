...
WHERE u.id > @after
  AND (sqlc.narg('email') IS NULL OR u.email = sqlc.narg('email'))
  AND (sqlc.narg('organization_id') IS NULL OR EXISTS (
    SELECT 1 FROM memberships m
    WHERE m.user_id = u.id AND m.organization_id = sqlc.narg('organization_id')
  ))
ORDER BY u.id ASC
LIMIT @max_rows;
...
WHERE m.id > @after
  AND (sqlc.narg('user_id') IS NULL OR m.user_id = sqlc.narg('user_id'))
  AND (sqlc.narg('organization_id') IS NULL OR m.organization_id = sqlc.narg('organization_id'))
ORDER BY m.id ASC
LIMIT @max_rows;
...
VALUES (@id, @email, @organization_id, @token, sqlc.narg('inviter_user_id'), @expires_at)
RETURNING *;
...
SET
  state = 'revoked',
  revoked_at = COALESCE(revoked_at, @ts),
  updated_at = @ts
WHERE id = @id
RETURNING *;
