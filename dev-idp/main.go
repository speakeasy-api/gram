...
func run() error {
	address := flag.String("address", envOr("GRAM_DEVIDP_ADDRESS", "127.0.0.1:35291"), "HTTP listener address")
	controlAddress := flag.String("control-address", envOr("GRAM_DEVIDP_CONTROL_ADDRESS", "127.0.0.1:35292"), "HTTP listener address for the health/control server")
	...
	if *clientSecret == "" || *clientSecret == "unset" {
		return errors.New("GRAM_IDP_CLIENT_SECRET must be generated by ./zero before starting dev-idp")
	}
	...
	if backend == devidpworkos.BackendWorkOS {
		if *workosKey == "" || *workosKey == "unset" {
			return errors.New("GRAM_DEVIDP_BACKEND=workos requires WORKOS_API_KEY to be a real WorkOS API key")
		}
